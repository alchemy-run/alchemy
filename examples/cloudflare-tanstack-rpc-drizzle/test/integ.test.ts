...
describe.each([true, false])("CloudflareTanstackRpcDrizzleExample (dev: %p)", (dev) => {
  ...
  test(
    ...
    Effect.gen(function* () {
      const client = yield* RpcClient.make(TodoRpcs);
      const result = yield* client.toggleTodo({ id: 2_147_483_000, done: true }).pipe(Effect.flip);
