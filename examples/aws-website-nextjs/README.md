...
The site is **effectful**: `src/site.ts` passes an Effect program as the
...
- `app/page.jsx` is server-rendered in the Lambda on every request and
  reads the `GREETING` environment value declared in `src/site.ts` via
...
effect program is pinned `remote()` in `src/site.ts`, so `/api/message`
