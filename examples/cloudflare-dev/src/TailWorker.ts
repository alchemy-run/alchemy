...
export default {
  async fetch(request: Request, env: { EVENTS: TailEventsKV }): Promise<Response> {
    ...
    if (url.pathname === "/events") {
      ...
      const batches = await Promise.all(list.keys.map((key) => env.EVENTS.get(key.name)));
  ...
  async tail(events: TraceItemLite[], env: { EVENTS: TailEventsKV }): Promise<void> {
    const producer = events[0]?.scriptName ?? "unknown";
    await env.EVENTS.put(`evt:${producer}:${Date.now()}`, JSON.stringify(events));
  },
};
