...
Deploys a real Nuxt application with `Neon.Website.Nuxt`.

A Nuxt SSR page reads `GREETING`, hydrates a Vue counter, and calls a Nitro `/api/hello` endpoint. `/about` is prerendered.

Native dev and desktop/mobile interactions have passed. With a proper workspace dependency installation, the production build, safe artifact staging, and Node 24 Fetch page/asset/HEAD checks pass. Live deployment and updates remain unverified.
