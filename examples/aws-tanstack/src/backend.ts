 ...
 * `enqueue` method) and CONSUMES it — single-handler delivery
...
/**
 * ONE deployment serves the TanStack Start app AND a typed backend — the
 * maximal AWS shape (Serve/DESIGN.md "MaxSite", minus DOs/Workflows which
 * AWS has no analogue for): an Effect `fetch` API (including a streaming
 * route and a request-scope finalizer route), a queue produced to AND
 * consumed on the same Lambda, and RPC methods for the value-form
 * `createClient` used by the TanStack server functions in
 ...
 *
 * HTTP composition lives in src/server.ts (TanStack's native server-entry
 * convention) — the mount file. Everything platform-shaped (the queue
 * consumer's event-source mapping) derives from the `yield*`
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      fetch: Effect.gen(function* () {
        ...
        // Queue producer over HTTP (the value-form `enqueue` method stays
        // covered through the TanStack server functions).
        ...
        // Reached only through the mount's admin gate (src/server.ts).
