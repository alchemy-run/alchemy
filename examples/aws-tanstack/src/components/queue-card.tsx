...
import { enqueueJob, getProcessed } from "../server/visits.ts";
...
// its server function and returns immediately — the consumer on the
// backend class catches up out of band. After a send, poll `getProcessed`
// (bounded, once per second) until the count moves so the catch-up —
// queue → consumer → KV → UI — is visible.
export function QueueCard({
  ...
  const send = () =>
    startTransition(async () => {
      ...
      await enqueueJob({ data: text || "hello queue" });
      for (let i = 0; i < 15; i++) {
        await sleep(1000);
