...
/**
 * ONE Worker serves the SvelteKit app AND a typed backend — the maximal
 ...
 * methods for the value-form `createClient` used by kit's server code
 * (+page.server.ts load/actions, +server.ts routes).
 *
 * HTTP composition lives in src/hooks.server.ts — the mount. Everything
 * platform-shaped (queue consumer, DO/Workflow class exports, bindings)
 * derives from the `yield*` registrations below.
 */
export default class Site extends SvelteKit<Site>()(
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      // ── Value-form methods (createClient from kit server code) ──
