/**
 * The DEV leg of the Waku MaxSite example (Serve/DESIGN.md test doctrine):
 * the same stack under `Test.make({ dev: true })` — local simulators, the
 * waku dev server (rsc environment in workerd via the Cloudflare vite
 * plugin), and the SAME mount middleware (src/middleware/mount.ts)
 * composing dispatch. Every platform surface the live leg proves is driven
 * here against the local runtime: the mount's custom routes, DO (monotonic
 * + streaming RPC), Workflow to completion, queue produce→consume, and the
 * request-scope finalizer.
 */
...
test(
  ...
  Effect.gen(function* () {
    ...
    const denied = yield* client.get(`${base}/api/admin/secret`);
    expect(denied.status).toBe(403);

    const passed = yield* client.execute(
      HttpClientRequest.get(`${base}/api/admin/secret`).pipe(
...
test(
  "dev: request-scope finalizer lands",
  Effect.gen(function* () {
    const { url } = yield* stack;
    const base = url.replace(/\/+$/, "");
...
test(
  "dev: the waku SSR page serves through the same pipeline",
  Effect.gen(function* () {
    const { url } = yield* stack;
    const base = url.replace(/\/+$/, "");
    // First SSR compile in the dev child is slow — generous retry.
    const client = yield* HttpClient.HttpClient;
