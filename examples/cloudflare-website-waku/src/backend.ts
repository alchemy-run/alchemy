...
// this module joins the waku RSC server graph (the generated worker entry
// imports it), and in dev the vite module runner evaluates every module in
// that graph. The provider barrel would drag the entire IaC engine into
// it — the service-level subpaths keep it to the construct + capability
// slice.
...
/**
 * ONE Worker serves the Waku app AND a typed backend — the maximal shape
 * (Serve/DESIGN.md "MaxSite"): an Effect `fetch` API (including a
 * streaming route and a request-scope finalizer route), a Durable Object,
 * a durable Workflow, a queue produced to AND consumed on the same class,
 * and RPC methods for the value-form `createClient` used by the waku RSC
 * pages in src/lib/backend.ts.
 *
 * HTTP composition lives in src/middleware/mount.ts — waku's framework
 * hook (the mount file). Everything platform-shaped (queue consumer,
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      fetch: Effect.gen(function* () {
        ...
        // Request-scope finalizer: settles after the mount's site.fetch
        // resolves (inline in this composition — no ctx is passed). The
        // test polls /api/kv?key=finalizer-last to observe it.
      ...
      // ── RPC methods (value-form createClient, used by the RSC pages) ──
