...
/**
 * ONE Lambda serves the SvelteKit app AND a typed backend — the maximal
 ...
 * on the same Lambda, and value-form methods for kit's own server code
 * (`+page.server.ts` load + form actions, `+server.ts` routes) via
 * `createClient(Backend)`.
 *
 * HTTP composition lives in src/hooks.server.ts — the mount. Everything
 * platform-shaped (the queue consumer's event-source mapping) derives
 * from the `yield*` registrations below; nothing is configured elsewhere.
 */
export default class Site extends SvelteKit<Site>()(
  ...
  Effect.gen(function* () {
    ...
    return {
      // ── Effect HTTP API (paths the hooks.server.ts mount routes here) ──
      fetch: Effect.gen(function* () {
        ...
        // Reached only through the mount's admin gate (hooks.server.ts).
