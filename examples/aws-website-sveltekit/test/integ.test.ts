...
test(
  ...
  Effect.gen(function* () {
    ...
    // CONSUMER runs out of band on the SAME server Lambda (single-handler
    // entry), whose event-source mapping was registered by the same
    // backend module.
...
test(
  "the mount's own route answers without kit (healthz)",
  Effect.gen(function* () {
    const url = yield* base;
...
test(
  "kit's own +server.ts route stays kit's (the mount's exclusion glob)",
  Effect.gen(function* () {
    const url = yield* base;
    // /api/processed is carved OUT of the mount's claim — kit's own
    // +server.ts (value-form client inside) answers it.
    const res = yield* getWhenReady(`${url}/api/processed`);
    expect(res.status).toBe(200);
