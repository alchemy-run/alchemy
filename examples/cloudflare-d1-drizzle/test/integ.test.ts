...
test(
  ...
  Effect.gen(function* () {
    ...
    // The raw @effect/sql-d1 client sees the same rows.
    const sqlResponse = yield* HttpClient.get(`${baseUrl}/sql`);
    expect(sqlResponse.status).toBe(200);
    const sqlBody = (yield* sqlResponse.json) as unknown as {
      rows: Array<{ id: number; name: string; email: string }>;
    };
    expect(sqlBody.rows.some((row) => row.id === createdUser.id)).toBe(true);
