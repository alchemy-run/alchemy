...
The order (each step independently testable; 1–3 are pure, no provider needed):
...
3. **Step machine (`Step.ts`)** — `Command`/`Feedback` with ids; truncated-batch rule (`finish.reason === "length"` + tool calls ⇒ fail wholesale); the StreamPart → KernelEvent total mapping; `StepState` survives `structuredClone`.
4. **Stage-B turn driver (memory kernel)** — `streamText` + `disableToolCallResolution`; emission points (Begin before I/O, terminal after commit); RW-lock tool scheduler off `Tool.Readonly`/`Destructive` annotations; `ToolInterceptor` pass-through; transactional budget decrement off `FinishPart.usage`.
5. **Admission ledger + Coordinator reduction** — join/coalesce-wake/interrupt over the ledger (DO single-threading replaces the mutex; memory kernel uses a per-ring fiber).
...
7. **Stage-C loop runtime** — idle-wake continuation admission; halt-as-tool (`resolve`/`give_up`); N-consecutive `Refused` ratification; `prompts/` kernel assets with `kernelAssetsHash` folded into `promptHash`.
