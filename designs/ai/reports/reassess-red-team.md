# Red team: did we over-generalize Agent into Process?

Adversarial review of the Process abstraction, prompted by the owner:
...
too much is reliant on an LLM making every decision."* Method: steelman both
sides against the code (`src/AI/{Process,Agent,Kernel}.ts`), the org example
(`examples/agent-chat-web/src/{org,server}.ts`), the design (§1, §2.9, §8),
and the four sibling reassessments. Verdict at the end.

---

## 1. The case for "too far"
...
| Piece | Authored by a user in the shipped example? | Honest classification |
|---|---|---|
...
as term vocabulary. The renderer already confesses this: control refs render
...
deterministic-orchestration report proves the org's one real Process (the
...
## 2. The case for keeping Process
...
**2.2 Term vs shape — the distinction that dissolves most of the fight.**
Split the word "Process":

- **(b) Process-as-service-shape** — `ProcessService<Out, In, Err>`. This is
  load-bearing everywhere and survives any verdict: it is the unit of
...
- **(a) Process-as-term** — a prose charter the kernel interprets. This is
  what the "too far" charge actually targets. And even here the keep case has
...
## 3. The over-LLM audit

The charge is **sustained against the example and the defaults**, not against
the capability set:
...
2. **Routing-by-model.** "Decide which member(s) should handle it"
   ...
   to do a `switch`.
3. **Prose legislating against failure modes code cannot have.** "You NEVER
   write prose into the thread" (`org.ts:47-50`) exists only because the
   coordinator is a model; the deterministic variant cannot commit the sin.
4. **The default is the LLM even though the doctrine isn't.** §2.9 already
   ...
   (`KernelMemory.ts:896-931`, perpetual §0.3) — the feared standing LLM loop
...
Taken together, the four siblings already prescribe the fix the "too far"
worry is groping toward:
...
- **control-refs (shape C)**: control *wiring* moves to a structured config
  ...
  "term costume" charge — without losing the typed channels (inference gets
...
defensible core: **a prose-charted, budgeted, judged, typed-exit goal job** —
...
**The over-generalization charge is sustained against the surface and the
defaults, and overruled against the architecture.** The minimal honest
vocabulary, named precisely:
...
No rename. `Agent` and `Process` are the right two names, with the documented
...
the tutorial, and it is also the honest answer to "have we gone too far":
