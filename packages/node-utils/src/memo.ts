...
import { gitignoreRulesToGlobs } from "./ignore-rules.ts";
import { sha256, sha256Object } from "./hash.ts";

const initialCwd = process.cwd();

const isPathWithin = (directory: string, candidate: string, base: string) => {
  const relative = NodePath.relative(
    NodePath.resolve(base, directory),
 ...
 */
const Memo = (runtimeDirectory: string) =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const runtimeBase = process.cwd();
    const dotAlchemy = runtimeDirectory;
    ...
    const listFiles = Effect.fn(function* (
      ...
      const excludeRuntime = !isPathWithin(
        dotAlchemy,
        options.cwd,
        runtimeBase,
      );
...
export const hashDirectory = Effect.fn(function* (props: {
  ...
  const service = yield* Memo(props.runtimeDirectory ?? ".alchemy");
...
export const hashViteInput = Effect.fn(function* <E, R>(
  ...
  const hashWorkspaceDirectory = (cwd: string, memo?: MemoOptions) =>
    hashDirectory({
      cwd: path.resolve(resolvedRoot, cwd),
      memo,
      runtimeDirectory,
    }).pipe(Effect.map((hash) => `${relativeToRoot(cwd)}:${hash}`));
