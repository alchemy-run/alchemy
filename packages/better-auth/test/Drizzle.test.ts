...
describe("BetterAuth (drizzle)", () => {
  it.live("uses generated Relations v2 for sign-up, sign-in, and joined sessions", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const migration = yield* fs.readFileString(
        yield* path.fromFileUrl(new URL("./fixtures/drizzle-auth.sql", import.meta.url)),
      );
      ...
      const db = yield* Effect.sync(() => drizzle({ client, relations: schema.authRelations }));
  ...
  it.live("accepts a request-scoped Postgres client", () =>
    Effect.gen(function* () {
      const { drizzle } = yield* Effect.promise(() => import("drizzle-orm/node-postgres"));
      ...
      const database = Effect.gen(function* () {
        const client = yield* Effect.acquireRelease(
          ...
          (pool) =>
            Effect.promise(() => pool.end()).pipe(Effect.tap(() => Effect.sync(() => released++))),
        );
        return yield* Effect.sync(() => drizzle({ client }));
      });
      const service = yield* Database.pipe(Effect.provide(Drizzle(database, { provider: "pg" })));
      expect(acquired).toBe(0);
      expect(typeof (yield* service.runtime.pipe(Effect.scoped))).toBe("function");
