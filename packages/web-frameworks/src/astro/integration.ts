...
export const SERVER_ENTRYPOINT =
  "@alchemy.run/web-frameworks/astro/entrypoints/server";

/** The production endpoint of the passthrough image service. */
export const IMAGE_PASSTHROUGH_ENDPOINT = fileURLToPath(
  import.meta
    .resolve("@alchemy.run/web-frameworks/astro/image-passthrough-endpoint"),
);
...
const assertSupportedOptions = (options: DistilledCloudflareOptions): void => {
  ...
  if (offending.length > 0) {
    throw new Error(
      "@alchemy.run/web-frameworks/astro: unsupported @astrojs/cloudflare option(s):\n" +
        offending
    ...
    hooks: {
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              ...
              cloudflarePlugins,
              {
                name: "@alchemy.run/web-frameworks/astro:cf-imports",
                enforce: "pre",
                resolveId: {
              ...
              {
                name: "@alchemy.run/web-frameworks/astro:environment",
              ...
              {
                enforce: "post",
                name: "@alchemy.run/web-frameworks/astro:cf-externals",
      ...
      "astro:config:done": ({
        ...
        if (config.adapter !== undefined && !_injectedAdapterMarker) {
          throw new Error(
            `@alchemy.run/web-frameworks/astro: the Astro config declares the adapter "${config.adapter.name}", ` +
        ...
        injectTypes({
          ...
          content:
            '/// <reference types="@alchemy.run/web-frameworks/astro/types.d.ts" />',
        });
        setAdapter({
          name: "@alchemy.run/web-frameworks/astro",
          adapterFeatures: {
