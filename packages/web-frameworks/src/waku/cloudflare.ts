/**
 * The Cloudflare Workers deploy target for `@alchemy.run/web-frameworks/waku` — the ONLY
 ...
 *   adapter (`@alchemy.run/web-frameworks/waku/adapter`): behavior-identical to upstream
  ...
  makeDeployTarget({
    ...
    adapter: () =>
      Effect.try({
        try: () =>
          fileURLToPath(
            import.meta.resolve("@alchemy.run/web-frameworks/waku/adapter"),
          ),
        catch: (cause) =>
          new DeployTargetError({
            platform: "cloudflare",
            message:
              "Failed to resolve the @alchemy.run/web-frameworks/waku adapter module",
            cause,
          }),
      }),
    vitePlugins: (context) =>
