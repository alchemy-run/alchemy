...
`packages/cloudflare-frameworks/core/README.md` → "Architecture: frameworks × deploy
...
| `./source` (`src/source.ts`)         | Alchemy `Cloudflare.Worker` source provider (`{ provider: "@alchemy.run/cloudflare-frameworks/waku/source" }`) wrapping the framework with the cloudflare target | `cloudflare-runtime` (types)       |
