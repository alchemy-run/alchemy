...
export const pollUntil = async <T>(
  ...
  for (let attempt = 0; attempt < tries; attempt++) {
    ...
    if (reason !== undefined) {
      ...
      throw new Error(`Gave up waiting for ${what}: ${reason}.${detail ? `\n${detail}` : ""}`);
    }
...
export function fetchOk(url: string | URL, options?: PollOptions): Promise<Response>;
...
export async function fetchOk(
  ...
  const optionsOnly =
    explicitOptions === undefined && ("tries" in initOrOptions || "delayMs" in initOrOptions);
  const init = optionsOnly ? undefined : (initOrOptions as RequestInit);
  const options = optionsOnly ? (initOrOptions as PollOptions) : (explicitOptions ?? {});
...
export const parseOutputUrl = (output: string, key: string): string | undefined => {
  const escaped = key.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return output.match(
    new RegExp(`\\b${escaped}(?:\\s*:\\s*|\\s+)['"]?(http[^\\s'",]+)['"]?(?=[\\s,}])`),
  )?.[1];
};

export const parseOutputValue = (output: string, key: string): string | undefined => {
  const escaped = key.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return output.match(new RegExp(`\\b${escaped}(?:\\s*:\\s*|\\s+)['"]?([^\\s'",]+)`))?.[1];
};
...
export class DevCli {
  ...
  constructor(options: DevCliOptions) {
    ...
    this.alchemyBin =
      options.alchemyBin ?? path.join(this.root, "node_modules", "alchemy", "bin", "alchemy.js");
  ...
  start(): void {
    ...
    const child = spawn("bun", [this.alchemyBin, "dev", "--stage", this.stage], {
      cwd: this.root,
      detached: true,
      env: this.env,
  ...
  outputUrlWhenReady(key: string, options?: PollOptions): Promise<string> {
    return this.pollUntil(`${key} url in stack outputs`, () => this.outputUrl(key), options);
  }
  ...
  destroy(options: { readonly timeout?: number } = {}): void {
    const result = spawnSync("bun", [this.alchemyBin, "destroy", "--stage", this.stage, "--yes"], {
      cwd: this.root,
      env: this.env,
      stdio: "inherit",
