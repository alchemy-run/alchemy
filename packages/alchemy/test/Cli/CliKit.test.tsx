...
it.effect(
  "reports terminal-native progress on TTY output",
  () =>
    Effect.gen(function* () {
...
it(
  "loads state columns and file contents only when selected",
  async () => {
...
it(
  "ignores state explorer responses from before a refresh",
  async () => {
...
it(
  "prints stack outputs using inspect without wrapping long lines",
  () => {
...
it(
  "renders detailed plans as nested YAML",
  () => {
    const { service } = makeStatic();
...
it(
  "keeps the plan summary visible and updates it during apply",
  () => {
    const { service } = makeStatic();
    const plan = {
      ...planWith([
...
it(
  "keeps apply totals on top and destroy progress on the last row",
  () => {
    const { service } = makeStatic();
    const plan = {
      ...planWith(
...
it(
  "binding rows mirror their host resource and stay out of totals",
  () => {
    const { service } = makeStatic();
    const worker = {
...
it(
  "binding rows follow a failed host",
  () => {
    const { service } = makeStatic();
    const worker = {
      ...updateNode({ a: 1 }, { a: 2 }, "Worker"),
...
it.effect(
  "keeps native progress active until the apply outcome settles",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = yield* makeLive();
      const tree = new PlanTree(
...
it.effect(
  "renders completed dev plans as a hideable output view",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = yield* makeLive();
      const plan = {
...
it.effect(
  "removes the dev apply widget when its generation scope closes",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = yield* makeLive();
      const plan = {
...
it.effect(
  "keeps the dev plan widget hidden across hot reloads",
  () =>
    Effect.gen(function* () {
...
it(
  "renders drift details without detailed mode",
  () => {
    const { service } = makeStatic();
...
it(
  "replaces provider details with refresh progress in place",
  () => {
    const { service } = makeStatic();
    const output = service.output.format(
      <ProfileDetailsBody
...
it(
  "sizes provider blocks by providerBlockHeight",
  () => {
    const { service } = makeStatic();
    const output = service.output.format(
...
it(
  "marks the focused provider with the pointer and no rails",
  () => {
    const { service } = makeStatic();
    const output = service.output.format(
      <ProfileDetailsBody
...
it(
  "windows the profile dashboard's providers to the terminal height",
  () => {
...
it.live(
  "scrolls the profile dashboard to the focused provider",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
      const { service, stdout } = yield* makeLive({ stdin });
...
it(
  "renders input frames inline by default and keeps a stacked variant",
  () => {
    const { service } = makeStatic();
...
it(
  "renders prompt frames with an indented body and no rail",
  () => {
    const { service } = makeStatic();
    const output = service.output.format(
...
it(
  "honors explicit color settings for static output",
  () => {
    const { service } = makeStatic();
    const view = <Text color="green">ready</Text>;
    const styled = service.output.format(view, { colors: true });
    expect(styled).toContain("\u001B[");
    expect(stripVTControlCharacters(styled)).toBe("ready");
    expect(service.output.format(view, { colors: false })).toBe("ready");
  },
  { tags: ["unit", "local"] },
);

it.effect(
  "keeps stack output URLs reachable",
  () =>
    Effect.gen(function* () {
...
it.effect(
  "renders the built-in layout components without writing",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = makeStatic();
...
it.effect(
  "renders confirmations as a compact segmented choice",
  () =>
    Effect.gen(function* () {
      const { service } = makeStatic();
...
it.effect(
  "preserves zero in primitive-array views",
  () =>
    Effect.gen(function* () {
      const { service } = makeStatic();
      const rendered = yield* service.output.render([
...
it(
  "does not animate work before it starts",
  () => {
...
it.effect(
  "prints headings and data views through one service",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = makeStatic();
...
it.effect(
  "fails input operations when no terminal input is available",
  () =>
    Effect.gen(function* () {
      const { service } = makeStatic();
...
it.effect(
  "progress handles are updateable and settle only once",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = makeStatic();
...
it.effect(
  "tears down store-driven live views idempotently",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = yield* makeLive();
...
it.effect(
  "does not let one closing view tear down a newer live view",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = yield* makeLive();
...
it.effect(
  "progress settles into success and failure status output",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = makeStatic();
      yield* Effect.scoped(
...
it.effect(
  "task collapses success and failure into status output",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = makeStatic();
      yield* service.task(
...
it.effect(
  "status output composes as a normal view",
  () =>
    Effect.gen(function* () {
      const { service } = makeStatic();
      const rendered = yield* service.output.render(
...
it.effect(
  "uses ASCII fallbacks when Unicode is unavailable",
  () =>
    Effect.gen(function* () {
...
it.effect(
  "does not decorate resource stderr as a semantic failure",
  () =>
    Effect.gen(function* () {
...
it.effect(
  "runs interactive components inside the owned session",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = yield* makeLive();
...
it.effect(
  "restores the terminal after an alternate-screen interaction",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = yield* makeLive();
...
it.effect(
  "treats the terminal DEL byte as text-field backspace",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
...
it.effect(
  "strips control characters from pasted text-field input",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
      const { service } = yield* makeLive({ stdin });
...
it.effect(
  "shows the whole typed value while width remains available",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
      const { service, stdout } = yield* makeLive({ stdin });
...
it.effect(
  "shows a text prompt default before it is accepted",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
      const { service, stdout } = yield* makeLive({ stdin });
...
it.effect(
  "erases the multi-select filter with the terminal DEL byte",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
      const { service } = yield* makeLive({ stdin });
...
it.effect(
  "filters searchable selects without stealing Enter",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
      const { service } = yield* makeLive({ stdin });
...
it.effect(
  "keeps required cycle edits open until something changes",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
      const { service } = yield* makeLive({ stdin });
...
it.effect(
  "reopens browser authorization from the waiting screen",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
      const { service } = yield* makeLive({ stdin });
...
it.effect(
  "shows a device authorization code",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
      const { service, stdout } = yield* makeLive({ stdin });
...
it.effect(
  "toggles every visible multi-select choice on ctrl+a",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
      const { service } = yield* makeLive({ stdin });
...
it.effect(
  "returns an explicit undefined menu back target on Escape",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
      const { service } = yield* makeLive({ stdin });
...
it.effect(
  "cleans up a cancelled standalone prompt",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = yield* makeLive();
...
it.effect(
  "releases the renderer when a running prompt is interrupted",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
      const { service } = yield* makeLive({ stdin });
...
it.effect(
  "closes leaked live handles when their scope closes",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = yield* makeLive();
...
it.effect(
  "fails the active prompt when a screen throws during render",
  () =>
    Effect.gen(function* () {
...
it.effect(
  "cancels a screen with no cancel wiring on ctrl+c",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
      const { service, stdout } = yield* makeLive({ stdin });
...
it.effect(
  "keeps one renderer alive for an Effect-driven application",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = yield* makeLive();
...
it.effect(
  "provides CliKit once as a scoped injectable service",
  () => {
    const stdout = new CaptureStream();
    return Effect.gen(function* () {
...
it.effect(
  "uses append-only progress when input is disabled on a TTY",
  () => {
...
it.effect(
  "composes widgets before and after divided key hints",
  () =>
    Effect.gen(function* () {
      const { service } = makeStatic();
      const rendered = yield* service.output.render(
...
it(
  "wraps individual key hints in narrow terminals",
  () => {
    const { service } = makeStatic();
...
it.effect(
  "tabs scroll horizontally instead of wrapping when overflowing",
  () =>
    Effect.gen(function* () {
      const { service } = makeStatic();
...
it(
  "tabsWindow keeps the active tab visible within the available width",
  () => {
...
it.effect(
  "renders nuke scan counts and outstanding providers",
  () =>
    Effect.gen(function* () {
      const { service } = makeStatic();
...
it.effect(
  "updates individual nuke plan rows during deletion",
  () =>
    Effect.gen(function* () {
      const { service } = makeStatic();
      const tree = new PlanTree(
...
it.effect(
  "renders nuke execution outcomes in the shared plan",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = yield* makeLive();
...
it.effect(
  "closes nuke live progress when the operation fails",
  () =>
    Effect.gen(function* () {
      const { service, stdout } = yield* makeLive();
      yield* Effect.gen(function* () {
...
it.effect(
  "renders fixed-width Braille progress bars with partial cells",
  () =>
    Effect.gen(function* () {
      const { service } = makeStatic();
...
it.effect(
  "falls back to ASCII for progress bars without Unicode",
  () =>
    Effect.gen(function* () {
      const { service } = yield* makeLive({ input: false, unicode: false });
      const output = yield* service.output.render(
...
it.live(
  "defaults nuke plan approval to Cancel",
  () =>
    Effect.gen(function* () {
      const stdin = new InputStream();
      const { service, stdout } = yield* makeLive({ stdin });
...
it.effect(
  "keeps unnamed and duplicate nuke targets as individual rows",
  () =>
    Effect.gen(function* () {
      const { service } = makeStatic();
      const targets = [
