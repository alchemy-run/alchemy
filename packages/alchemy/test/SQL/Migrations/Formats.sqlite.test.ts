...
describe("alchemy format", (it) => {
  ...
  it.effect("upgrades the legacy Alchemy 3-column table in place without replaying", () =>
    Effect.gen(function* () {
      const db = new Database(":memory:");
  ...
  it.effect("upgrades the oldest 2-column shape, reading names from the primary column", () =>
    Effect.gen(function* () {
      const db = new Database(":memory:");
      db.run("CREATE TABLE __alchemy_migrations (id TEXT PRIMARY KEY, applied_at TEXT NOT NULL);");
      db.run(
  ...
  it.effect("converts a wrangler-shaped table in place when it IS the resolved table", () =>
    Effect.gen(function* () {
      const db = new Database(":memory:");
      db.run(
  ...
  it.effect("legacy rows recorded as dir/migration.sql keep matching directory records", () =>
  ...
  it.effect("fails the upgrade when a recorded migration matches no local file", () =>
    Effect.gen(function* () {
      const db = new Database(":memory:");
      db.run(
...
describe("one-way conversion from foreign tables", (it) => {
  it.effect("adopts a wrangler-migrated database: history copied, table frozen", () =>
    Effect.gen(function* () {
      const db = new Database(":memory:");
  ...
  it.effect("conversion fails when foreign history references a missing local file", () =>
    Effect.gen(function* () {
      const db = new Database(":memory:");
      db.run(
        "CREATE TABLE d1_migrations (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE, applied_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL);",
  ...
  it.effect("adopts a state-lost legacy 3-column d1_migrations as a conversion source", () =>
  ...
  it.effect("prefers drizzle history when multiple foreign sources exist", () =>
    Effect.gen(function* () {
      ...
      const rows = migrationRows(db);
      expect(rows.map((r) => r.name)).toEqual(["20240101000000_init", "20240102000000_add_posts"]);
  ...
  it.effect("adopts a prisma-migrated database via _prisma_migrations", () =>
    Effect.gen(function* () {
      ...
      // Frozen source.
      expect(
        (db.query("SELECT COUNT(*) AS n FROM _prisma_migrations;").all() as Array<{ n: number }>)[0]
          .n,
      ).toBe(2);
    }),
  );
