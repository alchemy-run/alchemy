...
import { handleSqlRoutes, type UserRow } from "./routes.ts";
 ...
 * client) over a Hyperdrive connection to a Neon Postgres origin. Mounts the
 * shared dialect-agnostic routes plus the Postgres-only surface:
 * `withTransaction` (commit + rollback) and multi-row `sql.updateValues`.
 */
export default class SqlPostgresWorker extends Cloudflare.Worker<SqlPostgresWorker>()(
  ...
  Effect.gen(function* () {
    ...
    // The same origin through `SQL.PostgresLayer` — resolved as the generic
    // `SqlClient` service, proving the Layer wiring end-to-end.
    const layerSql = yield* Effect.gen(function* () {
      return yield* Sql.SqlClient;
    }).pipe(Effect.provide(SQL.PostgresLayer({ url: hd.connectionString })));
