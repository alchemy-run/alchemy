...
const waitUntilGone = (path: string) =>
  tagmanager.getAccountsContainers({ path }).pipe(
...
test.provider.skipIf(!hasGcpCreds)(
  "getAccountsContainers on a missing container fails with a typed tag",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();

      const error = yield* Effect.flip(
        tagmanager.getAccountsContainers({
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const created = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* GCP.Tagmanager.Container("Web", {
            account: accountPath,
            usageContext: ["web"],
      ...
      expect(created.path).toContain("/containers/");
      expect(created.account).toEqual(accountPath);
      expect(created.usageContext).toEqual(["web"]);
      expect(created.notes).toEqual("marketing site");

      const fetched = yield* tagmanager.getAccountsContainers({
        path: created.path,
      });
      ...
      expect(updated.notes).toEqual("marketing site v2");
