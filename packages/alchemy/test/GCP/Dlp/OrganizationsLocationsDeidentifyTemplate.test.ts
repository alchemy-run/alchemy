...
const waitUntilGone = (name: string) =>
  dlp.getOrganizationsLocationsDeidentifyTemplates({ name }).pipe(
...
test.provider.skipIf(!hasGcpCreds)(
  "getOrganizationsLocationsDeidentifyTemplates on a missing template fails with a typed tag",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
      ...
      const error = yield* Effect.flip(
        dlp.getOrganizationsLocationsDeidentifyTemplates({
          name: `${organization}/locations/${location}/deidentifyTemplates/alchemy-missing`,
        }),
      );
...
test.provider.skipIf(!hasGcpCreds)(
  "create, update, and delete a location-scoped organization deidentify template",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
      ...
      if (organization.length === 0) {
        const error = yield* Effect.flip(
          dlp.createOrganizationsLocationsDeidentifyTemplates({
      ...
      const access = yield* dlp
        .listOrganizationsLocationsDeidentifyTemplates({
          parent,
          pageSize: 1,
        })
        .pipe(
      ...
      const created = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* GCP.Dlp.OrganizationsLocationsDeidentifyTemplate(
            "RedactPhone",
            {
              organization,
              location,
              displayName: "redact-phone",
              description: "redact phones",
      ...
      expect(created.name).toEqual(
        `${parent}/deidentifyTemplates/${created.templateId}`,
      );

      const fetched = yield* dlp.getOrganizationsLocationsDeidentifyTemplates({
        name: created.name,
      });
        ...
        Effect.gen(function* () {
          return yield* GCP.Dlp.OrganizationsLocationsDeidentifyTemplate(
            "RedactPhone",
            {
              ...
              displayName: "redact-phone-v2",
              description: "redact phones v2",
              deidentifyConfig: redactConfig,
            },
          );
        }),
      ...
      expect(updated.displayName).toEqual("redact-phone-v2");
