    ...
    Effect.gen(function* () {
      ...
      const error = yield* Effect.flip(
        sasportal.getNodesDevices({
          name: "nodes/missing/nodes/missing/devices/alchemy-missing",
        }),
      );
...
test.provider.skipIf(!hasGcpCreds || !!process.env.GCP_TEST_SASPORTAL)(
  "createNodesNodesDevices without entitlement fails with a typed tag",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();

      const error = yield* Effect.flip(
        sasportal.createNodesNodesDevices({
