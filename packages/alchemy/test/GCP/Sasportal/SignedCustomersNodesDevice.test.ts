...
test.provider.skipIf(!hasGcpCreds || !!process.env.GCP_TEST_SASPORTAL)(
  "createSignedCustomersNodesDevices without entitlement fails with a typed tag",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();

      const error = yield* Effect.flip(
        sasportal.createSignedCustomersNodesDevices({
    ...
    Effect.gen(function* () {
      ...
      const created = yield* stack.deploy(
        Effect.gen(function* () {
          ...
          const device = yield* GCP.Sasportal.SignedCustomersNodesDevice(
            "Cbsd",
            {
              parent: parent.name,
