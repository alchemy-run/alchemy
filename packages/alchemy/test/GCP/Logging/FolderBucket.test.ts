...
const waitUntilGone = (name: string) =>
  logging.getFoldersLocationsBuckets({ name }).pipe(
    Effect.map((bucket) =>
...
test.provider.skipIf(!hasGcpCreds)(
  "create, update, replace, and delete a folder logging bucket",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
      ...
      const created = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* GCP.Logging.FolderBucket("AppLogs", {
      ...
      const fetched = yield* logging.getFoldersLocationsBuckets({
        name: created.name,
      });
      ...
      const fetchedUpdate = yield* logging.getFoldersLocationsBuckets({
        name: created.name,
      });
      ...
      const fetchedReplacement = yield* logging.getFoldersLocationsBuckets({
        name: replaced.name,
      });
