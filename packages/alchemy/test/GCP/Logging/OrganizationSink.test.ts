...
test.provider.skipIf(!hasGcpCreds)(
  "getOrganizationsSinks on a missing sink fails with NotFound or Forbidden",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      if (organization.length === 0) {
        const error = yield* Effect.flip(
          logging.createOrganizationsSinks({
            parent: "organizations/0",
            body: {
              name: "alchemy-probe",
              destination: `logging.googleapis.com/projects/${project}/locations/global/buckets/_Default`,
            },
          }),
        );
