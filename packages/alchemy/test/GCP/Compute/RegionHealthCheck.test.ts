...
test.provider.skipIf(!hasGcpCreds)(
  "create, update, and delete a region health check",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
      ...
      const created = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* GCP.Compute.RegionHealthCheck("Api", {
            region: "us-central1",
      ...
      const fetched = yield* compute.getRegionHealthChecks({
      ...
      expect(lastSegment(fetched.region)).toEqual("us-central1");
      ...
      const refetched = yield* compute.getRegionHealthChecks({
      ...
      const afterReplace = yield* compute.getRegionHealthChecks({
