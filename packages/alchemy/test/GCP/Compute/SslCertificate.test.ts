...
const waitUntilGone = (sslCertificateName: string) =>
  compute
    .getSslCertificates({ project, sslCertificate: sslCertificateName })
    .pipe(
...
test.provider.skipIf(!hasGcpCreds)(
  "create, replace, and delete an ssl certificate",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
      ...
      const created = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* GCP.Compute.SslCertificate("FrontendTls", {
      ...
      const fetched = yield* compute.getSslCertificates({
        project,
      ...
      const listed = yield* compute.listSslCertificates({
        project,
        maxResults: 500,
      });
      expect(
      ...
      const refetched = yield* compute.getSslCertificates({
        project,
