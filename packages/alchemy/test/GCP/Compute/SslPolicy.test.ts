...
test.provider.skipIf(!hasGcpCreds)(
  "create, update, and delete an ssl policy",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
      ...
      const created = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* GCP.Compute.SslPolicy("FrontendTls", {
      ...
      const fetched = yield* compute.getSslPolicies({
      ...
      const listed = yield* compute.listSslPolicies({
        project: created.project,
        maxResults: 500,
      });
      expect(
      ...
      const refetched = yield* compute.getSslPolicies({
