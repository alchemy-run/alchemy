...
const waitUntilGone = (courseId: string, userId: string) =>
  classroom.getCoursesStudents({ courseId, userId }).pipe(
...
test.provider.skipIf(!hasGcpCreds)(
  "getCoursesStudents on a missing student fails with a typed tag",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();

      const error = yield* Effect.flip(
        classroom.getCoursesStudents({
...
test.provider.skipIf(!runLifecycle)(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const course = yield* ensureCourse("alch-student");
      const courseId = course.id ?? "";
      const userId = classroomUser!;

      const created = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* GCP.Classroom.CoursesStudent("Ada", {
            courseId,
            userId,
          });
        }),
      ...
      const fetched = yield* classroom.getCoursesStudents({
        courseId,
