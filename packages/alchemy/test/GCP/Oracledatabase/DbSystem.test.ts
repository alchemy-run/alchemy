...
test.provider.skipIf(!hasGcpCreds)(
  "getProjectsLocationsDbSystems on a missing system fails with a typed tag",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
      ...
      const page = yield* oracle
        ...
        .pipe(
          Effect.catchTag("Forbidden", () =>
            Effect.succeed({ dbSystems: [] as const }),
          ),
        );
      expect(Array.isArray(page.dbSystems ?? [])).toEqual(true);
