...
test.provider.skipIf(!hasGcpCreds)(
  "getProjectsLocationsExascaleDbStorageVaults on a missing vault fails with a typed tag",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
      ...
      const page = yield* oracle
        .listProjectsLocationsExascaleDbStorageVaults({
          parent: `projects/${project}/locations/-`,
          pageSize: 10,
        })
        .pipe(
          Effect.catchTag("Forbidden", () =>
            Effect.succeed({ exascaleDbStorageVaults: [] as const }),
          ),
        );
      expect(Array.isArray(page.exascaleDbStorageVaults ?? [])).toEqual(true);
