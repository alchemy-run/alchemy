...
test.provider.skipIf(!hasGcpCreds)(
  "getProjectsLocationsFeaturestores on a missing store fails with a typed tag",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
...
test.provider.skipIf(!runLifecycle)(
  "create, update, and delete a featurestore",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
      ...
      const created = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* GCP.AIPlatform.Featurestore("Features", {
      ...
      expect(created.name).toContain("/featurestores/");
      expect(created.location).toEqual("us-central1");
      expect(created.labels).toMatchObject({ env: "test" });

      const fetched = yield* aiplatform.getProjectsLocationsFeaturestores({
        name: created.name,
      });
      ...
      const updated = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* GCP.AIPlatform.Featurestore("Features", {
            featurestoreId: created.featurestoreId,
            location: "us-central1",
