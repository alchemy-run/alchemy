...
test.provider("replaces a node pool for create-only VM settings", () =>
  Effect.gen(function* () {
    ...
    expect(nestedVirtualization).toEqual({
      action: "replace",
      deleteFirst: true,
    });
    ...
    // metadata/false-valued booleans must not be compared against user
    // input that never mentioned those keys.
    const adoptionInput = {
      ...input,
      olds: undefined,
    } as const;
    ...
    expect(adoptedOtherMetadataKey).toEqual({
      action: "replace",
      deleteFirst: true,
    });
...
test.provider.skipIf(!runLifecycle)(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      expect(created.workloadMetadataConfig?.mode).toEqual("GKE_METADATA");
