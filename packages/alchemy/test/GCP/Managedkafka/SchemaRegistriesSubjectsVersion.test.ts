...
test.provider.skipIf(!hasGcpCreds)(
  "getProjectsLocationsSchemaRegistriesSubjectsVersions on a missing version fails with a typed tag",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();

      const error = yield* Effect.flip(
        kafka.getProjectsLocationsSchemaRegistriesSubjectsVersions({
          name: `projects/${project}/locations/us-central1/schemaRegistries/alchemy_missing/subjects/missing/versions/1`,
        }),
      );
...
test.provider.skipIf(!runLifecycle)(
  "create and delete a schema subject version",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
      ...
      expect(created.version.name).toContain("/subjects/");
