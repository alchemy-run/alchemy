...
const waitUntilGone = (
  ...
  appengine
    .getProjectsLocationsApplicationsDomainMappings({
      projectsId,
      locationsId,
      applicationsId,
...
test.provider.skipIf(!hasGcpCreds || !!process.env.GCP_TEST_APPENGINE)(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const error = yield* Effect.flip(
        appengine.createProjectsLocationsApplicationsDomainMappings({
          projectsId: project,
          locationsId: location,
          applicationsId: project,
          body: {
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const created = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* GCP.Appengine.ApplicationsDomainMapping("Www", {
            domain,
            sslSettings: { sslManagementType: "AUTOMATIC" },
          });
        }),
