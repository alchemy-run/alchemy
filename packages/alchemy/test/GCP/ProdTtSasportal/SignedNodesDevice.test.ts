  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const error = yield* Effect.flip(
        sas.getNodesDevices({
          name: "nodes/0/devices/alchemy-signed-missing",
        }),
      );
...
test.provider.skipIf(!hasGcpCreds || !!process.env.GCP_TEST_PROD_TT_SASPORTAL)(
  "createSignedNodesDevices is Forbidden without SAS Portal access",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();

      const error = yield* Effect.flip(
        sas.createSignedNodesDevices({
          parent: "nodes/0",
          body: {
    ...
    Effect.gen(function* () {
      ...
      const created = yield* stack.deploy(
        Effect.gen(function* () {
          ...
          return yield* GCP.ProdTtSasportal.SignedNodesDevice("Cbsd", {
            node: parent.name,
            encodedDevice: probeJwt,
            installerId: "ALCHEMYCPI",
