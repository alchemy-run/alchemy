    ...
    Effect.gen(function* () {
      ...
      const error = yield* Effect.flip(
        sas.getDeploymentsDevices({
          name: "customers/0/deployments/0/devices/alchemy-signed-missing",
        }),
      );
...
test.provider.skipIf(!hasGcpCreds || !!process.env.GCP_TEST_PROD_TT_SASPORTAL)(
  "createSignedCustomersDeploymentsDevices is Forbidden without SAS Portal access",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();

      const error = yield* Effect.flip(
        sas.createSignedCustomersDeploymentsDevices({
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const created = yield* stack.deploy(
        Effect.gen(function* () {
          const site = yield* GCP.ProdTtSasportal.CustomersDeployment("Site", {
            customer: customer!,
            displayName: "signed-site",
          });
          return yield* GCP.ProdTtSasportal.SignedCustomersDeploymentsDevice(
            "Cbsd",
            {
