    ...
    Effect.gen(function* () {
      ...
      const error = yield* Effect.flip(
        sas.getNodesDeployments({
          name: "customers/0/nodes/0/deployments/alchemy-missing",
        }),
      );
...
test.provider.skipIf(!hasGcpCreds || !!process.env.GCP_TEST_PROD_TT_SASPORTAL)(
  "createCustomersNodesDeployments is Forbidden without SAS Portal access",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();

      const error = yield* Effect.flip(
        sas.createCustomersNodesDeployments({
    ...
    Effect.gen(function* () {
      ...
      const created = yield* stack.deploy(
        Effect.gen(function* () {
          ...
          return yield* GCP.ProdTtSasportal.CustomersNodesDeployment("Site", {
            node: parent.name,
