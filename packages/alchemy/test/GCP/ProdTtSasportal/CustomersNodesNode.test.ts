  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const error = yield* Effect.flip(
        sas.getNodesNodes({
          name: "customers/0/nodes/0/nodes/alchemy-missing",
        }),
      );
...
test.provider.skipIf(!hasGcpCreds || !!process.env.GCP_TEST_PROD_TT_SASPORTAL)(
  "createCustomersNodesNodes is Forbidden without SAS Portal access",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();

      const error = yield* Effect.flip(
        sas.createCustomersNodesNodes({
    ...
    Effect.gen(function* () {
      ...
      const created = yield* stack.deploy(
        Effect.gen(function* () {
          ...
          return yield* GCP.ProdTtSasportal.CustomersNodesNode("Building", {
            node: parent.name,
            displayName: "bldg-a",
          });
        }),
