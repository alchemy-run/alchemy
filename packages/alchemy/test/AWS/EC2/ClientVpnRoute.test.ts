...
// Routes require associated target networks, whose AWS provisioning and ENI cleanup can exceed 90 seconds.
describe.skipIf(process.env.AWS_TEST_CLIENT_VPN_ASSOCIATION !== "1")(
  "Client VPN routes (AWS_TEST_CLIENT_VPN_ASSOCIATION=1)",
  () => {
    ...
    const prerequisites = beforeAll(deploy(Stack), { timeout: 120_000 });
    afterAll(
      destroy(Stack).pipe(
        Effect.andThen(
