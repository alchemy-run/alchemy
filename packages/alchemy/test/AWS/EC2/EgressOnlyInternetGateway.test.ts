...
test.provider("list enumerates the deployed Egress-Only Internet Gateway", (stack) =>
  Effect.gen(function* () {
    yield* stack.destroy();
    ...
    expect(
      all.some((x) => x.egressOnlyInternetGatewayId === eigw.egressOnlyInternetGatewayId),
    ).toBe(true);
