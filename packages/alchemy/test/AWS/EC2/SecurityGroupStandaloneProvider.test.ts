...
const standaloneTags = [
  { key: "alchemy::id", value: "StandaloneRule" },
] as const;
...
const makeHarness = (initialRules: ReadonlyArray<MockRule>) => {
  ...
  const client = HttpClient.make((request) =>
    Effect.sync(() => {
      ...
      } else if (action === "RevokeSecurityGroupIngress") {
        body = revokeRules(state.revokedIngress);
      ...
      } else if (action === "AuthorizeSecurityGroupIngress") {
        state.authorizedIngress += 1;
        state.rules.push(
          rule(`sgr-inline-new-${state.authorizedIngress}`, false),
        );
      }
  ...
  const reconcile = (
    egress: [] | undefined,
    update: boolean,
    ingress?: SecurityGroup["Props"]["ingress"],
  ) =>
    Effect.gen(function* () {
    ...
    Effect.gen(function* () {
      ...
      yield* provider.delete({
        ...
        output: {
          ...
          ipProtocol: "tcp",
          isEgress: false,
          securityGroupRuleId: "sgr-standalone-ingress",
        },
        session,
      });
    }).pipe(Effect.provide(providers));

  return { deleteStandaloneRule, reconcile, state };
};

describe("SecurityGroup provider reconciliation", () => {
  it.effect(
    "leaves standalone rules to the SecurityGroupRule lifecycle",
    () => {
      const { deleteStandaloneRule, reconcile, state } = makeHarness([
        rule("sgr-standalone-ingress", false, standaloneTags),
        rule("sgr-inline-ingress", false),
        rule("sgr-standalone-egress", true, standaloneTags),
        rule("sgr-inline-egress", true),
      ]);
      return Effect.gen(function* () {
        yield* reconcile([], true, [
          { cidrIpv4: "10.0.0.0/16", ipProtocol: "tcp" },
        ]);
        expect(state.revokedIngress).toEqual(["sgr-inline-ingress"]);
        expect(state.revokedEgress).toEqual(["sgr-inline-egress"]);
        expect(state.authorizedIngress).toBe(1);

        yield* deleteStandaloneRule();
        expect(state.revokedIngress).toEqual([
          "sgr-inline-ingress",
          "sgr-standalone-ingress",
        ]);
        expect(state.rules.map((item) => item.securityGroupRuleId)).toEqual([
          "sgr-standalone-egress",
          "sgr-inline-new-1",
          "sgr-authorized-1",
        ]);
      });
