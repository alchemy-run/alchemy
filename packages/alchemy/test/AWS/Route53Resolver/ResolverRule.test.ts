...
test.provider(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      expect(deployed.rule.domainName.replace(/\.$/, "")).toBe("corp.alchemy-r53r-test.internal");
      ...
      expect(rule.ResolverRule?.ResolverEndpointId).toBe(deployed.endpoint.resolverEndpointId);
      expect(rule.ResolverRule?.TargetIps?.map((t) => t.Ip)).toEqual(["192.168.10.10"]);
      ...
      const updated = yield* stack.deploy(make("192.168.10.11", "corp.alchemy-r53r-test.internal"));
      expect(updated.rule.resolverRuleId).toBe(deployed.rule.resolverRuleId);
      const afterUpdate = yield* r53r.getResolverRule({
        ResolverRuleId: deployed.rule.resolverRuleId,
      });
      expect(afterUpdate.ResolverRule?.TargetIps?.map((t) => t.Ip)).toEqual(["192.168.10.11"]);

      // Replacement: change the domain name — new rule, old rule deleted.
      const replaced = yield* stack.deploy(
        make("192.168.10.11", "corp2.alchemy-r53r-test.internal"),
      );
      expect(replaced.rule.resolverRuleId).not.toBe(deployed.rule.resolverRuleId);
      expect(replaced.rule.domainName.replace(/\.$/, "")).toBe("corp2.alchemy-r53r-test.internal");
