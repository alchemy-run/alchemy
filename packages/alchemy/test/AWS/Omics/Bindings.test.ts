...
import OmicsBindingsFunctionLive, { OmicsBindingsFunction } from "./bindings-handler";
...
test.provider("listReadSets on a nonexistent sequence store fails with a typed tag", () =>
  Effect.gen(function* () {
    const error = yield* Effect.flip(omics.listReadSets({ sequenceStoreId: "0000000000" }));
...
test.provider("listReferences on a nonexistent reference store fails with a typed tag", () =>
  Effect.gen(function* () {
    const error = yield* Effect.flip(omics.listReferences({ referenceStoreId: "0000000000" }));
    expect(["ResourceNotFoundException", "ValidationException"]).toContain(error._tag);
  }),
);
...
describe("Omics Bindings (E2E)", () => {
  ...
  test.provider.skipIf(!RUN_LIVE)("all 8 capabilities initialize in the runtime", () =>
    Effect.gen(function* () {
  ...
  test.provider.skipIf(!RUN_LIVE)("ListRuns round-trips at the account level", () =>
    Effect.gen(function* () {
  ...
  test.provider.skipIf(!RUN_LIVE)("GetReadSetMetadata + GetRun surface the typed not-found", () =>
    Effect.gen(function* () {
