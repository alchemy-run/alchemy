...
for (const fifo of [false, true]) {
  provider(
    `precreate refuses a foreign ${fifo ? "FIFO" : "standard"} queue with unresolved mutable props`,
    (stack) =>
      Effect.gen(function* () {
        yield* stack.destroy();
        const ownerProps = {
        ...
        const conflict = yield* stack
          ...
          .pipe(Effect.flip);
        expect(JSON.stringify(conflict)).toContain("QueuePrecreateConflict");
        expect(JSON.stringify(conflict)).toContain("not owned by this stack");
...
provider(
  "precreate rejects a same-name FIFO-to-standard replacement without deleting messages",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
      const original = yield* stack.deploy(Queue("Queue", { fifo: true }));
      const tags = yield* SQS.listQueueTags({ QueueUrl: original.queueUrl });
      yield* SQS.sendMessage({
        QueueUrl: original.queueUrl,
        MessageBody: "preserved-message",
        MessageGroupId: "owner",
        MessageDeduplicationId: "owner",
      });
      const conflict = yield* stack
        .deploy(
          Queue("Queue", {
        ...
        .pipe(Effect.flip);
      expect(JSON.stringify(conflict)).toContain("QueuePrecreateConflict");
      expect(JSON.stringify(conflict)).toContain("existing queue is FIFO");
      const attributes = yield* SQS.getQueueAttributes({
        QueueUrl: original.queueUrl,
        AttributeNames: ["QueueArn", "FifoQueue"],
      });
      expect(attributes.Attributes?.QueueArn).toBe(original.queueArn);
      expect(attributes.Attributes?.FifoQueue).toBe("true");
      expect(
        (yield* SQS.listQueueTags({ QueueUrl: original.queueUrl })).Tags,
      ).toEqual(tags.Tags);
      const messages = yield* SQS.receiveMessage({
        QueueUrl: original.queueUrl,
        WaitTimeSeconds: 10,
      ...
      yield* stack.destroy();
      yield* assertQueueDeleted(original.queueUrl);
    }),
  { timeout: 120_000 },
);
...
provider(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      // SQS reserves a deleted name for 60s; leave the bounded create retry 40s.
...
provider(
  "foreign-tagged queue converges policies during explicit adoption",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
      ...
      const takenOver = yield* stack
        .deploy(
          Effect.gen(function* () {
            const queue = yield* Queue("Different", { queueName });
            yield* queue.bind`SelfPolicy`({
              policyStatements: [
                {
                  Effect: "Allow",
      ...
      yield* waitForQueueTags(
        takenOver.queueUrl,
        (tags) => tags["alchemy::id"] === "Different",
      );
      ...
      Effect.ensuring(
        deleteQueueIfExists(TAKEOVER_QUEUE_NAME).pipe(Effect.orDie),
      ),
    ),
);
