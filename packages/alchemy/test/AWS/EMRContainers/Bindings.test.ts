...
describe.sequential("EMRContainers Bindings", () => {
  beforeAll(
    Effect.gen(function* () {
      yield* Effect.logInfo("EMRContainers test setup: destroying previous resources");
      ...
      yield* Effect.logInfo(`EMRContainers test setup: probing readiness at ${readinessUrl}`);
      yield* HttpClient.get(readinessUrl).pipe(
        ...
        Effect.tapError((error) =>
          Effect.logWarning(`EMRContainers test setup: fixture not ready yet (${String(error)})`),
        ),
  ...
  describe("DescribeJobTemplate", () => {
    test.provider("reads the bound template's data (injected template id)", (_stack) =>
      Effect.gen(function* () {
  ...
  describe("ListJobTemplates", () => {
    test.provider("enumerates the account's templates including the fixture's", (_stack) =>
      Effect.gen(function* () {
  ...
  describe("ListVirtualClusters", () => {
    test.provider("enumerates the account's virtual clusters (IAM grant + call path)", (_stack) =>
      Effect.gen(function* () {
