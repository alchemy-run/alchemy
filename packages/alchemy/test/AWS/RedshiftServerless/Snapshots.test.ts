...
import SnapshotFunctionLive, { SnapshotFunction } from "./fixtures/snapshot-handler";
...
// near-zero cost.
test.provider("getSnapshot on a nonexistent snapshot fails with ResourceNotFoundException", () =>
  Effect.gen(function* () {
...
test.provider("deleteSnapshot on a nonexistent snapshot fails with ResourceNotFoundException", () =>
  Effect.gen(function* () {
    const error = yield* Effect.flip(
...
// in afterAll.
describe.skipIf(!process.env.AWS_TEST_REDSHIFT)("RedshiftServerless Snapshot Bindings", () => {
  beforeAll(
    Effect.gen(function* () {
      yield* Effect.logInfo("Snapshot bindings setup: destroying previous run");
      ...
      Effect.flatMap((res) =>
        res.status === 200
          ...
          : Effect.flatMap(res.text, (body) =>
              Effect.fail(new Error(`${request.url} -> ${res.status}: ${body}`)),
            ),
      ),
    );
      ...
      Effect.gen(function* () {
        ...
        const settled = (yield* json(
          ...
          Effect.repeat({
            ...
            until: (body): boolean =>
              (body as { status?: string }).status?.toUpperCase() === "AVAILABLE",
            times: 36,
          }),
        )) as {
          snapshotName?: string;
        ...
        const listed = (yield* json(HttpClientRequest.get(`${baseUrl}/snapshots`))) as {
        ...
        const updated = (yield* json(
          HttpClientRequest.post(`${baseUrl}/snapshot/retention?name=${SNAPSHOT_NAME}`),
        ...
        const recovery = (yield* json(HttpClientRequest.get(`${baseUrl}/recovery-points`))) as {
          count: number;
        };
        expect(recovery.count).toBeGreaterThanOrEqual(0);
        const restores = (yield* json(HttpClientRequest.get(`${baseUrl}/table-restores`))) as {
          count: number;
        };
        expect(restores.count).toBeGreaterThanOrEqual(0);

        // DeleteSnapshot removes it; out-of-band read confirms it is gone.
        yield* json(HttpClientRequest.delete(`${baseUrl}/snapshot?name=${SNAPSHOT_NAME}`));
        const gone = yield* serverless.getSnapshot({ snapshotName: SNAPSHOT_NAME }).pipe(
          Effect.map((r) => r.snapshot?.status ?? "PRESENT"),
          Effect.catchTag("ResourceNotFoundException", () => Effect.succeed("GONE" as const)),
          Effect.repeat({
            schedule: Schedule.spaced("5 seconds"),
            until: (status): boolean => status === "GONE" || status === "DELETING",
            times: 12,
          }),
        );
