...
import MediaConnectTestFunctionLive, { MediaConnectTestFunction } from "./fixtures/handler";
...
describe.sequential("MediaConnect Bindings", () => {
  beforeAll(
    Effect.gen(function* () {
      yield* Effect.logInfo("MediaConnect test setup: destroying previous resources");
      ...
      yield* Effect.logInfo(`MediaConnect test setup: probing readiness at ${readinessUrl}`);
      yield* HttpClient.get(readinessUrl).pipe(
        ...
        Effect.tapError((error) =>
          Effect.logWarning(`MediaConnect test setup: fixture not ready yet (${String(error)})`),
        ),
  ...
  describe("StopFlow", () => {
    test.provider("stopping a STANDBY flow is the typed BadRequestException", () =>
      Effect.gen(function* () {
