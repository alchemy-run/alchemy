...
const assertConfigurationDeleted = (name: string) =>
  findConfiguration(name).pipe(
    Effect.flatMap((config) =>
      config === undefined ? Effect.void : Effect.fail(new ConfigurationStillExists({ name })),
    ),
    Effect.retry({
...
test.provider(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      expect(config.playbackConfigurationArn).toContain(":playbackConfiguration/");
      expect(config.playbackEndpointPrefix).toContain("mediatailor");
      expect(config.sessionInitializationEndpointPrefix).toContain("mediatailor");

      // out-of-band verification via distilled
      const created = yield* findConfiguration(config.name);
      expect(created?.AdDecisionServerUrl).toBe("https://ads.example.com/vast?ip=[client_ip]");
      expect(created?.VideoContentSourceUrl).toBe("https://origin.example.com/live");
      ...
      expect(updated.playbackConfigurationArn).toBe(config.playbackConfigurationArn);

      const afterUpdate = yield* findConfiguration(config.name);
      expect(afterUpdate?.AdDecisionServerUrl).toBe("https://ads.example.com/vast/v2");
      ...
      expect(afterUpdate?.ManifestProcessingRules?.AdMarkerPassthrough?.Enabled).toBe(true);
...
test.provider(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      expect(second.playbackConfigurationArn).not.toBe(first.playbackConfigurationArn);
      ...
      const replacement = yield* findConfiguration("alchemy-test-playback-config-b");
      expect(replacement?.PlaybackConfigurationArn).toBe(second.playbackConfigurationArn);
