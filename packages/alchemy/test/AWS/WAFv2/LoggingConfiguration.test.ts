...
class LoggingConfigStillExists extends Data.TaggedError("LoggingConfigStillExists")<{
  readonly resourceArn: string;
}> {}

const assertLoggingDeleted = (resourceArn: string) =>
  wafv2.getLoggingConfiguration({ ResourceArn: resourceArn }).pipe(
    Effect.flatMap(() => Effect.fail(new LoggingConfigStillExists({ resourceArn }))),
...
test.provider(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      expect(created.LoggingConfiguration?.LogDestinationConfigs?.[0]).toContain(
        "aws-waf-logs-alchemy-wafv2-test",
      );
      ...
      expect(updated.LoggingConfiguration?.LoggingFilter?.DefaultBehavior).toBe("KEEP");
      expect(updated.LoggingConfiguration?.RedactedFields?.[0]?.SingleHeader?.Name).toBe(
        "authorization",
      );
