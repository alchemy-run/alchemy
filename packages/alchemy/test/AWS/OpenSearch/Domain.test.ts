...
test.provider("describeDomain on a nonexistent domain fails with ResourceNotFoundException", () =>
  Effect.gen(function* () {
...
const assertDomainDeleting = (name: string) =>
  Effect.gen(function* () {
    const status = yield* opensearch.describeDomain({ DomainName: name }).pipe(
      Effect.map((response) =>
        response.DomainStatus.Deleted === true || response.DomainStatus.Processing === true
          ? ("deleting" as const)
          : ("active" as const),
      ),
    ...
    if (status === "active") {
      return yield* Effect.fail(new Error(`domain '${name}' still exists and is not deleting`));
    }
  }).pipe(
    Effect.retry({
...
test.provider.skipIf(!process.env.AWS_TEST_SLOW)(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      expect(described.DomainStatus.EncryptionAtRestOptions?.Enabled).toBe(true);
      expect(described.DomainStatus.NodeToNodeEncryptionOptions?.Enabled).toBe(true);
      expect(described.DomainStatus.ClusterConfig.InstanceType).toBe("t3.small.search");
      expect(described.DomainStatus.DomainEndpointOptions?.EnforceHTTPS).toBe(true);
      ...
      expect(AWS.IAM.normalizePolicyDocument(described.DomainStatus.AccessPolicies ?? "")).toBe(
        AWS.IAM.normalizePolicyDocument(accessPolicies),
      );
      ...
      // and flip Processing.
      const changeIdBefore = described.DomainStatus.ChangeProgressDetails?.ChangeId;
      ...
      expect(redescribed.DomainStatus.ChangeProgressDetails?.ChangeId).toBe(changeIdBefore);
