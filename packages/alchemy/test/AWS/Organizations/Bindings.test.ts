      ...
      test.provider("reads the organization", (_stack) =>
        Effect.gen(function* () {
          ...
          if (response.ok) {
            ...
            expect(["AWSOrganizationsNotInUseException"]).toContain(
              response.tag,
            );
          }
        }),
