...
const describeMetricFilter = Effect.fn(function* (logGroupName: string, filterName: string) {
  const described = yield* logs
    .describeMetricFilters({ logGroupName, filterNamePrefix: filterName })
    .pipe(
      Effect.catchTag("ResourceNotFoundException", () => Effect.succeed({ metricFilters: [] })),
    );
  return (described.metricFilters ?? []).find((filter) => filter.filterName === filterName);
});

class MetricFilterStillExists extends Data.TaggedError("MetricFilterStillExists")<{
...
const assertMetricFilterDeleted = (logGroupName: string, filterName: string) =>
  describeMetricFilter(logGroupName, filterName).pipe(
    Effect.flatMap((filter) =>
      filter === undefined ? Effect.void : Effect.fail(new MetricFilterStillExists({ filterName })),
    ),
    Effect.retry({
...
test.provider(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const observedCreated = yield* describeMetricFilter(created.logGroupName, created.filterName);
      expect(observedCreated?.filterPattern).toBe("?ERROR");
      expect(observedCreated?.metricTransformations?.[0]?.metricName).toBe("AlchemyTestErrorCount");
      ...
      const observedUpdated = yield* describeMetricFilter(updated.logGroupName, updated.filterName);
      ...
      expect(yield* describeMetricFilter(replaced.logGroupName, replaced.filterName)).toBeDefined();
      yield* assertMetricFilterDeleted(created.logGroupName, created.filterName);

      yield* stack.destroy();
      yield* assertMetricFilterDeleted(replaced.logGroupName, replaced.filterName);
