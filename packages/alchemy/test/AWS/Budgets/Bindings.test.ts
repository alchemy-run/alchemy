...
import BudgetsTestFunctionLive, { BudgetsTestFunction, fixtureBudgetName } from "./handler";
...
describe.sequential("Budgets Bindings", () => {
  beforeAll(
    Effect.gen(function* () {
      yield* Effect.logInfo("Budgets test setup: destroying previous resources");
      ...
      yield* Effect.logInfo(`Budgets test setup: probing readiness at ${readinessUrl}`);
      yield* HttpClient.get(readinessUrl).pipe(
        ...
        Effect.tapError((error) =>
          Effect.logWarning(`Budgets test setup: fixture not ready yet (${String(error)})`),
        ),
  ...
  describe("DescribeBudget", () => {
    test.provider("reads the fixture budget's definition", (_stack) =>
      Effect.gen(function* () {
        const response = yield* send(HttpClientRequest.get(`${baseUrl}/budget`)).pipe(
          Effect.flatMap((r) => r.json),
        );
  ...
  describe("DescribeBudgetPerformanceHistory", () => {
    test.provider("reads the budget's history (fresh budget: >= 0 periods)", (_stack) =>
      Effect.gen(function* () {
        const response = yield* send(HttpClientRequest.get(`${baseUrl}/history`)).pipe(
          Effect.flatMap((r) => r.json),
        );
  ...
  describe("DescribeNotificationsForBudget", () => {
    test.provider("lists the fixture notification threshold", (_stack) =>
      Effect.gen(function* () {
        const response = yield* send(HttpClientRequest.get(`${baseUrl}/notifications`)).pipe(
          Effect.flatMap((r) => r.json),
        );
  ...
  describe("DescribeSubscribersForNotification", () => {
    test.provider("lists the fixture notification's subscriber", (_stack) =>
      Effect.gen(function* () {
        const response = yield* send(HttpClientRequest.get(`${baseUrl}/subscribers`)).pipe(
          Effect.flatMap((r) => r.json),
        );
  ...
  describe("DescribeBudgetActionsForBudget", () => {
    test.provider("lists the fixture action in STANDBY", (_stack) =>
      Effect.gen(function* () {
        const response = yield* send(HttpClientRequest.get(`${baseUrl}/actions`)).pipe(
          Effect.flatMap((r) => r.json),
        );
  ...
  describe("DescribeBudgetActionHistories", () => {
    test.provider("reads the action's event history (>= CREATE_ACTION)", (_stack) =>
      Effect.gen(function* () {
        const response = yield* send(HttpClientRequest.get(`${baseUrl}/action-histories`)).pipe(
          Effect.flatMap((r) => r.json),
        );
  ...
  describe("ExecuteBudgetAction", () => {
    test.provider("the grant works — execute returns a typed budgets outcome", (_stack) =>
      Effect.gen(function* () {
        const response = yield* send(HttpClientRequest.get(`${baseUrl}/execute-reset`)).pipe(
          Effect.flatMap((r) => r.json),
        );
        expect(["ok", "ResourceLockedException", "InvalidParameterException"]).toContain(
