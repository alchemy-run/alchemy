...
test.provider(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const { scheduleName, taskDefinitionArn, clusterArn } = yield* stack.deploy(
        Effect.gen(function* () {
          ...
          const schedule = yield* every("alchemy-test-ecs-every-heartbeat", "cron(0 3 * * ? *)", {
            cluster,
            task,
            subnets,
          });
      ...
      expect(described.Target?.EcsParameters?.TaskDefinitionArn).toBe(taskDefinitionArn);
      expect(described.Target?.EcsParameters?.LaunchType).toBe("FARGATE");
      expect(
        described.Target?.EcsParameters?.NetworkConfiguration?.awsvpcConfiguration?.Subnets,
      ).toEqual(subnets);
