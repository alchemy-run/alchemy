...
//
// The fixture's `main` points at a temp project OUTSIDE the repository (so
// the bundle `cwd` — the nearest `package.json` — resolves nothing bare);
// its `main.ts` just re-exports the fixture by absolute path. We deploy it
// (Docker build + ECR push), launch it once on Fargate, and assert the task
// runs to completion with container exit code 0. With the bootstrap's
// imports left external the container dies at boot with
// `Cannot find module '@effect/platform-bun'` (exit code 1).
//
...
test.provider.skipIf(!process.env.AWS_TEST_SLOW || !!process.env.FAST)(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      // Materialise the isolated project: a package.json (so the bundle cwd
      // stops here) and a main.ts re-exporting the fixture's Layer.
      yield* fs.makeDirectory(isolatedProjectDir, { recursive: true });
      yield* fs.writeFileString(
        `${isolatedProjectDir}/package.json`,
        JSON.stringify({
          name: "alchemy-test-ecs-isolated-project",
          private: true,
          type: "module",
        }),
      );
      ...
      try {
        ...
        yield* fs
          .remove(isolatedProjectDir, { recursive: true })
          .pipe(Effect.ignore);
      }
    }),
  { timeout: 900_000 },
