  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const out = yield* stack.deploy(
        Effect.gen(function* () {
          // Public networking (no NAT): VPC + IGW + one public subnet with a
          // route to the IGW; the task gets a public IP to pull the image.
          const vpc = yield* Vpc("ByoVpc", {
            cidrBlock: "10.75.0.0/16",
            enableDnsSupport: true,
            enableDnsHostnames: true,
          });
          const igw = yield* InternetGateway("ByoIgw", { vpcId: vpc.vpcId });
          ...
          const routeTable = yield* RouteTable("ByoRouteTable", {
            vpcId: vpc.vpcId,
          });
          yield* Route("ByoRoute", {
            routeTableId: routeTable.routeTableId,
            destinationCidrBlock: "0.0.0.0/0",
            gatewayId: igw.internetGatewayId,
          });
          yield* RouteTableAssociation("ByoRouteAssoc", {
            routeTableId: routeTable.routeTableId,
