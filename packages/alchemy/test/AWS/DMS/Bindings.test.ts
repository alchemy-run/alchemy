...
describe.sequential("DMS Bindings", () => {
  beforeAll(
    Effect.gen(function* () {
      ...
      yield* Effect.logInfo(`DMS test setup: probing readiness at ${readinessUrl}`);
      yield* HttpClient.get(readinessUrl).pipe(
        ...
        Effect.tapError((error) =>
          Effect.logWarning(`DMS test setup: fixture not ready yet (${String(error)})`),
        ),
  ...
  describe("DescribeSchemas", () => {
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          const response = (yield* send(HttpClientRequest.get(`${baseUrl}/schemas`)).pipe(
            Effect.flatMap((r) => r.json),
          )) as {
            schemas: unknown[];
            fault: string | null;
          ...
          if (response.fault !== null) {
            expect(["ResourceNotFoundFault", "InvalidResourceStateFault"]).toContain(
              response.fault,
            );
          } else {
  ...
  describe("DescribeRefreshSchemasStatus", () => {
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          const response = (yield* send(HttpClientRequest.get(`${baseUrl}/refresh-status`)).pipe(
            Effect.flatMap((r) => r.json),
          )) as {
            status: string | null;
            fault: string | null;
  ...
  describe("DescribeEvents", () => {
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          const response = (yield* send(HttpClientRequest.get(`${baseUrl}/events`)).pipe(
            Effect.flatMap((r) => r.json),
          )) as { count: number };
  ...
  describe("DescribeEndpointSettings", () => {
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          const response = (yield* send(HttpClientRequest.get(`${baseUrl}/endpoint-settings`)).pipe(
            Effect.flatMap((r) => r.json),
          )) as { count: number };
  ...
  describe("DescribeOrderableReplicationInstances", () => {
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          const response = (yield* send(HttpClientRequest.get(`${baseUrl}/orderable`)).pipe(
            Effect.flatMap((r) => r.json),
          )) as { count: number };
          expect(response.count).toBeGreaterThan(0);
        }),
  ...
  describe("DescribeReplicationTasks", () => {
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          const response = (yield* send(HttpClientRequest.get(`${baseUrl}/tasks`)).pipe(
            Effect.flatMap((r) => r.json),
          )) as {
            count: number;
            fault: string | null;
  ...
  describe("StartReplicationTask", () => {
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          const response = (yield* send(HttpClientRequest.post(`${baseUrl}/task/start`)).pipe(
            Effect.flatMap((r) => r.json),
          )) as { fault: string };
          expect([
  ...
  describe("StopReplicationTask", () => {
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          const response = (yield* send(HttpClientRequest.post(`${baseUrl}/task/stop`)).pipe(
            Effect.flatMap((r) => r.json),
          )) as { fault: string };
          expect(["ResourceNotFoundFault", "InvalidResourceStateFault"]).toContain(response.fault);
        }),
      { timeout: 60_000 },
    );
  });
  ...
  describe("DescribeTableStatistics", () => {
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          const response = (yield* send(HttpClientRequest.get(`${baseUrl}/task/tables`)).pipe(
            Effect.flatMap((r) => r.json),
          )) as { fault: string };
          expect([
            "ResourceNotFoundFault",
  ...
  describe("ReloadTables", () => {
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          const response = (yield* send(HttpClientRequest.post(`${baseUrl}/task/reload`)).pipe(
            Effect.flatMap((r) => r.json),
          )) as { fault: string };
          expect(["ResourceNotFoundFault", "InvalidResourceStateFault"]).toContain(response.fault);
        }),
  ...
  describe("DescribeReplications", () => {
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          const response = (yield* send(HttpClientRequest.get(`${baseUrl}/replications`)).pipe(
            Effect.flatMap((r) => r.json),
          )) as {
            count: number;
            fault: string | null;
  ...
  describe("StopReplication", () => {
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          const response = (yield* send(HttpClientRequest.post(`${baseUrl}/replication/stop`)).pipe(
            Effect.flatMap((r) => r.json),
          )) as { fault: string };
          expect([
            "ResourceNotFoundFault",
    ...
    test.provider(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(rule?.EventPattern).toContain("DMS Replication Task State Change");
        }),
      { timeout: 60_000 },
    );
  });
