...
describe.sequential("SSM Bindings", () => {
  beforeAll(
    Effect.gen(function* () {
      ...
      yield* Effect.logInfo(`SSM test setup: probing readiness at ${readinessUrl}`);
      yield* HttpClient.get(readinessUrl).pipe(
        ...
        Effect.tapError((error) =>
          Effect.logWarning(`SSM test setup: fixture not ready yet (${String(error)})`),
        ),
  ...
  describe("GetParameter", () => {
    ...
    test.provider("decrypts a SecureString parameter through the binding", (_stack) =>
      Effect.gen(function* () {
  ...
  describe("GetParameters", () => {
    test.provider("reads String and SecureString parameters in one call", (_stack) =>
      Effect.gen(function* () {
        ...
        expect(values).toEqual(["bound-secret-value", "plain-config-value"].sort());
      }),
    );
  });

  describe("GetParameterHistory", () => {
    test.provider("reads the deployed version from the parameter's history", (_stack) =>
      Effect.gen(function* () {
  ...
  describe("PutParameter", () => {
    test.provider("writes a new version of the bound parameter at runtime", (_stack) =>
      Effect.gen(function* () {
  ...
  describe("Label/UnlabelParameterVersion", () => {
    test.provider("labels the latest version and removes the label again", (_stack) =>
      Effect.gen(function* () {
  ...
  describe("GetParametersByPath", () => {
    test.provider("reads the subtree under the bound path-root parameter", (_stack) =>
      Effect.gen(function* () {
        const response = (yield* getJson("/by-path")) as {
          parameters: Array<{ name: string; value: string }>;
        };
        const child = response.parameters.find(
          (parameter) => parameter.name === "/alchemy-test/ssm-bindings/root/child",
        );
    ...
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          ...
          yield* assertParameterDeleted("/alchemy-test/ssm-bindings/root/child");
        }),
      { timeout: 120_000 },
    );
  });
