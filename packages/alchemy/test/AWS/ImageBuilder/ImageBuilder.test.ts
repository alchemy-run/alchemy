...
test.provider("getComponent on a nonexistent ARN fails with ResourceNotFoundException", () =>
  Effect.gen(function* () {
...
test.provider("getImage/deleteImage on a nonexistent ARN fail with ResourceNotFoundException", () =>
  Effect.gen(function* () {
    const { accountId, region } = yield* AWSEnvironment.current;
    ...
    const deleteError = yield* Effect.flip(imagebuilder.deleteImage({ imageBuildVersionArn: arn }));
