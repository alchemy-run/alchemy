...
test.provider.skipIf(!!process.env.FAST)("stage lifecycle, settings updates, and list", (stack) =>
  Effect.gen(function* () {
    yield* stack.destroy();
    ...
    expect(
      all.some((item) => item.restApiId === stage.restApiId && item.stageName === stage.stageName),
    ).toBe(true);
