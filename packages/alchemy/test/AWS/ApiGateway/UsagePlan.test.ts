...
test.provider.skipIf(!!process.env.FAST)("create and delete usage plan", (stack) =>
  Effect.gen(function* () {
    yield* stack.destroy();
...
test.provider.skipIf(!!process.env.FAST)("usage plan throttle updates in place", (stack) =>
  Effect.gen(function* () {
    yield* stack.destroy();
...
test.provider.skipIf(!!process.env.FAST)("list enumerates the deployed usage plan", (stack) =>
  Effect.gen(function* () {
    yield* stack.destroy();
