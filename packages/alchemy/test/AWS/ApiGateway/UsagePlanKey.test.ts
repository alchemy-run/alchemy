...
test.provider.skipIf(!!process.env.FAST)("create and delete usage plan key association", (stack) =>
  Effect.gen(function* () {
    yield* stack.destroy();
...
test.provider.skipIf(!!process.env.FAST)("list enumerates the deployed usage plan key", (stack) =>
  Effect.gen(function* () {
    yield* stack.destroy();
