    ...
    test.provider(
      ...
      (stack) =>
        Effect.gen(function* () {
          ...
          expect(deployed.siteB.kvNamespace).not.toBe(
            deployed.siteA.kvNamespace,
          );
    ...
    test.provider(
      ...
      (stack) =>
        Effect.gen(function* () {
          ...
          const editedIndex = (yield* fs.readFileString(
            root.indexPath,
          )).replace("ROUTER_VITE_ROOT_PAGE", "ROUTER_VITE_ROOT_PAGE_V2");
