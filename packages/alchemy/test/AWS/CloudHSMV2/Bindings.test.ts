...
describe.sequential("CloudHSMV2 Bindings", () => {
  beforeAll(
    Effect.gen(function* () {
      yield* Effect.logInfo("CloudHSMV2 test setup: destroying previous resources");
      ...
      yield* Effect.logInfo(`CloudHSMV2 test setup: probing readiness at ${readinessUrl}`);
      yield* HttpClient.get(readinessUrl).pipe(
        ...
        Effect.tapError((error) =>
          Effect.logWarning(`CloudHSMV2 test setup: fixture not ready yet (${String(error)})`),
        ),
  ...
  describe("DescribeClusters", () => {
    test.provider("filter on a nonexistent id returns an empty page", (_stack) =>
      Effect.gen(function* () {
  ...
  describe("DeleteBackup", () => {
    test.provider("surfaces the typed not-found tag for a nonexistent backup", (_stack) =>
      Effect.gen(function* () {
        const response = yield* getJson("/backup-delete");
        expect((response as any).tag).toBe("CloudHsmResourceNotFoundException");
      }),
    );
  });
  ...
  describe("CopyBackupToRegion", () => {
    test.provider("surfaces a typed tag for a nonexistent backup (proving the grant)", (_stack) =>
      Effect.gen(function* () {
        const response = yield* getJson("/backup-copy");
        expect(["CloudHsmResourceNotFoundException", "CloudHsmInvalidRequestException"]).toContain(
          (response as any).tag,
        );
      }),
    );
  ...
  describe("InitializeCluster", () => {
    test.provider("surfaces a typed tag for a nonexistent cluster (proving the grant)", (_stack) =>
      Effect.gen(function* () {
  ...
  describe("GetResourcePolicy", () => {
    test.provider("reaches service-side validation (proving the grant)", (_stack) =>
      Effect.gen(function* () {
        const response = yield* getJson("/policy-get");
        expect(["CloudHsmInvalidRequestException", "CloudHsmResourceNotFoundException"]).toContain(
          (response as any).tag,
        );
      }),
    );
