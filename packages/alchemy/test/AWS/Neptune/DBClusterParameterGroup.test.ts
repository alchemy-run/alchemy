...
const assertGone = (name: string) =>
  neptune.describeDBClusterParameterGroups({ DBClusterParameterGroupName: name }).pipe(
...
test.provider(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const userParams = yield* readUserParameters(group.dbClusterParameterGroupName);
      expect(
        userParams.find((p) => p.ParameterName === "neptune_query_timeout")?.ParameterValue,
      ).toBe("180000");
      ...
      expect(updated.dbClusterParameterGroupName).toBe(group.dbClusterParameterGroupName);
      const updatedParams = yield* readUserParameters(group.dbClusterParameterGroupName);
      expect(
        updatedParams.find((p) => p.ParameterName === "neptune_query_timeout")?.ParameterValue,
      ).toBe("60000");
      ...
      const resetParams = yield* readUserParameters(group.dbClusterParameterGroupName);
      expect(resetParams.find((p) => p.ParameterName === "neptune_query_timeout")).toBeUndefined();
