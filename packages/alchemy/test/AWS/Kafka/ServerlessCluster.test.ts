...
test.provider("describeClusterV2 on a nonexistent cluster fails with NotFoundException", () =>
  Effect.gen(function* () {
...
test.provider("getBootstrapBrokers on a nonexistent cluster fails with a typed tag", () =>
  Effect.gen(function* () {
...
test.provider("listTopics on a nonexistent cluster fails with a typed tag", () =>
  Effect.gen(function* () {
    const ClusterArn = yield* nonexistentClusterArn;
...
test.provider("describeTopic on a nonexistent cluster fails with a typed tag", () =>
  Effect.gen(function* () {
    const ClusterArn = yield* nonexistentClusterArn;
    const error = yield* Effect.flip(
...
test.provider("createTopic on a nonexistent cluster fails with a typed tag", () =>
  Effect.gen(function* () {
    const ClusterArn = yield* nonexistentClusterArn;
    const error = yield* Effect.flip(
      Kafka.createTopic({
