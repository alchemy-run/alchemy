...
describe.sequential("Signer Bindings", () => {
  beforeAll(
    Effect.gen(function* () {
      ...
      yield* Effect.logInfo(`Signer test setup: probing readiness at ${readinessUrl}`);
      yield* HttpClient.get(readinessUrl).pipe(
        ...
        Effect.tapError((error) =>
          Effect.logWarning(`Signer test setup: fixture not ready yet (${String(error)})`),
        ),
  ...
  describe("ListSigningPlatforms / GetSigningPlatform", () => {
    test.provider("reads the AWS-managed platform catalog", (_stack) =>
      Effect.gen(function* () {
        ...
        const platform = (yield* getJson("/platform?id=AWSLambda-SHA384-ECDSA")) as {
          platformId: string;
    ...
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          ...
          if (!started.ok) {
            throw new Error(`StartSigningJob failed: ${started.tag}: ${started.message}`);
          }
          ...
          if (revocation.ok) {
            ...
            expect(["ValidationException", "BadRequestException"]).toContain(revocation.tag);
          }
  ...
  describe("SignPayload", () => {
    test.provider("synchronously signs a Notation payload with the bound profile", (_stack) =>
      Effect.gen(function* () {
        ...
        if (signed.ok) {
          ...
          expect(["ValidationException", "BadRequestException"]).toContain(signed.tag);
        }
      }),
    );
  });
