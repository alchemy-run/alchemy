...
test.provider("create and delete an HTTP health check with default name", (stack) =>
  Effect.gen(function* () {
...
test.provider("update mutable props in place (same id), then no-op redeploy", (stack) =>
  Effect.gen(function* () {
    const zoneId = yield* resolveZoneId;
...
test.provider("changing type HTTP→TCP updates in place; delete is idempotent", (stack) =>
  Effect.gen(function* () {
    const zoneId = yield* resolveZoneId;
...
test.provider("list enumerates the deployed health check across zones", (stack) =>
  Effect.gen(function* () {
    const zoneId = yield* resolveZoneId;
