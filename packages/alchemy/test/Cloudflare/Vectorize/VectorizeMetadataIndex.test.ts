...
const metaIndexPoll = Schedule.max([Schedule.spaced("5 seconds"), Schedule.recurs(48)]);
...
const multiMetaIndexPoll = Schedule.max([Schedule.spaced("5 seconds"), Schedule.recurs(66)]);
...
describe.skipIf(!!process.env.FAST)("Cloudflare.Vectorize.MetadataIndex", () => {
  test.provider(
      ...
      Effect.gen(function* () {
        ...
        const { index, meta } = yield* stack.deploy(
          Effect.gen(function* () {
            ...
            const meta = yield* Cloudflare.Vectorize.MetadataIndex("MetaIdx", {
        ...
        const entries = yield* poll({
          ...
          predicate: (entries) => entries.some((e) => e.propertyName === "category"),
          schedule: metaIndexPoll,
        });
        expect(entries.find((e) => e.propertyName === "category")?.indexType).toBe("String");
  ...
  test.provider(
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        expect(entries.find((e) => e.propertyName === "category")?.indexType).toBe("String");
        expect(entries.find((e) => e.propertyName === "price")?.indexType).toBe("Number");
      ...
      Effect.gen(function* () {
        ...
        const { index: newIndex, meta: newMeta } = yield* stack.deploy(
          Effect.gen(function* () {
            ...
            const meta = yield* Cloudflare.Vectorize.MetadataIndex("ReplaceMeta", {
              indexName: index.indexName,
              propertyName: "tag",
              indexType: "string",
            });
  ...
  test.provider(
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        const provider = yield* Provider.findProvider(Cloudflare.Vectorize.MetadataIndex);
        ...
        const all = yield* poll({
          ...
          predicate: (all) =>
            all.some((x) => x.indexName === index.indexName && x.propertyName === "category"),
          schedule: metaIndexPoll,
        });

        const entry = all.find(
          (x) => x.indexName === index.indexName && x.propertyName === "category",
        );
...
const listMetadataIndexes = Effect.fn(function* (accountId: string, indexName: string) {
  return yield* vectorize.listIndexMetadataIndexes({ accountId, indexName }).pipe(
