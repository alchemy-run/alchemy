...
// Cloudflare can route post-replacement events to the deleted subscription's Queue.
test.provider.skipIf(
  process.env.CLOUDFLARE_TEST_VECTORIZE_IMMEDIATE_REPLACEMENT !== "1",
)(
  "Vectorize immediately routes post-replacement events to the current subscription",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
      ...
      const create = (name: string) =>
        Effect.gen(function* () {
          ...
          yield* Effect.logInfo("Native replacement create", {
        ...
        Effect.gen(function* () {
          ...
          for (const message of batch.messages ?? []) {
            if (message.body) {
              ...
              yield* Effect.logInfo("Native replacement receipt", {
                elapsedMs: (yield* Clock.currentTimeMillis) - start,
                queueId,
                event,
              });
      ...
      const identity = yield* create(`vec-${b}-after-replace`);
      yield* Effect.gen(function* () {
        yield* pull(a);
        yield* pull(b);
      }).pipe(
      ...
      const deliveries = received.filter(
        ({ event }) => event.payload.name === identity,
      );
      yield* Effect.logInfo("Native replacement result", {
        identity,
        deliveries,
      });
      expect(
        deliveries.some(
          ({ queueId, event }) =>
            queueId === b && event.metadata.eventSubscriptionId === current.id,
        ),
      ).toBe(true);
