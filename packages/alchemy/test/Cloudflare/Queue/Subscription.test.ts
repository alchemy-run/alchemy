...
describe.sequential("Subscription", () => {
  test.provider("create r2 event subscription into a queue and destroy it", (stack) =>
    Effect.gen(function* () {
  ...
  test.provider("update mutable props in place (same subscriptionId)", (stack) =>
    Effect.gen(function* () {
      ...
      expect([...updated.subscription.events].sort()).toEqual(["bucket.created", "bucket.deleted"]);
  ...
  test.provider("replaces the subscription when the source changes", (stack) =>
    Effect.gen(function* () {
      ...
      expect(replaced.subscription.subscriptionId).not.toEqual(initial.subscription.subscriptionId);
  ...
  test.provider("list enumerates the deployed subscription", (stack) =>
    Effect.gen(function* () {
      ...
      expect(all.some((s) => s.subscriptionId === deployed.subscription.subscriptionId)).toBe(true);
