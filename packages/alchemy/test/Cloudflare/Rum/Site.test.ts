...
test.provider("create and delete a host (gray-clouded) site", (stack) =>
  Effect.gen(function* () {
    ...
    const site = yield* stack.deploy(
      Cloudflare.Rum.RumSite("HostSite", {
...
test.provider("update mutable props in place (same siteTag)", (stack) =>
  Effect.gen(function* () {
    ...
    const initial = yield* stack.deploy(
      Cloudflare.Rum.RumSite("UpdateSite", {
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const zoneSite = yield* stack.deploy(
        Cloudflare.Rum.RumSite("FlipSite", {
          zoneTag: zone.id,
          autoInstall: true,
        }),
      );
...
test.provider("list enumerates the deployed RUM site", (stack) =>
  Effect.gen(function* () {
    ...
    const deployed = yield* stack.deploy(
      Cloudflare.Rum.RumSite("ListSite", {
        host: `list.${zoneName}`,
      }),
    );

    const provider = yield* Provider.findProvider(Cloudflare.Rum.RumSite);
...
test.provider("recreates after out-of-band delete", (stack) =>
  Effect.gen(function* () {
    ...
    const site = yield* stack.deploy(
      Cloudflare.Rum.RumSite("HealSite", {
        host: `heal.${zoneName}`,
      }),
    );
