...
test.provider("list enumerates service bindings across prefixes (read-only)", (stack) =>
  Effect.gen(function* () {
    yield* stack.destroy();

    const provider = yield* Provider.findProvider(Cloudflare.Addressing.ServiceBinding);
