  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const connector = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* Cloudflare.Tunnel.TunnelWarpConnector("SiteA", {
...
test.provider(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const connector = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* Cloudflare.Tunnel.TunnelWarpConnector("ListSite", {
      ...
      const provider = yield* Provider.findProvider(
        Cloudflare.Tunnel.TunnelWarpConnector,
      );
