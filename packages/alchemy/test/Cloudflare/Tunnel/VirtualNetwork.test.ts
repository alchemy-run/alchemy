...
test.provider("create, verify, and destroy a virtual network", (stack) =>
  Effect.gen(function* () {
    ...
    const vnet = yield* stack.deploy(
      Cloudflare.Tunnel.TunnelVirtualNetwork("BasicVnet", {
...
test.provider("update name and comment in place (same id)", (stack) =>
  Effect.gen(function* () {
    ...
    const initial = yield* stack.deploy(
      Cloudflare.Tunnel.TunnelVirtualNetwork("UpdateVnet", {
...
test.provider("recreates after out-of-band delete", (stack) =>
  Effect.gen(function* () {
    ...
    const vnet = yield* stack.deploy(
      Cloudflare.Tunnel.TunnelVirtualNetwork("HealVnet", {
...
test.provider("list enumerates the deployed virtual network", (stack) =>
  Effect.gen(function* () {
    ...
    const deployed = yield* stack.deploy(
      Cloudflare.Tunnel.TunnelVirtualNetwork("ListVnet", {
    ...
    const provider = yield* Provider.findProvider(
      Cloudflare.Tunnel.TunnelVirtualNetwork,
    );
