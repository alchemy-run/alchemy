...
const rawConnect = Effect.fn(function* (url: string) {
    ...
    Effect.sync(() => {
      ...
      socket.addEventListener("error", (event) =>
        Deferred.doneUnsafe(
          ...
          Exit.fail(
            new Error(
              `WebSocket handshake failed: ${"message" in event ? event.message : event.type}`,
            ),
          ),
        ),
      );
...
describe.concurrent.each([
  ...
  const stack = beforeAll(
    Effect.gen(function* () {
      ...
      const http = yield* HttpClient.HttpClient;
      yield* http.get(`${output.url}/ready`).pipe(
      ...
      yield* rawConnect(`${output.url}/rpc/ready`).pipe(
        Effect.scoped,
        Effect.retry({ schedule: Schedule.spaced("1 second"), times: 8 }),
      );
      return output;
  ...
  test(
    ...
    Effect.gen(function* () {
      ...
      const http = yield* HttpClient.HttpClient;
      const response = yield* http.get(`${url}/stats/roundtrip`);
      expect(response.status).toBe(200);
  ...
  test(
    ...
    Effect.gen(function* () {
      ...
      const http = yield* HttpClient.HttpClient;
      const status = yield* http.get(`${url}/stats/disconnect`).pipe(
      ...
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          if (mode === "unary") {
            ...
            const response = yield* http.post(`${url}/release/${name}/${mode}`);
            expect(response.status).toBe(200);
