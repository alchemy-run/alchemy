...
layer(NodeServices.layer)("readPrebuiltWorkerBundle", (it) => {
  ...
  it.effect(
    "uploads file contents byte-for-byte",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
  ...
  it.effect(
    "does not walk outside the entry's directory",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
  ...
  it.effect(
    "hash is stable across reads and sensitive to module changes",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
  ...
  it.effect(
    "fails with BundleError when the entry does not exist",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
