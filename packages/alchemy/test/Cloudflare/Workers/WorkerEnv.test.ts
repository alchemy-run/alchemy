...
const CONFIG_REDACTED_VALUE = (process.env.CONFIG_REDACTED = "config-redacted-value");
...
const CONFIG_NESTED_HOST_VALUE = (process.env.CONFIG_NESTED_HOST = "config-nested-host-value");
...
describe.concurrent("Cloudflare.Worker env bindings", () => {
  ...
  test.provider("Output env values classify by resolved value on the wire", () =>
    Effect.gen(function* () {
  ...
  test(
    ...
    Effect.gen(function* () {
      ...
      const body = yield* expectUrlContains(`${effectUrl}/env`, '"STR":"hello"', {
        timeout: "60 seconds",
  ...
  test(
    ...
    Effect.gen(function* () {
      ...
      const body = yield* expectUrlContains(`${effectUrl}/config-runtime`, '"CONFIG_STR"', {
        timeout: "60 seconds",
  ...
  test(
    ...
    Effect.gen(function* () {
      ...
      const body = yield* expectUrlContains(`${effectUrl}/config`, '"CONFIG_STR"', {
        timeout: "60 seconds",
