...
const bundleWorkerEntry = Effect.fn(function* (entry: string, generated = false) {
  ...
  const graph = new Map<string, { isEntry: boolean; importedIds: readonly string[] }>();
    ...
    Effect.promise(() =>
      rolldown({
        ...
        plugins: [
          ...
          generated
            ? virtualEntry(makeEffectVirtualEntry({}, { name: "runtime-entry", stage: "test" }))
            : undefined,
          {
            ...
            moduleParsed(info) {
              graph.set(info.id, {
                ...
                importedIds: [...info.importedIds, ...info.dynamicallyImportedIds],
              });
            },
          },
        ],
    ...
    (bundle) =>
      Effect.gen(function* () {
        ...
        for (let f = chunkQueue.shift(); f !== undefined; f = chunkQueue.shift()) {
          ...
          chunkQueue.push(...chunks.get(f)!.imports, ...chunks.get(f)!.dynamicImports);
        }
        const code = [...reachableChunks].map((f) => chunks.get(f)!.code).join("\n");
        ...
        const packages = new Set([...modules].map(packageOf).filter((name) => name !== undefined));
...
layer(NodeServices.layer)("alchemy/Cloudflare/Bridge (runtime-only entry)", (it) => {
  it.effect("exports the bridge factories the generated Worker entry imports", () =>
    Effect.gen(function* () {
      const { exports } = yield* bundleWorkerEntry("src/Cloudflare/Bridge.ts");
  ...
  it.effect("keeps planner tooling out of /Bridge's dependency graph", () =>
    Effect.gen(function* () {
      const { code, sources, packages } = yield* bundleWorkerEntry("src/Cloudflare/Bridge.ts");
  ...
  it.effect("keeps planner tooling out of imports added by the generated wrapper", () =>
    Effect.gen(function* () {
  ...
  it.effect("the alchemy/Cloudflare namespace does pull that tooling", () =>
    Effect.gen(function* () {
      const { code, sources, packages } = yield* bundleWorkerEntry("src/Cloudflare/index.ts");
  ...
  it.effect("detects local tooling behind a dynamic import", () =>
    Effect.gen(function* () {
      ...
      expect(sources.filter(isPlannerSource)).toContain("src/Cloudflare/LocalRuntime.ts");
      expect(packages.has("workerd")).toBe(true);
      expect(code).toContain("require.resolve");
    }),
  );
  ...
  it.effect("preserves the public runtime service identities", () =>
    Effect.sync(() => {
      expect(Cloudflare.WorkerEnvironment).toBe(WorkerRuntime.WorkerEnvironment);
      expect(Cloudflare.WorkerExecutionContext).toBe(WorkerRuntime.WorkerExecutionContext);
      expect(Cloudflare.Workflows.WorkflowEvent).toBe(WorkflowRuntime.WorkflowEvent);
      expect(Cloudflare.Workflows.WorkflowStep).toBe(WorkflowRuntime.WorkflowStep);
      expect(Cloudflare.Workflows.WorkflowStepContext).toBe(WorkflowRuntime.WorkflowStepContext);
  ...
  it.effect("alchemy/Cloudflare still re-exports the same factories", () =>
    Effect.sync(() => {
      ...
      expect(Cloudflare.makeDurableObjectBridge).toBe(Bridge.makeDurableObjectBridge);
  ...
  describe(`runtime entry (${dev ? "local" : "live"})`, () => {
    ...
    test.provider("application importing Worker.ts serves requests", (stack) =>
      Effect.gen(function* () {
        ...
        const response = yield* requestWorker(HttpClientRequest.get(worker.url!), {
