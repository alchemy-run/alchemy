 ...
 *
 * Exercises:
 *  - `Cloudflare.Workers.task` durable steps (greet + finalize)
 *  - `Cloudflare.Workers.sleep` between steps
...
export default class TestWorkflow extends Cloudflare.Workflow<TestWorkflow>()(
  ...
  Effect.gen(function* () {
    return Effect.fn(function* (input: { value: string }) {
      ...
      const greeted = yield* Cloudflare.Workers.task(
        "greet",
        Effect.succeed(`Hello, ${input.value}!`),
      );

      yield* Cloudflare.Workers.sleep("cooldown", "1 second");

      const finalized = yield* Cloudflare.Workers.task(
        "finalize",
        Effect.succeed({
          greeting: greeted,
