...
test.provider("create and delete a zone-scoped ip rule", (stack) =>
  Effect.gen(function* () {
    ...
    const rule = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Firewall.FirewallAccessRule("DefaultRule", {
          zoneId,
...
test.provider("update mode and notes in place (same ruleId)", (stack) =>
  Effect.gen(function* () {
    ...
    const initial = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Firewall.FirewallAccessRule("UpdateRule", {
          zoneId,
...
test.provider("changing the configuration triggers replacement", (stack) =>
  Effect.gen(function* () {
    ...
    const initial = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Firewall.FirewallAccessRule("ReplaceRule", {
          zoneId,
...
test.provider("list enumerates the deployed zone-scoped rule", (stack) =>
  Effect.gen(function* () {
    ...
    const rule = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Firewall.FirewallAccessRule("ListRule", {
          zoneId,
    ...
    const provider = yield* Provider.findProvider(
      Cloudflare.Firewall.FirewallAccessRule,
    );
...
test.provider(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const rule = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* Cloudflare.Firewall.FirewallAccessRule("AccountRule", {
