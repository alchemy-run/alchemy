 ...
 * written or dropped.
 */
test.provider("adopts a drizzle-kit-migrated database via one-way conversion", (stack) =>
  Effect.gen(function* () {
    const { accountId } = yield* yield* CloudflareEnvironment;
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
 ...
 */
test.provider("adopts a wrangler-migrated database via one-way conversion", (stack) =>
  Effect.gen(function* () {
    const { accountId } = yield* yield* CloudflareEnvironment;
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
 ...
 */
test.provider("converts a legacy 2-column d1_migrations table into __alchemy_migrations", (stack) =>
  Effect.gen(function* () {
    const { accountId } = yield* yield* CloudflareEnvironment;
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    ...
    expect(columns.map((c) => c.name)).toEqual(["id", "hash", "created_at", "name", "applied_at"]);
...
test.provider("clones a database by passing the source resource directly", (stack) =>
  Effect.gen(function* () {
    const { accountId } = yield* yield* CloudflareEnvironment;
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
...
// adoption.
test.provider("existing database (matching name) is silently adopted without --adopt", (stack) =>
  Effect.gen(function* () {
    const { accountId } = yield* yield* CloudflareEnvironment;
