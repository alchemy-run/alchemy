...
test.provider("pins a toggle setting and restores the original value on destroy", (stack) =>
  Effect.gen(function* () {
...
test.provider("updates a numeric setting in place and keeps the captured initial value", (stack) =>
  Effect.gen(function* () {
    const zoneId = yield* resolveZoneId;
...
test.provider("changing settingId replaces — old setting restored, new setting pinned", (stack) =>
  Effect.gen(function* () {
    const zoneId = yield* resolveZoneId;
