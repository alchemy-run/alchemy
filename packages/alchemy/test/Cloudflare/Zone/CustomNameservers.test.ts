  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const customNs = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* Cloudflare.Zone.ZoneCustomNameservers("CustomNs", {
            zoneId,
            enabled: false,
          });
        }),
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const provider = yield* Provider.findProvider(
        Cloudflare.Zone.ZoneCustomNameservers,
      );
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const enabled = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* Cloudflare.Zone.ZoneCustomNameservers("AcnsList", {
            zoneId,
            enabled: true,
          });
        }),
...
test.provider.skipIf(!acnsZoneId)(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const enabled = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* Cloudflare.Zone.ZoneCustomNameservers("AcnsToggle", {
            zoneId,
            enabled: true,
          });
        }),
