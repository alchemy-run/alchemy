...
test.provider("create, verify, and destroy a location", (stack) =>
  Effect.gen(function* () {
    ...
    const location = yield* stack.deploy(
      Cloudflare.Gateway.GatewayLocation("BasicLocation", {
...
test.provider("update name and ecsSupport in place (same id)", (stack) =>
  Effect.gen(function* () {
    ...
    const initial = yield* stack.deploy(
      Cloudflare.Gateway.GatewayLocation("UpdateLocation", {
...
test.provider("recreates after out-of-band delete", (stack) =>
  Effect.gen(function* () {
    ...
    const location = yield* stack.deploy(
      Cloudflare.Gateway.GatewayLocation("HealLocation", {
...
test.provider("list enumerates deployed gateway locations", (stack) =>
  Effect.gen(function* () {
    ...
    const location = yield* stack.deploy(
      Cloudflare.Gateway.GatewayLocation("ListLocation", {
    ...
    const provider = yield* Provider.findProvider(
      Cloudflare.Gateway.GatewayLocation,
    );
