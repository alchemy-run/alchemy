...
describe.sequential("HostnameAssociation", () => {
  ...
  test.provider("associates hostnames with an uploaded CA and destroys before the cert", (stack) =>
    Effect.gen(function* () {
      const { accountId } = yield* yield* CloudflareEnvironment;
      const zoneId = yield* resolveZoneId;
      ...
      const { cert, assoc } = yield* stack.deploy(
        Effect.gen(function* () {
          ...
          const assoc = yield* Cloudflare.CertificateAuthorities.HostnameAssociation("CaHosts", {
            zoneId,
  ...
  test.provider("changing the certificate key replaces the association", (stack) =>
    Effect.gen(function* () {
      const { accountId } = yield* yield* CloudflareEnvironment;
      const zoneId = yield* resolveZoneId;
      ...
      yield* waitForHostnames(zoneId, [`mtls-replace.${zoneName}`], second.cert.mtlsCertificateId);
