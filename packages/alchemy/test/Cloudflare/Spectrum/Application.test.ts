  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const app = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* Cloudflare.Spectrum.SpectrumApplication("Ssh", {
            zoneId,
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const error = yield* stack
        .deploy(
          Effect.gen(function* () {
            return yield* Cloudflare.Spectrum.SpectrumApplication("Adopted", {
              zoneId,
              dns: { type: "CNAME", name: dnsName },
              protocol: "tcp/22",
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const provider = yield* Provider.findProvider(
        Cloudflare.Spectrum.SpectrumApplication,
      );
...
test.provider.skipIf(!entitledZoneId)(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const app = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* Cloudflare.Spectrum.SpectrumApplication("Listed", {
            zoneId,
            dns: { type: "CNAME", name: dnsName },
            protocol: "tcp/22",
            originDirect: ["tcp://192.0.2.1:22"],
