  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const op = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* Cloudflare.ApiShield.ApiShieldOperation("DefaultOp", {
            zoneId,
            method: "GET",
            host: zoneName,
            endpoint: ENDPOINT_DEFAULT,
...
test.provider("changing the method triggers replacement", (stack) =>
  Effect.gen(function* () {
    ...
    const initial = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.ApiShield.ApiShieldOperation("ReplaceOp", {
          zoneId,
          method: "GET",
          host: zoneName,
          endpoint: ENDPOINT_REPLACE,
...
test.provider("list enumerates the deployed API Shield operation", (stack) =>
  Effect.gen(function* () {
    ...
    const op = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.ApiShield.ApiShieldOperation("ListOp", {
          zoneId,
          method: "GET",
          host: zoneName,
          endpoint: ENDPOINT_LIST,
    ...
    const provider = yield* Provider.findProvider(
      Cloudflare.ApiShield.ApiShieldOperation,
    );
