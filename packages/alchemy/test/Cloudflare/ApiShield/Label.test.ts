...
test.provider("create, update description in place, destroy a label", (stack) =>
  Effect.gen(function* () {
    ...
    const label = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.ApiShield.ApiShieldLabel("DefaultLabel", {
          zoneId,
          name: NAME_DEFAULT,
          description: "v1",
        }).pipe(adopt(true));
...
test.provider("renaming a label triggers replacement", (stack) =>
  Effect.gen(function* () {
    ...
    const initial = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.ApiShield.ApiShieldLabel("RenameLabel", {
          zoneId,
          name: NAME_RENAME_A,
...
test.provider(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const label = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* Cloudflare.ApiShield.ApiShieldLabel(
            "GeneratedNameLabel",
            {
              zoneId,
            },
...
test.provider("list enumerates the deployed label", (stack) =>
  Effect.gen(function* () {
    ...
    const deployed = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.ApiShield.ApiShieldLabel("ListLabel", {
          zoneId,
          name: NAME_LIST,
          description: "listed",
        }).pipe(adopt(true));
    ...
    const provider = yield* Provider.findProvider(
      Cloudflare.ApiShield.ApiShieldLabel,
    );
