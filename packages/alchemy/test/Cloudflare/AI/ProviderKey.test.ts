...
test.provider("provisions a Secret + GatewayProvider with the correct naming contract", (stack) =>
  Effect.gen(function* () {
    yield* stack.destroy();
...
test.provider("value rotation updates in place; alias change cascades a replacement", (stack) =>
  Effect.gen(function* () {
    yield* stack.destroy();
...
test.provider("destroying the provider key removes the secret from the surviving store", (stack) =>
  Effect.gen(function* () {
    yield* stack.destroy();
