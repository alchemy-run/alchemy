...
test.provider(
  "create and delete a widget with default name",
  (stack) =>
    Effect.gen(function* () {
  ...
  { tags: ["provider:cloudflare", "provider:cloudflare:turnstile", "live"] },
);

test.provider(
  "update mutable props in place (same sitekey)",
  (stack) =>
    Effect.gen(function* () {
      const { accountId } = yield* yield* CloudflareEnvironment;
...
test.provider(
  "list enumerates the deployed widget",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
