...
test.provider("create and delete a widget with default name", (stack) =>
  Effect.gen(function* () {
    ...
    const widget = yield* stack.deploy(
      Cloudflare.Turnstile.TurnstileWidget("DefaultWidget", {
        domains: [zoneName],
        mode: "managed",
      }),
    );
...
test.provider("update mutable props in place (same sitekey)", (stack) =>
  Effect.gen(function* () {
    ...
    const initial = yield* stack.deploy(
      Cloudflare.Turnstile.TurnstileWidget("UpdateWidget", {
...
test.provider("recreates after out-of-band delete", (stack) =>
  Effect.gen(function* () {
    ...
    const widget = yield* stack.deploy(
      Cloudflare.Turnstile.TurnstileWidget("HealWidget", {
...
test.provider("list enumerates the deployed widget", (stack) =>
  Effect.gen(function* () {
    ...
    const widget = yield* stack.deploy(
      Cloudflare.Turnstile.TurnstileWidget("ListWidget", {
    ...
    const provider = yield* Provider.findProvider(
      Cloudflare.Turnstile.TurnstileWidget,
    );
