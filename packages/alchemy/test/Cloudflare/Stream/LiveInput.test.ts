  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const input = yield* stack.deploy(
        Cloudflare.Stream.StreamLiveInput("BroadcastInput", {
...
test.provider(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const input = yield* stack.deploy(
        Cloudflare.Stream.StreamLiveInput("HealInput", {
...
test.provider.skipIf(!process.env.CLOUDFLARE_TEST_STREAM_LIST)(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const input = yield* stack.deploy(
        Cloudflare.Stream.StreamLiveInput("ListInput", {
          meta: { name: "alchemy-stream-list-input" },
        }),
      );

      const provider = yield* Provider.findProvider(
        Cloudflare.Stream.StreamLiveInput,
      );
