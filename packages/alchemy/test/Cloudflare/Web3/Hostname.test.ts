...
test.provider("list returns a well-typed array of web3 hostnames", (stack) =>
  Effect.gen(function* () {
    const provider = yield* Provider.findProvider(Cloudflare.Web3.Web3Hostname);
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const hostname = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* Cloudflare.Web3.Web3Hostname("ListHostname", {
            zoneId: zone.id,
            name,
