  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const created = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* Cloudflare.Web3.Web3Hostname("Gateway", {
            zoneId,
            name,
            target: "ipfs",
            dnslink: "/ipns/onboarding.ipfs.cloudflare.com",
...
test.provider.skipIf(!entitledZoneId)(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const program = (entries: Cloudflare.Web3.Web3ContentListEntry[]) =>
        Effect.gen(function* () {
          const gateway = yield* Cloudflare.Web3.Web3Hostname("Universal", {
            zoneId,
            name,
            target: "ipfs_universal_path",
          });
          const list = yield* Cloudflare.Web3.Web3HostnameContentList(
            "Blocklist",
            {
              zoneId,
