  ...
  (stack) =>
    Effect.gen(function* () {
      const provider = yield* Provider.findProvider(
        Cloudflare.Web3.Web3HostnameContentList,
      );
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const deployed = yield* stack.deploy(
        Effect.gen(function* () {
          const gateway = yield* Cloudflare.Web3.Web3Hostname("ListGateway", {
            zoneId: zone.id,
            name,
            target: "ipfs_universal_path",
          }).pipe(adopt(true));
          const contentList = yield* Cloudflare.Web3.Web3HostnameContentList(
            "ListBlocklist",
            {
              zoneId: zone.id,
