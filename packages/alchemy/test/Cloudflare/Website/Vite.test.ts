...
describe.concurrent(
  "Vite",
  { tags: ["provider:cloudflare", "provider:cloudflare:website"] },
  () => {
    test.provider(
...
`,
      ...
      (stack) =>
        Effect.gen(function* () {
          ...
          expect(render.html).toContain(
            "Worker render via the ssr environment.",
          );
    ...
    devTest.provider(
      ...
      { tags: ["provider:cloudflare:worker", "local"], timeout: 120_000 },
    );
    ...
    devTest.provider(
      ...
      { tags: ["provider:cloudflare:r2", "local"], timeout: 360_000 },
    );
  },
);
