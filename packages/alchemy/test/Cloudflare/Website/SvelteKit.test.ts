...
describe.concurrent(
  ...
  () => {
    test.provider(
        ...
        Effect.gen(function* () {
          ...
          const kvRead = yield* fetchJsonReady<{ value: string | null }>(
            ...
            Effect.filterOrFail(
              (r) => r.value === kvValue,
              (r) =>
                new Error(`kv value not visible yet: ${JSON.stringify(r)}`),
            ),
            Effect.retry({
      ...
      { tags: ["provider:cloudflare:kv"], timeout: 360_000 },
    );
    ...
    class NamespaceStillExists extends Data.TaggedError(
      "NamespaceStillExists",
    ) {}
    ...
    class PageResponseMismatch extends Data.TaggedError(
      "PageResponseMismatch",
    )<{
      url: string;
      expected: string;
      actual: string;
          ...
          .pipe(
            Effect.flatMap((res) =>
              res.status === 200
                ? res.text
                : Effect.fail(
                    new Error(`form action not ready: ${res.status}`),
                  ),
            ),
            Effect.retry({
              schedule: Schedule.min([
