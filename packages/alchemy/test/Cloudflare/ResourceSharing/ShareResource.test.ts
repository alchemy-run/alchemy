...
test.provider(
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const provider = yield* Provider.findProvider(Cloudflare.ResourceSharing.ShareResource);
      ...
      if (recipientAccountId) {
        ...
        expect(all.some((e) => e.shareResourceId === entry.shareResourceId)).toBe(true);
        // The deployed entry surfaces in the read Attributes shape.
        const found = all.find((e) => e.shareResourceId === entry.shareResourceId)!;
