    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        const domain = yield* stack.deploy(
          Effect.gen(function* () {
            return yield* Cloudflare.Registrar.RegistrarDomain("Domain", props);
          }),
        );
  ...
  test.provider(
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        const domain = yield* stack.deploy(
          Effect.gen(function* () {
            return yield* Cloudflare.Registrar.RegistrarDomain("Domain", {
              domainName,
              autoRenew: flipped,
            });
          }),
  ...
  test.provider("list enumerates registrar domains on the account", (stack) =>
    Effect.gen(function* () {
      ...
      const provider = yield* Provider.findProvider(
        Cloudflare.Registrar.RegistrarDomain,
      );
