...
test.provider("create, verify, and destroy an OIDC IdP", (stack) =>
  Effect.gen(function* () {
    ...
    const idp = yield* stack.deploy(
      Cloudflare.Access.AccessIdentityProvider("BasicOidc", {
...
test.provider("update name and config in place (same id)", (stack) =>
  Effect.gen(function* () {
    ...
    const initial = yield* stack.deploy(
      Cloudflare.Access.AccessIdentityProvider("UpdateOidc", {
...
test.provider("list enumerates the deployed IdP", (stack) =>
  Effect.gen(function* () {
    ...
    const deployed = yield* stack.deploy(
      Cloudflare.Access.AccessIdentityProvider("ListOidc", {
    ...
    const provider = yield* Provider.findProvider(
      Cloudflare.Access.AccessIdentityProvider,
    );
...
test.provider("changing the type replaces the IdP", (stack) =>
  Effect.gen(function* () {
    ...
    const oidc = yield* stack.deploy(
      Cloudflare.Access.AccessIdentityProvider("ReplaceIdp", {
