...
test.provider("create, verify out-of-band, and destroy tag", (stack) =>
  Effect.gen(function* () {
    ...
    const tag = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Access.AccessTag("BasicTag", {});
      }),
    );
...
test.provider("rename replaces the tag", (stack) =>
  Effect.gen(function* () {
    ...
    const tag = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Access.AccessTag("RenameTag", {
...
test.provider("list enumerates the deployed access tag", (stack) =>
  Effect.gen(function* () {
    ...
    const tag = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Access.AccessTag("ListTag", {
          name: "alchemy-test-access-tag-list",
        });
      }),
    );

    const provider = yield* Provider.findProvider(Cloudflare.Access.AccessTag);
