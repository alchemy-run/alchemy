...
test.provider("create, update rules, and delete group", (stack) =>
  Effect.gen(function* () {
    ...
    const group = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Access.AccessGroup("BasicGroup", {
...
test.provider("rename updates the group in place", (stack) =>
  Effect.gen(function* () {
    ...
    const group = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Access.AccessGroup("RenameGroup", {
...
test.provider("list enumerates the deployed access group", (stack) =>
  Effect.gen(function* () {
    ...
    const group = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Access.AccessGroup("ListGroup", {
          include: [{ emailDomain: { domain: "example.com" } }],
        });
      }),
    );
...
test.provider("group can be referenced from an access policy", (stack) =>
  Effect.gen(function* () {
    ...
    const { group, policy } = yield* stack.deploy(
      Effect.gen(function* () {
        const group = yield* Cloudflare.Access.AccessGroup("PolicyGroup", {
          include: [{ emailDomain: { domain: "example.com" } }],
        });
        const policy = yield* Cloudflare.Access.AccessPolicy("GroupPolicy", {
          decision: "allow",
