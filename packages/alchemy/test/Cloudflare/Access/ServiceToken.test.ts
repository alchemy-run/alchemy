...
test.provider("create, update duration, and delete service token", (stack) =>
  Effect.gen(function* () {
    ...
    const token = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Access.AccessServiceToken("BasicToken", {});
      }),
    );
...
test.provider("list enumerates the deployed service token", (stack) =>
  Effect.gen(function* () {
    ...
    const token = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Access.AccessServiceToken("ListToken", {});
      }),
    );

    const provider = yield* Provider.findProvider(
      Cloudflare.Access.AccessServiceToken,
    );
...
test.provider("incrementing clientSecretVersion rotates the secret", (stack) =>
  Effect.gen(function* () {
    ...
    const token = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Access.AccessServiceToken("RotateToken", {});
      }),
    );
    expect(token.clientSecret).toBeDefined();
