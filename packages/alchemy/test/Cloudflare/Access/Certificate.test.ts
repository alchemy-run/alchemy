  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const cert = yield* stack.deploy(
        Effect.gen(function* () {
          return yield* Cloudflare.Access.AccessCertificate("BasicCert", {
            certificate: CA_CERT_1,
          });
        }),
      );
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const provider = yield* Provider.findProvider(
        Cloudflare.Access.AccessCertificate,
      );

      if (entitled) {
        const deployed = yield* stack.deploy(
          Effect.gen(function* () {
            return yield* Cloudflare.Access.AccessCertificate("ListCert", {
              certificate: CA_CERT_1,
            });
          }),
        );
