...
test.provider("create and delete basic allow policy", (stack) =>
  Effect.gen(function* () {
    ...
    const policy = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Access.AccessPolicy("BasicAllowPolicy", {
          decision: "allow",
...
test.provider("update mutates includes without replacing", (stack) =>
  Effect.gen(function* () {
    ...
    const initial = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Access.AccessPolicy("UpdatePolicy", {
          decision: "allow",
          include: [{ emailDomain: { domain: "example.com" } }],
          adopt: true,
        });
...
test.provider("adopts an out-of-band reusable policy", (stack) =>
  Effect.gen(function* () {
    ...
    const adopted = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Access.AccessPolicy("AdoptPolicy", {
          name,
          decision: "allow",
          include: [{ emailDomain: { domain: "example.com" } }],
          adopt: true,
...
test.provider("list enumerates the deployed reusable policy", (stack) =>
  Effect.gen(function* () {
    ...
    const policy = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Access.AccessPolicy("ListPolicy", {
          decision: "allow",
          include: [{ emailDomain: { domain: "example.com" } }],
        });
      }),
