...
test.provider("create and delete an A record with default props", (stack) =>
  Effect.gen(function* () {
    ...
    const record = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Dns.DnsRecord("DefaultA", {
          zoneId,
          name: NAME_DEFAULT,
          type: "A",
...
test.provider("updating mutable fields patches in place", (stack) =>
  Effect.gen(function* () {
    ...
    const initial = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Dns.DnsRecord("UpdateA", {
          zoneId,
          name: NAME_UPDATE,
          type: "A",
...
test.provider("changing the record type triggers replacement", (stack) =>
  Effect.gen(function* () {
    ...
    const initial = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Dns.DnsRecord("ReplaceRecord", {
          zoneId,
          name: NAME_REPLACE,
          type: "A",
  ...
  (stack) =>
    Effect.gen(function* () {
      ...
      const error = yield* stack
        .deploy(
          Effect.gen(function* () {
            return yield* Cloudflare.Dns.DnsRecord("AdoptedRecord", {
              zoneId,
              name: NAME_ADOPT,
              type: "A",
...
test.provider("list enumerates the deployed DNS record", (stack) =>
  Effect.gen(function* () {
    ...
    const record = yield* stack.deploy(
      Effect.gen(function* () {
        return yield* Cloudflare.Dns.DnsRecord("ListedA", {
          zoneId,
          name: NAME_LIST,
          type: "A",
    ...
    const provider = yield* Provider.findProvider(Cloudflare.Dns.DnsRecord);
