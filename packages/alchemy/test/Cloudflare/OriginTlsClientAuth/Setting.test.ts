    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        const setting = yield* stack.deploy(
          Cloudflare.OriginTlsClientAuth.OriginTlsClientAuthSetting("Aop", {
            zoneId,
            enabled: true,
          }),
        );
  ...
  test.provider("updates the enabled flag in place", (stack) =>
    Effect.gen(function* () {
      ...
      const enabled = yield* stack.deploy(
        Cloudflare.OriginTlsClientAuth.OriginTlsClientAuthSetting("AopUpdate", {
          zoneId,
          enabled: true,
        }),
      );
  ...
  test.provider("list enumerates the setting across all zones", (stack) =>
    Effect.gen(function* () {
      ...
      const provider = yield* Provider.findProvider(
        Cloudflare.OriginTlsClientAuth.OriginTlsClientAuthSetting,
      );
