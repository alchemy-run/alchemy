...
describe.sequential("AiGatewayDataset", () => {
  test.provider.skipIf(SKIP_NON_EPHEMRAL_ACCOUNT_TESTS)(
    "create, update, delete a dataset on a gateway",
    (stack) =>
      Effect.gen(function* () {
  ...
  test.provider.skipIf(SKIP_NON_EPHEMRAL_ACCOUNT_TESTS)(
    "replaces dataset when the gateway changes",
    (stack) =>
      Effect.gen(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        ...
        const initial = yield* stack.deploy(
          Effect.gen(function* () {
            ...
            const dataset = yield* Cloudflare.AiGatewayDataset(
              "ReplaceDataset",
              {
  ...
  test.provider.skipIf(SKIP_NON_EPHEMRAL_ACCOUNT_TESTS)(
    "recreates a dataset after out-of-band delete",
    (stack) =>
      Effect.gen(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
  ...
  test.provider.skipIf(SKIP_NON_EPHEMRAL_ACCOUNT_TESTS)(
    "list enumerates datasets across gateways",
    (stack) =>
      Effect.gen(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
        ...
        expect(
          all.some((d) => d.datasetId === deployed.dataset.datasetId),
        ).toBe(true);
