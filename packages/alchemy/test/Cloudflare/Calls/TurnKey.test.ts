...
test.provider(
  "create and delete a TURN key with default name",
  (stack) =>
    Effect.gen(function* () {
  ...
  { tags: ["provider:cloudflare", "provider:cloudflare:calls", "live"] },
);

test.provider(
  "update name in place (same keyId, key preserved)",
  (stack) =>
    Effect.gen(function* () {
      const { accountId } = yield* yield* CloudflareEnvironment;
...
test.provider(
  "list enumerates the deployed TURN key",
  (stack) =>
    Effect.gen(function* () {
      yield* stack.destroy();
