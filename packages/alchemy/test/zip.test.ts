...
test(
  "zipCode is deterministic for nested package paths",
  async () => {
    const build = () =>
      Effect.runPromise(
        zipCode("export default 1", [
...
test(
  "zipFiles preserves binary data, Unicode, empty files, and input order independence",
  async () => {
    const files = [
...
test(
  "zipFiles records fixed timestamps and Unix file types and permissions",
  async () => {
