...
layer(NodeServices.layer)("Bundle.build default text module types", (it) => {
  it.effect(
    "inlines a bare .sql import as a string default export",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "bundles a drizzle-kit durable-sqlite migrations layout",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
  ...
  it.effect(
    "inlines bare .txt and .html imports as strings",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
  ...
  it.effect(
    "caller-provided moduleTypes override the defaults",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
