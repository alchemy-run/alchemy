/**
 * Dashboard v2 document core: the pure fold (events → document), minimal
...
// ── real journal fixture ───────────────────────────────────────────────────

describe("document fold — real deploy journal", () => {
  test.provider("folds a live deploy journal into a document", (stack) =>
    Effect.gen(function* () {
      yield* stack.deploy(
        Effect.gen(function* () {
      ...
      const states = yield* store.getAll!({
        stack: stack.name,
        stage: "test",
      });
      ...
      expect(spansA.length).toBeGreaterThanOrEqual(1);
      expect(spansB.length).toBeGreaterThanOrEqual(1);
      ...
      expect(waterfall.length).toBeGreaterThanOrEqual(2);
...
describe("document fold — structure passes", () => {
  it.effect("plan overlay adds nodes and deletion ghosts, never removes", () =>
    Effect.sync(() => {
      ...
      expect(doc.structure.nodes.size).toBe(3);
    ...
    () =>
      Effect.sync(() => {
        ...
        const unknown = applyEvent(
          doc,
          { kind: "quantum-flux", id: "a" } as unknown as ApplyEvent,
          7,
        );
...
describe("document snapshot + patch protocol", () => {
  it.effect("snapshot → fromSnapshot round-trips", () =>
    Effect.sync(() => {
      ...
      setApproval(doc, { plan: { anything: true }, requestedAt: 4 });
  ...
  it.effect("the patch stream reproduces every mutation", () =>
    Effect.sync(() => {
      ...
      track(setApproval(doc, { plan: { v: 1 }, requestedAt: 11 }));
