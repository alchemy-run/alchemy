 ...
 * history comes from the in-memory fixture. No cloud, no real stacks.
 */
...
import { makeHistory, successfulDeployJournal } from "./fixtures/history.ts";
  ...
  test.provider(
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        // The scratch store is shared by every test file in the run — scope
        // the catalog to this stack so target resolution is deterministic.
        const scoped: StateService = {
          ...store,
          listStacks: () => Effect.succeed([stack.name]),
        };

        // no stack/stage options: the viewer resolves them from the store
        yield* withViewer({ state: scoped, history: fixture.history }, (base) =>
          Effect.gen(function* () {
            ...
            expect(stacks.body).toEqual([
              { stack: stack.name, stages: [stack.stage] },
            ]);
        ...
        // without a history the document still serves; history 404s
        yield* withViewer({ state: scoped }, (base) =>
          Effect.gen(function* () {
            const doc = yield* getJson<DocumentSnapshot>(
              `${base}/api/v2/document`,
            );
            ...
            const history = yield* getJson(`${base}/api/v2/deployments`);
  ...
  test.provider(
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        const store = yield* getStore;
        yield* withViewer(
          { state: store, stack: stack.name, stage: stack.stage },
          (base) =>
            Effect.gen(function* () {
              const doc = yield* getJson<DocumentSnapshot>(
                `${base}/api/v2/document?stack=${encodeURIComponent(stack.name)}&stage=${encodeURIComponent(stack.stage)}`,
              );
              expect(doc.status).toBe(200);
