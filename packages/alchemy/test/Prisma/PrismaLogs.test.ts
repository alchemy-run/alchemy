...
describe(
  ...
  () => {
      ...
      withWebSocketServer((server) =>
        Effect.gen(function* () {
          ...
          expect(String(error)).toContain(
            "exceeds the 1048576-byte frame limit",
          );
    ...
    it.effect(
      "reconnects a transient close from the latest confirmed byte",
      () =>
        withWebSocketServer((server) =>
          Effect.gen(function* () {
            const url = yield* listenUrl(server);
      ...
      () =>
        withWebSocketServer((server) =>
          Effect.gen(function* () {
            ...
            expect(String(error)).toContain(
              "made no progress after 3 reconnect",
            );
          }),
        ),
    );
