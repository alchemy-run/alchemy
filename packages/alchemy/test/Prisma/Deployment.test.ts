...
describe(
  ...
  () => {
    ...
    it.effect(
      "streams file-backed artifacts with a fixed Content-Length",
      () => {
    ...
    it.effect(
      ...
      () => {
        ...
        return Effect.gen(function* () {
          ...
          expect(deployments.get("version-replacement")?.status).toBe(
            "running",
          );
    ...
    it.effect(
      "allows null HTTP port to request the Foundry 8080 default",
      () => {
        const calls: Array<[string, unknown?]> = [];
        const client = {
          createAppDeployment: (appId: string, input: unknown) =>
    ...
    it.effect(
      "replaces when triggers cannot be resolved while planning",
      () => {
        const client = {} as PrismaManagementClient;
