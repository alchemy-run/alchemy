...
describe(
  ...
  () => {
    ...
    it.effect(
      ...
      () => {
        ...
        return Effect.gen(function* () {
          ...
          expect(redactedValue(env[keys.password] ?? undefined)).toBe(
            "password",
          );
        }).pipe(
    ...
    it.effect(
      ...
      () => {
        ...
        const runtime = {
          ...
          env: {},
          set: (id: string) =>
            Effect.succeed(id.replaceAll(/[^a-zA-Z0-9]/g, "_")),
          get: <T>(key: string): Effect.Effect<T> => {
    ...
    it.effect(
      "rejects conflicting App branch inputs",
      () => {
    ...
    it.effect(
      "rejects conflicting Database branch inputs",
      () => {
        const { client } = makeClient();
    ...
    it.effect(
      "reads existing Prisma resources for adoption and refresh",
      () => {
        const calls: Call[] = [];
        const database = {
          id: "database-1",
        ...
        return Effect.gen(function* () {
          ...
          const connectionProvider =
            yield* Provider.findProvider(PrismaConnection);
          ...
          const versionProvider =
            yield* Provider.findProvider(PrismaDeployment);
          const envProvider = yield* Provider.findProvider(
            PrismaEnvironmentVariable,
          );
          const repoProvider = yield* Provider.findProvider(
    ...
    it.effect(
      "treats Prisma 404 during delete as already gone",
      () => {
        const calls: Call[] = [];
    ...
    it.effect(
      "reconciles a Prisma custom domain through the client",
      () => {
        const { client, calls } = makeClient();
    ...
    it.effect(
      ...
      () => {
        ...
        Object.assign(client, {
          ...
          createAppDomain: () =>
            Effect.die("must not claim an existing domain"),
        });
      ...
      () => {
        ...
        return Effect.gen(function* () {
          const domainProvider =
            yield* Provider.findProvider(PrismaCustomDomain);
          const error = yield* domainProvider
            .reconcile(
              reconcileInput("CustomDomain", {
                app: "service-1",
    ...
    it.effect(
      "normalizes Prisma custom domain hostnames when matching",
      () => {
        const { client, calls } = makeClient();
        Object.assign(client, {
          listAppDomains: (appId: string) =>
    ...
    it.effect(
      "rejects Prisma custom domains on non-default branches",
      () => {
        const { client, calls } = makeClient();
        Object.assign(client, {
          getBranch: (id: string) =>
    ...
    it.effect(
      "starts a direct deployment only after observing status",
      () => {
        const calls: Call[] = [];
        let status = "new";
          ...
          getDeployment: (id: string) =>
            Effect.sync(() => {
              ...
              return {
                ...
                previewDomain:
                  status === "running"
                    ? "version-1.preview.prisma.build"
                    : null,
                createdAt,
              };
            }),
    ...
    it.effect(
      "refuses to mutate system-managed environment variables",
      () => {
        const calls: Call[] = [];
    ...
    it.effect(
      "validates Prisma environment variable writes locally",
      () => {
        const calls: Call[] = [];
        const client = {
          listEnvironmentVariables: (query: unknown) =>
    ...
    it.effect(
      "updates mutable Prisma resources from observed state",
      () => {
        const calls: Call[] = [];
        const client = {
          getDatabase: (id: string) =>
    ...
    it.effect(
      ...
      () => {
        ...
        const database = {
          ...
          connections: [
            databaseConnection("database-clone", "connection-clone"),
          ],
          project: resourceRef("projects", "project-1", "app"),
          region: { id: "us-east-1", name: "US East" },
    ...
    it.effect(
      "rejects non-atomic explicit-name database branch creation",
      () => {
        let created = false;
        const client = {
          createDatabase: () =>
    ...
    it.effect(
      "ensures a default database on an existing Prisma project",
      () => {
          ...
          if (
            ...
            return envelope(
              wireCreatedDatabase({
                ...
                connections: [
                  wireConnection({
                    directConnectionString: "postgres://direct",
                  }),
                ],
              }),
              { status: 201 },
        ...
        return Effect.gen(function* () {
          ...
          expect(
            fake.captured.find((request) => request.method === "POST")
              ?.bodyJson,
      ...
      { tags: ["provider:prisma:project"] },
    );

    it.effect(
      "clears previously managed project settings",
      () => {
        const fake = makeFakeManagementApi((request) => {
    ...
    it.effect(
      "renames a project without clearing unmanaged settings",
      () => {
        const fake = makeFakeManagementApi((request) => {
          if (
            request.pathname === "/v1/projects/project-1" &&
    ...
    it.effect(
      "deletes Prisma resources through their management APIs",
      () => {
        const calls: Call[] = [];
    ...
    it.effect(
      "rejects direct delete for a default Prisma database",
      () => {
        const calls: Call[] = [];
        const client = {
          getDatabase: (id: string) =>
    ...
    it.effect(
      "checks live database default state before deleting",
      () => {
        const calls: Call[] = [];
        const client = {
          getDatabase: (id: string) =>
    ...
    it.effect(
      "rejects default Prisma database delete without a session",
      () => {
        const calls: Call[] = [];
        const client = {
          getDatabase: (id: string) =>
    ...
    it.effect(
      "deletes a database when stale state says it is default",
      () => {
        const calls: Call[] = [];
        const client = {
          getDatabase: (id: string) =>
    ...
    it.effect(
      "rejects direct delete for a default Prisma branch",
      () => {
        const calls: Call[] = [];
        const client = {
          getBranch: (id: string) =>
    ...
    it.effect(
      "checks live branch default state before deleting",
      () => {
        const calls: Call[] = [];
        const client = {
          getBranch: (id: string) =>
    ...
    it.effect(
      "rejects default Prisma branch delete without a session",
      () => {
        const calls: Call[] = [];
        const client = {
          getBranch: (id: string) =>
    ...
    it.effect(
      "deletes a branch when stale state says it is default",
      () => {
        const calls: Call[] = [];
        const client = {
          getBranch: (id: string) =>
    ...
    it.effect(
      "refuses to adopt explicit identities after create races",
      () => {
        const calls: Call[] = [];
        ...
        const client = {
          ...
          listBranches: (projectId: string, query: unknown) =>
            Effect.sync(() => {
              calls.push(["listBranches", { projectId, query }]);
              const gitName = (query as { gitName?: string } | undefined)
                ?.gitName;
              if (gitName === "preview") {
          ...
          createEnvironmentVariable: (input: unknown) =>
            Effect.gen(function* () {
              ...
              visible.add("env");
              return yield* Effect.fail(
                apiConflict("/v1/environment-variables"),
              );
            }),
          updateEnvironmentVariable: (id: string, input: unknown) =>
            Effect.sync(() => {
        ...
        return Effect.gen(function* () {
          ...
          expect(Redacted.value(connectionOut.password!)).toBe(
            "rotated-password",
          );
