...
it.effect("derives row schemas without a client and preserves nullable fields", () =>
  Effect.gen(function* () {
...
it.effect("maps lists and namespaces without including navigation relations", () =>
  Effect.sync(() => {
...
it.effect("preserves enums, bigint, decimals, bytes, temporal strings, and JSON", () =>
  Effect.sync(() => {
...
it.effect("emits deterministic standalone expressions for the same contract", () =>
  Effect.sync(() => {
...
it.effect("rejects unsupported codecs instead of silently accepting unknown rows", () =>
  Effect.sync(() => {
    const custom = {
      ...contract,
      domain: {
