...
describe("rewriteEmittedTypes", (it) => {
  it.effect("maps leaked @internal/extension-* specifiers to @prisma/orm-extension-*", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
...
`);
  ...
  it.effect("still maps postgres @internal/* prefixes when an extension specifier is present", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const dtsPath = yield* writeDts(`
...
`);
  ...
  it.effect("fails when an unmapped @internal/* specifier remains", () =>
    Effect.gen(function* () {
      const dtsPath = yield* writeDts(`type Leak = { package: "@internal/unknown-pkg/types" };\n`);
