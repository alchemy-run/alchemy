...
describe(
  ...
  () => {
    ...
    it.effect(
      "rejects conflicting branch attachment inputs",
      () => {
        const client = {} as PrismaManagementClient;
    ...
    it.effect(
      "rejects invalid production and dev ports",
      () => {
        const client = {} as PrismaManagementClient;
    ...
    it.effect(
      "rejects unassigned branch attachment for Compute deploys",
      () => {
        const client = {} as PrismaManagementClient;
    ...
    it.effect(
      "rejects skipCodeUpload without a version to fork",
      () => {
        const calls: Array<[string, unknown]> = [];
        const client = {
          getApp: (id: string) => {
    ...
    it.effect(
      "rejects effect-native Compute without a main module",
      () => {
        const client = {} as PrismaManagementClient;
    ...
    it.effect(
      "rejects effect-native Compute with an external build",
      () => {
        const client = {} as PrismaManagementClient;
    ...
    it.effect(
      "reads the live deployment through the canonical route",
      () => {
        const calls: Array<[string, unknown]> = [];
        const client = {
          listApps: (projectId: string, query: unknown) => {
    ...
    it.effect(
      "returns the preview URL for unpromoted Compute deploys",
      () => {
        const calls: Array<[string, unknown?]> = [];
        let status = "new";
        const client = {
    ...
    it.effect(
      "forwards command build output limits before cloud mutation",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "fails when Prisma omits an upload URL for app artifacts",
      () => {
        const calls: Array<[string, unknown?]> = [];
        const client = {
          getApp: () => {
    ...
    it.effect(
      "deletes created deployment when artifact upload fails",
      () => {
        const calls: Array<[string, unknown?]> = [];
        const client = {
          getApp: () =>
    ...
    it.effect(
      "deletes created deployment when start fails",
      () => {
        const calls: Array<[string, unknown?]> = [];
        const client = {
          getApp: (id: string) => {
    ...
    it.effect(
      "uploads a pre-created artifact from artifactPath",
      () => {
        let uploaded:
    ...
    it.effect(
      "validates Compute env vars before remote writes",
      () => {
        const calls: Array<[string, unknown]> = [];
        const client = {
          getApp: (id: string) =>
    ...
    it.effect(
      "merges binding env into Compute deployment env",
      () => {
        const calls: Array<[string, unknown]> = [];
        const client = {
          getApp: (id: string) => {
    ...
    it.effect(
      "removes env vars from previously managed bindings",
      () => {
        const calls: Array<[string, unknown]> = [];
        const byKey = new Map([
          [
    ...
    it.effect(
      "does not expose redacted env values in Compute outputs",
      () => {
        const calls: Array<[string, unknown]> = [];
        const client = {
          getApp: (id: string) => {
    ...
    it.effect(
      "runs a build command and uploads the built archive",
      () => {
        const calls: Array<[string, unknown]> = [];
    ...
    it.effect(
      "auto-builds a Bun app before uploading",
      () => {
        const calls: Array<[string, unknown]> = [];
        let uploaded:
          | { url: string; contentType: string | undefined; bytes: Uint8Array }
    ...
    it.effect(
      "deletes env vars removed from Compute props on update",
      () => {
        const calls: Array<[string, unknown]> = [];
        const byKey = new Map([
          [
    ...
    it.effect(
      "deletes Compute env vars on provider destroy",
      () => {
        const calls: Array<[string, unknown]> = [];
        const client = {
          listEnvironmentVariables: (query: unknown) => {
    ...
    it.effect(
      "skips system-managed Compute env vars on provider destroy",
      () => {
        const calls: Array<[string, unknown]> = [];
        const byKey = new Map([
          [
    ...
    it.effect(
      "deletes Compute env vars when old props are missing",
      () => {
        const calls: Array<[string, unknown]> = [];
        const client = {
          listEnvironmentVariables: (query: { key: string }) => {
