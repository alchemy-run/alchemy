...
describe(
  "Prisma Compute static-site build",
  { tags: ["provider:prisma", "provider:prisma:compute", "local"] },
  () => {
    it.live(
      "packages a prebuilt index without running a command",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
    ...
    it.effect(
      "scopes archive ignores to the static output",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
    ...
    it.effect(
      "rejects unsafe output and index paths",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const root = yield* fs.makeTempDirectory({
    ...
    it.effect(
      "fails when the build output or index page is missing",
      () =>
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
