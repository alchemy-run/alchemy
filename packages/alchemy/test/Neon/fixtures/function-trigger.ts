...
export default {
  async fetch(request: Request) {
    if (new URL(request.url).pathname === "/events") return Response.json([...events.values()]);
    ...
    try {
      event = Schema.decodeUnknownSync(FunctionTriggerEnvelope)(await request.json());
    } catch {
      return new Response(null, { status: 400 });
    }
    if (event.invocation_id !== invocation) return new Response(null, { status: 400 });
