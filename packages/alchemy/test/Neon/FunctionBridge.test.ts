...
for (const cancellation of ["abort", "body"] as const)
  test.live(`Function bridge closes the handler scope on ${cancellation} cancellation`, () =>
    Effect.gen(function* () {
      const finalized = yield* Deferred.make<void>();
      const streamFinalized = yield* Deferred.make<void>();
      const runtime = yield* Effect.sync(() => makeFunctionRuntimeContext("Bridge"));
      let closed = false;
      yield* runtime.route(
        "/stream",
        Effect.gen(function* () {
          ...
          return HttpServerResponse.stream(
            Stream.fromEffect(
              Effect.gen(function* () {
                ...
                return yield* Effect.sync(() => new TextEncoder().encode("first"));
              }),
            ).pipe(
      ...
      if (cancellation === "abort") yield* Effect.sync(() => controller.abort());
      else yield* Effect.tryPromise(() => reader.cancel());
      yield* Effect.all([Deferred.await(finalized), Deferred.await(streamFinalized)]).pipe(
        Effect.timeout("1 second"),
        Effect.ensuring(Effect.tryPromise(() => reader.cancel()).pipe(Effect.ignore)),
      );
...
for (const cancellation of ["abort", "body"] as const)
  test.live(`Function bridge preserves backpressure until ${cancellation} cancellation`, () =>
    Effect.gen(function* () {
      const runtime = yield* Effect.sync(() => makeFunctionRuntimeContext("Backpressure"));
      const finalized = yield* Deferred.make<void>();
      const streamFinalized = yield* Deferred.make<void>();
      let produced = 0;
      let closed = false;
          ...
          Effect.gen(function* () {
            ...
            return HttpServerResponse.stream(
              Stream.fromEffectRepeat(
                Effect.sync(() => new TextEncoder().encode(String(++produced))),
              ).pipe(Stream.ensuring(Deferred.succeed(streamFinalized, undefined))),
            );
          }),
        )
        .pipe(
      ...
      yield* Effect.gen(function* () {
        ...
        yield* Deferred.await(streamFinalized).pipe(Effect.timeout("1 second"));
        yield* Deferred.await(finalized).pipe(Effect.timeout("1 second"));
        expect(closed).toBe(true);
      }).pipe(Effect.ensuring(Effect.tryPromise(() => reader.cancel()).pipe(Effect.ignore)));
    }),
  );
...
for (const outcome of ["complete", "error"] as const)
  test.live(`Function bridge finalizes a streamed ${outcome} once`, () =>
    Effect.gen(function* () {
      const runtime = yield* Effect.sync(() => makeFunctionRuntimeContext("Completion"));
      const events: string[] = [];
      yield* runtime.route(
        "/",
        Effect.gen(function* () {
      ...
      const request = yield* Effect.sync(() => new Request("https://function.test/"));
      const response = yield* Effect.tryPromise(() => bridge.fetch(request));
      expect(response.status).toBe(201);
      expect(response.headers.get("x-stream-test")).toBe("preserved");
      const result = yield* Effect.tryPromise(() => response.arrayBuffer()).pipe(Effect.exit);
...
for (const cancellation of ["close", "abort"] as const)
  test.live(
    ...
    () =>
      Effect.gen(function* () {
        const runtime = yield* Effect.sync(() => makeFunctionRuntimeContext("Upgrade"));
        const finalized = yield* Deferred.make<void>();
        ...
        expect(count).toBe(0);
        yield* Effect.sync(() =>
          cancellation === "close" ? socket.dispatchEvent(new Event("close")) : controller.abort(),
        );
        expect(count).toBe(0);
...
test.effect("Function bridge closes bodyless request scopes without consuming a stream", () =>
  Effect.gen(function* () {
    const runtime = yield* Effect.sync(() => makeFunctionRuntimeContext("Bodyless"));
    let finalized = 0;
    let consumed = 0;
    yield* runtime.route(
      "/",
