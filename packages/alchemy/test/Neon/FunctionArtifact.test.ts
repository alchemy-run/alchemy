...
test.effect("prebuilt directory and ZIP share a deterministic root-entry artifact", () =>
  Effect.gen(function* () {
...
test.effect("changing a native entrypoint changes uploaded bytes and digest", () =>
  Effect.gen(function* () {
    const first = yield* buildFunctionArtifact({
      ...scope,
...
test.effect("native bundle targets Node without a Bun or workerd bootstrap", () =>
  Effect.gen(function* () {
