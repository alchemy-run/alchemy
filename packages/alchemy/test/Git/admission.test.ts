 ...
 */
import { BACKPRESSURE_BYTES, RETAIN_BYTES } from "@/Git/Store/StreamingSource.ts";

const MiB = 1024 * 1024;
const spilled = Math.ceil((RETAIN_BYTES + BACKPRESSURE_BYTES + STAGE_BATCH_BYTES) / MiB);
...
describe("pushPermitsFor", () => {
  ...
  test("an in-memory push is charged by its size; tiny pushes stay cheap", () => {
    ...
    expect(spilled + 4 * pushPermitsFor(1024)).toBeLessThanOrEqual(PUSH_MEMORY_BUDGET_MB);
  });
});
