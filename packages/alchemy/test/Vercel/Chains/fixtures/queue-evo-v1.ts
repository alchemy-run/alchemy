/**
 * QueueEvolution chain — CYCLE 1 fixture: Topic schema v1 (no `note`),
 * default subscribe options. Same two-function shape as the flagship
 * Subscribe.test.ts fixture: public routes produce/drain, the platform
 * push-delivers to the generated consumer function, which echoes into
 * `Echoes` for cross-function readback.
 */
...
export default class QueueEvoFn extends Vercel.Function<QueueEvoFn>()(
  ...
  Effect.gen(function* () {
    ...
    // Sends into a topic with no trigger fail 503 — give the echo topic its
    // own no-op trigger (acks under this group; readback keeps its own).
    ...
    return {
      fetch: Effect.gen(function* () {
        ...
        return yield* HttpServerResponse.json({ ok: true, stage: 1 });
      }),
    };
  }).pipe(
    Effect.provide([
