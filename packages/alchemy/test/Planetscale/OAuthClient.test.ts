...
import { OAUTH_CLIENT_ID, exchange, usesCurrentClient } from "@/Planetscale/OAuthClient.ts";
...
describe("PlanetScale OAuth client credentials", () => {
  it("maps the token response onto stored credentials", async () => {
    ...
    const credentials = await Effect.runPromise(
      exchange("code").pipe(Effect.provideService(FetchHttpClient.Fetch, fetch)),
    );
    ...
    expect(credentials.expires).toBeGreaterThanOrEqual(before + 3_600_000 - 1_000);
  });
