...
describe.skipIf(!process.env.PLANETSCALE_TEST).concurrent("PostgresDatabase", () => {
  ...
  test.provider.skipIf(!process.env.PLANETSCALE_DEPLOY_TEST)(
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        expect(
          all.some((db) => db.organization === database.organization && db.name === database.name),
        ).toBe(true);
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        const { updatedDatabase } = yield* stack.deploy(
          Effect.gen(function* () {
            const updatedDatabase = yield* Planetscale.PostgresDatabase("PostgresDatabaseCRUD", {
              name,
              clusterSize: "PS_20",
  ...
  test.provider(
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        const { database } = yield* stack.deploy(
          Effect.gen(function* () {
            const database = yield* Planetscale.PostgresDatabase("PostgresDatabaseCustomBranch", {
              name,
              clusterSize: "PS_10",
              defaultBranch,
            });
  ...
  test.provider(
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        expect(database.migrationsHashes["0001_create_widgets.sql"]).toEqual(expect.any(String));
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        const { newDatabase } = yield* stack.deploy(
          Effect.gen(function* () {
            const newDatabase = yield* Planetscale.PostgresDatabase("PostgresDatabaseWrongArch", {
              arch: "x86",
              clusterSize: "PS_10",
            });
            return {
  ...
  test.provider(
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        const { database } = yield* stack.deploy(
          Effect.gen(function* () {
            const database = yield* Planetscale.PostgresDatabase("PostgresDatabaseRetainRemoval", {
              region: { slug: "us-east" },
              clusterSize: "PS_10",
