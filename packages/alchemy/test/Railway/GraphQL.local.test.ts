...
import { ResourceDeletionPending, waitUntilDeleted } from "@/Railway/GraphQL.ts";

describe("Railway deletion confirmation", () => {
  it.effect("fails with resource identity when bounded polls never confirm absence", () =>
    Effect.gen(function* () {
      let reads = 0;
      ...
      const fiber = yield* waitUntilDeleted("Railway.Project", "project-1", absent).pipe(
        Effect.flip,
  ...
  it.effect("preserves the query's typed aggregate failure during polling", () =>
    Effect.gen(function* () {
