...
/**
 * Build a kit `Handle` that answers matched effect routes and calls
 * `resolve(event)` on passthrough/miss. Env comes from
 * `event.platform.env` when the adapter provides it (Cloudflare), falling
 * back to the bridge's resolution ladder (Node dev → `process.env`).
 */
