...
import { CloudflareControl } from "../../AlchemyControl/CloudflareControl.ts";
import { CloudflareTokenControl } from "../../AlchemyControl/CloudflareTokenControl.ts";
import type { CreatedCloudflareToken } from "../../AlchemyControl/Surface.ts";
...
const bootstrapCommand = Command.make(
  ...
  instrumentCommand(
    ...
    Effect.fn(function* ({ envFile, profile, force, workerName }) {
      ...
      yield* (yield* CloudflareControl).bootstrap({
        workerName,
        force,
        profile: profileName,
...
const teardownCommand = Command.make(
  ...
  instrumentCommand(
    ...
    Effect.fn(function* ({ envFile, profile, workerName, yes: approved }) {
      ...
      const profileName = yield* resolveProfileName(envFile, profile);
      yield* (yield* CloudflareControl).teardown({
        workerName,
        profile: profileName,
        envFile: Option.getOrUndefined(envFile),
      });
...
const createTokenCommand = Command.make(
  ...
  instrumentCommand(
    ...
    Effect.fn(function* ({ envFile, allPermissions, name, accountId }) {
      ...
      const tokens = yield* CloudflareTokenControl;
      const catalog = yield* tokens.catalog(credentials);
      ...
      const result = yield* tokens.create({
        credentials,
        plan,
      });
      yield* Console.log(
...
const stateLogsCommand = Command.make(
  ...
  instrumentCommand(
    ...
    Effect.fn(function* ({
      ...
      const cloudflare = yield* CloudflareControl;
      const target = {
        profile: profileName,
        workerName,
        envFile: Option.getOrUndefined(envFile),
      ...
      if (follow) {
        ...
        yield* cloudflare.stateLogs
          .tail(target)
          .pipe(Stream.runForEach((line) => Console.log(formatLine(line))));
        return;
      }
      const lines = yield* cloudflare.stateLogs.entries({
        ...target,
        limit,
