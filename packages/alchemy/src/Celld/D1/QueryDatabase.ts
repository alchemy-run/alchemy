 ...
 * `yield* Celld.D1.QueryDatabase(db)`.
 *
 * ### Querying a Database
 * **Example:** Querying a database inside a Worker
 * ```typescript
 * const db = yield* Celld.D1.QueryDatabase(MyDatabase);
...
/**
 * Effect-native wrapper around a Celld D1 prepared statement.
 *
...
export class PreparedStatement {
  ...
  /**
   * Materialize the underlying Celld prepared statement against
...
/** Wrap a native binding without changing SQL source, row values, or metadata. */
export const makeQueryDatabaseClient = (
  raw: Effect.Effect<runtime.D1Database>,
): QueryDatabaseClient => ({
  raw,
  prepare: (query) => new PreparedStatement(query, [], raw),
  exec: (query) =>
    Effect.flatMap(raw, (database) =>
      Effect.promise(() => database.exec(query)),
    ),
  batch: (statements) =>
    Effect.flatMap(raw, (database) =>
      Effect.promise(() =>
        database.batch(
          statements.map((statement) => statement._build(database)),
        ),
      ),
    ),
});
...
export interface QueryDatabaseClient {
  /**
   * An Effect that resolves to the raw underlying Celld D1Database binding.
