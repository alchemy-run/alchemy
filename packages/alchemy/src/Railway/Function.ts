...
type ProjectResponseServicesEdgesItemNode = railway.Result<"Service!", typeof selection>;
...
const latestRuntimeImage = () =>
  railway.functionRuntime({ name: FUNCTION_RUNTIME_NAME }, { latestVersion: { image: true } }).pipe(
...
const fetchDeployLogs = (deploymentId: string | undefined) =>
  deploymentId === undefined || deploymentId.length === 0
    ? Effect.succeed("")
    : railway.deploymentLogs({ deploymentId, limit: 80 }, { severity: true, message: true }).pipe(
        Effect.map((rows) =>
          rows
...
const instanceSettingsDelta = (input: {
  ...
  if (input.props.region !== undefined && (instance?.region ?? undefined) !== input.props.region) {
