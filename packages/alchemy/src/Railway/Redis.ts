...
export class RedisUrlMissing extends Data.TaggedError(
  "Railway.RedisUrlMissing",
)<{
  name: string;
}> {}

export class RedisCommandError extends Data.TaggedError(
  "Railway.RedisCommandError",
)<{
  command: string;
  cause: unknown;
}> {}
...
/**
 * Build a `redis://` URL. Encodes the password. Used by laptop tests
 * against a {@link TcpProxy} and by the runtime client.
 */
export const redisConnectionUrl = (input: {
  host: string;
  port: number;
  password: string;
  username?: string;
...
/**
 * Send one Redis command over RESP. Used by the Read/Write clients and
 * by laptop tests against a public TcpProxy URL.
 */
export const runRedisCommand = (
  url: string,
  command: string,
  args: readonly string[] = [],
): Effect.Effect<unknown, RedisCommandError> =>
  Effect.tryPromise({
    try: async () => {
      const Client = (
        globalThis as {
      ...
      const socket = await Bun.connect({
        ...
        port: Number(parsed.port || (tls ? 6380 : REDIS_PORT)),
        tls,
      });
...
> =>
  Effect.gen(function* () {
    ...
    return yield* runRedisCommand(resolved, command, args);
  });
