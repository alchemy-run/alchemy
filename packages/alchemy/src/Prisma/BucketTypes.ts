 ...
 */
export class BucketError extends Data.TaggedError("PrismaBucketError")<{
  message: string;
  cause: Error;
}> {}
...
export interface ListResult {
  ...
  truncated: boolean;
  /**
   * Cursor to pass as `ListOptions.cursor` for the next page, when truncated.
   */
...
export interface PresignOptions {
  ...
  expiresIn?: number;
  /**
   * `Content-Type` signed into the URL. An uploader using a presigned PUT URL
   * must send exactly this `Content-Type` or the signature check fails.
   */
  contentType?: string;
}

/**
 * The values a bound `Prisma.BucketKey` carries into the host runtime. Each is
 * read back out of the host environment, hence the {@link RuntimeContext}
 * requirement.
 *
...
export interface BucketCredentials {
   ...
   */
  endpoint: Effect.Effect<string, never, RuntimeContext>;
  /**
   ...
   */
  accessKeyId: Effect.Effect<string, never, RuntimeContext>;
  /**
   * S3 secret access key of the bound bucket key.
   */
  secretAccessKey: Effect.Effect<
