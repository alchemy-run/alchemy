    ...
    try: () => {
      ...
      const encode = (value: unknown): string => {
        ...
        if (Array.isArray(value)) {
          if (globalThis.Object.getOwnPropertyNames(value).length !== value.length + 1)
            throw new Error("Sparse arrays and array properties are unsupported");
          encoded = `[${Array.from({ length: value.length }, (_, index) => {
            const descriptor = globalThis.Object.getOwnPropertyDescriptor(value, String(index));
            if (!descriptor || !descriptor.enumerable || !("value" in descriptor))
              throw new Error("Sparse arrays, non-enumerable fields and accessors are unsupported");
          ...
          encoded = `{${globalThis.Object.keys(value)
            ...
            .map((key) => {
              ...
              if (!("value" in descriptor)) throw new Error("JSON accessors are unsupported");
...
export const storageBodyBytes = (body: Stream.Stream<Uint8Array, Error> | undefined) =>
  body === undefined
...
const desiredBytes = Effect.fn(function* (props: ObjectProps<unknown>) {
  ...
  return typeof body === "string" ? yield* Effect.sync(() => new TextEncoder().encode(body)) : body;
});

const observeObject = Effect.fn(function* (bucket: BucketAttributes, key: string, json: boolean) {
...
export const ObjectProvider = () =>
  Provider.succeed(ObjectResource, {
    ...
    read: Effect.fn(function* ({ fqn, olds, output }) {
      ...
      const attrs = yield* observeObject(bucket, output?.key ?? olds.key, isJson(olds));
    ...
    reconcile: Effect.fn(function* ({ fqn, news }) {
      ...
      if (globalThis.Object.keys(applicationMetadata).length !== entries.length) {
        return yield* new ObjectDecodeError({
      ...
      const options: StoragePutOptions = {
        ContentType:
          news.contentType ?? (isJson(news) ? "application/json" : "application/octet-stream"),
      ...
      if (
        ...
        (yield* serializeObjectValue(current.metadata)) !== (yield* serializeObjectValue(metadata))
      ) {
    ...
    delete: Effect.fn(function* ({ output }) {
      ...
      yield* client.delete(output.key).pipe(Effect.catchTag("NoSuchBucket", () => Effect.void));
    }),
  });
