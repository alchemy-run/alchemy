...
import { nativeArtifactError, neonRuntimeTarget as runtimeTarget } from "../NativeArtifact.ts";
...
export const WebsiteArtifact = Resource<WebsiteArtifact>("Neon.WebsiteArtifact");

/** Packaging failed before any Function deployment. */
export class WebsiteArtifactError extends Data.TaggedError("WebsiteArtifactError")<{
...
const native = (file: string) => /\.(?:node|dylib|dll|so(?:\.\d+)*)$/.test(file);
...
const fail = (message: string) => Effect.fail(new WebsiteArtifactError({ message }));
...
const platformPackage = Effect.fn(
  function* (name: string, version: string) {
    if (!/^@img\/sharp-(?:libvips-)?linux-arm64$/.test(name) || !exactVersion.test(version))
      return yield* fail("Sharp must declare exact Linux ARM64 platform package versions.");
    ...
    if (
      ...
      return yield* fail(`Invalid registry identity or integrity for ${name}@${version}.`);
    ...
    if (
      ...
      return yield* fail("Platform packages must use the canonical npm registry tarball.");
    ...
    const files = yield* Effect.try({
      try: () => {
        ...
        if (integrity !== metadata.dist!.integrity) throw new Error("Registry integrity mismatch");
        const tar = gunzipSync(compressed, { maxOutputLength: MAX_BYTES });
        const files = new Map<string, Uint8Array>();
        const text = (start: number, size: number) =>
          new TextDecoder().decode(tar.subarray(start, start + size)).replace(/\0.*$/s, "");
        let entries = 0;
        for (let offset = 0; offset < tar.length;) {
          ...
          if (![0, 48, 53].includes(type!)) throw new Error("Non-regular tar entry");
        ...
        if (manifest.name !== name || manifest.version !== version || !compatible(manifest))
...
export const stageWebsiteArtifact = Effect.fn(function* (props: WebsiteArtifactProps) {
  ...
  if (props.layout !== "next" && root === dist)
    return yield* fail("Output must be a dedicated directory, not the application root.");
  if (!within(root, dist))
  ...
  const standaloneConfig =
    props.layout === "next" &&
    entry !== undefined &&
    (yield* fs.readFileString(entry)).includes("__NEXT_PRIVATE_STANDALONE_CONFIG");
  ...
  const collect: (
  ...
  ) => Effect.Effect<void, WebsiteArtifactError | PlatformError> = Effect.fn(function* (
    source,
    boundary,
    ancestors = new Set(),
    ...
    if (excluded(real)) return yield* fail("An output symlink targets a sensitive file.");
    ...
    if (stat.type === "Directory") {
      ...
      for (const name of (yield* fs.readDirectory(source)).sort()) {
        if (name !== "node_modules")
          yield* collect(path.join(source, name), boundary ?? real, next, runtimePackage);
      }
    } else if (stat.type === "File") {
      files.add(source);
      if (!runtimePackage && !props.static && javascript(source)) seeds.add(source);
  ...
  for (const extra of extras ?? []) {
    if (standaloneConfig && /^next\.config\./.test(path.basename(extra.source))) continue;
  ...
  if (props.layout === "next") {
    for (const name of ["next.config.mts", "next.config.cts"]) {
      ...
      if (!standaloneConfig && (yield* fs.exists(config))) yield* collect(config);
    }
  ...
  for (const manifest of manifests) {
    ...
    for (const file of parsed.files) {
      ...
      if (excluded(source)) return yield* fail("A Next.js trace selects a sensitive file.");
  ...
  for (const file of tracedFiles) if (!sourceOnly(file)) files.add(path.resolve(traceBase, file));
  ...
  const packageRoot = (file: string) => {
    ...
    return parts.slice(0, index + (parts[index + 1]!.startsWith("@") ? 3 : 2)).join(path.sep);
  };
  ...
  for (const file of files) {
    ...
    if (pkg && path.basename(pkg) === "sharp" && javascript(real)) sharpRoots.add(pkg);
  }
  for (const sharpRoot of sharpRoots) {
    if (!within(workspace, sharpRoot))
      return yield* fail("Sharp escapes the application workspace.");
    const installed = yield* fs.readFileString(path.join(sharpRoot, "package.json"));
    const sharp = yield* Effect.try(() => JSON.parse(installed) as PackageManifest);
    const packages = ["@img/sharp-linux-arm64", "@img/sharp-libvips-linux-arm64"];
  ...
  const isReplaced = (file: string) => [...replaced].some((pkg) => within(pkg, file));
  for (const file of files) {
    const real = yield* fs.realPath(file);
    if (!within(workspace, file) || !within(workspace, real))
      return yield* fail("A traced dependency escapes the application workspace.");
    if (excluded(file) || excluded(real)) {
      const sensitive = path
        .relative(workspace, excluded(file) ? file : real)
        .replaceAll("\\", "/")
        .replace(/[^a-zA-Z0-9_./@+-]/g, "_");
      return yield* fail(`A traced dependency selects a sensitive file: ${sensitive}`);
    }
  ...
  if (nextPackageRoot) links.set(path.join(root, "node_modules/next"), nextPackageRoot);
  let base = root;
  for (const file of [...selected, ...generated.keys(), ...links.keys(), ...links.values()])
  ...
  const copy = Effect.fn(function* (source: string, dest: string) {
    ...
    if (++entries > MAX_ENTRIES || bytes > MAX_BYTES)
      return yield* fail("Materialized artifact exceeds its entry or expanded byte limit.");
    ...
    if (native(source)) {
      ...
      if (pkg && (yield* fs.exists(path.join(pkg, "package.json")))) {
        ...
        const manifest = yield* Effect.try(() => JSON.parse(text) as PackageManifest);
        if (!compatible(manifest)) return yield* fail("Native package excludes Linux ARM64 glibc.");
      }
    }
  ...
  for (const [alias, target] of links) {
    ...
    if (index < 0 || target.endsWith(`${path.sep}node_modules${path.sep}${name}`)) continue;
    if (selected.has(target)) yield* copy(target, staged(alias));
    else
      for (const file of sorted)
        if (within(target, file))
          yield* copy(file, path.join(staged(alias), path.relative(target, file)));
  }
  if (props.static) {
...
export const packageWebsiteArtifact = Effect.fn(function* (props: WebsiteArtifactProps) {
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  const { directory } = yield* stageWebsiteArtifact(props);
  const files: ZipFile[] = [];
  const walk: (relative: string) => Effect.Effect<void, PlatformError> = Effect.fn(
    function* (relative) {
      for (const name of (yield* fs.readDirectory(path.join(directory, relative))).sort()) {
    ...
    Effect.gen(function* () {
      ...
      return {
        ...
        diff: Effect.fn(function* ({ news, output }) {
          if (!isResolved(news) || !output) return { action: "update" as const };
          if (!(yield* fs.exists(output.artifactPath))) return { action: "update" as const };
          if ((yield* sha256(yield* fs.readFile(output.artifactPath))) !== output.hash)
            return { action: "update" as const };
          // Dependencies outside the framework root can change without its build hash.
          const { hash } = yield* packageWebsiteInChild(news).pipe(Effect.scoped);
          return {
            action: hash === output.hash ? ("noop" as const) : ("update" as const),
          };
        }),
        reconcile: Effect.fn(function* ({ id, news }) {
          const name = yield* createPhysicalName({ id, maxLength: 80 });
          const directory = path.join(initialCwd, ".alchemy", "neon-websites", name);
          const { archive, hash } = yield* packageWebsiteInChild(news).pipe(Effect.scoped);
