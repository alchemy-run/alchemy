...
export class GkebackupOperationPending extends Data.TaggedError(
  "GCP.Gkebackup.OperationPending",
)<{
  operation: string;
}> {}
...
export class ResourceStillExists extends Data.TaggedError(
  "GCP.Gkebackup.ResourceStillExists",
)<{
  name: string;
}> {}
...
export const rfc1035 = (name: string, fallback = "gkebackup"): string => {
  let next = name
    .toLowerCase()
    .replace(/[^a-z0-9-]/g, "-")
    .replace(/-+/g, "-")
...
export const projectName = (value: string | undefined, project: string) => {
...
const READY_STATES = new Set(["READY", "SUCCEEDED", "DEACTIVATED", "IN_USE"]);
...
const alreadyExists = (error: gkebackup.GoogleRpcStatus | undefined) =>
  error?.code === 6 ||
  (error?.message ?? "").toUpperCase().includes("ALREADY_EXISTS");

const isNotFoundStatus = (error: gkebackup.GoogleRpcStatus | undefined) =>
  error?.code === 5 ||
  (error?.message ?? "").toLowerCase().includes("not found");

const isIgnorable = (
  error: gkebackup.GoogleRpcStatus | undefined,
...
export const waitForOperation = (
  operation: gkebackup.GoogleLongrunningOperation,
  options?: {
    notFoundOk?: boolean;
    times?: number;
  ...
  Effect.gen(function* () {
    ...
    if (operation.done === true) {
      if (operation.error && !isIgnorable(operation.error, options)) {
        return yield* new GkebackupOperationFailed({
          operation: name ?? "",
    ...
    const getOperation = gkebackup.getProjectsLocationsOperations({ name });
    const resolved =
      options?.notFoundOk === true
        ? getOperation.pipe(
            Effect.catchTag("NotFound", () =>
              Effect.succeed<gkebackup.GoogleLongrunningOperation>({
                name,
                done: true,
              }),
            ),
    ...
    return yield* resolved.pipe(
      Effect.filterOrFail(
        ...
        () => new GkebackupOperationPending({ operation: name }),
      ),
      Effect.flatMap((current) => {
        const error = current.error;
        if (error && !isIgnorable(error, options)) {
          return Effect.fail(
            new GkebackupOperationFailed({
              operation: name,
      ...
      Effect.retry({
        while: (error) => error._tag === "GCP.Gkebackup.OperationPending",
...
export const waitUntilExists = <A, E extends { readonly _tag: string }, R>(
  ...
  get.pipe(
    ...
    Effect.retry({
      while: (error) => error._tag === "GCP.Gkebackup.ResourceNotResolved",
      times: 8,
...
export const waitUntilGone = <A, E extends { readonly _tag: string }, R>(
  ...
  get.pipe(
    ...
    Effect.asVoid,
    Effect.retry({
      while: (error) => error._tag === "GCP.Gkebackup.ResourceStillExists",
      times: 10,
...
> =>
  get.pipe(
    ...
    Effect.retry({
      while: (error) =>
        error._tag === "GCP.Gkebackup.ResourceNotReady" ||
        error._tag === "GCP.Gkebackup.ResourceNotResolved",
      times: options?.times ?? 10,
      schedule: Schedule.spaced(options?.interval ?? "3 seconds"),
    }),
  );
