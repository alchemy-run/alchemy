...
export class FirebasedataconnectOperationFailed extends Data.TaggedError(
  "GCP.Firebasedataconnect.OperationFailed",
)<{
  operation: string;
  message: string;
}> {}

export class FirebasedataconnectOperationPending extends Data.TaggedError(
  "GCP.Firebasedataconnect.OperationPending",
)<{
  operation: string;
}> {}
...
const alreadyExists = (error: firebasedataconnect.Status | undefined) =>
  error?.code === 6 ||
  (error?.message ?? "").toUpperCase().includes("ALREADY_EXISTS");

const isNotFoundStatus = (error: firebasedataconnect.Status | undefined) =>
  error?.code === 5 ||
  (error?.message ?? "").toLowerCase().includes("not found");

const isIgnorable = (
  error: firebasedataconnect.Status | undefined,
...
export const waitForOperation = (
  operation: firebasedataconnect.Operation,
  options?: {
    notFoundOk?: boolean;
    times?: number;
  ...
  Effect.gen(function* () {
    ...
    if (operation.done === true) {
      if (operation.error && !isIgnorable(operation.error, options)) {
        return yield* new FirebasedataconnectOperationFailed({
          operation: name ?? "",
    ...
    const getOperation = firebasedataconnect.getProjectsLocationsOperations({
      name,
    });
    const resolved =
      options?.notFoundOk === true
        ? getOperation.pipe(
            Effect.catchTag("NotFound", () =>
              Effect.succeed<firebasedataconnect.Operation>({
                name,
                done: true,
              }),
            ),
    ...
    return yield* resolved.pipe(
      Effect.filterOrFail(
        ...
        () => new FirebasedataconnectOperationPending({ operation: name }),
      ),
      Effect.filterOrFail(
        (current) => !current.error || isIgnorable(current.error, options),
        (current) =>
          new FirebasedataconnectOperationFailed({
            operation: name,
            message: current.error?.message ?? "operation failed",
          }),
      ),
      Effect.retry({
        while: (error) => error instanceof FirebasedataconnectOperationPending,
