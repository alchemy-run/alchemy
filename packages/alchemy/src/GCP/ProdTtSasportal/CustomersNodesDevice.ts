...
export type CustomersNodesDevice = Resource<
  "GCP.ProdTtSasportal.CustomersNodesDevice",
...
/**
 * A Citizens Broadband Radio Service Device (CBSD) under a SAS Portal customer node in the production-test (prod-tt) environment.
 *
 ...
 * ```typescript
 * const device = yield* GCP.ProdTtSasportal.CustomersNodesDevice("Cbsd", {
 *   node: node.name,
...
export class CustomersNodesDeviceNotResolved extends Data.TaggedError(
  "GCP.ProdTtSasportal.CustomersNodesDeviceNotResolved",
)<{
  name: string;
}> {}
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
      ...
      if (current === undefined) {
        const created = yield* sas
          .createCustomersNodesDevices({ parent, body })
          .pipe(
