...
import {
  ...
  hasOwnershipMarker,
  ignoreMissing,
  listAllCustomerNodeDevices,
  listCustomerNodesDevices,
  ownedByAlchemy,
  ownershipLabels,
  replaceOnIdentity,
  sameJson,
...
export type SignedCustomersNodesDeviceProps = {
  /**
...
export type SignedCustomersNodesDevice = Resource<
  "GCP.ProdTtSasportal.SignedCustomersNodesDevice",
  SignedCustomersNodesDeviceProps,
  {
    /** Full resource name. */
    name: string;
...
/**
 * A CPI-signed CBSD under a SAS Portal customer node in the production-test (prod-tt) environment.
 *
 ...
 * ```typescript
 * const device = yield* GCP.ProdTtSasportal.SignedCustomersNodesDevice("Cbsd", {
 *   node: parent.name,
 *   encodedDevice: jwt,
 *   installerId: "CPI-123",
 ...
 */
export const SignedCustomersNodesDevice = Resource<SignedCustomersNodesDevice>(
  "GCP.ProdTtSasportal.SignedCustomersNodesDevice",
);

export class SignedCustomersNodesDeviceNotResolved extends Data.TaggedError(
  "GCP.ProdTtSasportal.SignedCustomersNodesDeviceNotResolved",
)<{
  name: string;
}> {}

export const SignedCustomersNodesDeviceProvider = () =>
  Provider.succeed(SignedCustomersNodesDevice, {
    ...
    read: Effect.fn(function* ({ id, olds, output }) {
      ...
      if (existing === undefined) {
        existing = yield* findOwned(
          yield* listCustomerNodesDevices(parent),
          id,
        );
      }
    ...
    list: () =>
      Effect.gen(function* () {
        const items = yield* listAllCustomerNodeDevices();
        return items
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
      ...
      if (current === undefined) {
        current = yield* findOwned(yield* listCustomerNodesDevices(parent), id);
      }

      if (current === undefined) {
        const created = yield* sas
          .createSignedCustomersNodesDevices({
            parent,
            body: {
          ...
          .pipe(
            Effect.catchTag("Conflict", () =>
              listCustomerNodesDevices(parent).pipe(
      ...
      if (current === undefined) {
        return yield* new SignedCustomersNodesDeviceNotResolved({
          name: name || parent,
        });
      }
