...
import {
  ...
  hasOwnershipMarker,
  ignoreMissing,
  listAllCustomerDeploymentsDevices,
  listCustomerDeploymentsDevices,
  ownedByAlchemy,
  ownershipLabels,
  replaceOnIdentity,
  sameJson,
...
export type SignedCustomersDeploymentsDeviceProps = {
  /**
   * Parent SAS deployment (`customers/{customer}/deployments/{deployment}`).
...
export type SignedCustomersDeploymentsDevice = Resource<
  "GCP.ProdTtSasportal.SignedCustomersDeploymentsDevice",
  SignedCustomersDeploymentsDeviceProps,
  {
    /** Full resource name. */
    name: string;
...
/**
 * A CPI-signed CBSD under a SAS Portal customer deployment in the production-test (prod-tt) environment.
 *
 ...
 * ```typescript
 * const device = yield* GCP.ProdTtSasportal.SignedCustomersDeploymentsDevice("Cbsd", {
...
export class SignedCustomersDeploymentsDeviceNotResolved extends Data.TaggedError(
  "GCP.ProdTtSasportal.SignedCustomersDeploymentsDeviceNotResolved",
)<{
  name: string;
}> {}

export const SignedCustomersDeploymentsDeviceProvider = () =>
  Provider.succeed(SignedCustomersDeploymentsDevice, {
    ...
    read: Effect.fn(function* ({ id, olds, output }) {
      ...
      if (existing === undefined) {
        existing = yield* findOwned(
          yield* listCustomerDeploymentsDevices(parent),
          id,
        );
      }
    ...
    list: () =>
      Effect.gen(function* () {
        const items = yield* listAllCustomerDeploymentsDevices();
        return items
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
      ...
      if (current === undefined) {
        const created = yield* sas
          .createSignedCustomersDeploymentsDevices({
            parent,
            body: {
          ...
          .pipe(
            Effect.catchTag("Conflict", () =>
              listCustomerDeploymentsDevices(parent).pipe(
      ...
      if (current === undefined) {
        return yield* new SignedCustomersDeploymentsDeviceNotResolved({
          name: name || parent,
        });
      }
