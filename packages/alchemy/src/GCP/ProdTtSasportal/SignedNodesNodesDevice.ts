...
export type SignedNodesNodesDevice = Resource<
  "GCP.ProdTtSasportal.SignedNodesNodesDevice",
...
/**
 * A CPI-signed CBSD under a nested SAS Portal node in the production-test (prod-tt) environment.
 *
 ...
 * ```typescript
 * const device = yield* GCP.ProdTtSasportal.SignedNodesNodesDevice("Cbsd", {
 *   node: parent.name,
 *   encodedDevice: jwt,
 *   installerId: "CPI-123",
...
export class SignedNodesNodesDeviceNotResolved extends Data.TaggedError(
  "GCP.ProdTtSasportal.SignedNodesNodesDeviceNotResolved",
)<{
  name: string;
}> {}
