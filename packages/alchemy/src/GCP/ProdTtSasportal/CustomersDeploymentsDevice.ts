...
export type CustomersDeploymentsDeviceProps = {
  /**
   * Parent SAS deployment (`customers/{customer}/deployments/{deployment}` or a node deployment name).
...
export type CustomersDeploymentsDevice = Resource<
  "GCP.ProdTtSasportal.CustomersDeploymentsDevice",
...
/**
 * A Citizens Broadband Radio Service Device (CBSD) under a SAS Portal customer deployment in the production-test (prod-tt) environment.
 *
 ...
 * ```typescript
 * const device = yield* GCP.ProdTtSasportal.CustomersDeploymentsDevice("Cbsd", {
...
export class CustomersDeploymentsDeviceNotResolved extends Data.TaggedError(
  "GCP.ProdTtSasportal.CustomersDeploymentsDeviceNotResolved",
)<{
  name: string;
}> {}
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
      ...
      if (current === undefined) {
        const created = yield* sas
          .createCustomersDeploymentsDevices({ parent, body })
          .pipe(
