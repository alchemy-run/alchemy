...
export const itemOwnedByAlchemy = (id: string, item: CatalogItem) =>
  Effect.gen(function* () {
    if (yield* ownedByAlchemy(id, item.description)) return true;
...
export const findOwnedCatalogItem = (
  id: string,
  parent: string,
  hinted?: string,
) =>
  Effect.gen(function* () {
    if (hinted !== undefined && hinted.length > 0) {
      const existing = yield* getCatalogItem(hinted);
      if (existing !== undefined) return existing;
    }
    const items = yield* listCatalogItems(parent);
    for (const item of items) {
      if (yield* itemOwnedByAlchemy(id, item)) return item;
    }
    return undefined as CatalogItem | undefined;
  });
