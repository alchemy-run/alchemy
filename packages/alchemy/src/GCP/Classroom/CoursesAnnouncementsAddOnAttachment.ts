...
export type CoursesAnnouncementsAddOnAttachmentProps = AddOnAttachmentBody & {
  /**
   * Parent course id. Immutable — changing it replaces the attachment.
   */
  courseId: string;
  /**
   * Parent announcement id (`itemId`). Immutable — changing it
...
export type CoursesAnnouncementsAddOnAttachment = Resource<
  "GCP.Classroom.CoursesAnnouncementsAddOnAttachment",
  CoursesAnnouncementsAddOnAttachmentProps,
  {
...
/**
 * A Classroom add-on attachment on an announcement.
 *
 ...
 * Parent course, announcement, and attachment id are identity. Title,
 * view URIs, due date, and points update in place.
 *
 * ### Creating an Add-On Attachment
 * **Example:** Partner-first attachment
 * ```typescript
 * const attachment = yield* GCP.Classroom.CoursesAnnouncementsAddOnAttachment(
 *   "Quiz",
 *   {
 ...
 * @category Classroom
 */
export const CoursesAnnouncementsAddOnAttachment =
  Resource<CoursesAnnouncementsAddOnAttachment>(
    "GCP.Classroom.CoursesAnnouncementsAddOnAttachment",
  );

export class CoursesAnnouncementsAddOnAttachmentNotResolved extends Data.TaggedError(
  "GCP.Classroom.CoursesAnnouncementsAddOnAttachmentNotResolved",
)<{
  courseId: string;
  itemId: string;
  attachmentId: string;
...
const findOwned = (courseId: string, itemId: string, id: string) =>
  Effect.gen(function* () {
    const items = yield* listAnnouncementAddOnAttachments(courseId, itemId);
...
const listOwnedAttachments = () =>
  Effect.gen(function* () {
    ...
    const attachments = yield* Effect.forEach(
      ...
      (courseId) =>
        Effect.gen(function* () {
          const posts = yield* listAnnouncements(courseId);
          const nested = yield* Effect.forEach(
            posts.flatMap((post) =>
              post.id !== undefined && post.id.length > 0 ? [post.id] : [],
            ),
            (itemId) => listAnnouncementAddOnAttachments(courseId, itemId),
            { concurrency: 2 },
          );
          return nested.flat();
        }),
...
export const CoursesAnnouncementsAddOnAttachmentProvider = () =>
  Provider.succeed(CoursesAnnouncementsAddOnAttachment, {
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
      ...
      if (current === undefined) {
        const created = yield* classroom
          .createCoursesAnnouncementsAddOnAttachments({
            courseId,
            itemId,
      ...
      if (current === undefined) {
        return yield* new CoursesAnnouncementsAddOnAttachmentNotResolved({
          courseId,
          itemId,
      ...
      if (updateMask.length > 0) {
        current = yield* classroom.patchCoursesAnnouncementsAddOnAttachments({
          courseId,
          itemId,
    ...
    delete: Effect.fn(function* ({ output }) {
      ...
      yield* classroom
        .deleteCoursesAnnouncementsAddOnAttachments({
