...
export class VmmigrationOperationFailed extends Data.TaggedError(
  "GCP.Vmmigration.OperationFailed",
)<{
  operation: string;
  message: string;
}> {}

export class VmmigrationOperationPending extends Data.TaggedError(
...
export const rfc1035 = (name: string, fallback = "vm"): string => {
  let next = name
    .toLowerCase()
    .replace(/[^a-z0-9-]/g, "-")
    .replace(/-+/g, "-")
...
export const sourceOf = (value: string, project: string, location: string) =>
...
const alreadyExists = (error: vm.Status | undefined) =>
  error?.code === 6 ||
  (error?.message ?? "").toUpperCase().includes("ALREADY_EXISTS");

const isNotFoundStatus = (error: vm.Status | undefined) =>
  error?.code === 5 ||
  (error?.message ?? "").toLowerCase().includes("not found");

const isIgnorable = (
  error: vm.Status | undefined,
  ...
  Effect.gen(function* () {
    ...
    if (operation.done === true) {
      if (operation.error && !isIgnorable(operation.error, options)) {
        return yield* new VmmigrationOperationFailed({
          operation: name ?? "",
    ...
    const getOperation = vm.getProjectsLocationsOperations({ name });
    const resolved =
      options?.notFoundOk === true
        ? getOperation.pipe(
            Effect.catchTag("NotFound", () =>
              Effect.succeed<vm.Operation>({
                name,
                done: true,
              }),
            ),
    ...
    return yield* resolved.pipe(
      Effect.filterOrFail(
        ...
        () => new VmmigrationOperationPending({ operation: name }),
      ),
      Effect.flatMap((current) => {
        const error = current.error;
        if (error && !isIgnorable(error, options)) {
          return Effect.fail(
            new VmmigrationOperationFailed({
              operation: name,
      ...
      Effect.retry({
        while: (error) => error._tag === "GCP.Vmmigration.OperationPending",
