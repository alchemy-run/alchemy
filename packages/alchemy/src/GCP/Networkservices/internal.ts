...
export class NetworkservicesOperationFailed extends Data.TaggedError(
...
export class NetworkservicesOperationPending extends Data.TaggedError(
  "GCP.Networkservices.OperationPending",
)<{
  operation: string;
}> {}
...
const alreadyExists = (error: networkservices.Status | undefined) =>
  error?.code === 6 ||
  (error?.message ?? "").toUpperCase().includes("ALREADY_EXISTS");

const isNotFoundStatus = (error: networkservices.Status | undefined) =>
  error?.code === 5 ||
  ...
  Effect.gen(function* () {
    ...
    if (operation.done === true) {
      if (operation.error) {
        ...
        return yield* new NetworkservicesOperationFailed({
          operation: name ?? "",
    ...
    const getOperation = networkservices.getProjectsLocationsOperations({
      name,
    });
    const resolved =
      options?.notFoundOk === true
        ? getOperation.pipe(
            Effect.catchTag("NotFound", () =>
              ...
              } satisfies networkservices.Operation),
            ),
          )
        : getOperation.pipe(
            Effect.retry({
    ...
    return yield* resolved.pipe(
      Effect.filterOrFail(
        ...
        () => new NetworkservicesOperationPending({ operation: name }),
      ),
      Effect.flatMap((current) => {
        ...
        return Effect.fail(
          new NetworkservicesOperationFailed({
            operation: name,
      ...
      Effect.retry({
        while: (error) => error._tag === "GCP.Networkservices.OperationPending",
        times: options?.times ?? 10,
        schedule: Schedule.spaced(options?.delay ?? "4 seconds"),
      }),
    );
  });
