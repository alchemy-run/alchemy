...
export type NodesDeviceProps = {
  ...
  parent: string;
  /**
   * Server-assigned device resource name. Immutable — changing it
   * replaces the device.
   */
  name?: string;
  /**
...
/**
 * A Spectrum Access System (SAS) Portal CBSD under a node.
 *
 * SAS Portal devices have no labels field, so Alchemy stamps ownership
 * into `displayName` for `list` / nuke. Parent node, FCC id, and serial
...
export class NodesDeviceNotResolved extends Data.TaggedError(
  "GCP.Sasportal.NodesDeviceNotResolved",
)<{
  parent: string;
  name: string;
}> {}
...
export const NodesDeviceProvider = () =>
  Provider.succeed(NodesDevice, {
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
      ...
      if (current === undefined) {
        return yield* new NodesDeviceNotResolved({
          parent,
