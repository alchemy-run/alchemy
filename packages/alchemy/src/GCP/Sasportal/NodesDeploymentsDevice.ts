...
export type NodesDeploymentsDevice = Resource<
  "GCP.Sasportal.NodesDeploymentsDevice",
...
/**
 * A Spectrum Access System (SAS) Portal CBSD under a node deployment.
 *
...
export class NodesDeploymentsDeviceNotResolved extends Data.TaggedError(
  "GCP.Sasportal.NodesDeploymentsDeviceNotResolved",
)<{
  parent: string;
  name: string;
}> {}
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
      ...
      if (current === undefined) {
        const created = yield* sasportal
          .createNodesDeploymentsDevices({
            parent,
            body: desired,
          })
          .pipe(
            Effect.catchTag("Conflict", () =>
              findOwnedNodeDeploymentDevice(id, parent).pipe(
