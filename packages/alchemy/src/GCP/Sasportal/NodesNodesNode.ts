...
export type NodesNodesNodeProps = {
  ...
  parent: string;
  /**
   * Server-assigned node resource name (`nodes/{node}/nodes/{node}/nodes/{node}`).
...
export type NodesNodesNode = Resource<
  ...
  NodesNodesNodeProps,
  {
    /** Resource name `nodes/{node}/nodes/{node}/nodes/{node}`. */
    name: string;
...
/**
 * A Spectrum Access System (SAS) Portal node nested under another nested node.
 *
 ...
 * **Example:** Nested node
 * ```typescript
 * const child = yield* GCP.Sasportal.NodesNodesNode("Sector", {
 *   parent: child.name,
