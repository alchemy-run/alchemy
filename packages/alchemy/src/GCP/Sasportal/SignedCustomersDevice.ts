...
export type SignedCustomersDevice = Resource<
  "GCP.Sasportal.SignedCustomersDevice",
...
/**
 * A CPI-signed Spectrum Access System (SAS) Portal CBSD under a customer.
 *
 ...
 * ```typescript
 * const device = yield* GCP.Sasportal.SignedCustomersDevice("Cbsd", {
...
export class SignedCustomersDeviceNotResolved extends Data.TaggedError(
  "GCP.Sasportal.SignedCustomersDeviceNotResolved",
)<{
  parent: string;
  name: string;
}> {}
...
export const SignedCustomersDeviceProvider = () =>
  Provider.succeed(SignedCustomersDevice, {
    ...
    read: Effect.fn(function* ({ id, olds, output }) {
      ...
      if (existing === undefined) {
        const found =
          ...
          (yield* scanOwnedCustomerDevice(id, parent));
        existing = found?.row;
    ...
    list: () =>
      Effect.gen(function* () {
        const env = yield* GcpEnvironment.current;
        const parents = yield* listCustomers();
        ...
        const created = yield* sasportal
          ...
          .pipe(
            Effect.catchTag("Conflict", () =>
              Effect.gen(function* () {
                const found = yield* findOwnedCustomerDevice(id, parent);
