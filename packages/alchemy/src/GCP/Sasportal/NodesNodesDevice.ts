...
export type NodesNodesDevice = Resource<
  "GCP.Sasportal.NodesNodesDevice",
...
/**
 * A Spectrum Access System (SAS) Portal CBSD under a nested node.
 *
 ...
 * ```typescript
 * const device = yield* GCP.Sasportal.NodesNodesDevice("Cbsd", {
 *   parent: child.name,
...
export class NodesNodesDeviceNotResolved extends Data.TaggedError(
  "GCP.Sasportal.NodesNodesDeviceNotResolved",
)<{
  parent: string;
  name: string;
}> {}
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
      ...
      if (current === undefined) {
        const created = yield* sasportal
          .createNodesNodesDevices({
            parent,
            body: desired,
          })
          .pipe(
            Effect.catchTag("Conflict", () =>
              findOwnedNodeNodeDevice(id, parent).pipe(
