...
import {
  ...
  expandPath,
  findDeviceBySerial,
  findOwnedNodeDeploymentDevice,
  frequencyRangesOf,
  getDeploymentDevice,
  hasOwnershipMarker,
  lastSegment,
  listNodeDeploymentDevices,
...
export type SignedNodesDeploymentsDeviceProps = {
  /**
   * Parent SAS deployment under a node, as `nodes/{node}/deployments/{deployment}`.
   * Immutable — changing it replaces the device.
   */
  parent: string;
  /**
   * Server-assigned device resource name
   * (`nodes/{node}/deployments/{deployment}/devices/{device}`). Immutable — changing it
   * replaces the device.
   */
  name?: string;
  /**
...
export type SignedNodesDeploymentsDevice = Resource<
  "GCP.Sasportal.SignedNodesDeploymentsDevice",
  SignedNodesDeploymentsDeviceProps,
  {
    /** Resource name `nodes/{node}/deployments/{deployment}/devices/{device}`. */
    name: string;
...
/**
 * A CPI-signed Spectrum Access System (SAS) Portal CBSD under a node deployment.
 *
...
export const SignedNodesDeploymentsDeviceProvider = () =>
  Provider.succeed(SignedNodesDeploymentsDevice, {
    ...
    read: Effect.fn(function* ({ id, olds, output }) {
      ...
      if (existing === undefined) {
        const found =
          ...
          (yield* scanOwnedNodeDeploymentDevice(id, parent));
        existing = found?.row;
    ...
    list: () =>
      Effect.gen(function* () {
        const env = yield* GcpEnvironment.current;
        const parents = yield* walkNodeDeployments();
        const pages = yield* Effect.forEach(
          ...
          (entry) => {
            ...
            return parent.length === 0
              ? Effect.succeed([])
              : listNodeDeploymentDevices(parent).pipe(
                  Effect.map((rows) =>
                    rows
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
      ...
      if (current === undefined) {
        const created = yield* sasportal
          .createSignedNodesDeploymentsDevices({
            parent,
          ...
          .pipe(
            Effect.catchTag("Conflict", () =>
              Effect.gen(function* () {
                const found = yield* findOwnedNodeDeploymentDevice(id, parent);
      ...
      if (current === undefined) {
        return yield* new SignedNodesDeploymentsDeviceNotResolved({
          parent,
