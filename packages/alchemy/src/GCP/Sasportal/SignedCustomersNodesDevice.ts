...
export type SignedCustomersNodesDevice = Resource<
  "GCP.Sasportal.SignedCustomersNodesDevice",
...
/**
 * A CPI-signed Spectrum Access System (SAS) Portal CBSD under a customer node.
 *
 ...
 * ```typescript
 * const device = yield* GCP.Sasportal.SignedCustomersNodesDevice("Cbsd", {
 *   parent: node.name,
 *   encodedDevice: jwt,
 *   installerId: "cpi-1",
...
export class SignedCustomersNodesDeviceNotResolved extends Data.TaggedError(
  "GCP.Sasportal.SignedCustomersNodesDeviceNotResolved",
)<{
  parent: string;
  name: string;
}> {}
        ...
        const created = yield* sasportal
          ...
          .pipe(
            Effect.catchTag("Conflict", () =>
              Effect.gen(function* () {
                const found = yield* findOwnedCustomerNodeDevice(id, parent);
