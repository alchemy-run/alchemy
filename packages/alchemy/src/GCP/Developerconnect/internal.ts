...
export class DeveloperconnectOperationFailed extends Data.TaggedError(
  "GCP.Developerconnect.OperationFailed",
)<{
  operation: string;
  message: string;
}> {}

export class DeveloperconnectOperationPending extends Data.TaggedError(
  "GCP.Developerconnect.OperationPending",
)<{
  operation: string;
}> {}
...
const alreadyExists = (error: developerconnect.Status | undefined) =>
  error?.code === 6 ||
  (error?.message ?? "").toUpperCase().includes("ALREADY_EXISTS");

const isNotFoundStatus = (error: developerconnect.Status | undefined) =>
  error?.code === 5 ||
  (error?.message ?? "").toLowerCase().includes("not found");

const isIgnorable = (
  error: developerconnect.Status | undefined,
  ...
  Effect.gen(function* () {
    ...
    if (operation.done === true) {
      if (operation.error && !isIgnorable(operation.error, options)) {
        return yield* new DeveloperconnectOperationFailed({
          operation: name ?? "",
    ...
    const getOperation = developerconnect.getProjectsLocationsOperations({
      name,
    });
    const resolved =
      options?.notFoundOk === true
        ? getOperation.pipe(
            Effect.catchTag("NotFound", () =>
              Effect.succeed<developerconnect.Operation>({
                name,
                done: true,
              }),
            ),
    ...
    return yield* resolved.pipe(
      Effect.filterOrFail(
        ...
        () => new DeveloperconnectOperationPending({ operation: name }),
      ),
      Effect.filterOrFail(
        (current) => !current.error || isIgnorable(current.error, options),
        (current) =>
          new DeveloperconnectOperationFailed({
            operation: name,
            message: current.error?.message ?? "operation failed",
          }),
      ),
      Effect.retry({
        while: (error) => error instanceof DeveloperconnectOperationPending,
