...
export class MigrationcenterOperationFailed extends Data.TaggedError(
  "GCP.Migrationcenter.OperationFailed",
)<{
  operation: string;
  message: string;
}> {}

export class MigrationcenterOperationPending extends Data.TaggedError(
...
export class ResourceNotResolved extends Data.TaggedError(
  "GCP.Migrationcenter.ResourceNotResolved",
)<{
  name: string;
}> {}

export class ResourceStillExists extends Data.TaggedError(
  "GCP.Migrationcenter.ResourceStillExists",
)<{
  name: string;
}> {}
...
export const rfc1035 = (name: string, fallback = "mc"): string => {
  let next = name
    .toLowerCase()
    .replace(/[^a-z0-9-]/g, "-")
    .replace(/-+/g, "-")
...
const alreadyExists = (error: mc.Status | undefined) =>
  error?.code === 6 ||
  (error?.message ?? "").toUpperCase().includes("ALREADY_EXISTS");

const isNotFoundStatus = (error: mc.Status | undefined) =>
  error?.code === 5 ||
  (error?.message ?? "").toLowerCase().includes("not found");

const isIgnorable = (
  error: mc.Status | undefined,
  ...
  Effect.gen(function* () {
    ...
    if (operation.done === true) {
      if (operation.error && !isIgnorable(operation.error, options)) {
        return yield* new MigrationcenterOperationFailed({
          operation: name ?? "",
    ...
    const getOperation = mc.getProjectsLocationsOperations({ name });
    const resolved =
      options?.notFoundOk === true
        ? getOperation.pipe(
            Effect.catchTag("NotFound", () =>
              Effect.succeed<mc.Operation>({
                name,
                done: true,
              }),
            ),
    ...
    return yield* resolved.pipe(
      Effect.filterOrFail(
        ...
        () => new MigrationcenterOperationPending({ operation: name }),
      ),
      Effect.flatMap((current) => {
        const error = current.error;
        if (error && !isIgnorable(error, options)) {
          return Effect.fail(
            new MigrationcenterOperationFailed({
              operation: name,
      ...
      Effect.retry({
        while: (error) => error._tag === "GCP.Migrationcenter.OperationPending",
...
export const encodeOwnershipLine = (
  ...
  const combined = `${marker} ${trimmed}`;
  return combined.length <= maxLength ? combined : marker;
};
