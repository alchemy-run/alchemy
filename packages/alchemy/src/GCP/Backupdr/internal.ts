...
export class BackupdrOperationFailed extends Data.TaggedError(
...
export class BackupdrOperationPending extends Data.TaggedError(
  "GCP.Backupdr.OperationPending",
)<{
  operation: string;
}> {}
...
const alreadyExists = (error: backupdr.Status | undefined) =>
  error?.code === 6 ||
  (error?.message ?? "").toUpperCase().includes("ALREADY_EXISTS");

const isNotFoundStatus = (error: backupdr.Status | undefined) =>
  error?.code === 5 ||
  (error?.message ?? "").toLowerCase().includes("not found");

const isIgnorable = (
  error: backupdr.Status | undefined,
  ...
  Effect.gen(function* () {
    ...
    if (operation.done === true) {
      if (operation.error && !isIgnorable(operation.error, options)) {
        return yield* new BackupdrOperationFailed({
          operation: name ?? "",
    ...
    const getOperation = backupdr.getProjectsLocationsOperations({ name });
    const resolved =
      options?.notFoundOk === true
        ? getOperation.pipe(
            Effect.catchTag("NotFound", () =>
              Effect.succeed<backupdr.Operation>({
                name,
                done: true,
              }),
            ),
    ...
    return yield* resolved.pipe(
      Effect.filterOrFail(
        ...
        () => new BackupdrOperationPending({ operation: name }),
      ),
      Effect.flatMap((current) => {
        const error = current.error;
        if (error && !isIgnorable(error, options)) {
          return Effect.fail(
            new BackupdrOperationFailed({
              operation: name,
      ...
      Effect.retry({
        while: (error) => error._tag === "GCP.Backupdr.OperationPending",
