...
export interface QueryState {
  ...
  /** Slash-separated stack/stage/resource path. Omit for the root. */
  readonly path?: string;
  /** Include every descendant instead of immediate children. */
...
export interface ReadState {
  ...
  /** Slash-separated stack/stage/resource path. Omit for the root. */
  readonly path?: string;
  /** Read every descendant when the path identifies a directory. */
  readonly recursive?: boolean;
}

export interface DeleteState {
  readonly source?: StateSource;
  /** Slash-separated stack/stage/resource path. */
  readonly path: string;
...
const allFiles = Effect.fn(function* (state: StateService) {
...
export const listState = Effect.fn(function* (
  state: StateService,
  { path, recursive = false }: QueryState,
) {
...
export const readState = Effect.fn(function* (
  state: StateService,
  { path, recursive = false }: ReadState,
) {
  const parts = yield* parsePath(path);
...
export const deleteState = Effect.fn(function* (
  state: StateService,
  { path, recursive = false }: DeleteState,
) {
  const parts = yield* parsePath(path);
  if (parts.length === 0) {
