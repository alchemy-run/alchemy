 ...
 * outputs, and (when the store keeps one) deployment history and journals.
 * Point it at any {@link StateService} (typically `makeHttpStateStore`
 * against a deployed state-store worker) and serve the
 * `@alchemy.run/dashboard` SPA next to it (same-origin) and the full
 * dashboard works: structure, decorations, timelines, deployment picker,
 * outputs.
 *
 ...
 * - `GET /api/v2/deployments`          — deployment records, newest first (404 without history)
...
export interface ViewerOptions {
   ...
   * without it `/api/v2/deployments*` 404 and the SPA hides its picker.
   */
   ...
   * deployment is live — without a history, every tick).
...
export const viewer = (options: ViewerOptions) => {
  ...
  return Effect.gen(function* () {
    ...
    if (route === "/api/v2/events") {
      ...
      // poll: with a history the deployment record is re-read every tick
      // (cheap) and the full states/outputs only when the record changed
      // or a run is live; without one, states/outputs are re-read every
      // tick. The host, its poller, and the store subscriptions all live
