 ...
 * - browser-safe: only types are imported, so the SPA bundles it as-is
 *
...
/** What the last apply DID to a node — the terminal statuses' verdicts. */
...
/**
 * What a plan WOULD do to a node. Mirrors the engine's plan actions
 * (`Plan.CRUD["action"]` minus `noop`, plus `run` for stack actions).
 */
 ...
 * status a resource reports within a deployment resets its timeline.
 */
...
/**
 * One node in the structural graph. Carries the stable identity fields plus
 * the plan/ghost baseline set by the rebuild passes ({@link applyStates},
 * {@link applyPlan}, {@link applyStructure}). Live, event-driven state
 * (status transitions, apply results, notes) lives in {@link Decoration} —
 * events never mutate structure.
 */
...
/** A plan awaiting browser-side approval. */
export interface DocumentApproval {
  /** the approval's id — what the browser posts back its decision under */
  id: string;
  /** the plan awaiting approval (opaque to the document core) */
  plan: unknown;
  requestedAt: number;
}
 ...
 * fall back to the logicalId index (fan-out to every fqn sharing the
 * logicalId), finally to the bare id (root-level fqns ARE their logicalId).
 */
const targetsOf = (
  doc: DeploymentDocument,
...
export const applyPlan = (
  ...
  for (const planNode of planNodes) {
    // deletions and orphans exist(ed) in the cloud; creates don't yet
    const leaving =
      planNode.action === "delete" || planNode.action === "orphaned";
    ...
    if (node === undefined) {
      node = {
        ...
        kind: "resource",
        type: planNode.type,
        status: leaving ? "created" : "pending",
        bindings: [],
...
/**
 * O(1) incremental fold of one event — live apply events (status-change,
...
/** Set/refresh the deployment record (from `DeploymentHistory.get`/`list`). */
...
/**
 * Replay a stored journal (from `DeploymentHistory.readEvents`) through
  ...
  for (const envelope of events) {
    ...
    const enriched =
      eventFqn === undefined && envelope.fqn !== undefined
        ? ({ ...event, fqn: envelope.fqn } as DashboardEvent)
        : event;
