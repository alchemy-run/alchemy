...
/**
 * Dashboard document host (in-process transport): owns ONE mutable
 * {@link DeploymentDocument} per `(stack, stage)` and turns every fold
 * mutation into debounced patch frames for subscribers.
 *
 * The host is the thin glue between the durable spine (the state store and,
 * when available, its deployment history) and the pure document core: it
 * reads, the fold computes, subscribers receive `snapshot`-then-`patches`
 * frames. No rendering semantics live here.
 */

/** One frame of the wire protocol (SSE `data:` payload). */
...
export interface DocumentHostOptions {
   ...
   * without it the document still hydrates from persisted states and live
   * apply events, but no past run is restored and the deployment record
   * only ever comes from the live reporter's bookends.
   */
...
export interface DocumentHost {
  ...
  readonly applyEvent: (event: DashboardEvent) => Effect.Effect<void>;
   ...
   * the history-backed record (version, initiator) when the store has one.
   */
   ...
   * waiting briefly for a journaling engine's end to land; if the persisted
   * end still hasn't surfaced (remote stores are eventually consistent and
   * the CLI exits shortly after done) or there is no history at all,
   * synthesize the end with the tee-reported `outcome` so the run settles
   * NOW.
   */
  readonly deploymentDone: (
    outcome?: "succeeded" | "failed",
  ) => Effect.Effect<void>;
  readonly setApproval: (approval: {
    id: string;
    plan: unknown;
  }) => Effect.Effect<void>;
 ...
 *
 * 1. persisted states (`list` + per-fqn `get`)
 * 2. with a history: the newest deployment record + a full journal re-fold
 *    — an OPEN record means the server (re)started mid-deploy and the fold
 *    rebuilds the in-flight picture; a CLOSED record restores the last
 *    deployment's decorations/op-spans
 * 3. stack outputs
 *
...
export const make: (
  ...
  const { state, history, stack, stage } = options;
  ...
  const refreshStates = () =>
    Effect.gen(function* () {
      ...
      // updated, adopted, replaced, ran) must survive the rebuild even when
      // the readback misses them — remote stores are eventually consistent
      // and can lag the writes the run just made. Deleted/orphaned/skipped
      // verdicts don't protect: retiring those is the point.
        ...
        for (const [fqn, decoration] of doc.decorations) {
          if (
            ...
            (decoration.applyResult === "created" ||
              ...
              decoration.applyResult === "adopted" ||
  ...
  const refreshDeployment = () =>
    Effect.gen(function* () {
      ...
      // deployment — a journaling engine's begin commits concurrently with
      // the reporter's start post. Folding that older ended record in
    ...
    Effect.gen(function* () {
      if (history !== undefined) {
        ...
        if (doc.deployment !== undefined && doc.deployment.live) {
          // a journaling engine's end races the reporter's done post —
          // give it one beat
      ...
      if (doc.deployment !== undefined && doc.deployment.live) {
        // The persisted end never surfaced (or there is no history at all).
        // The done post itself is authoritative that the run ENDED —
        // synthesize the end bookend. Outcome comes from the tee; fall back
        // to what the decorations say.
        const at = yield* Clock.currentTimeMillis;
        const derived =
          outcome ??
  ...
  if (newest !== undefined && history !== undefined) {
  ...
  // the history until it closes. The apply/done tee can race a journaling
  // engine's end (or the deploy may not be teeing at all — another host, a
  // killed process), and a record that ends out-of-band would otherwise
  // stay `live: true` in the document forever.
  if (history !== undefined) {
    yield* Effect.forkScoped(
      Effect.gen(function* () {
        for (;;) {
