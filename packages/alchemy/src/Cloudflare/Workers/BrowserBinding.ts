...
import { Browser, type BrowserClient, BrowserError, type BrowserResponse } from "./Browser.ts";
 ...
 */
export const BrowserBinding = makeBindingLayer<Browser, cf.BrowserRun, BrowserClient>(
  Browser,
  (raw): BrowserClient => {
    const respond = (
      action: string,
    ...
    const jsonAction = <T>(
      ...
      respond(action, options).pipe(
        Effect.flatMap((response) => tryPromise(() => response.json() as Promise<T>)),
      );
      ...
      respond(action, options).pipe(
        Effect.map((response) =>
          Stream.fromReadableStream({
            evaluate: () => response.body as any as ReadableStream<Uint8Array<ArrayBufferLike>>,
            onError: (cause) =>
              new BrowserError({
    ...
    return {
      raw,
      fetch: (...args) =>
        raw.pipe(Effect.flatMap((binding) => tryPromise(() => binding.fetch(...args)))),
      quickAction,
...
const failResponse = (
  ...
  tryPromise(() => response.text()).pipe(
    Effect.flatMap((body) => {
      ...
      const message =
        (cause as cf.BrowserRunErrorResponse | undefined)?.errors?.[0]?.message ??
