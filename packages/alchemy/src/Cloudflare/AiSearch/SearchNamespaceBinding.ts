...
import { makeClient, tryAiSearch } from "./Search.ts";
import {
  type SearchNamespaceClient,
  SearchNamespace,
} from "./SearchNamespace.ts";

/**
 * Runtime layer for {@link SearchNamespace}.
 */
export const SearchNamespaceBinding = Layer.effect(
  SearchNamespace,
  Effect.gen(function* () {
    ...
    return Effect.fn(function* (namespace: Namespace) {
      ...
      } satisfies SearchNamespaceClient;
    });
  }),
);
