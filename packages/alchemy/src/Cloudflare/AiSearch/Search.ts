...
/**
 * Bind an {@link Instance} to a Worker and obtain the Effect-native
 * AI Search client (`search`, `chatCompletions`, `info`, `stats`). The
 * single-instance `ai_search` binding resolves directly to a runtime
 * `Instance`.
 *
 * `Search` is a single identifier that is simultaneously the binding's Context
 * tag, its type, and the callable — `yield* Cloudflare.AiSearch.Search(instance)`.
 *
 * Provide {@link SearchBinding} in the Worker's runtime layer.
 *
 * @binding
 * @category AI
 *
 ...
 * handlers.
 * ```typescript
 * const search = yield* Cloudflare.AiSearch.Search(instance);
 *
 * return {
...
export interface Search extends Binding.Service<
  Search,
  "Cloudflare.AiSearch.Search",
  (instance: Instance) => Effect.Effect<SearchClient>
> {}

export const Search = Binding.Service<Search>("Cloudflare.AiSearch.Search");
 ...
 *
 * Wraps the standalone `Instance` runtime binding (the binding the
 ...
 * one from `Cloudflare.AiSearch.Search(instance)` (or a namespace client's
 * `.get(instanceName)`) during the Worker's init phase.
 */
export interface SearchClient {
  /**
   * Effect resolving to the raw underlying Cloudflare `Instance`
...
/**
 * Build an {@link SearchClient} from an Effect that lazily resolves the raw
