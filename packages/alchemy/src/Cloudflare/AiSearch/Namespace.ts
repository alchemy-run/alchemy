...
const NamespaceTypeId = "Cloudflare.AiSearch.Namespace" as const;
 ...
 * Bind the namespace with `Cloudflare.AiSearch.SearchNamespace(namespace)`,
 ...
 * Provide {@link SearchNamespaceBinding} in the Worker's runtime
 * layer.
 ...
 *   Effect.gen(function* () {
 *     const ns = yield* Cloudflare.AiSearch.SearchNamespace(Docs);
 *
 *     return {
 ...
 *       }),
 *     };
 *   }).pipe(Effect.provide(Cloudflare.AiSearch.SearchNamespaceBinding)),
 * ) {}
 * ```
 *
 * @section Binding to an Async Worker
 * For a vanilla `async fetch` Worker, pass the namespace under `Worker.env`.
 * `InferEnv` types `env.SEARCH` as the runtime `Namespace` handle.
 ...
 */
export const Namespace = Resource<Namespace>(NamespaceTypeId);
