...
const InstanceTypeId = "Cloudflare.AiSearch.Instance" as const;
type InstanceTypeId = typeof InstanceTypeId;

/**
 * The kind of data source an AI Search instance indexes.
 */
export type InstanceSourceType = "r2" | "web-crawler";
...
export type InstanceProps = {
   ...
   * account-provided `default` namespace. Pass an `Namespace`'s
 ...
 * unspecified). Pass an {@link Namespace}'s `name` to group related
 ...
 * `Cloudflare.AiSearch.Search(instance)`, which attaches the
 ...
 * {@link SearchBinding} in the Worker's runtime layer.
 ...
 *   Effect.gen(function* () {
 *     const search = yield* Cloudflare.AiSearch.Search(SearchInstance);
 *
 *     return {
 ...
 * `env.SEARCH` as the runtime `Instance` handle.
 ...
 */
export const Instance = Resource<Instance>(InstanceTypeId);
 ...
 */
const toMutableBody = (news: InstanceProps) => ({
...
const toAttributes = (
  ...
  type: (normalize(instance.type) ?? "r2") as InstanceSourceType,
