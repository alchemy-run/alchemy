...
export const ImagesTag =
  <Self>() =>
  <Id extends string, Req = never>(
    id: Id,
    options: {
      resource: ImagesLike | Effect.Effect<ImagesLike, never, Req>;
    },
  ) =>
    makeBindingTag<Self, Id, ImagesClient, never, ImagesBinding | Req>(
      id,
      ImagesBinding.bind(options.resource),
    );
