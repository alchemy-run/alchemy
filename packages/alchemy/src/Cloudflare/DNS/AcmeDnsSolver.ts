 ...
 * **Example:**
 * ```typescript
 * const account = yield* LetsEncrypt;
...
const RUNTIME_PROPAGATION_DELAY = "60 seconds";
  ...
  return {
    ...
    cleanup: (record) =>
      Effect.suspend(() => {
        ...
        return client.deleteDnsRecord(id).pipe(
          Effect.tap(() => Effect.sync(() => published.delete(keyOf(record)))),
          Effect.asVoid,
