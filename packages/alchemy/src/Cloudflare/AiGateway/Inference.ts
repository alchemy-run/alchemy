...
/**
 * Binding service that turns a {@link GatewayResource} resource
 * into a typed {@link InferenceClient} for Worker runtime code. Wraps
 ...
 * `Layer`.
 *
 * Bind a {@link GatewayResource} to a Worker and obtain the
 * Effect-native AI Gateway client (`run`, `getUrl`, `model`, …).
 *
 * `Inference` is a single identifier that is simultaneously the binding's
 * Context tag, its type, and the callable —
 * `yield* Cloudflare.AiGateway.Inference(gateway)`.
 *
 * @binding
 * @product AI Gateway
 * @category AI
 ...
 * ```
 *
 * Provide {@link InferenceBinding} in the worker's runtime layer
 * to resolve the underlying Cloudflare AI binding at request time.
 */
export interface Inference extends Binding.Service<
  Inference,
  "Cloudflare.AiGateway.Inference",
  (gateway: GatewayResource) => Effect.Effect<InferenceClient>
> {}

export const Inference = Binding.Service<Inference>(
  "Cloudflare.AiGateway.Inference",
);
 ...
 * `Cloudflare.AiGateway.Inference(gateway)` inside a Worker's init phase.
 */
