...
export const AiGatewayTag =
  ...
  <Id extends string, Req = never>(
    ...
    options: {
      resource:
        | AiGatewayResource
        | Effect.Effect<AiGatewayResource, never, Req>;
    },
  ) =>
    makeBindingTag<Self, Id, AiGatewayClient, never, AiGatewayBinding | Req>(
      id,
      AiGatewayBinding.bind(options.resource),
    );
