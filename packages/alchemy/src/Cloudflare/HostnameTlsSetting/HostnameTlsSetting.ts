...
export const HostnameTlsSettingProvider = () =>
  Provider.succeed(HostnameTlsSetting, {
    ...
    list: Effect.fn(function* () {
      ...
      // of the three TLS settings and flatten the bare-array responses
      // into one row per (settingId, hostname) override.
    ...
    reconcile: Effect.fn(function* ({ news }) {
      const { zoneId, settingId, hostname } = news;

      // LIST preserves missing-as-undefined without relying on GET errors.
      const observed = yield* findSetting(zoneId, settingId, hostname);
...
/** Find an override in the collection; a missing hostname is undefined. */
