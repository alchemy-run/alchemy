...
const setArgs = (key: string, value: string | Uint8Array, options?: SetOptions): Arg[] => {
...
export const makeRead = (url: Url): ReadClient => ({
  ...
  ping: (message) =>
    command(url, "PING", message === undefined ? [] : [message]).pipe(Effect.map(asString)),
  ...
  hmget: (key, ...fields) =>
    command(url, "HMGET", [key, ...fields]).pipe(Effect.map(asNullableStringArray)),
