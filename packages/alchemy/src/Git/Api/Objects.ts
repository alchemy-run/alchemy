...
/** Reads one commit. */
export const GetCommit = HttpApiEndpoint.get("commit", "/repos/:owner/:repo/commits/:oid", {
  params: RepoOidPath,
  success: CommitInfo,
...
export const GetTree = HttpApiEndpoint.get("tree", "/repos/:owner/:repo/trees/:oid", {
  params: RepoOidPath,
...
export const GetBlob = HttpApiEndpoint.get("blob", "/repos/:owner/:repo/blobs/:oid", {
  params: RepoOidPath,
  success: Schema.Struct({
    oid: Oid,
 ...
 */
export const GetDiff = HttpApiEndpoint.get("diff", "/repos/:owner/:repo/commits/:oid/diff", {
  params: RepoOidPath,
  success: CommitDiff,
  error: [RepoNotFound, ObjectNotFound, WrongObjectType],
});
 ...
 */
export const Compare = HttpApiEndpoint.get("compare", "/repos/:owner/:repo/compare", {
  params: RepoPath,
  query: Schema.Struct({
  ...
  success: Comparison,
  error: [RepoNotFound, RefNotFound, ObjectNotFound, WrongObjectType, NoMergeBase],
});
 ...
 */
export const GetBlobRaw = HttpApiEndpoint.get("blobRaw", "/repos/:owner/:repo/blobs/:oid/raw", {});

/**
 * A file at `?path=` under `?ref=` (refname or oid; the default branch
 * when absent), walked tree by tree, as an octet-stream.
 */
export const GetFile = HttpApiEndpoint.get("file", "/repos/:owner/:repo/file", {});

/** The `objects` group, mounted at `/api/v1`. */
export class Objects extends HttpApiGroup.make("objects")
  .add(GetCommit, GetLog, GetTree, GetBlob, GetDiff, Compare, GetBlobRaw, GetFile)
  .prefix("/api/v1") {}
