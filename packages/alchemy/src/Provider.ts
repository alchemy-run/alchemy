...
/**
 * Resolve the data plane for a HOSTLESS (account-scoped) deploy-time
 * binding client — a plan-time `execute` with no resource among its
 * arguments (`AWS.EC2.getAmi`, region-wide lookups). With no bound
 * resource whose mode could anchor the routing, the run default decides:
 * a plain deploy targets the real cloud, and an `alchemy dev` run targets
 * the binding's cloud emulator — resolved as the unique `localDataPlane`
 * registered under the binding id's cloud namespace (`AWS.EC2.GetAmi` →
 * providers typed `AWS.*` → floci). All of a cloud's dual registrations
 * share one module-memoized plane by construction; if several distinct
 * planes ever match, the binding's own service prefix (`AWS.EC2.*`) is
 * preferred, and a remaining ambiguity dies rather than guessing. Zero
 * matching planes → live (nothing emulates that cloud).
 */
export const describeHostlessDataPlane = (
  bindingId: string,
): Effect.Effect<DataPlaneResolution> =>
  Effect.gen(function* () {
    const mode = yield* defaultProviderMode;
    if (mode !== "local") return { kind: "live" as const };
    const segments = bindingId.split(".");
    const cloudPrefix = `${segments[0]}.`;
    const servicePrefix = `${segments.slice(0, 2).join(".")}.`;
    const context = yield* Effect.context<never>();
    const collect = (prefix: string) => {
      const planes = new Set<() => Layer.Layer<any, any, never>>();
        ...
        if (isProviderCollectionService(value)) {
          for (const [type, provider] of Object.entries(value.providers)) {
            if (
              ...
              provider.modes !== undefined &&
              provider.localDataPlane !== undefined
            ) {
              planes.add(provider.localDataPlane);
            }
          }
        } else if (
          typeof key === "string" &&
          ...
          value.localDataPlane !== undefined
        ) {
          planes.add(value.localDataPlane);
        }
      }
      return [...planes];
    };
    let planes = collect(cloudPrefix);
    if (planes.length > 1) {
      const scoped = collect(servicePrefix);
      if (scoped.length > 0) planes = scoped;
    }
    if (planes.length === 0) return { kind: "live" as const };
    if (planes.length > 1) {
      return yield* Effect.die(
        `Hostless binding client ${bindingId} matches ${planes.length} ` +
          "distinct local data planes — cannot pick one. Register the " +
          "cloud's dual providers with a single module-memoized dataPlane " +
          "layer reference.",
      );
    }
    return { kind: "local" as const, layer: planes[0]!() };
  });

/**
 * Run `effect` against the data plane a hostless (account-scoped) call
 * should target — see {@link describeHostlessDataPlane}. In an `alchemy
 * dev` run this provides the cloud's emulator context closest around the
 * effect (so e.g. an AMI or availability-zone lookup in stack composition
 * needs no real credentials); in a plain deploy the effect runs untouched
 * against the ambient live environment.
 */
export const withHostlessDataPlane =
  (bindingId: string) =>
  <A, E, R>(effect: Effect.Effect<A, E, R>): Effect.Effect<A, E, R> =>
    describeHostlessDataPlane(bindingId).pipe(
      Effect.flatMap((plane) =>
        plane.kind === "local"
          ? (Effect.provide(effect, plane.layer) as Effect.Effect<A, E, R>)
          : effect,
      ),
    );
