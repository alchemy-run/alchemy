...
export const makeScheduledEvents = <State>(
  DurableObjectState: Context.Key<
    State,
    {
      readonly storage: Pick<DurableObjectStorage, "transaction" | "sql">;
      readonly raw: { readonly storage: cf.DurableObjectStorage };
    }
  >,
) => {
   ...
   */
  const scheduleEvent = Effect.fn(function* (
    id: string,
    runAt: Date,
    payload: unknown,
    repeatMs?: number,
   ...
   */
  const cancelEvent = Effect.fn(function* (id: string) {
    yield* ensureTable;
    const ctx = yield* DurableObjectState;
   ...
   */
  const listEvents: Effect.Effect<
    ScheduledEvent[],
    never,
   ...
   * ```
   */
  const processScheduledEvents: Effect.Effect<
    ScheduledEvent[],
    never,
    State | RuntimeContext
  > = Effect.gen(function* () {
   ...
   */
  const reconcileAlarm: Effect.Effect<void, never, State | RuntimeContext> =
    Effect.gen(function* () {
      const ctx = yield* DurableObjectState;
  ...
  return { scheduleEvent, cancelEvent, listEvents, processScheduledEvents };
};
