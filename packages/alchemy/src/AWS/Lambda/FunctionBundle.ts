      ...
      yield* Effect.fail(
        new MissingServeMountError({
          message:
            ...
            `framework's own idiom (\`mount(Site)\` from alchemy/Serve in ` +
            `a fetch-shaped server entry, a Next.js catch-all route file, ` +
            `SvelteKit's hooks.server.ts, or a Nuxt server middleware), or ` +
            `set \`server: { verify: false }\` to skip this check.`,
...
// The whole entry flow — internal extension registration (the SIGTERM
// Shutdown window), instance scope, the sandbox layer stack, and the
// exports handler dispatch — lives in the real module
// \`alchemy/AWS/Lambda/EntryRuntime\` so plain effect Lambdas and the
// framework-composite wrapper (\`alchemy/AWS/Serve\`) share one
// implementation.
