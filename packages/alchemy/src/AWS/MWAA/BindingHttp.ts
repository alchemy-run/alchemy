 ...
 */
export const makeMWAAEnvironmentHttpBinding = <I extends { Name: string }, A, E, R>(options: {
 ...
 * (default `Admin`).
 */
export const makeMWAAAirflowRoleHttpBinding = <I extends { Name: string }, A, E, R>(options: {
  ...
  Effect.gen(function* () {
    ...
    return Effect.fn(function* (environment: Environment, roleOptions?: AirflowRoleOptions) {
      const Name = yield* environment.environmentName;
            ...
            policyStatements: [
              {
                ...
                Resource: [
                  environment.arn.pipe(
                    Output.map((arn) => `${arn.replace(":environment/", ":role/")}/${airflowRole}`),
                  ),
                ],
              },
            ],
