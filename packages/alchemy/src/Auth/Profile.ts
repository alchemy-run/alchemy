    ...
    const normalizeProfile = (name: string, value: unknown): Profile => {
      ...
      if (
        ...
        return {
          ...
          providers: dropLegacyEnvEntries(candidate.providers),
        };
      }
      return {
        id: name,
        providers: dropLegacyEnvEntries(value as Profile["providers"]),
      };
    };
...
/**
 * Warn (once per provider per process) that environment variables are
 ...
 */
const warnedEnvironmentProviders = new Set<string>();
const warnEnvironmentCredentials = (
  ...
  Effect.gen(function* () {
    ...
    if (warnedEnvironmentProviders.has(provider)) return;
    warnedEnvironmentProviders.add(provider);
    yield* Console.warn(
