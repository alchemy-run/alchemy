 ...
 */
export const isPathWithin = (directory: string, candidate: string, base: string): boolean => {
  const relative = path.relative(path.resolve(base, directory), path.resolve(base, candidate));
  return (
    relative === "" ||
