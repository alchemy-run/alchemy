...
import { Ask, AskHub, AskHubMemory } from "./Ask.ts";
...
import { EventBus, EventBusMemory } from "./EventBus.ts";
import { type Event, isWorldOwned } from "./Event.ts";
...
import { TraceStore, TraceStoreMemory } from "./TraceStore.ts";
 ...
 *
 * Deliberately absent: auto-delivery and ANY subscription (canon §2 +
 * kernel-pruning ruling — `AI.when` is a pure input declaration;
 * delivery is ALWAYS explicit outside code via `send`/`dispatch`/
 * `steer`, and run endings are exclusively `settle(key, event)` from
 * the component that consumed the wire), `AI.check`/`AI.fold` agents at
...
/**
 * The interpretation CORE — its components are ordinary REQUIREMENTS
 * (`R`), never polled options (kernel-pruning ruling, 2026-07-17: no
 * silent defaults — and no `serviceOption`, which is a soft default in
 * disguise: "absent" becomes an implicit configuration the core
 * silently interprets). An assembly NAMES an implementation for each,
 * including the explicit absences:
 *
 * - {@link TraceStore} — the durable record; `TraceStoreMemory` in the
 *   reference assembly.
 * - {@link AskHub} — the Ask park protocol; `AskHubMemory`, or
 *   `AskHubNone` for an assembly without asks/approvals (the first ask
 *   dies with an assembly error).
 * - {@link EventBus} — `ctx.emit` fan-out to same-harness subscribers;
 *   `EventBusMemory`, or `EventBusNone` to drop fan-out (`ctx.emit`
 *   still writes its durable Trace row — the contract half). The
 *   kernel never subscribes anything: run endings are exclusively
 *   `settle(key, event)` from the component that consumed the wire.
 *
 * Use {@link memory} (the reference ASSEMBLY) unless you are composing
 * a bespoke kernel.
 */
export const memoryCore: Layer.Layer<
  Kernel,
  never,
  LanguageModel.LanguageModel | TraceStore | AskHub | EventBus
> = Layer.effect(
  Kernel,
  Effect.gen(function* () {
    ...
    const makeRing = Effect.fn(function* (options: {
      ...
      const runTurn: RunTurn = Effect.fn(function* ({ session, seed, input }) {
        const emit = (
          type: string,
          cause: string,
          kind: string,
                      ...
                      Effect.andThen(
                        Effect.tap(
                          Deferred.make<unknown>().pipe(
                            Effect.tap((seat) =>
                              Effect.sync(() => settleSeats.set(session, seat)),
                            ),
                          ),
                          () =>
                            emitRow(
    ...
    const interpretProcess = Effect.fn(function* (term: {
      ...
      // ── perpetual rings (AI.never): each work item is served as
      // ONE kernel-default turn (agent semantics per item); the RING
      // never resolves. Health is the Trace, not an exit.
      if (halt?.mode === "never") {
        const steerCell: SteerCell = {};
        const children = new Set<{ readonly cancel: Effect.Effect<void> }>();
        const compiled = yield* compileTools(
      ...
      // ── externally settled (kernel-pruning ruling, 2026-07-17): a
      // charter with NO halt. The kernel just runs the loop — no
      // resolve/give_up tools; the model works the item with its own
      // tools, then the run PARKS. The COMPONENT (the implementation
      // Layer that consumed the wire) owns the end: `settle(key,
      // event)` completes the run's seat, correlated by the KEY the
      // run was admitted under — never by kernel subscriptions. The
      // model may CAUSE the world's event (closing the issue), but
      // the run settles on the component DELIVERING it (reconciler
      // doctrine: observation > claim). A run-key steer
      // (`steer(runKey, msg)`) wakes the parked run for another work
      // round with the steered input.
      if (halt === undefined) {
        const steerCell: SteerCell = {};
        const children = new Set<{ readonly cancel: Effect.Effect<void> }>();
        const compiled = yield* compileTools(
          ...
          runItem: (item, run) =>
            Effect.gen(function* () {
              ...
              while (true) {
                ...
                // the settling event out in the world. Keyed steers
                // that raced the round join its input.
                ...
                if (outcome._tag === "Interrupted") {
                  return yield* Effect.die(
                    new Error("interim: interrupted settled-run"),
                  );
                }
                // PARK until the component settles the run (immediate
                // if its delivery already raced ahead) — or a run-key
                // steer wakes it for another work round
                yield* run.emitRow(
                  run.session,
                  "run.parked",
                  run.session,
                ...
                const wake = yield* Effect.raceFirst(
                  Effect.map(run.awaitSettle, (event) => ({
      ...
      const checkRef = term.refs.find(isCheck) as Check<any, any[]> | undefined;
      const machineCheck =
      ...
      >([
        [
          ...
          (params) => {
            ...
            const decoded = S.decodeUnknownResult(haltSchema as never)(parsed);
            ...
            while (true) {
              ...
              if (outcome._tag === "BudgetExceeded") {
                return yield* Effect.fail(
                  new BudgetExceeded({
                    loop: termName,
                    limit: outcome.limit === "tokens" ? "tokens" : "iterations",
      ...
      const service = yield* makeRing({
        ...
        runItem: (item, run) => {
          ...
          const ctx: ProcessContext = {
            ...
            emit: ((typeOrSource: string | Event<any>, payload?: unknown) =>
              typeof typeOrSource === "string"
                ...
                : // owner gate (canon §2a ruling 4): a world-owned
                  ...
                  : Effect.andThen(
                      // the CONTRACT half: one durable Trace row,
                      // always. Fan-out is the assembly's business —
                      // its named bus (EventBusNone drops it).
            ...
            run: <A, E, R>(
              ...
              Effect.gen(function* () {
                ...
                yield* Effect.asVoid(
                  run.emitRow(run.session, "child.started", runId, "started", {
                    runId,
                    agent,
                  }),
                );
...
/**
 * The reference ASSEMBLY — {@link memoryCore} plus the memory
 * components, named EXPLICITLY (kernel-pruning ruling: components are
 * Layers an assembly names, never defaults an implementation invents).
 * Layers are memoized by reference, so a harness that also provides
 * `AskHubMemory`/`EventBusMemory` (to hold the answering/publishing
 * side) shares the same instances with this kernel.
 */
export const memory: Layer.Layer<Kernel, never, LanguageModel.LanguageModel> =
  memoryCore.pipe(
    Layer.provide([AskHubMemory, EventBusMemory, TraceStoreMemory]),
  );
