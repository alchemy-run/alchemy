 ...
 *   export is `makeWebsiteEntryExports(...)` from `alchemy/Cloudflare/Serve`
 ...
 *
 * The AWS half of the mount design lives in `./aws.ts`
 * (`renderAwsLambdaEntry`): the generated nitro entry that grafts nitro's
 * aws-lambda streaming handler verbatim and adds the program's non-fetch
 * listener dispatch through `makeFrameworkFunctionHandler` — no injected
 * middleware exists on any cloud anymore.
 *
