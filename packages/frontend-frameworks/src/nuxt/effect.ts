 ...
 *
 * - **Dev** ({@link renderDevHandler}): a nitro middleware handler (h3 v1
 *   `__is_handler__` marker, injected through `nitro.handlers` on the dev
 *   overrides layer) that scopes itself to the effect routes and delegates
 *   to `alchemy/serve/nitro`'s `toEventHandler`. It runs inside nitro's dev
 *   SSR worker thread, where `event.context.cloudflare.env` is served by
 *   the platform proxy — so the same bindings resolve in dev, against the
 *   local simulators. Non-fetch handlers (queue/scheduled/DO classes) are
 *   production-only on Nuxt for now: the dev server runs nitro's dev
 *   preset entry, not the deploy entry.
 *
...
/**
 * Render the dev middleware handler. The route matcher is inlined (same
 * glob dialect as Cloudflare's `runWorkerFirst` rules / alchemy's
 * `Serve/Routes.ts`: `*` matches any run including `/`, leading `!` is an
 * exclusion and exclusions win) so the generated module only imports
 * `alchemy/serve/nitro` and the site module.
 */
export const renderDevHandler = (options: {
  readonly sitePath: string;
  readonly routes: ReadonlyArray<string>;
}): string =>
  [
    "// Generated by alchemy (Cloudflare.Website.Nuxt effect dev middleware) — do not edit.",
    ...
    "",
    "const handler = toEventHandler(Site);",
    "const middleware = async (event) => {",
    '  const raw = event.path ?? event.node?.req?.url ?? "/";',
    '  const pathname = raw.split("?")[0];',
    "  if (!matches(pathname)) {",
    "    return; // outside the effect routes — nitro continues",
    "  }",
    "  return handler(event); // undefined on passthrough — nitro continues",
    "};",
