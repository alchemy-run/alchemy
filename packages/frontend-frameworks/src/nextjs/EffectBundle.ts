...
export interface NextjsEffectEntry {
  ...
  /**
   * Effect-claim path globs (the engine default). NOT consumed by the
   ...
   * retired dev front dispatch (kept for descriptor-hash stability).
   */
...
export const makeEffectEntrySource = (
  ...
  return [
    ...
    `import { makeWebsiteEntryExports${needsDo ? ", DurableObjectBridge" : ""}${needsWf ? ", WorkflowBridge" : ""} } from "alchemy/Cloudflare/Serve";`,
