...
export const missingBuildScriptMessage = (configPath: string): string =>
  ...
  `build your Next.js app. Add \`"build": "next build"\` to the scripts in ` +
  `package.json, or set \`buildCommand: "${DEFAULT_BUILD_COMMAND}"\` in ${configPath}.`;
        ...
        yield* fs
          .writeFileString(
            ...
            makeDefaultOpenNextConfig({
              buildCommand: hasBuildScript ? undefined : DEFAULT_BUILD_COMMAND,
            }),
          )
          .pipe(
            Effect.mapError(
