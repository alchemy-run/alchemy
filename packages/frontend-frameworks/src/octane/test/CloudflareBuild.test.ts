...
const fixture = Effect.gen(function* () {
  ...
  const source = yield* path.fromFileUrl(new URL("./fixtures/cloudflare", import.meta.url));
...
describe("Octane Cloudflare build", () => {
  it("preserves native plugins without mutating their server hooks", async () => {
    ...
    const plugins = await run(clientPlugins([Promise.resolve([native, false]), custom]));
    expect(plugins.map((plugin) => plugin.name)).toEqual([native.name, custom.name]);
  ...
  it("rejects a missing native plugin instead of silently dropping SSR", async () => {
    const error = await run(clientPlugins([{ name: "unrelated" }]).pipe(Effect.flip));
  ...
  it("builds and serves an adapter-free app with native plugins, CSS, routes, and bindings", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(yield* fs.exists(path.join(root, "output/client/index.html"))).toBe(false);
        expect(yield* fs.exists(path.join(root, "output/client/.vite"))).toBe(false);
        ...
        const worker = yield* Effect.tryPromise(
          () =>
            import(
              /* @vite-ignore */ pathToFileURL(path.join(root, "output/server/worker.js")).href
            ),
        );
        ...
        const rpcId = yield* Effect.sync(() =>
          createHash("sha256").update("/src/rpc.tsx#greet").digest("hex").slice(0, 8),
        );
        ...
        expect(yield* Effect.tryPromise(() => rpc.text())).toContain('"hello Worker"');
  ...
  it("preserves Cloudflare module imports in the Worker bundle", async () => {
    await run(
      Effect.gen(function* () {
        ...
        const output = yield* build(root);
        expect(
          output.serverModules?.map((module) => Buffer.from(module.content).toString()).join("\n"),
  ...
  it("cleans generated files after a server-build failure without rewriting config", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(yield* fs.exists(path.join(root, "output/client/index.html"))).toBe(true);
        expect(
          (yield* fs.readDirectory(root)).filter((entry) =>
            entry.startsWith(".alchemy-octane-worker-"),
          ),
  ...
  it("rejects incompatible adapter declarations", async () => {
    await run(
      Effect.gen(function* () {
        ...
        yield* fs.writeFileString(
          configPath,
          config.replace("defineConfig({", 'defineConfig({ adapter: { name: "vercel" },'),
        );
        const error = yield* buildCloudflare(root).pipe(Effect.flip);
        expect(error.message).toContain('"vercel" is incompatible with Cloudflare');
      }),
    );
  }, 30_000);
});
