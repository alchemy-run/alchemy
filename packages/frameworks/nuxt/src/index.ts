/**
 * `@alchemy.run/frameworks/nuxt` — Nuxt integration implementing framework-core's
 * `Framework` service, with the deploy target passed as a value (Cloudflare
 * Workers by default, via the `@alchemy.run/frameworks/nuxt/cloudflare` subpath
 * module).
 *
 ...
 * behind `@alchemy.run/frameworks/nuxt/cloudflare`.
 */
import type { Framework } from "@alchemy.run/frameworks/core";
...
/**
 * The e2e-harness factory contract (`framework: "@alchemy.run/frameworks/nuxt"` in
 * `e2e.config.ts`).
 */
const factory = (
