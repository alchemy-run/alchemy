/**
 * `@alchemy.run/frameworks/sveltekit` — SvelteKit integration implementing
 * framework-core's `Framework` service, with the deploy target passed as a
 * value (Cloudflare Workers by default, via the
 * `@alchemy.run/frameworks/sveltekit/cloudflare` subpath module).
 *
 ...
 * `@alchemy.run/frameworks/sveltekit/cloudflare`.
 */
...
/**
 * The e2e-harness factory contract (`framework: "@alchemy.run/frameworks/sveltekit"`
 * in `e2e.config.ts`).
 */
const factory = (
  options: HarnessOptions,
