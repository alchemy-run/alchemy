...
export const SERVER_ENTRYPOINT =
  "@alchemy.run/frameworks/astro/entrypoints/server";

/** The production endpoint of the passthrough image service. */
export const IMAGE_PASSTHROUGH_ENDPOINT =
  "@alchemy.run/frameworks/astro/image-passthrough-endpoint";
...
const assertSupportedOptions = (options: DistilledCloudflareOptions): void => {
  ...
  if (offending.length > 0) {
    throw new Error(
      "@alchemy.run/frameworks/astro: unsupported @astrojs/cloudflare option(s):\n" +
        offending
    ...
    hooks: {
      "astro:config:setup": ({ command, config, updateConfig, logger }) => {
        ...
        if (config.adapter === undefined) {
          ...
          updateConfig({
            adapter: { name: "@alchemy.run/frameworks/astro", hooks: {} },
          });
        }
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              ...
              cloudflarePlugins,
              {
                name: "@alchemy.run/frameworks/astro:cf-imports",
                enforce: "pre",
                resolveId: {
              ...
              {
                name: "@alchemy.run/frameworks/astro:environment",
              ...
              {
                enforce: "post",
                name: "@alchemy.run/frameworks/astro:cf-externals",
      ...
      "astro:config:done": ({
        ...
        if (config.adapter !== undefined && !_injectedAdapterMarker) {
          throw new Error(
            `@alchemy.run/frameworks/astro: the Astro config declares the adapter "${config.adapter.name}", ` +
        ...
        injectTypes({
          ...
          content:
            '/// <reference types="@alchemy.run/frameworks/astro/types.d.ts" />',
        });
        setAdapter({
          name: "@alchemy.run/frameworks/astro",
          adapterFeatures: {
