...
export function createWorkerdPrerenderer(
    ...
    if (serverUrl === undefined) {
      throw new Error(
        "The workerd prerender server is not running. " +
          "This is a bug in @alchemy.run/frameworks/astro — `setup()` must run before the prerender protocol is driven.",
      );
    }
    return serverUrl;
  };
  ...
  return {
    name: "@alchemy.run/frameworks/astro:workerd-prerenderer",
  ...
  if (entryIndex === -1) {
    throw new Error(
      `No prerender entry chunk (prerender-entry.*.mjs) found in "${outputDir}". ` +
        "This is a bug in @alchemy.run/frameworks/astro — the prerender environment should have built the worker server entrypoint.",
    );
  }
