...
```ts
import * as Astro from "@alchemy.run/frameworks/astro";
import cloudflare from "@alchemy.run/frameworks/astro/cloudflare";
...
The package separates two concerns (see `packages/frameworks/core/README.md`,
...
| **Cloudflare target**            | `src/cloudflare.ts` (subpath `@alchemy.run/frameworks/astro/cloudflare`), `src/integration.ts`, `src/config-plugin.ts`, `src/prerenderer.ts`, `src/prerender-environment.ts`, `src/prerender-middleware.ts`, `src/runtime/**` | The `AstroTarget` factory plus everything Cloudflare: the forked `@astrojs/cloudflare` integration over our vite plugin, the vendored runtime entrypoints, the `virtual:astro-cloudflare:config` plugin, the workerd prerenderer (build-side driver of the `__astro_*` protocol over cloudflare-runtime), the dev node-prerender middleware plugin. |
| **alchemy source provider**      | `src/source.ts` (subpath `@alchemy.run/frameworks/astro/source`)                                                                                                                                                              | The alchemy `Cloudflare.Workers` source module (structural `SourceProvider` mirror): `build`/`hash`/`dev` for the alchemy Worker resource. Cloudflare-specific by definition; constructs the Cloudflare target directly.                                                                                                                            |
...
Omitting `target` defaults to `"@alchemy.run/frameworks/astro/cloudflare"`, so
...
| -------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `target`       | `AstroTargetInput`  | Deploy target (value / factory / specifier). Default: `"@alchemy.run/frameworks/astro/cloudflare"`.                                                                                                                                 |
...
export default Options.make({
  ...
  framework: "@alchemy.run/frameworks/astro",
});
...
- **Swapped plugin.** `@cloudflare/vite-plugin` → our plugin, `main` pinned to
  the vendored server entrypoint (`@alchemy.run/frameworks/astro/entrypoints/server`),
...
- **Kept (build-output parity).** The `astro:build:done` pass mirrors
  ...
  (`@alchemy.run/frameworks/astro/types.d.ts`).
...
- **`astro:env` at build time.** The alchemy source provider
  (`@alchemy.run/frameworks/astro/source`) feeds the Worker's literal env values
