...
- **Framework packages** (`@alchemy.run/frameworks/waku`, `.../astro`,
...
- **Per-framework target modules** own the halves that are neither generic
  ...
  subpath of its framework package (e.g. `@alchemy.run/frameworks/waku/cloudflare`
  exporting the `WakuCloudflareTarget`). A future AWS target is a new subpath
  (`@alchemy.run/frameworks/waku/aws`) implementing the same seams — no change to
...
```ts
// packages/frameworks/waku/src/cloudflare.ts (illustrative)
...
1. Its options gain `target?: <Framework>TargetInput` (a
   ...
   cloudflare subpath specifier (e.g. `"@alchemy.run/frameworks/waku/cloudflare"`).
...
(`packages/cloudflare-test-tools/e2e/src/CloudflareTarget.ts`) — serving built output is the
...
- **Waku** — `WakuTarget` extends `DeployTarget` with the adapter-path hook
  (today: the wrangler-free fork at `@alchemy.run/frameworks/waku/adapter`, selected
