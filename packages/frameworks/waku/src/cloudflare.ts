/**
 * The Cloudflare Workers deploy target for `@alchemy.run/frameworks/waku` — the ONLY
 ...
 *   adapter (`@alchemy.run/frameworks/waku/adapter`): behavior-identical to upstream
  ...
  makeDeployTarget({
    ...
    adapter: () =>
      Effect.try({
        try: () =>
          fileURLToPath(
            import.meta.resolve("@alchemy.run/frameworks/waku/adapter"),
          ),
        catch: (cause) =>
          new DeployTargetError({
            platform: "cloudflare",
            message:
              "Failed to resolve the @alchemy.run/frameworks/waku adapter module",
            cause,
          }),
      }),
    vitePlugins: (context) =>
