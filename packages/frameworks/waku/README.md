# @alchemy.run/frameworks/waku

Wrangler-free [Waku](https://waku.gg) integration. Implements
`@alchemy.run/frameworks/core`'s `Framework` service — programmatic
...
`@alchemy.run/frameworks/waku/cloudflare`.

## Architecture: framework half × deploy-target half

The package separates two concerns (see
`packages/frameworks/core/README.md` → "Architecture: frameworks × deploy
...
| `./cloudflare` (`src/cloudflare.ts`) | **Cloudflare target**: `WakuTarget` implementation — injects `@alchemy.run/cloudflare-runtime/vite`, selects the `./adapter` fork           | `cloudflare-vite-plugin`           |
...
| `./source` (`src/source.ts`)         | Alchemy `Cloudflare.Worker` source provider (`{ provider: "@alchemy.run/frameworks/waku/source" }`) wrapping the framework with the cloudflare target | `cloudflare-runtime` (types)       |
...
### The cloudflare target (`@alchemy.run/frameworks/waku/cloudflare`)
...
```ts
import wakuFramework, { make } from "@alchemy.run/frameworks/waku";

make({
  /** Deploy target: value | (config) => target | module specifier |
   *  harness carriage. Default: "@alchemy.run/frameworks/waku/cloudflare",
...
- a **target value** — full type safety over the config; build it yourself:
  ...
  ```ts
  import cloudflareTarget from "@alchemy.run/frameworks/waku/cloudflare";
...
export default Options.make({
  framework: "@alchemy.run/frameworks/waku",
  target: {
    cloudflare: {
      worker: {
...
```ts
import wakuFramework from "@alchemy.run/frameworks/waku";
