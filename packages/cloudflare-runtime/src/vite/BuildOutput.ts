...
import { cachedFunction } from "@alchemy.run/node-utils/cached-function";
import { sha256, sha256Object } from "@alchemy.run/node-utils/hash";
import * as Data from "effect/Data";
export class ViteBuildError extends Data.TaggedError("ViteBuildError")<{
...
const bundleErrorFromUnknown = (cause: unknown) =>
  new ViteBuildError({
    message: cause instanceof Error ? cause.message : String(cause),
    cause,
  });
const bundleOutputFromFiles = (files: [BundleFile, ...BundleFile[]]) =>
  sha256Object(files.map(({ path, hash }) => ({ path, hash }))).pipe(
    Effect.map((hash) => ({ files, hash })),
  );
...
export interface ViteBuildOutput {
  ...
  readonly serverBundle: Effect.Effect<
...
export const viteBuildOutputPlugin = Effect.fn(function* ({
  ...
  const serverChunks = new Map<
    string,
    Effect.Effect<BundleFile, ViteBuildError>
  >();
  ...
  const makeServerBundle = () => {
    ...
    const server: NonEmptyArray<Effect.Effect<BundleFile, ViteBuildError>> = [
