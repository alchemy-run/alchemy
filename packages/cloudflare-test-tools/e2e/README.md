...
export default Options.make({
  framework: "@alchemy.run/frameworks/waku", // ← optional; omit for the Vite default
  target: {
...
2. **A package specifier string** (e.g. `"@alchemy.run/frameworks/waku"`) — resolved
...
```ts
// @alchemy.run/frameworks/waku — src/index.ts
import type { Options } from "@alchemy.run/cloudflare-test-tools/e2e/Options";
import { Framework } from "@alchemy.run/frameworks/core";
...
Defined in `@alchemy.run/frameworks/core` (`Framework.ts`):
...
>()("@alchemy.run/frameworks/core/Framework") {}
```
