---
name: alchemy-aws-service
description: Bring an AWS service to full Alchemy coverage.
---
...
Load before adding a new AWS service or finishing an incomplete AWS service.
...
- usually named `packages/alchemy/src/AWS/<Service>/<Operation>.ts` (callable + types) with the impl layer in `<Operation>Http.ts` (AWS runtime impls call the distilled HTTP API authenticated by the Lambda's IAM role; `Http`, not `Binding` — `Binding` is a Cloudflare native-worker concept)
...
1. Service-level abstraction in `packages/alchemy/src/AWS/<Service>/...`
2. Runtime-specific implementation in places like:
   - `packages/alchemy/src/AWS/Lambda/...`
   - `packages/alchemy/src/Process/...`
...
1. `packages/alchemy/src/AWS/<Service>/index.ts`
2. `packages/alchemy/src/AWS/Providers.ts`
...
- `packages/alchemy/test/AWS/<Service>/Bindings.test.ts`
...
- `packages/alchemy/test/AWS/<Service>/handler.ts`
...
- the service-level abstraction lives in `packages/alchemy/src/AWS/DynamoDB/Stream.ts`
- the Lambda runtime implementation lives in `packages/alchemy/src/AWS/Lambda/TableEventSource.ts`
