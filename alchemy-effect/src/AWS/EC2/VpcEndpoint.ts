...
export const VpcEndpointProvider = Provider.effect(
  VpcEndpoint,
  Effect.gen(function* () {
    ...
    return {
      ...
      update: Effect.fn(function* ({ id, news, olds, output, session }) {
        ...
        if (news.vpcEndpointType === "Gateway" || !news.vpcEndpointType) {
          ...
          if (removeRouteTableIds.length > 0) {
            modifications.RemoveRouteTableIds = removeRouteTableIds as string[];
            hasModifications = true;
          }
        }
        ...
        if (
          ...
          if (addSecurityGroupIds.length > 0) {
            modifications.AddSecurityGroupIds = addSecurityGroupIds as string[];
            hasModifications = true;
          }
      ...
      delete: Effect.fn(function* ({ output, session }) {
        ...
        yield* ec2
          ...
          .pipe(
            Effect.catchTag("InvalidVpcEndpointId.NotFound", () => Effect.void),
          );
