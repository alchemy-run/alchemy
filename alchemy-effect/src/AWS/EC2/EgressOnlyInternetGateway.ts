...
export const EgressOnlyInternetGatewayProvider = Provider.effect(
  ...
  Effect.gen(function* () {
        ...
        .pipe(
          ...
          Effect.flatMap((gw) =>
            gw
              ? Effect.succeed(gw)
              : Effect.fail(
                  new Error(`Egress-Only Internet Gateway ${eigwId} not found`),
                ),
          ),
        );
    ...
    return {
      ...
      create: Effect.fn(function* ({ id, news, session }) {
        ...
        yield* session.note(`Egress-Only Internet Gateway created: ${eigwId}`);
