...
export const TableProvider = Provider.effect(
  Table,
  Effect.gen(function* () {
        ...
        const requested = bindings
          .flatMap((binding) =>
            (binding as { data?: TableBinding }).data?.streamSpecification
              ...
              : (binding as TableBinding).streamSpecification?.StreamEnabled ===
                  true
                ? [
                    normalizeStreamSpecification(
    ...
    return Table.Provider.of({
      ...
      create: Effect.fn(function* ({ id, news, session, bindings }) {
        ...
        const streamSpecification = yield* resolveStreamSpecification(bindings);
        yield* session.note(
