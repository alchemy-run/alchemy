...
function renderPage(doc: PageDoc, resolve: LinkResolver): string {
  const description =
    stripLinkTags(firstParagraph(stripCopyMarkers(doc.summary).trim())) ||
...
async function main() {
  ...
  for (const page of pending) {
    ...
    if (await writeIfChanged(outputPath, renderPage(page.doc, resolve))) {
      written++;
    }
  }
  ...
  console.log(
    `Done. Wrote ${written} changed of ${pending.length} resource pages (skipped ${skipped} untagged${
