...
function renderPage(doc: PageDoc, resolve: LinkResolver): string {
  const description =
    stripLinkTags(firstParagraph(doc.summary)) || `API reference for ${doc.title}`;
  const frontmatter = [
    "---",
...
function buildServiceItems(pages: PageEntry[]): SidebarItem[] {
  ...
  for (const [label, productPages] of byLabelKey) {
    ...
    items.push({
      label,
      collapsed: true,
      items: productPages.map((p) => ({ label: p.resource, link: p.link })).sort(byLabel),
    });
  }
...
function buildProvidersSidebar(entries: PageEntry[]): SidebarItem[] {
  ...
  for (const provider of orderedKeys([...byProvider.keys()], PROVIDER_ORDER)) {
    ...
    const items: SidebarItem[] = [];
    for (const cat of [...categorized.keys()].sort((a, b) => a.localeCompare(b))) {
      items.push({
        label: cat,
        collapsed: true,
    ...
    if (categorized.size === 0 && pages.length <= FLAT_PROVIDER_MAX_PAGES) {
      ...
      items.push(...uncategorized.map((p) => ({ label: p.resource, link: p.link })).sort(byLabel));
    } else {
...
async function main() {
  ...
  await fs.writeFile(path.join(config.outRoot, "index.mdx"), referenceIndex, "utf8");
