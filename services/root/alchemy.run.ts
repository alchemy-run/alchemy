/**
 * The company, deployed — everything on Cloudflare, nothing local:
 *
 * - {@link ApiWorker} (src/ApiWorker.ts) — the backend: the Root
 *   Thread (the Head's session), the team's sessions as Durable
 *   Objects, the triage queue / task ledger / proposals / calls on
 *   SQLite DOs, GitHub by webhook + token bindings, workspaces on
 *   MicroVMs;
