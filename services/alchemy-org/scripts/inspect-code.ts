...
const info = await page.evaluate(() => {
  const blocks = [...document.querySelectorAll("[data-streamdown]")].map(
    (el) => el.getAttribute("data-streamdown"),
  );
  const shadow = [...document.querySelectorAll("main *")].filter((el) => el.shadowRoot).length;
  const errors: string[] = [];
