...
export const TypedErrorsLive = TypedErrors.make`
  Growing distilled's typed error unions. Every error an operation can
  produce in practice MUST be a tagged error in its type-level union —
  the catch-alls (UnknownCloudflareError, CloudflareHttpError, the
  out-of-union NotFound/BadRequest) exist only to SURFACE gaps and are
  never handled in alchemy code.
  ...
  2. Read any existing patch at
     ...
     resource-specific tag (WidgetNotFound, never a bare NotFound):

         { "errors": { "WidgetNotFound": [{ "code": 10404 }] } }

     Matchers combine "code", "status", and "message"
     ({ "includes": … }) — prefer the Cloudflare error code when one
     exists; fall back to status plus a message fragment when the API
     misuses HTTP statuses. Request-schema patch keys are camelCase;
     response keys are wire snake_case.
  3. Regenerate THAT service only, in one shell command:
     ...
     The test runner resolves distilled from src/*.ts directly — the
     regenerated union is visible to the very next test run; never
     wait on a build.
  4. Handle the now-typed tag with Effect.catchTag or a typed
     retry-while and re-run the tests.

  If a catchTag fails to typecheck, that IS the signal the union is
  missing the error — patch distilled; never loosen alchemy-side
