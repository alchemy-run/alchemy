...
/**
 * The HARNESS skill — the org's own conventions, for a change to
 * `services/alchemy-org` itself. The org is the harness that runs the
 * agents that maintain alchemy; a change here changes the hands that
 * make the next change. This is the lift: the doctrine a human coding
 * agent reads in `AGENTS.md` and the doctrine the org's own agents
 * activate are ONE source — this file. `AGENTS.md` is its rendering
 * (`bun scripts/agents-md.ts`), and `test/harness.test.ts` fails when
 * the two drift.
 *
 * Dormant until activated: the Engineer and the Reviewer both mention
 * it; the prose enters a session only when the work touches the org.
 * The teaching splices read-and-run tools only — both agents hold
 * them, so the skill is satisfiable by either Layer graph and grants
 * no editor the Reviewer must not have.
 */
export class Harness extends AI.Skill<Harness>()("Harness") {}

/**
 * The skill's Layer — and the teaching itself: `Skill.make` carries its
 * template and splices on the Layer (`AI.Teaching`), so this ONE tagged
 * template is what the agents activate and what `AGENTS.md` renders.
 * It splices read/run tools only, all of which both charters hold.
 */
export const HarnessGeneral = Harness.make`
...
/**
 * `AGENTS.md`, whole: a generated-file notice and the teaching rendered
 * the way the driver renders it for the org's own agents — tool splices
 * become the tools' names in backticks, so a human coding agent reads
 * `grep` where the Engineer holds `grep`.
 */
export const renderAgentsMd = (): string =>
  [
