...
export const LiveTestingLive = LiveTesting.make`
  ...
  - Hitting the timeout wall IS the failure — read the partial output
    ...
    for 10s; that list is where a hang lives), find the unbounded
    retry or infinite pagination, fix the cause; never just re-run
    hoping.
  - Never poll for provisioning slower than ~90s — skipIf-gate behind
    an env var with the exact typed error recorded, and keep an
    ungated probe test that asserts the typed tag so the gate stays
    honest.
  - Three-iteration budget: a suite not green after ~3 fix iterations
    whose blocker is platform behavior gets implemented fully, gated,
    verified skip-clean, and reported honestly.`;
