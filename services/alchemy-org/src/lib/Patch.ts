/**
 * The apply-patch engine — the Codex-style patch grammar as one
 * self-contained module: the operation TYPES, the pure PARSER (a
 * malformed patch cannot touch the workspace), and the guarded APPLY
 * (parse -> path-check -> digest-check -> simulate -> stage -> write,
 * with best-effort rollback). The tool surface lives in
 * tools/ApplyPatch.ts; this is the physics.
 */
