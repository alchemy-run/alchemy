...
const content = AI.Thing("content", S.String)`
  The requested page of the retained output — a trailing note names the
  next offset when more remains.`;

export class ReadOutput extends (AI.Tool<ReadOutput>(import.meta)("readOutput")`
  Read complete tool output retained under ${outputId} — answers
  ${AI.out(content)}. Page large artifacts with ${offset} and ${limit}.
  IDs are scoped to the current local toolbox and reveal no host
  filesystem path.`) {}
...
export const ReadOutputLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return ((input: { outputId: string; offset?: number; limit?: number }) =>
      pageArtifact(artifacts, input).pipe(
        Effect.map((page) => ({ content: page })),
      )) as never;
  }),
);
