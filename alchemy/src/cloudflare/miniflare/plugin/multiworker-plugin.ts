...
const SharedOptionsSchema = z
  ...
  .transform((sharedOptions) => {
    ...
    return {
      ...sharedOptions,
      allWorkers:
        sharedOptions.allWorkers ??
        new Set(sharedOptions.workers.map((w) => w.name)),
      workersWithAssets:
        sharedOptions.workersWithAssets ??
        new Set(
          sharedOptions.workers.filter((w) => w.assets).map((w) => w.name),
        ),
      fallbackWorker:
        (sharedOptions.fallbackWorker ?? sharedOptions.workers.length > 1)
          ? sharedOptions.workers[1].name
          : undefined,
    };
  });
...
> = {
  ...
  async getServices({ options, workerNames, sharedOptions }) {
    ...
    const globalOptions = sharedOptions as unknown as z.infer<
      typeof SharedOptionsSchema
    >;
    ...
    const defaultWorker =
      (globalOptions.fallbackWorker ?? workerNames.length > 1)
        ? workerNames[1]
        : globalOptions.fallbackWorker;

    const allPossibleWorkerNames = globalOptions.allWorkers.union(
      new Set(workerNames),
    );
