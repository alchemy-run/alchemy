...
export interface Alchemy {
   ...
   * @param fqn Optional fully qualified name to scope the rotation (defaults to root FQN)
   *
   * @example
   * // Rotate password for all resources
   * const app = await alchemy("my-app");
   * await alchemy.rotatePassword(
   ...
   * );
   */
  rotatePassword: typeof rotatePassword;
...
_alchemy.env = env;
_alchemy.rotatePassword = rotatePassword;
