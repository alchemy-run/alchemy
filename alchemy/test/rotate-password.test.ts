...
import { rotatePassword } from "../src/rotate-password.ts";
...
describe("Password Rotation", () => {
  test("rotates password for secrets", async () => {
    ...
    await rotatePassword(oldPassword, newPassword);
    ...
    console.log(await stageScope2.state.all());
    ...
    console.log(rotatedSecret, normalData);
  ...
  //   await expect(
  //     rotatePassword("same-password", "same-password"),
