...
export const worker = await Worker("api", {
  ...
  dev: { port: 8787 },                    // Local dev: { port?, remote?, tunnel? } OR { url } to connect to existing dev server
...
Module scope initialization (when appropriate):
```ts
...
:::caution
**IO is not allowed at module scope** in Cloudflare Workers. You cannot access `env` bindings, make fetch requests, or perform any IO operations during module initialization. Only perform IO inside handler functions (`fetch`, `queue`, `scheduled`, etc.).
:::

:::tip
Use module scope for expensive setup like class instantiation or constant initialization. Configure with runtime values (like API keys from `env`) inside your handler functions to avoid cold start penalties while respecting the no-IO-at-module-scope constraint.
:::
...
:::tip
Type augmentation ensures your runtime code only accesses bindings defined in your infrastructure. This prevents undefined binding errors and provides full IntelliSense support across your project.
:::
