...
4. Install `alchemy` and configure `dev`, `deploy` and `destroy` in the `backend` app.
    ...
    ```diff lang="json" title="apps/backend/package.json"
    {
      ...
      "scripts": {
        "build": "tsc -b",
    +    "dev": "alchemy dev --app backend --env-file ../../.env --root-dir ../../",
    +    "deploy": "alchemy deploy --app backend --env-file ../../.env --root-dir ../../",
    +    "destroy": "alchemy destroy --app backend --env-file ../../.env --root-dir ../../"
      },
      "dependencies": {
    +    "alchemy": "catalog:"
      }
    ...
    :::caution
    Make sure to use `--root-dir ../../` and `--env-file ../../.env` in all of your apps so that state and `.env` is synchronized.
    :::
...
8. Use `--app frontend` and `--root-dir ../../` in your frontend's `dev`, `deploy` and `destroy` scripts:
    ...
    ```diff lang="json" title="apps/frontend/package.json"
    {
      ...
      "scripts": {
        "build": "tsc -b",
    +    "dev": "alchemy dev --app frontend --root-dir ../../  --env-file ../../.env",
    +    "deploy": "alchemy deploy --app frontend --root-dir ../../  --env-file ../../.env",
    +    "destroy": "alchemy destroy --app frontend --root-dir ../../  --env-file ../../.env"
      },
      "dependencies": {
        "backend": "workspace:*",
        "alchemy": "catalog:"
