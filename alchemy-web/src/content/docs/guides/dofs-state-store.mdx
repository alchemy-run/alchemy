...
:::warning
The `DOFSStateStore` is deprecated. Use the [DOStateStore](/guides/do-state-store) instead.
:::

DOFSStateStore provides a [State Store](/concepts/state-store) using a Cloudflare Worker and the [Durable Object File System (DOFS)](https://www.npmjs.com/package/dofs) package.
...
DOFSStateStore requires Cloudflare authentication. See the [Cloudflare Auth Guide](/guides/cloudflare) for setup options:
...
:::tip
The same Cloudflare credentials used for other resources work with DOFSStateStore.
:::

## How It Works

1. **Worker Creation**: DOFSStateStore automatically deploys a Cloudflare Worker with Durable Objects
