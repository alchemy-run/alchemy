...
2. Install alchemy dependencies:
...
4. Create your `alchemy.run.ts` file:
...
5. Create the worker entrypoint file:
...
1. Update your `alchemy.run.ts` to import the [`KVNamespace`](../providers/cloudflare/kv-namespace) Resource:

   ```typescript
   import { KVNamespace } from "alchemy/cloudflare";
   ```

2. Create a new `KVNamespace`:

   ```ts
   const kv = await KVNamespace("my-app-storage");
   ```

3. Bind the `KVNamespace` to the `Worker`:
...
4. Update your `./src/worker.ts` to use these bindings:
...
Alchemy supports running in a "dev mode" that runs your Workers locally with hot module reloading and continuous deployment of your infrastructure.

<Steps>

1. Run `alchemy.run.ts` script with the `--watch` flag to start local development:
...
2. Configure the `KVNamespace` resource to run remotely instead of locally:

    ```typescript
    const kv = await KVNamespace("my-app-storage", {
      dev: {
        // use the actual remote KV namespace instead of a local one
        remote: true,
      },
    });
    ```
    ...
    :::tip
    Development mode emulates most Cloudflare resources locally for faster iteration and no API costs. For resources that need remote access, you can configure them with `dev: { remote: true }`. Learn more in the [Development Mode guide](./guides/dev.md).
    :::
