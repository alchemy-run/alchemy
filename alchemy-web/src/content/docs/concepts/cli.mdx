...
Here are some examples of how to use the `exec` command:

```sh
# run a script in read-only mode
alchemy exec ./scripts/my-script.ts

# watch for changes and re-run
alchemy exec ./scripts/my-script.ts --watch

# specify a stage
alchemy exec ./scripts/my-script.ts --stage dev

# use an environment file
alchemy exec ./scripts/my-script.ts --env-file .env.prod
```
:::

:::note[Equivalent Runtime Commands]

You can also run the exec command directly with your preferred JavaScript runtime:
