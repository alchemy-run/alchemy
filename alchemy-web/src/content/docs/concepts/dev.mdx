...
A Worker can be configured to use a Cloudflare Tunnel for local development:
...
In `dev` mode, a temporary Tunnel will be created and bound to the remote Worker to route requests from the public internet to your locally running Worker.

:::tip
You can enable this for all Workers in your project by passing the `--tunnel` flag to the `alchemy dev` command.
