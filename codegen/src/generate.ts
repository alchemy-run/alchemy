  ...
  branch(
    ...
    Effect.gen(function* () {
      ...
      const loadFiles = Effect.gen(function* () {
        return {
          index: yield* readFile(indexPath),
          resources: (
            JSON.parse(yield* readFile(resourcesJson)) as {
              resources: string[];
            }
          ).resources,
        };
      ...
      if ((yield* exists(indexPath)) && (yield* exists(resourcesJson))) {
...
const designResource = (service: string, resource: string) =>
  branch(
    `design-${service}-${resource}`,
    Effect.gen(function* () {
      const session = yield* Session;
      const resourcePath = yield* designPath(service, `${resource}.md`);

      if (yield* exists(resourcePath)) {
        return yield* readFile(resourcePath);
      }
...
const designTestCases = (service: string, resource: string) =>
  branch(
    ...
    Effect.gen(function* () {
      ...
      if (yield* exists(testPath)) {
        return yield* readFile(testPath);
      }
...
const designLifecycle = (
  ...
  branch(
    `lifecycle-${service}-${resource}-${lifecycle}`,
    Effect.gen(function* () {
      const session = yield* Session;
      const resourcePath = yield* designPath(service, `${resource}.md`);
      const indexPath = yield* designPath(service, "index.md");
      ...
      if (yield* exists(lifecyclePath)) {
        return yield* readFile(lifecyclePath);
      }
      yield* Console.log(
...
const implementResourceContracts = (service: string, resources: string[]) =>
  branch(
    `contract-${service}-${resources.join("-")}`,
    Effect.gen(function* () {
      const session = yield* Session;
...
- [${resource} Delete Lifecycle Operation](${destroy})`;
      ...
      for (const resource of resources) {
        if (yield* exists(yield* sourcePath(service, `${resource}.ts`))) {
          return;
        }
      }
      ...
      yield* session.prompt({
        ...
        prompt: `Implement the resource contracts for the resources: ${resources.join(", ")}.
...
const implementResourceProvider = (service: string, resource: string) =>
  branch(
    ...
    Effect.gen(function* () {
      ...
      if (yield* exists(providerSrc)) {
        return yield* readFile(providerSrc);
      }
      ...
      yield* session.prompt({
        ...
        prompt: `Implement the resource provider for the ${resource} resource.
...
const implementResourceTestCases = (service: string, resource: string) =>
  branch(
    ...
    Effect.gen(function* () {
      ...
      if (yield* exists(testFile)) {
        return yield* readFile(testFile);
      }
      ...
      yield* session.prompt({
        ...
        prompt: `Implement the resource test cases for the ${resource} resource.
