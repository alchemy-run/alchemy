...
- Post-integration bridge/storage regressions passed all 11 cases, and the three short deployed runtime controls passed with both Functions and their project independently absent. The initial census found `young-sea-37145338`, created at `2026-09-20T07:41:34Z` during the guard-interrupted custom-domain run before its ID was saved. After matching its exact test name and creation time, an isolated recovery stack adopted that exact project and destroyed it through the ordinary provider lifecycle. The final independent census at `07:54:30Z` returned `projects: []`; no account-wide cleanup ran. Evidence: `.alchemy/neon-domain-recovery-deploy-final.log`, `.alchemy/neon-domain-recovery-destroy.log`, and `packages/alchemy/.alchemy/neon-readiness-census-final.json`.
- The merged SDK passed 542 core/Neon/XML tests, the strict Neon package typecheck, and all current-head companion CI checks at `0b8987558`. Scoped Alchemy checking passed. Full local workspace checking exceeded the 4 GiB owned-process ceiling and was stopped. CI then exposed a missing explicit `aws4fetch` test dependency and missing project references in the new Neon examples after main's source-export change. Declared the dependency, applied main's project-reference convention, and verified the affected tests plus all 13 example projects with sequential typechecks (2.9 GiB peak). After integrating main `3463790dd`, all CI checks passed at Alchemy `a9046f3d5`: full workspace types, CLI launcher checks, JSDoc validation, Linux/macOS/Windows shared-runtime tests, package previews, and the documentation preview build/deployment. The final-main bridge/SQL regression run passed 47 tests. Browser interaction with the preview exposed a documentation-only placement issue: prose headings after example sections are discarded by the generator. Moved the streaming warning before the examples so its heading renders and appears in page navigation; JSDoc generation and scoped types pass.
...
## Cancellation root-cause correction (2026-09-20)

The [host reproduction and upstream report](./stream-disconnect-reproduction.md) identify the remaining platform blocker. The previously published streaming watchdog was unsafe and has been removed. A paused reader is legitimate backpressure, not evidence of disconnection. A Node 24 reproduction showed the watchdog cancelling a paused consumer and ending its response early; an abandoned pending read escaped the watchdog entirely. The earlier diagnostic's throttled, fire-and-forget SQL writes did not establish the claimed zero-pull behavior. Its green runtime run demonstrated the workaround, not a root-cause fix.

A corrected live native-handler diagnostic awaited each recorded chunk before enqueueing it. Both connected controls delivered six chunks. After killed curl clients, the pull producer stopped at five chunks, while the push producer advanced from three to 26 over five seconds; neither recorded an abort or cancel. The complete run took 25 seconds, peaked at 808 MiB, and independently verified deletion of its owned project.

Inspection of the deployed Node language worker identified a concrete transport defect: `/runtime/runtime.js`'s `pumpReaderIdentity` waits only for `drain` after `res.write()` returns false. It does not handle premature response close/error or cancel its Web Stream reader. The extracted loop reproduced the leak over real local Node HTTP: the connection closed, but the producer was not cancelled and the pump remained pending. A cancellation-aware `pipeline(Readable.fromWeb(body), res)` comparison cancelled the producer and settled the pump. The deployed host repository is not accessible from this account, so that correction has not been applied or verified on Neon's platform. Compression peeking and request-signal propagation also need upstream coverage; changing response headers to select another transport path would only be a workaround.

Alchemy had a separate abort-propagation defect: closing the request scope did not reliably interrupt the independent response producer. The bridge now connects the actual request signal to a standard Web Streams pipeline. It lets producer cancellation run stream finalizers before the transferred request scope closes, without an idle deadline. Tests await both stream and request finalizers before performing any explicit reader cleanup, including pending-read and backpressure cases. The live disconnect test again expects native cancellation and Effect finalization; it does not assert that the host remains broken.

Focused bridge validation passed all nine cases in 2.8 seconds (552 MiB peak): pending-read abort/cancel, paused-consumer abort/cancel, ordinary completion, stream failure, native upgrade identity with both close and abort cleanup, and bodyless responses. The full workspace `tsc -b` attempt exceeded the 4 GiB owned-process ceiling; a native-compiler heap-limited attempt was stopped by system memory pressure at 2.6 GiB. Those interrupted attempts are not successful typechecks. A scoped check completed in two seconds at 885 MiB and reported `TS2322` in unchanged `FunctionProvider.ts:271`: the ZIP upload's `Uint8Array<ArrayBufferLike>` is not assignable to `BlobPart`. No clean typecheck is claimed.

The three short deployed runtime controls passed in 24.3 seconds: finite streaming and concurrent request isolation, HEAD/204 finalizers, `waitUntil`, and error responses. Normal destruction independently confirmed both Functions and their project absent. The two long disconnect/WebSocket cases were not rerun in that filtered pass. The host correction remains upstream-only and is not counted as fixed by these local or live controls.

**Write-only storage remains a documented-contract discrepancy.** The earlier operation matrix observed successful `PutObject`/`DeleteObject` and denied `GetObject`/`HeadObject`/`ListObjectsV2`/`ListBuckets` with a `storage:write`-only credential. Managed write clients request both read and write. The two passing storage tests assert observed behavior, not a correction to Neon's documented implied-read contract. This cancellation change does not alter storage permissions or those tests.
