...
const writePnpmWorkspace = async () => {
  ...
  const lines: string[] = [
    "# Auto-generated by test/smoke.test.ts — DO NOT COMMIT.",
    "# Mirrors `package.json#workspaces.{packages,catalog}` so pnpm 11's",
    "# `runDepsStatusCheck` can resolve `catalog:` deps in our examples.",
    "",
    "packages:",
  ...
  console.log(`→ wrote ${PNPM_WORKSPACE_PATH}`);
};

const removePnpmWorkspace = () => fs.rm(PNPM_WORKSPACE_PATH, { force: true });
