...
function CloudHero() {
  const providers = [
    { name: "Cloudflare", color: "#f38020", desc: "Workers · R2 · KV · D1 · Durable Objects · Queues · Workflows · Containers" },
    { name: "AWS", color: "#ff9900", desc: "Lambda · S3 · DynamoDB · SQS · Kinesis · IAM · EC2" },
    { name: "+ more", color: "var(--alc-walnut-500)", desc: "GitHub · Stripe · DNS · and a growing community ecosystem" },
  ];
  return (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, maxWidth: 1100, margin: "0 auto" }}>
        {providers.map(p => (
          <div key={p.name} style={{
            ...
            borderRadius: 12, padding: 22, display: "flex", flexDirection: "column", gap: 8,
          }}>
            <div style={{
              width: 34, height: 34, borderRadius: 8, background: p.color,
              display: "flex", alignItems: "center", justifyContent: "center",
              color: "#fff", fontFamily: "var(--alc-font-mono)", fontWeight: 700,
            }}>{p.name[0] === "+" ? "+" : p.name[0]}</div>
...
// A Stack entrypoint — the real shape
function CloudStackExample() {
  return (
    <Section padding="40px 32px 80px">
      <div style={{ textAlign: "center", marginBottom: 36 }}>
        <Eyebrow>the shape of a stack</Eyebrow>
        <h2 className="alc-h2" style={{ margin: "12px 0 12px" }}>Resources, bindings, outputs — one file.</h2>
        <p style={{ fontSize: 16, lineHeight: 1.65, color: "var(--alc-fg-2)", maxWidth: 620, margin: "12px auto 0" }}>
          A <code style={{ fontFamily: "var(--alc-font-mono)", fontSize: 14, color: "var(--alc-accent-deep)" }}>Stack</code> is an Effect that declares resources and returns typed outputs. If it compiles, it deploys.
        </p>
      </div>
...
function CloudResourcesAndWorker() {
  return (
    <Section padding="80px 32px" style={{ background: "var(--alc-bg-nav)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, maxWidth: 920, margin: "0 auto 16px" }}>
...
{"  "}{T.v("Effect")}.{T.f("gen")}({T.k("function")}* () {"{"}{"\n"}
{"    "}{T.c("// typed runtime clients · deploy-time bindings wired automatically")}{"\n"}
