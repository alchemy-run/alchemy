...
```typescript
// src/ApiService.ts — inside the host's Construct phase (a Cloudflare Worker or Lambda Function)
...
nothing, which is what makes it safe at construct/plan time — and each
...
```typescript
// src/ApiService.ts — end of the Construct phase
