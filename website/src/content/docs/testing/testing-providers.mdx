...
Inside the test body, the configured `providers` Layer is already in scope. Out-of-band SDK calls (`DynamoDB.describeTable`, `stripe.products.retrieve`, …) work without extra setup. Use them to assert that the cloud matches the resource's reported outputs, as in the [Stripe walkthrough](/infrastructure-as-code/custom-provider#test-the-lifecycle).

## Verify replacement

Change a `stables` field, or any field your `diff` flags as `replace`, between two `stack.deploy` calls with the same logical ID:
...
`test.provider`'s in-memory state is exposed as the `state` field on `ScratchStack` for advanced cases. For simpler seeds, call `stack.deploy(...)` once with the seed, then again with the actual test inputs. The second call sees the first call's output as existing state.
