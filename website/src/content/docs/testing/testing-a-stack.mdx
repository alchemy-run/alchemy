...
POST a JSON body with `HttpBody.jsonUnsafe`:
...
test(
  ...
  Effect.gen(function* () {
    ...
    const created = yield* HttpClient.post(`${url}/pastes`, {
      body: HttpBody.jsonUnsafe({ content: "hello" }),
    });
    expect(created.status).toBe(201);
    const { id } = (yield* created.json) as { id: string };

    const read = yield* HttpClient.get(`${url}/pastes/${id}`);
    expect(read.status).toBe(200);
  }),
);
```

The client does not fail on 4xx/5xx — assert on `response.status`
directly (a `404` is a value, not an error, unless you opt into
...
Vitest runs test files in parallel by default, so two files deploying
the same stage can race the first converge. Either serialize files:
...
or give each file its own stage if the suites are independent.
