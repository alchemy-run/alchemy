...
cold-start helpers. [Testing a Stack](/testing/testing-a-stack)
covers them.
...
It is usually the same one your `Stack` uses:
...
The state store that top-level `deploy(Stack)` and
`destroy(Stack)` use. `test.provider` does not use it. Defaults to
`localState()`, which stores state in `.alchemy/` on disk.
...
resources between runs. [Testing a Stack](/testing/testing-a-stack)
covers the run-against-an-existing-stack pattern built on it.
...
- **Vitest** uses `@effect/vitest`'s `it.live`, so tests stay
  Effect-aware. The default hook timeout is the same (120s).
