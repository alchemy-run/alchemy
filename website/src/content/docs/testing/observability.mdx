...
data point, and every `Effect.logInfo` ships a log line. Each fetch,
queue message, cron fire, and invocation also gets an
`http.server`-style root span automatically. A telemetry Layer
provided on the Function/Worker decides where the signals go:
...
Any OTLP endpoint works, including Axiom, an OTel collector, or any
OTLP-compatible vendor. Swap the layer to ship somewhere else. The
code emitting the signals never changes. Buffered telemetry is
flushed when each request's scope closes (via `waitUntil` on Cloudflare, so
...
(traces, logs, or metrics) to an OTLP collector. The Worker needs no
code changes:
...
into a higher-level alert (`AlarmRule: 'ALARM("HighErrors") OR
ALARM("HighLatency")'`). Stacks are per-stage, so `prod`, `staging`,
and `pr-42` each get their own dashboards and alarms.
