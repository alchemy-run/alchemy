...
runtime ships with the OTLP exporter built in — set the standard
`OTEL_*` env vars and signals flow. With alchemy the receiving end
...
root span automatically. Where the signals go is decided by
...
OTLP-compatible vendor. Swap the URL, ship somewhere else; the code
...
mechanics and the `Alchemy.Telemetry` override API for installing a
custom exporter Layer.
...
Mint a least-privilege ingest token and pass each signal's endpoint
and headers to the Worker through `env`:
...
`Axiom.otlpHeaders` composes the
`Authorization=Bearer <token>,X-Axiom-Dataset=<name>` string Axiom
expects. The endpoints are plain strings so they bind as
`plain_text`; the headers carry the `Redacted` token so they bind as
`secret_text` — see [Secrets](/environments/secrets). The built-in
telemetry reads these per signal and starts shipping — no exporter
code in the Worker.
