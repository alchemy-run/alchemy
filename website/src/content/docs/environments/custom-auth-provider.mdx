...
description: Plug a third-party API into alchemy's auth system — a lazy credentials service backed by an AuthProvider that resolves keys from a Profile, env vars, or a refreshable session.
---
...
registry — that's how the alchemy CLI discovers it. The factory also wraps
        ...
        Effect.flatMap((creds) =>
          creds == null
            ? Effect.fail(
                new AuthError({
                  message:
                    "Stripe stored credentials not found. Run: alchemy profile refresh default --provider Stripe",
                }),
              )
            : Effect.succeed({
                type: "apiKey" as const,
...
Two details matter. `resolveProviderConfig` applies the standard profile
selection order, then returns the stored config or runs `configure` and
persists the answer — so the first deploy configures its providers. A missing
explicit Profile fails with a command to create it. And the value returned to
`Layer.effect` is the *Effect*
...
alchemy profile edit --re-configure Stripe   # re-run the method picker
