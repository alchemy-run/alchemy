---
title: Stages
description: Stages are isolated instances of a Stack — live_sam, local_sam, staging, prod, pr-42 — each with their own state and physical names.
sidebar:
  hidden: true
---
...
(e.g. `live_sam`) and **`alchemy dev`** uses **`local_$USER`** (e.g.
`local_sam`). Each developer gets a personal cloud sandbox and a
separate local-dev sandbox without any config — so a bare `alchemy dev`
cannot replace resources you deployed.
...
$ alchemy dev
# emulates locally as stage `local_sam`

$ alchemy destroy --stage local_sam
...
3. `live_${USER}` for deploy / destroy / plan / logs / drift
   (`local_${USER}` for `alchemy dev`)
4. `live_unknown` / `local_unknown` if no user is set
...
| `local_<user>`    | Per-developer local-dev sandbox (`alchemy dev` default) |
