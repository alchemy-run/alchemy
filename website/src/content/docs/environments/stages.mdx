---
title: Stages
description: Stages are isolated instances of a Stack, such as live_sam, dev_sam, staging, prod, or pr-42. Each has its own state and physical names.
sidebar:
  hidden: true
---

A **stage** is an isolated instance of a [Stack](/infrastructure-as-code/stack).
Every deploy targets exactly one stage. Resources from different
stages never overlap. With one program, Alchemy gives each developer
their own sandbox, each PR its own preview, and production its own
dedicated environment.

## The default stage

If you don't pass `--stage`, **`alchemy deploy`** uses **`live_$USER`**
(e.g. `live_sam`) and **`alchemy dev`** uses **`dev_$USER`** (e.g.
`dev_sam`). Each developer gets a personal cloud sandbox and a
separate local-dev sandbox with no config, so a bare `alchemy
dev` cannot replace resources you deployed. Existing default `alchemy
dev` stacks keep the `dev_$USER` name. **`Test.make`** uses
**`test_$USER`** the same way, so two people running the suite
against one account don't collide.

```sh
$ whoami
sam

$ alchemy deploy
# deploys to stage `live_sam`

$ alchemy dev
# emulates locally as stage `dev_sam`

$ alchemy destroy --stage dev_sam
# tears down the local-dev stage
```

`--stage` and `$ALCHEMY_STAGE` still work on both commands. Pointing
`alchemy dev` at the same stage as a deploy (`--stage prod`, or a
shared `$ALCHEMY_STAGE`) is a live ⇄ local replacement of that stage.

The resolution order is:

1. `--stage <name>` flag
2. `$ALCHEMY_STAGE` environment variable (process env, `--env-file`, or `.env`)
3. `live_${USER}` for deploy / destroy / plan / logs / drift
   (`dev_${USER}` for `alchemy dev`)
4. `live_unknown` / `dev_unknown` if no user is set

## Common stage patterns

| Stage             | Purpose                                              |
| ----------------- | ---------------------------------------------------- |
| `live_<user>`     | Per-developer cloud sandbox (`alchemy deploy` default) |
| `dev_<user>`      | Per-developer local-dev sandbox (`alchemy dev` default) |
| `test_<user>`     | Per-developer test sandbox (`Test.make` default)     |
| `pr-<n>`          | Per-pull-request preview environment                 |
| `staging`         | Shared pre-production                                |
| `prod`            | Production                                           |

Stage names must match `[a-z0-9][-_a-z0-9]*`.

```sh
alchemy deploy --stage prod
alchemy deploy --stage pr-42
alchemy destroy --stage pr-42
```

## Isolation

Each stage gets its own:

- **State file**: the persisted record of what's deployed
- **Physical names**: `myapp-bucket-prod-<suffix>` vs
  `myapp-bucket-live-sam-<suffix>`
- **Logs and metrics**: scoped per deployed function/worker

As a result, deploying or destroying one stage never touches
another:

```sh
$ alchemy deploy --stage live_sam    # -> myapp-photos-live-sam-<16-char suffix>
$ alchemy deploy --stage pr-147      # -> myapp-photos-pr-147-...
$ alchemy deploy --stage prod        # -> myapp-photos-prod-...

$ alchemy destroy --stage pr-147     # only removes pr-147 resources
```

By default, generated physical names follow
`{stack}-{logicalId}-{stage}-{suffix}`. Characters that aren't letters,
digits, or hyphens (such as the `_` in `live_sam`) become `-` in physical
names. A provider can lowercase the name or truncate it to fit a service's
length limit.

## Per-stage configuration

The `Stack` service exposes the current stage, so the same program
can branch on it:

```typescript
import { Stack } from "alchemy/Stack";

Effect.gen(function* () {
  const stack = yield* Stack;

  const queue = yield* SQS.Queue("Jobs").pipe(
    RemovalPolicy.retain(stack.stage === "prod"),
  );
});
```

For resources declared at module scope, use
[`Stack.useSync`](/infrastructure-as-code/stack#stackusesync-for-module-scope-resources):

```typescript
export class JobFunction extends AWS.Lambda.Function<JobFunction>()(
  "JobFunction",
  Stack.useSync((stack) => ({
    main: import.meta.url,
    memory: stack.stage === "prod" ? 1024 : 512,
  })),
) {}
```

## Pull request previews

A common CI pattern creates a fresh stage for every PR, comments the
URL on the PR, and tears the stage down on merge.

```yaml
# .github/workflows/preview.yml
- run: alchemy deploy --stage pr-${{ github.event.number }} --yes

# on PR close:
- run: alchemy destroy --stage pr-${{ github.event.number }} --yes
```

See the [CI guide](/environments/ci) for a complete example.

## Stage vs profile

Stages isolate what is deployed (state, physical names).
[Profiles](/environments/profiles) isolate how alchemy authenticates
to your cloud providers. The two are independent. You can pair a
`prod` stage with a `prod` profile, or use the same credentials
across many stages:

```sh
alchemy deploy --stage prod --profile prod
alchemy deploy --stage pr-42 --profile default
```

For the full set of CLI flags, see the
[CLI reference](/cli#common-options).
