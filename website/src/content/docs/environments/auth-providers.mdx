...
Planetscale, `DIGITALOCEAN_TOKEN` (or `DIGITALOCEAN_ACCESS_TOKEN`,
which is what `doctl` sets) for DigitalOcean. When `CI` is set,
`configure` auto-selects the env method without prompting, so
