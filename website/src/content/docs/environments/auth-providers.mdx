...
user-editable JSON that may come from another alchemy version, so every
load decodes against this schema — an invalid entry fails with a
reconfigure hint instead of reaching provider code that matches
...
schema-validated at registration, and registering a `readEnvironment`
without it fails.
...
then `read` materializes credentials. Profile selection itself
...
explicitly — with a console warning naming the variables that won. Passing
