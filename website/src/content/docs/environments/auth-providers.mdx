...
> {
  ...
  ): Effect.Effect<Config | void, AuthError>;
...
`updateConfig` writes changed values (for example a refreshed OAuth token)
back to the profile. `read` must never prompt. Fail with `NeedsReauth`
instead.
...
`configure`/`login`. The factory also wraps `read`, `details`, and `logout` in
the same cross-process file lock, so two processes never refresh credentials at the
...
A Profile stores one document per provider at
`~/.alchemy/profiles/{profile}/{provider}.json`, holding the chosen `method`
and its values. See
[Profiles › Where profiles live](/environments/profiles#where-profiles-live).
Each cloud bridges that config to its credential service with a
`fromProfile`/`fromAuthProvider` Layer:
...
Refreshed tokens are written back to the profile through the `updateConfig`
callback that Alchemy passes to `read`. A failed refresh surfaces as
`NeedsReauth`, which the profile UI renders as "needs re-login."
