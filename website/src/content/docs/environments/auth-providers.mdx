...
`resolveProviderConfig` selects the current profile and loads its stored
config — or runs the provider's `configure` on first use and persists the
result — then `read` materializes credentials. Profile selection
itself (`--profile`, `$ALCHEMY_PROFILE`, `alchemy profile`) is covered
  ...
  creds.expires > Date.now() + 10_000
    ...
    : yield* OAuthClient.refresh(creds).pipe(
        ...
        Effect.mapError(
          (e) =>
            new AuthError({
              message: "Cloudflare OAuth refresh failed. Run: alchemy profile edit --re-configure Cloudflare",
              cause: e,
            }),
        ),
      );
