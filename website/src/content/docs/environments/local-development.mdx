...
the real cloud, into the [`local_$USER` stage](/environments/stages)
...
the local-dev stage down with `alchemy destroy --stage local_$USER`.
...
- [Stages](/environments/stages) — how `live_$USER`, `local_$USER`, `pr-42`, and `prod` stay isolated.
