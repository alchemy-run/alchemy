...
the real cloud, tracked in the engine-owned
[`local:<user>` stage](/environments/stages) so the session can
never replace a stage you `alchemy deploy`'d. A stack that mixes
emulated and live-only resources just works. Tear that stage down
with `alchemy destroy --dev`.
...
- [Stages](/environments/stages) — how `dev_$USER`, `local:<user>`, `pr-42`, and `prod` stay isolated.
