...
`Alchemy.live()` opts a resource out of local emulation — "run this live
even in dev":

```typescript
import * as Alchemy from "alchemy";

const worker = yield* Worker("Api", { ... }).pipe(Alchemy.live());
```
...
(an R2 Bucket, a D1 Database) already run live in dev — `live()` on them
is a no-op.

### Switching is a replacement

When a resource moves between local and live — you deploy after a dev
session, or add/remove `live()` — it's planned as a **replacement**: the
