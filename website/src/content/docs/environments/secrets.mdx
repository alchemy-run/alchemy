...
By default the local state store (`.alchemy/state/`) writes secrets
as plaintext markers:

```json
{ "apiKey": { "__redacted__": "sk-live-..." } }
```

Set `ALCHEMY_PASSWORD` to encrypt them at rest instead:

```bash
ALCHEMY_PASSWORD=$(openssl rand -base64 32) # save this somewhere safe
```

With the password set, every `Redacted` value is encrypted
(AES-256-GCM, key derived with scrypt) before it is written:
...
introspectable — only the secret payloads are ciphertext.
...
- **Change detection still works.** Secrets are decrypted when state
  ...
  before.
- **The same password must be used for every deploy** of that state
  (including CI). Reading encrypted state without the password (or
  with the wrong one) fails with an explicit error.
- **Existing plaintext state migrates automatically.** Old
  `__redacted__` markers still read fine; each resource is
  re-written encrypted the next time it is deployed.
- **It applies to every client-side state store** — local,
  S3 (`AWS.state()`), and HTTP. The Cloudflare-hosted state store
  (`Cloudflare.state()`) additionally encrypts every record
  server-side with its own generated key, whether or not
  `ALCHEMY_PASSWORD` is set; setting it there gives you end-to-end
  encryption where secrets leave your machine already encrypted.
- `alchemy state get` prints `__secret__` envelopes (not plaintext)
