...
- **S3 state** (`AWS.state()`) uses KMS envelope encryption with the
  ...
  permission is required. When readers can't reach KMS, set
  `ALCHEMY_PASSWORD` to use a password-derived key instead; opt out
  entirely with `AWS.state({ secretEncryption: "off" })`.
