...
Pass `registry` on deploy (`ghcr.io/org` or `docker.io/user`) so
the image can be pushed; it is unused under `alchemy dev`. Your
...
`errorPage` and `spa` apply only to `output: "static"`. `errorPage`
serves the built `404.html` for unmatched routes; `spa` answers
misses with the index page (200) instead.
...
are created. `registry` is unused in this mode. Wrap the site in
