---
title: Static sites
description: Deploy a static site to Railway with Railway.Website.StaticSite — a build command, a container Service, and a required image registry on deploy.
sidebar:
  order: 4
---
...
`command` and `outdir` are required. On the live path `registry` is
required too (`ghcr.io/org` or `docker.io/user`) — deploy dies with
`RegistryRequired` if it is missing. It is unused under
`alchemy dev`. Declare the site as a module-level const:
...
`registry` is unused, and `site.url` is the local address detected
from stdout (`dev.url` pins it when detection fails). `dev` also
accepts `cwd` and `env` overrides:
