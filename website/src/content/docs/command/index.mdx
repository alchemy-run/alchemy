...
```typescript
yield* Command.Exec("final-backup", {
  command: "true",
...
destroyed, and a non-zero exit fails the destroy — so make it
tolerate an already-gone target if it shouldn't block teardown.
