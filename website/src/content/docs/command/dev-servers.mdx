...
Under `alchemy dev` the server is also reachable through the shared ingress as `http://<name>.localhost:1337` (`Dev("Web", …)` → `web.localhost`), and `alchemy dev --tunnel` gives it a public `https://*.trycloudflare.com` URL. `dev.url` stays the server's own address — emulators running in Docker dial it directly — so the ingress and tunnel URLs are printed when the server starts and listed at `http://localhost:1337`. See [dev](/cli/dev#subdomains).
