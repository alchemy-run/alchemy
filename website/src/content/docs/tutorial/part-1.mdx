...
Install <code>alchemy@{alchemyVersion}</code>, Effect, and the Cloudflare
provider packages used by this tutorial. Alchemy keeps provider
integrations in opt-in packages, so a project imports `alchemy/Cloudflare`
only after those Cloudflare packages are present in its package manifest.
