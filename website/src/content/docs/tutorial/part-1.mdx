...
import { gettingStartedPkgs } from "../../../versions";
...
Install Alchemy, Effect, and the Cloudflare package group used by this
tutorial. The generated command includes the runtime packages, the
Cloudflare resource API, shared Distilled error types, Worker binding
types, and the Worker packaging path that `Cloudflare.state()` uses when
it bootstraps the account-level state-store Worker.
