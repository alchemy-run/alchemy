...
description: Receive messages from a Cloudflare Queue with Cloudflare.Queues.messages(queue, handler) — Effect-style stream handler with automatic ack/retry.
sidebar:
  order: 10
---
...
`Cloudflare.Queues.messages(queue, handler)` API.

`messages(...)` does both halves in one call: it registers a
...
`Cloudflare.Queues.messages(queue, handler)` registers a queue
...
`messages(...)` is a `Context.Service` call —
...
Without the live layer, the `messages` call fails at deploy with
...
`Cloudflare.Queues.messages(queue, handler)` is the same primitive
...
ack/retry, and the same surface as `AWS.SQS.messages(queue, handler)`.
