...
description: Receive messages from a Cloudflare Queue with Cloudflare.Queues.messages(queue).subscribe(...) — Effect-style stream handler with automatic ack/retry.
sidebar:
  order: 10
---
...
`Cloudflare.Queues.messages(queue).subscribe(handler)` registers a queue
...
- Cloudflare.Queues.messages<QueueMessageBody>(queueResource).subscribe(
+ Cloudflare.Queues.messages<QueueMessageBody>(queueResource, {
+   batchSize: 25,
+   maxRetries: 3,
...
`Cloudflare.Queues.messages(queue).subscribe(...)` is the same primitive
