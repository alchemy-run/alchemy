...
description: Receive messages from a Cloudflare Queue with Cloudflare.Queue.messages(queue).subscribe(...) — Effect-style stream handler with automatic ack/retry.
sidebar:
  order: 10
---
...
`Cloudflare.Queue.WriteQueue(...)` from the [Queue producer
...
`Cloudflare.Queue.QueueConsumer` resource that tells Cloudflare to
...
`Cloudflare.Queue.WriteQueue` for a typed sender. The sender exposes `send` /
...
`Cloudflare.Queue.messages(queue).subscribe(handler)` registers a queue
...
- Cloudflare.Queue.messages<QueueMessageBody>(queueResource).subscribe(
+ Cloudflare.Queue.messages<QueueMessageBody>(queueResource, {
+   batchSize: 25,
+   maxRetries: 3,
...
`Cloudflare.Queue.messages(queue).subscribe(...)` is the same primitive
