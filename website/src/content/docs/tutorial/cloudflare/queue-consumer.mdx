...
description: Receive messages from a Cloudflare Queue with Cloudflare.Queues.consumeQueue(queue, handler) — Effect-style stream handler with automatic ack/retry.
sidebar:
  order: 10
---
...
`consumeQueue(...)` does both halves in one call: it registers a
...
`consumeQueue(...)` next — it's the same handle, not a new one.

## Subscribe to incoming messages

`Cloudflare.Queues.consumeQueue(queue, handler)` registers a queue
...
`consumeQueue(queue, props)` accepts a settings object. Time fields are
...
`consumeQueue(...)` is a `Context.Service` call —
...
Without the live layer, the `consumeQueue` call fails at deploy with
...
`Cloudflare.Queues.consumeQueue(queue, handler)` is the same primitive
...
ack/retry, and the same surface as `AWS.SQS.consumeQueue(queue, handler)`.
