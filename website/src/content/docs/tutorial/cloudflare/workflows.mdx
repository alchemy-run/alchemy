...
`Cloudflare.KV.KVNamespace("KV")` is just a description — Alchemy
...
`Cloudflare.Workers.task("name", effect)` runs the inner Effect inside a
...
`Cloudflare.Workers.sleep("name", "2 seconds")` parks the workflow without
 ...
 export default class NotifyWorkflow extends Cloudflare.Workflow<NotifyWorkflow>()(
   ...
   Effect.gen(function* () {
     ...
     return Effect.gen(function* () {
       ...
       const event = yield* Cloudflare.Workers.WorkflowEvent;
       const { roomId, message } = event.payload as {
         roomId: string;
         message: string;
       };

       const stored = yield* Cloudflare.Workers.task("kv-roundtrip", /* ... */);
...
`Cloudflare.Workers.task`, so the response is checkpointed and the
