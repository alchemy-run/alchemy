...
Add an `AiSearch` construct pointed at the bucket. Pass the bucket's
`bucketName` as the `source`.
...
For an R2 source, the indexer needs permission to read your bucket.
Rather than make you provision that by hand, `AiSearch` mints it as
stable children of the construct:
...
The construct returns `{ instance, token }` so you can reach the
underlying `AiSearchInstance` when you need to bind it.

## Opt out of the managed token

By default (`token: "managed"`) Alchemy provisions the read token for
you, as shown above. Pass `token: "auto"` to opt out of that: the
construct skips the `AccountApiToken` + `AiSearchToken` children and
creates the instance with no `tokenId`, leaving the source credential
to the platform default instead of an explicit token you own.
...
The managed default (`token: "managed"`) is recommended — the token is
least-privilege, lives in your stack's state, and is torn down with
everything else on `destroy`.
