...
`Cloudflare.AiSearch.AiSearch` helper wires all of that up in a single call.
...
export const Docs = Cloudflare.R2.R2Bucket("Docs", {});
```
...
export const Search = Cloudflare.AiSearch.AiSearch("Search", {
  source: Docs,
});
```
