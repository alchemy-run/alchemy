...
+yield* DynamoDB.consumeStream(table, (stream) =>
+  stream.pipe(
...
That single `consumeStream(...)` call enables a stream on the table,
...
`consumeStream` and TypeScript will help you destructure
`NewImage`/`OldImage`:

```diff lang="typescript"
-yield* DynamoDB.consumeStream(table, {
+yield* DynamoDB.consumeStream<{ id: { S: string }; content: { S: string } }>(table, {
  streamViewType: "NEW_AND_OLD_IMAGES",
  startingPosition: "TRIM_HORIZON",
  batchSize: 10,
}, (stream) =>
