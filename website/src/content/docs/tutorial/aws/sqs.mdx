...
`SQS.consumeQueue(queue, ...)` is the consumer-side mirror
of `consumeBucket` / `stream`: a typed `Stream<SQSRecord>` you
...
export default class Worker extends AWS.Lambda.Function<Worker>()(
  ...
  Effect.gen(function* () {
    const queue = yield* Jobs;
+
+    yield* SQS.consumeQueue(queue, { batchSize: 10 }, (stream) =>
+      stream.pipe(
...
That single `consumeQueue(...)` call creates the Lambda event
...
`SQS.consumeQueue(queue)` and now sees one record per DynamoDB
