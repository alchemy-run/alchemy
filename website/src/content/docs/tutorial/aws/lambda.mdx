...
import { awsTutorialPkgs } from "../../../../versions";
...
<Code code={`mkdir my-app && cd my-app && bun init -y
bun add ${awsTutorialPkgs}`} lang="sh" />

The stack imports `alchemy/AWS`, so the AWS package group is part of the
project dependencies. The group includes the AWS client surface, Smithy
credential types, and Lambda handler type declarations used by AWS
