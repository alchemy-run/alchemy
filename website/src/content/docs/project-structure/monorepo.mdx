---
title: Monorepo
description: Two ways to organize an Alchemy monorepo. Use one Stack at the workspace root, or one Stack per package wired with typed references.
---

A monorepo is a workspace with a backend package and a frontend
that calls it. Alchemy also asks you to decide where the Stacks live.
...
one plan. The frontend reads the Worker's URL directly from the
in-memory Output. It needs no references and no deploy ordering.
...
- Start with a **Single Stack**. It has one plan, one state file,
  and no ordering constraints.
- Use **Multiple Stacks** when packages deploy on different
  cadences, such as different teams or different CI, or when the
  backend has consumers beyond this frontend.
- Use **Multiple Stacks** when you need to `destroy` one package
