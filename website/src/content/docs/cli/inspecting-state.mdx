...
Prints the state files tracked under that stack/stage — the paths
`state read` takes.
...
`state delete` is destructive but **local-only** — it deletes
alchemy's record, never the cloud resources themselves:
