---
title: state
description: Inspect and manage the state store — browse interactively, list stacks, stages, and resources, print persisted state, and delete entries.
---
...
resources with the persisted state of the selected resource. Mark
...
# a whole stage as one JSON document — made for jq
...
`output.json` cannot be deleted independently — it is removed with its
stage.
