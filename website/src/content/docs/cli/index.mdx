...
Each command has its own page, including [deploy](/cli/deploy), [plan](/cli/plan), [destroy](/cli/destroy), [drift](/cli/drift), [unsafe nuke](/cli/nuke), [dev](/cli/dev), [logs](/cli/logs), [profile](/cli/profile), [state](/cli/state), [provider check-env](/cli/provider), [AWS provider commands](/cli/aws), and [Cloudflare provider commands](/cli/cloudflare). The workflow guides cover [adopting resources](/cli/adopting-resources) and [inspecting state](/cli/inspecting-state).

## Common options

| Option              | Description                                                                                                                              |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `--stage <name>`    | Stage to target. See the fallback order below. Must match `[a-z0-9]+([-_a-z0-9]+)*`. |
| `--profile <name>`  | Auth profile from `~/.alchemy/profiles/<name>/`. Overrides `$ALCHEMY_PROFILE`. Defaults to `default`. |
...
Without `--stage`, Alchemy picks the stage in this order:

1. `$ALCHEMY_STAGE`, read from the process environment, `--env-file`,
   or `.env`.
2. `live_$USER` for `deploy`, `destroy`, `plan`, `logs`, and `drift`, or
   `dev_$USER` for `alchemy dev`. On Windows, `$USERNAME` stands in for
   `$USER`.
3. `live_unknown` or `dev_unknown` when no user is set.

See [Stages](/environments/stages) for how stages isolate deployments.

## Interactive mode

In an interactive terminal, `deploy`, `destroy`, and `plan` render a live TUI with the rendered plan, an arrow-key approval prompt, and per-resource apply progress that unmounts into the final output. Everywhere else the same commands emit plain line-oriented logs.
