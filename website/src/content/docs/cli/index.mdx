...
Every command operates on an `alchemy.run.ts` stack file (or a custom entrypoint passed as the positional `[file]`, which must exist) and targets a [stage](/concepts/stages).
...
Running these commands in a pipeline? See [CI](/guides/ci).
...
- [CI](/guides/ci) — run the CLI in pipelines
