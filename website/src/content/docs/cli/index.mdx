...
| Option              | Description                                                                                                                              |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `--stage <name>`    | Stage to target. Falls back to `$STAGE`, then `dev_$USER`. Must match `[a-z0-9]+([-_a-z0-9]+)*` (case-insensitive). `:` is reserved. |
| `--dev`             | Target the engine-owned `local:<user>` stage that `alchemy dev` uses. `destroy`, `plan`, `logs`, and `drift` accept it. |
