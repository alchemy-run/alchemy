...
│
├─ profile create|rename|edit|current|set-default|list|show|delete # manage credentials and accounts
│
...
Each command has its own page: [deploy](/cli/deploy), [plan](/cli/plan), [destroy](/cli/destroy), [unsafe nuke](/cli/nuke), [dev](/cli/dev), [tail](/cli/tail), [logs](/cli/logs), [profile](/cli/profile), [state](/cli/state), [aws](/cli/aws), [cloudflare](/cli/cloudflare) — plus two workflow guides, [Adopting Resources](/cli/adopting-resources) and [Inspecting State](/cli/inspecting-state).
...
`--profile` is genuinely common — `deploy`, `plan`, `destroy`, `dev`, `tail`, `logs`, and `state` all take it.
