...
The same table drives the provider-side contract: see [Adoption in Resource Lifecycle](/concepts/resource-lifecycle#adoption) for the engine's routing and [the `read` operation](/concepts/provider#read) for how a provider returns `Unowned(attrs)`.
...
- [Resource Lifecycle](/concepts/resource-lifecycle) — where adoption sits in the engine
- [Providers](/concepts/provider) — implementing `read` and `Unowned`
