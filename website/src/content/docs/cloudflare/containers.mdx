...
[Durable Object](/cloudflare/durable-objects): the DO owns the
...
Reach for a container when a [Worker](/cloudflare/workers) isn't
...
- [Run a Container](/cloudflare/guides/containers) — build the
  sandbox above out to a shell-executing container with RPC, HTTP
  proxying, stage-dependent config, and tests.

Related building blocks:

- [Durable Objects](/cloudflare/durable-objects) — every container
  is fronted by one.
- [Workers](/cloudflare/workers) — the entrypoint that reaches the
