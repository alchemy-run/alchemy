...
a [Worker](/cloudflare/workers), and every method you return becomes
...
This schemaless, typed-stub pattern is the recommended way to talk
...
[Bind to another Worker's Durable Object](/cloudflare/guides/cross-worker-durable-object).
...
methods and streaming without the ceremony. If you want
schema-*validated* procedures (payload validation, tagged error
schemas shared with external clients), reach for
[Effect RPC](/cloudflare/guides/effect-rpc) on a Worker instead.

## Where next

Guides that build on Durable Objects:

- [Accept WebSockets](/cloudflare/guides/hibernatable-websockets) —
  WebSocket connections that survive hibernation.
- [Bind to another Worker's Durable Object](/cloudflare/guides/cross-worker-durable-object)
  — one Worker hosts the DO, others get a typed stub.
- [Run a Container](/cloudflare/guides/containers) — a long-lived
  container managed by a DO.
- [Add a Workflow](/cloudflare/guides/workflows) — durable multi-step
  orchestration.

Related:

- [Workers](/cloudflare/workers) — the runtime every DO is reached
  through.
