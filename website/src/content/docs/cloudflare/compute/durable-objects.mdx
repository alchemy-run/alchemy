...
A method can also return an object whose methods keep running inside the
instance, such as a per-caller session. See
[RPC objects](/cloudflare/apis/rpc-objects#return-a-session-from-a-durable-object).
