...
For SQL tables, load checked-in files with
[`Cloudflare.SqlMigrations("./drizzle")`](/sql/effect-sql/migrations#durable-object-migrations)
in the outer Effect and apply them in the inner Effect before activation.
[Drizzle's Durable Object integration](/sql/drizzle/migrations#durable-object-migrations)
accepts the same migrations alongside your typed schema.
