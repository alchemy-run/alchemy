...
For objects that use SQL tables, follow the [Users SQL migration guide](/cloudflare/data/sql-migrations)
for construction-time capture, per-instance application, and SQL-only redeploys.
Generate and commit migration files with your schema changes before deploying. Load them in the outer Effect and apply them
in the inner Effect:
...
Use schema-based `RpcDurableObject` when external clients share an explicit
`RpcGroup`, need schema-validated payloads and tagged errors, or connect directly
to a named object over a hibernatable WebSocket. It also preserves schema-backed
values across the wire where the class-method bridge serializes plain values.
See [`Cloudflare.RpcDurableObject`](/cloudflare/apis/effect-rpc#sugar-cloudflarerpcdurableobject)
and the cross-provider [RoomRpcs example](/cloudflare/apis/effect-rpc#build-the-roomrpcs-example).
Class-method RPC remains the simpler choice for internal calls that do not need
...
The focused [durable schedules guide](/cloudflare/compute/durable-schedules)
uses the same archive callback as Celld and Rivet. The existing Document example
below also covers legacy schedule upgrades.
...
The two-phase declaration resolves services during construction and initializes
one instance during activation. Calls get fresh scopes. Storage transactions
cover the same object's native storage and callback scheduling, not remote work.

## Limitations

Class-method RPC is not schema-backed WebSocket RPC. Use explicit schemas for
external clients and authorize the selected object before forwarding requests.
Cloudflare's namespace transfer and placement rules are not portable host APIs.

## Related guides

- [Durable schedules](/cloudflare/compute/durable-schedules)
- [SQL migrations](/cloudflare/data/sql-migrations)
- [Celld Durable Objects](/celld/compute/durable-objects) · [Rivet Durable Objects](/rivet/compute/durable-objects)
