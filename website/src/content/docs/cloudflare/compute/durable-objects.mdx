...
Expose cancellation as another method on the object:

```diff lang="typescript"
return {
  save: /* unchanged */,
+  cancelSnapshot: (revision: string) => onSnapshot.cancel(revision),
};
```

Cancelling an absent ID succeeds. It removes pending work without
interrupting an already-running handler, and it can join the same storage
transaction as an application write. Scheduling and cancellation failures
use `Alchemy.CallbackError`.
