...
:::tip[Why `Cloudflare.DurableObject<Counter>()(...)` ?]
The empty `()` after `Cloudflare.DurableObject<Counter>` lets
TypeScript capture the class identity for the typed RPC stub. It's a
one-time bit of ceremony — the rest of your DO code looks completely
normal.
:::
