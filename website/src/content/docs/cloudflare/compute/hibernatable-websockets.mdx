...
+          socket.serializeAttachment({ userId: id });
...
`socket.serializeAttachment({ userId: id })` persists a structured-cloneable value
...
+  ) {
+    const attachment = socket.deserializeAttachment<{ userId: string }>();
+    if (!attachment) return;
+    const text =
+      typeof message === "string"
+        ? message
+        : new TextDecoder().decode(message);
+    const label = attachment.userId.slice(0, 8);
...
+    reason: string,
+  ) {
+    const attachment = ws.deserializeAttachment<{ userId: string }>();
+    if (attachment) sessions.delete(attachment.userId);
...
+      const data = socket.deserializeAttachment<{ userId: string }>();
+      if (data) sessions.set(data.userId, socket);
+    }
...
`{ userId }` we stashed in the upgrade step. Without this loop the
...
This versioned Session matches the [Celld](/celld/compute/hibernatable-websockets)
and [Rivet](/rivet/compute/hibernatable-websockets) examples. `userId` is a string;
`connectedAt` is a Date encoded as a string. The version detects incompatible
attachments; it does not migrate them automatically.
...
for (const socket of yield* state.getWebSockets()) {
  yield* socket.getAttachment(Session).pipe(
    Effect.tap((session) =>
      Effect.sync(() => sessions.set(session.userId, socket)),
    ),
...
including older sessions without a version or `connectedAt`. Your code chooses this behavior;
    ...
    return {
      ...
      webSocketMessage: Effect.fn(function* (socket, message) {
        ...
        const text = typeof message === "string"
          ...
          : new TextDecoder().decode(message);
-        for (const peer of sessions.values()) {
-          yield* peer.send(`[${attachment.userId.slice(0, 8)}] ${text}`);
-        }
+        yield* broadcast(`[${attachment.userId.slice(0, 8)}] ${text}`);
      }),
      webSocketClose: /* ... */,
+      broadcast,
    };
...
Native sockets and their attachments can outlive an idle activation. Rebuild
application indexes after wake and validate versioned data before using it.
Cloudflare's native auto-response can answer configured heartbeat pairs without
waking the object; Effect RPC owns its protocol heartbeat handling separately.

## Limitations

An attachment is not an authentication mechanism. In the example, the generated
ID is a session label; derive real user identity from an authenticated upgrade.
Cloudflare's native attachment limit is 16,384 bytes including serialization
overhead. Rivet instead requires JSON connection state, while Celld uses its own
native attachment backend. Do not overwrite reserved RPC recovery metadata.
Idle socket retention does not make unfinished calls replayable.

## Related guides

- [Effect RPC](/cloudflare/apis/effect-rpc#build-the-roomrpcs-example)
- [Celld hibernatable WebSockets](/celld/compute/hibernatable-websockets) · [Rivet hibernatable WebSockets](/rivet/compute/hibernatable-websockets)
