...
## Define an optional attachment codec

The unchecked methods above remain available. For validation, define an
Effect Schema codec for your application's attachment:
...
The application sees a `Date`; the attachment stores its encoded string.
Schemas with services retain their requirements: writing needs encoding
services, and reading needs decoding services.

## Encode typed attachments

Replace the unchecked write in `fetch` with `setAttachment`:
...
`setAttachment` encodes before calling native `serializeAttachment`. It adds
no envelope: unchecked reads see `{ id, joinedAt: "..." }`, and previously
stored unchecked values can be decoded if they match the codec's encoded
shape. The native structured-clone and 16,384-byte serialized-size limit still
apply; a schema accepting a value does not guarantee the platform can store
it. Serialization overhead counts toward that limit, not just string length.
...
This application closes sockets with absent, malformed, or obsolete
attachments. The wrapper never closes them automatically. A
`WebSocketAttachmentError` identifies `encode`, `decode`, `missing`, `read`,
or `write` in its `reason`; `cause` preserves schema errors and native
exceptions. Nullish attachments count as missing, even with `Schema.Unknown`.
Older sessions lacking `joinedAt` fail validation unless your codec migrates
them.
