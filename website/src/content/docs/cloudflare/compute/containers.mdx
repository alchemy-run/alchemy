...
a background consumer) are genuinely released on exit — something no
serverless runtime can offer, because workerd and Lambda have no
teardown hook. A hard kill still skips finalizers, as in any
