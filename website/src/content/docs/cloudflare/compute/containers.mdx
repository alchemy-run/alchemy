...
registry is provisioned. With [shared publication](#share-published-images-across-stages),
...
build-layer cache. The Container deploys the immutable digest, not the
mutable `buildcache` tag.

This applies to Dockerfile builds, inline Dockerfiles, and Effect-native
`main` builds. For an Effect-native Container, put `publish` in the props
passed to `.make()`, alongside `main`. Omitting `publish` keeps the
application-specific repository and does not opt into shared registry caching.

:::caution[Pin external build inputs]
Pin base images and downloaded dependencies when opting in. A changed
upstream image tag or package download is outside the build context and
cannot invalidate its content hash, so a matching cached image may still
be reused.
:::
