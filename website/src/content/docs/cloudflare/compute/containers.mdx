...
On Docker Desktop that alias reaches processes bound to `127.0.0.1`.
On native Linux Docker (including CachyOS/Arch) it is the bridge
gateway IP (`172.17.0.1`), so the process must also listen there —
Alchemy TCP-forwards `@prisma/dev`'s loopback ports onto every local
RFC1918 address automatically. A host server you start yourself
should bind `0.0.0.0` (or the bridge address), not `127.0.0.1`.

If the container still times out to `172.17.0.1` on Linux, a host
firewall is dropping docker→host INPUT (nftables/firewalld on
CachyOS). Trust the docker subnet, for example:
...
or an nftables equivalent that accepts `iifname docker0` / source
`172.16.0.0/12` on the prisma/dev ports.
