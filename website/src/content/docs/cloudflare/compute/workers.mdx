...
export default Alchemy.Stack(
  ...
  Effect.gen(function* () {
    ...
    // worker.url is Output<string | undefined> (undefined only when
    // `url: false`); assert the narrower type for the stack output
...
subdomain, and prints the URL as a stack output. See
[Outputs](/infrastructure-as-code/outputs#as) for how `.as<T>()`
narrows lazy `Output` values.

## Handle the request

Inside `fetch`, read the incoming request with `HttpServerRequest` —
method, path, headers, and body. A small JSON API:
  ...
  Effect.gen(function* () {
    return {
      fetch: Effect.gen(function* () {
        ...
        const url = new URL(request.url, "http://x"); // request.url is a path
...
`request.headers` is a plain record (e.g. `request.headers["host"]`
for building absolute URLs of the deployed Worker).
