...
Every URL that serves a Worker is collected in `worker.urls`, most
significant first, and `worker.url` is always `urls[0]`. The ranking:
the canonical custom domain, then its aliases, then the stable
`workers.dev` URL, then version preview URLs. Under `alchemy dev`,
`urls` is the local dev server's `[localhost, ...LAN]` addresses
instead — the URLs actually serving your session. `worker.urls` is
handy wholesale, e.g. as a CORS allow-list.

The `workersDev` prop controls the `workers.dev` surface: `true` (the
default) serves the stable URL and enables version previews, `false`
turns both off, and the object form
(`{ enabled, previewsEnabled }`) toggles them independently.

To serve the Worker from your own hostname, pass `domain` — a string,
or an object with `aliases` (additional hostnames that serve the
Worker) and `redirects` (hostnames that 301 to the canonical name,
path and query preserved, without ever invoking the Worker). Every
hostname is attached as a Cloudflare custom domain, so DNS records and
edge certificates are managed for you; the zone must already exist in
the account:
...
Redirect hostnames appear in `worker.domain` but never in
`worker.urls` — they serve no content. For zone setup, DNS records, and
route patterns, see the
