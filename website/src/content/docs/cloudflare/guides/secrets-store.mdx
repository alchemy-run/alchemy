...
- **Env vars** ([`Config.redacted`](/concepts/secrets)) — the value
  ...
  and per-app config. See [Secrets and env vars](/guides/secrets)
...
- [Workers](/cloudflare/workers) — the host the secret binds into.
- [Secrets and Config](/concepts/secrets) — how env-var bindings
  work under the hood.
- [Secrets and env vars](/guides/secrets) — wiring `.env` values
  into a Worker.
