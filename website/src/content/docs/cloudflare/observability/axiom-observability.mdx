...
1. **From the Worker** — OTLP endpoints and an ingest token as env
   vars; the [built-in telemetry](/infrastructure-as-effects/telemetry)
   exports your Effect spans, logs, and metrics directly.
...
Pass each dataset's endpoint and headers through the Worker's `env`
prop. That's the whole integration — the built-in telemetry reads
the standard `OTEL_*` variables per signal and exports every span,
log record, and metric your Worker produces, flushed as each request
completes:
...
const worker = yield* Cloudflare.Worker("Api", {
  ...
  env: {
    ...
    OTEL_EXPORTER_OTLP_METRICS_HEADERS: Axiom.otlpHeaders(ingest, metrics),
  },
});
```
...
Axiom expects. Values are routed by shape: the endpoint strings
become `plain_text` vars, and the headers — carrying the `Redacted`
bearer — become `secret_text` bindings, so the token never appears
in plaintext Worker config. Axiom routes ingest by the
`X-Axiom-Dataset` header, which is why each signal needs its own
headers var rather than one shared `OTEL_EXPORTER_OTLP_HEADERS`.
