...
At runtime, `Drizzle.mysql` is the MySQL counterpart of
`Drizzle.postgres` — it takes Hyperdrive's connection string and
returns a typed drizzle db whose queries are Effects (swap the `pg`
dependency for `mysql2` + `@effect/sql-mysql2`):
...
The pool is built lazily on the first query, memoized per execution,
and torn down when the event settles — the same lifecycle as
`Drizzle.postgres`. Two MySQL-specific behaviors are handled for
you: queries run over mysql2's **text protocol** (Hyperdrive does
not proxy MySQL prepared statements / `COM_STMT_PREPARE`), and the
pool defaults to `disableEval: true` because mysql2's JIT row
parsers compile via `new Function(...)`, which the Workers isolate
blocks.
...
- [Drizzle hub](/drizzle) — where `Drizzle.Schema`'s generation and
  diff semantics live, including [Migrations as resources](/drizzle/migrations)
