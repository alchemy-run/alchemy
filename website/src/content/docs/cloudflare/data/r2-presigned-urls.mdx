...
Both functions take `key` and `expiresIn` (seconds, 900 by default,
7 days at most). `contentType` is the header an uploader must send,
or the type a download is served with. `PresignGetObject` also takes
`contentDisposition`, for example `attachment; filename="report.pdf"`.

## Async Workers

Declare `S3Credentials` on `env`:
...
secret with credentials derived from the Worker's API token. Under
`alchemy dev`, it points at the local endpoint with local credentials.
`access` sets the token's permissions: `"read"`, `"write"`, or
`"read-write"` (the default).

In an Effect-native Worker, `yield* Cloudflare.R2.S3Credentials(Uploads)`
...
the bucket. The local endpoint allows any origin.
...
bindings. A bucket piped through `Alchemy.remote()` uses real R2 in
dev. Two gaps remain:

- It never answers `Expect: 100-continue`, which some S3 SDKs send on
  uploads. Presigned URLs used from `fetch` or a browser are
  unaffected.
- Presigned POST (HTML form) uploads aren't supported. Use `PUT`.
