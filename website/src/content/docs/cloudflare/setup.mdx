...
[profile](/concepts/profiles) at `~/.alchemy/profiles.json` — no
...
`CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` from the
environment instead — see the [CI guide](/guides/ci) for the full
...
- [Cloudflare overview](/cloudflare) — the map of building blocks and
  guides.
