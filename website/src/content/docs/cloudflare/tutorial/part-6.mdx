...
## Swapping or removing the exporter

Telemetry is just a Layer, so turning it off is removing
`Axiom.Telemetry` from the `Layer.mergeAll` — without an exporter
Layer, Effect's no-op tracer makes every span free. And because the
Axiom layer is sugar over the generic `Alchemy.Telemetry.otlp`
binding layer, any OTLP backend slots into the same position:
...
`Alchemy.Telemetry.layer(...)` goes one step further and installs
any custom exporter Layer. See
