...
- [`Foldkit`](/cloudflare/frontend/foldkit) — Foldkit apps; one
  `vite build`, client-only or server-rendered, with no asset-routing
  default.
...
defaults: one `vite build`, and no asset-routing default, because
the same project can ship client-only, prerendered, server-rendered,
or hybrid.
