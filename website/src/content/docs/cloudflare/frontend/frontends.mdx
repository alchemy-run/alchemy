...
description: Deploy Vite, Astro, Nuxt, SvelteKit, Waku, or any static build to Cloudflare Workers with first-class Website resources.
sidebar:
  label: Overview
  order: 1
---
...
Use the resource named after your framework. `Astro`, `Nuxt`,
`SvelteKit`, `Waku`, and `Octane` each drive their framework's own
programmatic build and know its output layout, config surface, and dev
server.
