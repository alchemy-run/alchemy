...
With `output: "static"` every page is prerendered at
...
`errorPage: "404.html"` serves the built `404.html` for
...
The resource exposes common serializable options (`site`, `base`,
`output`, `srcDir`, `publicDir`, `outDir`, `trailingSlash`) as flat
props for deploy-specific overrides; Astro merges them *over* the
config file:
...
- `output` — Alchemy defaults it to `"server"` (superseding a
  ...
  Opt into a fully prerendered site with `output: "static"` on the
  resource.
