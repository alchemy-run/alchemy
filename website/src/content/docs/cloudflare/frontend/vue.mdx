...
With `spa: true`, unmatched paths return `index.html` instead of
a 404, and vue-router resolves the route on the client. It is
sugar for `assets.notFoundHandling: "single-page-application"` —
an explicit `assets` config wins.
