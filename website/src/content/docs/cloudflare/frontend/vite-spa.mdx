...
`spa: true` is sugar for
`assets.notFoundHandling: "single-page-application"`. A site that
ships a real error page sets `errorPage: "404.html"` instead, and
`assets` still accepts the standard Cloudflare Worker
static-assets config for everything else — an explicit
`assets.notFoundHandling` wins over the sugar.
