...
By default the build runs `next build` through your package runner
(`bunx` / `npx` / `yarn` / `pnpm exec`, detected from the nearest
lockfile). Set `build: { command }` to override it:
