...
The Next.js build itself runs programmatically through your
project's own `next` install (no shell command, no `build` script
required). A `buildCommand` set in `open-next.config.ts` is still
honored.
