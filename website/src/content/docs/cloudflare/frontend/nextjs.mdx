...
Alchemy generates the OpenNext configuration without changing `next.config.*`.
No `open-next.config.ts` or Alchemy plugin is required. The default cache
serves prerendered pages from static assets; use [Writable ISR](#writable-isr)
for revalidation.
...
Alchemy selects the adapters and binds the cache namespaces, revalidation
queue, and Worker self-reference. The KV cache fills on demand; build-time
cache entries are not uploaded to KV. Pages using writable ISR must be able
to regenerate with runtime credentials and data sources.
...
Build options belong on the resource, alongside its hosting configuration.
