...
Alchemy injects `vite-plugin-cloudflare:alchemy`. Leave the official
`@cloudflare/vite-plugin` out of this configuration. The same `alchemy()`
configuration selects KV on Cloudflare, S3 on AWS, and Redis on Node targets.
...
The build sets `ALCHEMY_VINEXT_CACHE=kv`, and `alchemy()` includes the KV adapter
for ISR and `"use cache"`. Deployment uploads prerendered HTML and React Server
