...
Options set on the resource (`srcDir`, `outDir`, `basePath`) merge
over the file, per key — `outDir` maps to waku's `distDir`. The one
key Alchemy owns is `unstable_adapter` — setting it fails the build
