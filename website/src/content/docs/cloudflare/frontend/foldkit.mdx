...
`Foldkit` defaults to SPA fallback (`spa: true`), which returns
`index.html` for unmatched paths instead of a 404 — the Foldkit
runtime resolves the route once the app boots.

A Foldkit app that ships a real 404 page serves it with
`errorPage: "404.html"` instead (and `spa: false` turns the
