...
[Foldkit](https://foldkit.dev) is a TypeScript frontend framework
built on Effect, using the Elm architecture. Foldkit apps build
through Vite, so `Cloudflare.Website.Foldkit` deploys one as a
Cloudflare Worker with a single declaration: no adapter, no build
command, no output directory, no Wrangler configuration.

`Foldkit` is [`Cloudflare.Website.Vite`](/cloudflare/frontend/vite)
with a narrowed prop surface, so the Vite page is the reference for
everything it does. It runs one `vite build` over the project: the
client output deploys as the Worker's static assets, and Alchemy's
injected Cloudflare plugin builds a server bundle only when the
declaration names a `main` — your Foldkit config declares no server
environment of its own.

Foldkit scaffolds three rendering modes — SPA, SSG, and SSR — and
they want different asset routing, so `Foldkit` applies no routing
default. See [Asset routing](#asset-routing) for which to set.
...
(`@alchemy.run/cloudflare-runtime/vite`) and is **not compatible**
with `@cloudflare/vite-plugin` in your `vite.config.ts`.
`create-foldkit-app` does not add it, so this applies only if you
added it yourself:
