...
The generated Cloudflare adapter is configured via `adapter` on the
resource:
...
Don't set an `adapter` in your `sveltekit(...)` call — Alchemy
injects its own wrangler-free Cloudflare adapter (a declared adapter
is replaced with a warning). Every other kit option belongs in the
`sveltekit(...)` call in your `vite.config.*`.
