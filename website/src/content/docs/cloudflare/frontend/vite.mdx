...
- **`spa`** — serve `index.html` for unmatched paths so client-side
  routing takes over; sugar for
  `assets.notFoundHandling: "single-page-application"`.
- **`errorPage`** — serve the built `404.html` for unmatched paths;
  sugar for `assets.notFoundHandling: "404-page"`. Cloudflare's
  asset layer serves the nearest `404.html` — the name is fixed by
  the platform, so `"404.html"` is the only accepted value. An
  explicit `assets.notFoundHandling` wins over both sugars, and
