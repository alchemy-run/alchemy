...
- **`assets`** — a flat `AssetsConfig` for routing behavior:
  `runWorkerFirst`, `htmlHandling`, `notFoundHandling`, etc.
  `runWorkerFirst` accepts `true` (every request hits the Worker
  before asset matching) or a glob array like `["/api/*"]` (only
  matching paths do — everything else stays assets-first). The
