...
[Schemaless RPC](/apis/schemaless) documents the pattern, including what makes a member callable, how the typed client arises, and what crosses the wire. This page walks Cloudflare's pairings. Worker → Worker and Worker → Durable Object ride the platform's native JSRPC channel (values move by structured clone, and service bindings never traverse the public internet); Containers ride the generic fetch transport.
...
Any non-`fetch` function member returning an Effect or a Stream becomes callable remotely, and the class instance type *is* the interface, so there is no schema to declare. The full member rules live at [Schemaless RPC](/apis/schemaless).
...
Tagged failures from a Durable Object arrive at the caller as plain `{ _tag, ...fields }` structs. Declare the error classes on the Durable Object, and the calling side rebuilds each matching failure as a real instance, so `instanceof` and schema encoders see the class:

```typescript
export class Repo extends Cloudflare.DurableObject<Repo, RepoShape>()(
  "Repo",
  // matched by the class's static `identifier` (Schema tagged errors),
  // otherwise by class name, so the class name must equal its `_tag`
  { errors: [RepoNotFound] },
) {}
```

Undeclared errors still arrive as plain structs. `Effect.catchTag` matches either way, because it keys on `_tag`.
