...
The Worker example below uses HTTP and produces the same `HttpEffect` type,
so its wiring matches the HTTP API guide. Durable Objects also support
[hibernatable WebSocket RPC](#http-and-websocket-rpc); the shared
[RoomRpcs example](#build-the-roomrpcs-example) shows streaming and scoped clients.
...
[RpcDurableObject API reference](/providers/cloudflare/workers/rpcdurableobject).
...
## Build the RoomRpcs example

The following schema-backed Room is also used in the
[Cloudflare](/cloudflare/apis/effect-rpc) and [Rivet](/rivet/apis/effect-rpc) guides.
It is independent of the Task and Counter examples above.
...
export class Room extends Cloudflare.RpcDurableObject<Room>()(
  "Room",
  { schema: RoomRpcs },
) {}
```
...
application procedure. Deploy this Worker in your
[Cloudflare Stack](/cloudflare/setup).
...
- [Hibernatable WebSockets](/cloudflare/compute/hibernatable-websockets)
- [Celld Effect RPC](/celld/apis/effect-rpc) · [Rivet Effect RPC](/rivet/apis/effect-rpc)
