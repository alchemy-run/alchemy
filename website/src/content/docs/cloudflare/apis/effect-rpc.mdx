...
Create the browser's Effect RPC client in a Layer:
...
class CounterClient extends Context.Service<CounterClient>()("CounterClient", {
  make: RpcClient.make(CounterRpcs),
}) {}

const CounterClientLive = Layer.effect(CounterClient, CounterClient.make).pipe(
...
The Layer owns the client and connection lifetime, so callers do not need
an explicit `Effect.scoped`. Provide it around the application or component
program that shares the client, rather than constructing it for each call.
Concurrent calls and streams share the socket; Effect handles typed errors,
backpressure, and cancellation. Client cleanup is separate from server-side
