...
Opt in on the same class:

```diff lang="typescript"
-{ schema: CounterRpcs },
+{ schema: CounterRpcs, transport: "websocket" },
```

WebSockets use Effect RPC's JSON protocol. HTTP remains available with
NDJSON, so existing `getByName` clients continue to work. The inner
Effect must return a handler Layer for WebSocket support. Existing
implementations returning `RpcServer.toHttpEffect(...)` remain supported
for HTTP.

## Forward browser connections

Forward an incoming request to a named object from the hosting Worker:
...
The namespace's `fetch` forwards HTTP requests and WebSocket upgrades;
it is not an application RPC method. Authenticate and authorize the
request before forwarding it. Use application-controlled object names
when access must be restricted to a particular instance.

## Connect over a WebSocket

Use Effect's existing WebSocket RPC client in the browser:
...
const Connection = RpcClient.layerProtocolSocket().pipe(
  Layer.provide(
    Layer.mergeAll(
      Socket.layerWebSocket("wss://example.com/rpc").pipe(
...
const program = Effect.gen(function* () {
  const counter = yield* RpcClient.make(CounterRpcs);
...
Keep the connection layer around the entire program that uses the client,
not just `RpcClient.make`. Concurrent calls and streams share the socket;
Effect handles typed errors, backpressure, and cancellation.
...
from socket attachments and handles protocol heartbeats internally.
