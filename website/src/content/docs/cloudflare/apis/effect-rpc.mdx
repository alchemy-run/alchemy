...
use [Schemaless RPC](/rpc/schemaless) instead; for choosing between
the modalities, see [RPC](/rpc).
...
- **Internal calls** (Worker-to-Worker, Worker-to-DO) —
  [Schemaless RPC](/rpc/schemaless): typed clients with no schema
...
The full decision guide lives at [RPC](/rpc).
