...
[s2][d]• http://worker.localhost:1337[/d]
...
serves at `http://api.localhost:1337`. Durable Objects run inside
...
## One port, a subdomain per Worker

Every local Worker is reachable through one shared ingress on port
`1337` as `http://<name>.localhost:1337` — the name is the Worker's
logical id in kebab-case (`Api` → `api.localhost`, `MyApi` →
`my-api.localhost`), or whatever `dev: { subdomain }` says:

```typescript
export default class Api extends Cloudflare.Worker<Api>()(
  "Api",
  { main: import.meta.url, dev: { subdomain: "api" } },
  // ...
) {}
```

Each Worker keeps its own `http://localhost:<port>` too (it follows in
`worker.urls`), and `http://localhost:1337` alone forwards to the only
Worker or lists them all. Distinct origins make CORS behave like
production; for cookies shared across subdomains use a real-looking
domain with `alchemy dev --domain myapp.test` (see
[dev](/cli/dev#subdomains) and [hosts](/cli/hosts)).

## Expose local Workers to the internet

```sh
alchemy dev --tunnel
```

Every local Worker also gets a public `https://<random>.trycloudflare.com`
URL through a free Cloudflare quick tunnel — for webhooks, phones, and
external services that must call into your machine. A tunneled Worker's
`url` is the public one; `dev: { tunnel: true | false }` overrides the
flag per Worker. `cloudflared` is downloaded on first use if it isn't
installed. See [dev](/cli/dev#tunnels).
...
| ------------------------ | --------------------------------------------------------------------------------- |
| Workers                  | workerd, with hot reload, a `http://<name>.localhost:1337` URL (and an optional quick tunnel) |
