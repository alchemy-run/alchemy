...
[custom domains guide](/cloudflare/guides/custom-domains).
...
- [Custom domains](/cloudflare/guides/custom-domains) — serve
...
- [Durable Objects](/cloudflare/durable-objects) — stateful instances
  behind your Worker.
- [KV](/cloudflare/kv) — low-latency key-value reads at the edge.
- [Queues](/cloudflare/queues) — background work with at-least-once
  delivery.
- [Workflows](/cloudflare/workflows) — durable multi-step jobs that
  outlive a request.
- [Secrets & env](/cloudflare/secrets-env) — bind `.env` values and
  secrets into the Worker.
- [Domains](/cloudflare/domains) — zones, DNS records, and
  certificates.
