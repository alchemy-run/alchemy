...
Zone that holds your domain, the `domains` prop that attaches hostnames
...
— a hostname, or an object with aliases and redirects. Cloudflare
provisions the DNS record and certificate for you; the zone is inferred
from each hostname, so it must already exist in the account:
...
The canonical domain is the Worker's most significant URL, so it leads
`worker.urls` and becomes `worker.url` — ahead of the `workers.dev`
URL. `aliases` are additional hostnames that serve the Worker, and
`redirects` are hostnames that answer with a permanent redirect (HTTP
301, path and query preserved) to the canonical name without ever
invoking the Worker — ideal for domain migrations:
...
Redirects are implemented as rules in the zone's
`http_request_dynamic_redirect` phase, which runs before Workers —
redirected requests never hit your code. Redirect hostnames appear in
`worker.domain` but never in `worker.urls`, since they serve no
content.
