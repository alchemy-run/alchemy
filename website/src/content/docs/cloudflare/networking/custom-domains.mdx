...
The simplest way to serve a Worker on your domain is the `domains` prop.
Cloudflare provisions the DNS record and certificate for you; the zone
is inferred from each hostname, so it must already exist in the account:
...
(most relevant first — the `workers.dev` URL when enabled, then your
custom domains in the order you provided them), `worker.domains` holds
the matching hostnames, and `worker.url` is always `allUrls[0]`.
Disable `workers.dev` to make your first custom domain the primary URL:
