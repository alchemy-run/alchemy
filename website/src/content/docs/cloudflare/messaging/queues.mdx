...
`Cloudflare.Queues.QueueSink(queue)` exposes the queue as an Effect
[Sink](/infrastructure-as-effects/sinks). Run any `Stream` of message
bodies into it; each stream chunk becomes one `sendBatch` call, split
at Cloudflare's 100-message / 256 KB batch limits.
...
Combined with `consumeQueueMessages`, a Worker can consume one queue,
transform each batch, and drain it into another:
...
`QueueSinkBinding` builds on the native `WriteQueueBinding`, so it is
the only layer the sink needs. It also works under `alchemy dev`.
