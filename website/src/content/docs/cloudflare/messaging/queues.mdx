...
| `Cloudflare.AI.ModelResource` | `workersAi.model` | Batch events for the selected model |
...
`AI.ModelResource` validates a Cloudflare-managed catalog model and persists a non-owning handle. It neither deploys nor invokes the model. `yield* Cloudflare.AI.ModelResource.ref("Embeddings")` can be used in place of `model` after deployment. Batch events require asynchronous batch inference; ordinary synchronous inference does not emit them.
...
- [Workers AI model handle API reference](/providers/cloudflare/ai/modelresource)
