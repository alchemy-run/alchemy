...
MySQL (Vitess) family, see [MySQL](/planetscale/mysql).
...
[Migrations](/planetscale/migrations) for how SQL files are hashed
and applied, and the
...
into consumers like [Cloudflare Hyperdrive](/cloudflare/hyperdrive):
...
- [Migrations](/planetscale/migrations) — `migrationsDir` and
  `importFiles` on databases and branches.
- [Hyperdrive](/cloudflare/hyperdrive) — edge-pooled connections from
  Workers to your PlanetScale database.
- [Drizzle guide](/cloudflare/guides/drizzle) — generate migration SQL
