...
import { Api, ApiLive, AuthenticatedLive } from "./api.ts";

export const GitObjects = Cloudflare.R2.Bucket("GitObjects");

const GitLive = Git.Server.layer(Api, ApiLive).pipe( // Api: Git.Api behind your middleware
  Layer.provide(Git.HandlersLive), // shared implementations used by ApiLive
...
`Api`, `ApiLive`, and `AuthenticatedLive` are the application API, its
group implementations, and its middleware. [Getting Started](/git/getting-started)
defines all three; [Server](/git/blocks/server#handlers) explains their composition.
...
service. Endpoints use Effect's `HttpApiEndpoint`; implementations use
