...
import { Api, AuthenticatedLive } from "./api.ts";
...
const GitLive = Git.Server.layer(Api).pipe( // Api: Git.Api behind your middleware
  Layer.provide(Git.HandlersLive), // shared Git handlers
...
`Api` and `AuthenticatedLive` are the application API and its middleware. [Getting Started](/git/getting-started)
