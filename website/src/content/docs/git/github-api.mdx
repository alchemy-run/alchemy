...
Authentication accepts GitHub's `Authorization: token <t>` alongside
`Bearer` and Basic, because that is what `gh` and Octokit send. The
credential is whatever your `Authenticated` middleware verifies, and `/user`
answers with the principal it resolved.
