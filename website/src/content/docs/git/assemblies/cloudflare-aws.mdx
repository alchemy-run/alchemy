...
On Cloudflare, the same Worker and Durable Object namespaces as the
[Cloudflare assembly](/git/assemblies/cloudflare), minus the bucket.
...
those is a hop the R2 assembly does not make.

## When it is worth it

Take this assembly when the bytes have to be in AWS. Take the Lambda
