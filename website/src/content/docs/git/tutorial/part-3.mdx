...
behind it, so you implement it once, with whatever your application
uses to know who is calling. Anonymous is an answer, not an error: the
policy decides what anonymous may do.
...
`Git.Authenticated.make` takes an Effect that produces the resolver.
Resources, bindings, and the services the resolver needs are declared
in that Effect. The resolver is an Effect that runs once per request,
reads `HttpServerRequest`, and answers with a principal, or
`undefined` for anonymous:
...
export const AuthenticatedLive = Git.Authenticated.make(
  Effect.gen(function* () {
    ...
    return Effect.gen(function* () {
      ...
      // TODO: resolve a principal from the request
      return undefined;
    });
  }),
);
...
`getSession` reads the same `HttpServerRequest`:
...
groups, and yours is one more. This API is the boundary your
...
their own. [Part 4](/git/tutorial/part-4) writes the rules: who reads
what, and who may move `main`. The complete app, with a React UI that signs
