...
`Git.Authenticated.make` takes an `init` that runs when the layer is
built and returns the per-request resolver. An API key in the password
field of the remote names a user. Otherwise the session cookie does.
...
`Git.parseSecret` reads a bearer token or the Basic password, which is
what `git` sends and what `gh api` sends.
...
-  Layer.provideMerge(Git.AuthenticatedSecret({ principal: { id: "acme" } })),
);
```
