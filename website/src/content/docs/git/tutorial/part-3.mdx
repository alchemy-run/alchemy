...
The git routes are ordinary Effect `HttpApiEndpoint` values. Yours are
built the same way. `Me` declares the middleware it relies on so its
handler can read the `Session`:
...
An endpoint goes in a group, and a group goes in an API. Add the new
group to `Git.Api`, then put the middleware in front of every group:
...
`Server.layer` supplies Git’s default groups. Implement only the new
application endpoint, using `AppApi` so its middleware provides `Session`:
...
The group builder checks that every endpoint is implemented. Its setup
runs once, and the handlers run per request. Use `Effect.gen` in the
group builder when your own handlers need storage clients or bindings.

## The host

Pass the API and your `MeLive` group to `Git.Server.layer`. Provide
`Git.HandlersLive` to build the shared Git handlers, and provide the
middleware and storage layers as before:
...
Replace the old `Api` declaration with `AppApi`. The server registers
Git’s groups and adds the application group from `MeLive`.
...
has no user to answer with. Add a replacement `github` group. Spread the
default handlers and supply a new `user` handler:
...
The group is built against `AppApi`, so its middleware supplies `Session`
directly. All the other handlers come from `git.github`. Any Git handler
can be replaced this way. Pass the replacement alongside your application group:
