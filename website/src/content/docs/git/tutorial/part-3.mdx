...
Implement each group against `AppApi`. This replaces the `ApiLive`
assembly from Part 1. `Git.Handlers` supplies the Git handlers, while
`handle("me", ...)` implements the new endpoint:
...
Pass the API and its group layers to `Git.Server.layer`. Provide
...
Delete the old `Api` and `ApiLive` declarations; `AppApi` and
`AppApiLive` now include both the Git groups and the application group.
...
has no user to answer with. In `AppApiLive`, replace the `github` group
registration with this one. Spread the default handlers and supply a
new `user` handler:
...
can be replaced this way; `GitLive` still receives the same `AppApiLive`.
