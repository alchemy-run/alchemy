...
git route. Register each group against that API with `HttpApiBuilder.group`;
`Git.Handlers` holds the engine's reusable handlers:
...
`Server.layer(Git.Api, Git.ApiLive)` with `Git.HandlersLive` and no middleware. Part 3 replaces the
