...
to verify a push. This does not compile yet. `Git.Server` needs five
...
Refs, the object index, tokens, the commit graph, and pull requests
live in its SQLite, and a push's ref update is one transaction:
...
`Git.AuthTokens` is the smallest thing that secures a fresh host: a
deployer admin secret and per-repository tokens:
...
The file compiles. Five lines, five decisions, and each one is a Layer
you can replace later without touching the others.
...
The admin secret is read from your environment at deploy time and
bound to the Worker as a secret:
...
mints tokens. The reference for each line you wrote is under
[Building blocks](/git/blocks).
