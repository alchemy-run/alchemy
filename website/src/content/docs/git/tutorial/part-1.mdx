...
`Git.Authenticate` turns a request into a principal, or anonymous. The
smallest thing that secures a fresh host is one shared secret: whoever
presents it is `acme`, and repositories `acme` creates are `acme`'s:
