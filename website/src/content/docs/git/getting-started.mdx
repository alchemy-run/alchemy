...
`ApiLive` implements all six groups against `Api`, so each group sees its
middleware. `Git.HandlersLive` builds the shared storage clients and handlers.
`Server.layer(Api, ApiLive)` mounts those groups and the internal hash route;
the internal route stays outside the application middleware.
