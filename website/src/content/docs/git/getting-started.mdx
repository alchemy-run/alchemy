...
`Server.layer(Api)` registers the Git routes with your API’s middleware.
`Git.HandlersLive` builds their shared storage clients and handlers. The
internal hash route stays outside the application middleware.
