...
`Policy` is the same. `Authenticate` runs at the Worker, and the policy
...
| [Auth](/git/blocks/auth) | who is calling, and what they may do | `PolicyOwners`, `AuthenticateSecret`, `Authenticated` |
