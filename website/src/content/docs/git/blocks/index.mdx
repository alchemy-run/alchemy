...
A git host is six decisions: what serves HTTP, where refs and objects
live, how names resolve, where bulk bytes go, what verifies a push, and
who may do what. Each is a [Layer](/infrastructure-as-effects/layers),
and you make all six in one graph:
...
`Auth` is the same. `authenticate` runs at the Worker, and `authorize`
runs wherever the facts it needs live.

## The blocks

| Block | Decides | Ships with |
| --- | --- | --- |
| [Server](/git/blocks/server) | the HTTP surface and the Worker it runs in | `ServerLive` |
| [Repositories](/git/blocks/repositories) | where refs and objects live, the jobs that keep a repository small | `ReposDurableObject` |
...
- [Server](/git/blocks/server) — the three planes behind `git.fetch`,
  and mounting it under your own routes.
- [Choosing an assembly](/git/assemblies) — which lines change for
  which requirements.
