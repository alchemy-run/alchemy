...
Merge `AppRoutesLive` with the six Git group layers built against `AppApi`,
then pass the merged layer to `Git.Server.layer(AppApi, AppApiLive)`.
[Server](/git/blocks/server#handlers) shows the full assembly. The registry
and repository clients are resolved when the group builds; the handler
runs once per request behind the API's middleware.
