...
Merge this group layer with Git's group implementations and pass them to
`Git.Server.layer(AppApi, AppApiLive)` as shown in
[Server](/git/blocks/server#handlers). A replacement handler registered
against `AppApi` can also yield `Session` directly.

Hooks are independent of the API contract. They read application services
optionally, because a host may provide a hook without that middleware:
