...
[profile](/environments/profiles) at `~/.alchemy/profiles.json`. You
don't need environment variables. Login only happens through the
`profile` command. A deploy with nothing configured fails and prints the
