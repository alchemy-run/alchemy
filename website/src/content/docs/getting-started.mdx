...
import { dependencyRows, gettingStartedPkgs } from "../../versions";

export const pkgs = gettingStartedPkgs;
...
This page uses `Cloudflare.state()` and `alchemy/Cloudflare`, so the
install command includes Alchemy core packages plus the Cloudflare
packages needed to deploy the state-store Worker and create Cloudflare
resources.
...
Alchemy keeps provider packages explicit so projects install the provider
surfaces they import. A stack that uses more than one provider installs
the package groups for each provider surface it imports.
...
<table>
  ...
  <tbody>
    {dependencyRows.map((row) => (
      <tr>
        <td><code>{row.importSurface}</code></td>
        <td>{row.packages}</td>
      </tr>
    ))}
  </tbody>
...
The install command on this page combines the Cloudflare package group
with Alchemy core packages. An AWS-only stack would install the AWS
package group beside the core packages instead.
