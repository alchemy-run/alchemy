...
- [Add an AI Gateway](/tutorial/cloudflare/ai-gateway) — wire
  ...
  streaming.
- [Persist conversation state in a Durable Object](/tutorial/cloudflare/durable-object-chat-persistence)
  — back Effect AI's `Chat.Persistence` with `state.storage` so
  conversation history survives restarts and hibernation.
