...
| Branch on the current stage (e.g. PR previews → staging) | a conditional, see [Shared database across stages](/cloudflare/guides/shared-database) |
...
- [References](/concepts/references) — the underlying `Output.stackRef` and `Output.ref` operators.
