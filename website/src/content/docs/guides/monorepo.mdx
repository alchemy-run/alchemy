...
1. **Single-stack** — one `alchemy.run.ts` at the workspace root
   ...
   projects.
2. **Split-stack** — each package owns its own `alchemy.run.ts`
...
| Backend and frontend deploy on different cadences (different teams, CI)    | Split-stack |
| Backend has consumers besides the frontend, with their own deploy lifecycle | Split-stack |
| You want to be able to `destroy` the frontend without touching the backend | Split-stack |
...
- [`examples/monorepo-split-stack`](https://github.com/alchemy-run/alchemy-effect/tree/main/examples/monorepo-split-stack)
...
— that's the channel the runtime client (and, in the split-stack
...
| Concern                           | Single-stack                | Split-stack                              |
| --------------------------------- | --------------------------- | ---------------------------------------- |
