...
Resources are plain TypeScript values, so file layout is just
module design. Three rules cover it: one file per Resource or
Layer, group Resources that travel together, and keep
`alchemy.run.ts` as the composition root. This page walks a
project from one file to that shape.
...
`src/Sandbox.ts` (see
[Building with Layers](/guides/infrastructure-layers)).
...
- [Monorepo](/guides/monorepo) — the workspace-level version of
  this question: one Stack or one per package.
- [Stack References](/guides/stack-references) — when the split
  crosses Stack boundaries.
- [Building with Layers](/guides/infrastructure-layers) — promote
  a multi-Resource unit into a swappable Layer.
- [Stacks](/concepts/stack) — the Stack service, stages, and
