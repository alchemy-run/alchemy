...
import { awsPkgs, cloudflarePkgs } from "../../../versions";
...
The AWS command group loads its provider module when an `aws` subcommand
runs. The package group includes the AWS client surface, Smithy credential
types, and Lambda handler type declarations used across AWS resources:

<Code code={`bun add ${awsPkgs}`} lang="sh" />
...
The Cloudflare command group loads its provider module when a `cloudflare`
subcommand runs. The package group includes the Cloudflare API client,
shared Distilled error types, Worker binding types, and Worker packaging
path used by the state-store bootstrap flow:

<Code code={`bun add ${cloudflarePkgs}`} lang="sh" />
