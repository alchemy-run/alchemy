...
Mint a Cloudflare API token (`POST /user/tokens`) without leaving the
terminal. Useful for creating the `CLOUDFLARE_API_TOKEN` you hand to CI,
or a broad local token for `alchemy deploy`.
