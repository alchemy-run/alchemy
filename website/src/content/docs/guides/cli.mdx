...
targets a [**stage**](/concepts/stages) — an isolated environment
like `dev_sam`, `prod`, or `pr-42`.
...
See [Profiles](/concepts/profiles).

## `profile`

Inspect credentials stored in `~/.alchemy/profiles.json`. See
...
See [Stages](/concepts/stages).

## Where next

- [Stages](/concepts/stages) — how environments are isolated
- [Profiles](/concepts/profiles) — auth profiles and credentials
- [State Store](/concepts/state-store) — where resource state lives
- [CI](/guides/ci) — run these commands in CI
