...
The AWS command group loads AWS provider modules when an `aws`
subcommand runs. Projects that use these commands install the AWS
provider package alongside `alchemy`:

```sh
bun add @distilled.cloud/aws
```
...
The Cloudflare command group loads Cloudflare provider modules when a
`cloudflare` subcommand runs. Projects that use these commands install
the Cloudflare provider packages alongside `alchemy`:
