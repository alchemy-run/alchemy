...
In this guide, which builds on [Bindings](/concepts/binding), you'll
build two Workers that call each other, one step at a time.
...
([Functions & Servers](/concepts/platform)), not just Workers.
...
in [Inputs & Outputs](/concepts/outputs).

## Where next

- [Inputs & Outputs](/concepts/outputs) — the deferred references that make the cycle resolvable.
- [Layers](/concepts/layers) — package resources behind service interfaces.
- [Providers](/concepts/provider) — the `precreate` hook that reserves resources up front.
