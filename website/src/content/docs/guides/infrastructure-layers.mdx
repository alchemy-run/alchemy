...
`Cloudflare.KV.KVNamespace("Jobs")` describes a real resource. Yielding
...
+import * as Cloudflare from "alchemy/Cloudflare";
 ...
 export const JobServiceKV = Layer.effect(
   JobService,
   Effect.gen(function* () {
+    const Namespace = yield* Cloudflare.KV.KVNamespace("Jobs");
+
     return {
       // ...
     };
...
shares the same KV namespace, because `Cloudflare.KV.KVNamespace("Jobs")`
...
export const JobServiceR2 = Layer.effect(
  JobService,
  Effect.gen(function* () {
    const Bucket = yield* Cloudflare.R2.R2Bucket("Jobs");
