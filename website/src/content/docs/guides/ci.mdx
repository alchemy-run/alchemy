...
2. **Alchemy deploys the stack to that stage.** An isolated copy of
   ...
   [Stage state lives separately](/concepts/stages), so PRs can't
...
[profile](/concepts/profiles), create a separate `admin` profile:
...
- [Stages](/concepts/stages) — how `pr-42` and `prod` stay isolated.
- [Profiles](/concepts/profiles) — one credential bundle per environment.
- [Secrets & env on Cloudflare](/cloudflare/secrets-env) — wire app secrets into a Worker.
