---
title: Cross-stage references
description: Reference a deployed resource from a different stage of the same stack — point ephemeral PR-preview environments at long-lived shared infrastructure.
sidebar:
  order: 4
---

[Stages](/concepts/stages) make it cheap to spin up isolated copies
of a stack — per-developer, `pr-42`, `prod`. Most resources should be
isolated. But some — a Neon Postgres project, a shared S3 bucket,
a global rate limiter — are too expensive or too stateful to
re-provision per stage. PR-preview stages should *point at* the
shared instance instead.

`Resource.ref(id, { stage })` reads a deployed resource's
attributes from another stage of the same stack. The reference is
typed, lazy, and resolved at plan time against the persisted state
store — same machinery as
[`Output.ref`](/concepts/outputs#ref) and
[cross-stack references](/guides/cross-stack-references), just
...
whole stack's outputs (instead of a single resource) see
[Cross-stack references](/guides/cross-stack-references).
...
| Reference an entire stack's outputs         | `Backend.stage[name]` ([guide](/guides/cross-stack-references)) |
...
- [Stages](/concepts/stages) — naming conventions for `pr-*`,
  `dev_*`, `staging`, `prod`.
- [Cross-stack references](/guides/cross-stack-references) —
  whole-stack references via typed `Stack` handles.
