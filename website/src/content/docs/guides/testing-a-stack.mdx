...
Bun vs Vitest is a two-line import swap — see [Bun vs Vitest](/concepts/test-harness#bun-vs-vitest).
...
`beforeAll` accessors compose — seed data in a second `beforeAll` that tests `yield*` alongside the stack; see [Hooks](/concepts/test-harness#hooks).
...
The first run deploys; re-runs diff and skip unchanged Resources, and tests default to the isolated `test` [stage](/concepts/stages) so they never clobber your dev deployment.
...
How the client is wired is covered in [Test harness](/concepts/test-harness#httpclient-is-in-scope).
...
Locally, the deployed Stack plus cached state make re-runs near-instant. Hook semantics live in [Hooks](/concepts/test-harness#hooks).
...
See [State store](/concepts/state-store) for choosing a remote store.
...
Harness `deploy`/`destroy` never prompt for approval — `--yes` is only for CI workflows invoking the CLI (`alchemy deploy --stage pr-42 --yes`); see [Stage](/concepts/test-harness#stage) and the [CI guide](/guides/ci) for full workflow YAML.

## Where next

- [Testing](/concepts/testing) — the model behind the harness.
- [Test harness](/concepts/test-harness) — every helper, hook, and option on `Test.make`.
- [Testing Providers](/guides/testing-providers) — lifecycle-testing a custom provider with `test.provider`.
- [CI](/guides/ci) — running these suites in CI with per-PR stages.
...
- [State store](/concepts/state-store) — local vs remote state.
- [Stages](/concepts/stages) — isolating deployments per stage.
