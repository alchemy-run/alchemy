...
`Foldkit` always answers misses with `index.html` (200) so the
Foldkit runtime can resolve the route. This resource does not take
`spa` or `errorPage` — use
[`Hetzner.Website.Vite`](/hetzner/frontend/vite) when you need a
real 404 page.
