...
`Hetzner.Website.Vinext` runs `vinext build` for production and native
`vinext dev` locally. The built app runs on a Hetzner Cloud Server using vinext's production Node
server as a systemd service. Alchemy creates a server when `server` is omitted
and uploads `dist/` with the runtime dependency installation.
...
The same configuration selects the appropriate cache adapter for each target.
Leave Cloudflare plugins and Worker entrypoints out of this application.
...
The default data cache is process-local. To persist ISR and `"use cache"`
across restarts, supply an existing Redis endpoint through `env.REDIS_URL`.
The Website does not install or provision Redis. Protect credentials with
`Redacted.make` and keep the Redis service private.
...
Hetzner requires an existing DNS `zone` when setting `domain`.
See [Website domains](/hetzner/frontend/websites) for platform-specific DNS and
certificate configuration.

## Where next

- [vinext API reference](/providers/hetzner/website/vinext).
- [Runnable example](https://github.com/alchemy-run/alchemy/tree/main/examples/hetzner-website-vinext).
- [Website overview](/hetzner/frontend/websites).
