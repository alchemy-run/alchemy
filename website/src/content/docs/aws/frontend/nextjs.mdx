...
Your `next.config.*` is loaded and honored as-is — the build runs
Next's own programmatic build (no shell command, no `build` script
required), and Alchemy never rewrites the file. OpenNext is
configured through the `openNext` prop, deep-merged over Alchemy's
streaming default:
...
The merged config is written to a synthesized `open-next.config.ts`
(marked as generated and regenerated whenever the props change). If
you author your own `open-next.config.ts` — for example to use
function-valued overrides — it takes precedence and is honored
as-is:
...
The server function's environment is configured under
