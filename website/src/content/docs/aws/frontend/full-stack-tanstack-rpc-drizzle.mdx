...
Copy the example's [`schema.ts`](https://github.com/alchemy-run/alchemy/blob/main/examples/aws-dsql-drizzle/src/schema.ts) into `src/backend/`.

## 4. The backend RPC Lambda

`connectDatabase` uses the non-admin `app_user` role with verified TLS.
