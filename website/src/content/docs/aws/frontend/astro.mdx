...
error), and Astro options set on the resource (`site`, `base`,
`output`, ...) override the file's — see
...
`srcDir`, `publicDir`, `outDir`, `trailingSlash`) as flat props for
deploy-specific overrides; Astro merges them *over* the config
