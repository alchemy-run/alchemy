...
## Override the output directory and base path

The `vite` prop carries serializable overrides merged *over* your
config file. `outDir` is the build output directory (relative to
`rootDir`) and `base` is the public path the site deploys under:
...
Leave them unset and the resolved `build.outDir` from your
`vite.config.*` is used — Vite's own default is `dist`.
...
The resource takes no `environment` prop, so a value that only exists
...
The ACM certificate and Route 53 records are managed for you. A bare
...
Effect.gen(function* () {
  const router = yield* AWS.Website.Router("Router", {
    domain: { name: "example.com", hostedZoneId: "Z0123456789ABCDEFGHIJ" },
  });
...
[SolidStart](/aws/frontend/solidstart), and
