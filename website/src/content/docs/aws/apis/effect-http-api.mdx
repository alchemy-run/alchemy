...
Effect HTTP defines REST endpoints, with URLs, path params, query
strings, and typed bodies, behind a typed interface. Use it when your
consumers aren't Effect, or aren't TypeScript, and want a plain HTTP
client.

This page is the Lambda wiring. See
[Effect HTTP](/apis/effect-http) for the concept and [RPC](/apis) for
choosing between the RPC styles.

The [Lambda](/aws/compute/lambda) page serves HTTP with a hand-rolled
`fetch` handler that parses requests and routes by hand. As your API
grows, you lose type safety at the boundary. Payloads aren't
validated, response shapes aren't enforced, and errors slip through
untyped.

Effect's `HttpApi` module declares endpoints with schemas for
payloads, responses, and errors, and you implement handlers against
those schemas. The result is an `HttpEffect`, the type a Lambda's
`fetch` expects, so it plugs in directly.

The [Workers version of this guide](/cloudflare/apis/effect-http-api)
uses the same module. The spec and handlers are portable, and only the
host changes.
...
2. **Bind resources and construct handlers inside the Function's
   ...
   start, so do only pure construction here, such as bindings and
   layers, and never per-request work.
...
Nothing executes yet, because `JobApi` is a value-level description.
The client at the end of this guide uses the same constant.
...
Each binding does two jobs. At deploy time it attaches a
least-privilege IAM statement to the function's execution role, so
`GetItem` grants only `dynamodb:GetItem` on this table's ARN. At
...
into the API spec. It doesn't run the handlers, so it's safe to
...
satisfy `Job` or fail with `JobNotFound`, so mismatches fail at
compile time.

`Effect.orDie` converts DynamoDB's infrastructure errors
into defects, which the HttpApi runtime turns into 500s. The typed
error channel stays reserved for `JobNotFound`.

### 3d. Return `{ fetch }`

The return value of the constructor is the Function's runtime surface.
For an HTTP Lambda, that's an object with a `fetch` field.
...
`functionUrl: true` creates a public URL (`authType: "NONE"`), so the
schema validation is your only input boundary. Add auth for anything
that isn't public, and allow browser origins on the URL itself:
...
+    functionUrl: {
+      authType: "AWS_IAM", // require SigV4-signed callers
+      cors: { AllowOrigins: ["https://app.example.com"] },
+    },
+  },
```

To handle CORS in the app instead, on any host, provide
`HttpRouter.cors({ allowedOrigins: [...] })` as a layer. See
[Effect HTTP on Workers › Enable CORS](/cloudflare/apis/effect-http-api#3d-enable-cors).
...
validation errors. A missing job comes back as a 404, the
`httpApiStatus` declared on `JobNotFound`.
