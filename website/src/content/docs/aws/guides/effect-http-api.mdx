...
The [Lambda](/aws/lambda) page serves HTTP with a hand-rolled
...
[Workers version of this guide](/cloudflare/guides/effect-http-api);
...
- [Lambda](/aws/lambda) — how the Function class, Function URLs,
  and the test harness work.
- [Effect HTTP API on Workers](/cloudflare/guides/effect-http-api) —
  the same `HttpApi` spec deployed to Cloudflare.
- [DynamoDB](/aws/dynamodb) — the table resource and the rest of
  its bindings.
- [REST API (API Gateway v1)](/aws/guides/api-gateway) — put API
