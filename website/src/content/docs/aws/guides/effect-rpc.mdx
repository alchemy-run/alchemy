...
The [HTTP API guide](/aws/guides/effect-http-api) showed how to
...
construction or manual serialization. It's the same `Rpc` module
used in the
[Workers version of this guide](/cloudflare/guides/effect-rpc); the
group and handlers are portable, only the host changes.
...
Both approaches produce the same `HttpEffect` and wire into a Lambda
...
You can even combine them in the same Function — serve an HTTP API
...
[Infrastructure from Layers](/guides/infrastructure-layers) guide
explains the service-Layer pattern it uses.

## Where next

- [Lambda](/aws/lambda) — how the Function class, Function URLs, and
  the test harness work.
- [Effect HTTP API on Lambda](/aws/guides/effect-http-api) — the
  REST-style sibling of this guide, same wiring.
- [Effect RPC on Workers](/cloudflare/guides/effect-rpc) — the same
  `RpcGroup` deployed to Cloudflare, plus streaming and Durable
  Object routing.
- [DynamoDB](/aws/dynamodb) — the table resource and the rest of its
  bindings.
