...
function's [Output](/concepts/outputs) instead:
...
commonly an [SNS](/aws/sns) topic that fans out to email, Slack, or
PagerDuty:
...
work against a stock [Lambda](/aws/lambda) function with zero
...
[Observability](/concepts/observability): Effect emits OpenTelemetry
...
- [Observability](/concepts/observability) — OTel exporters as
  Layers, plus the cross-cloud version of alerts-in-code.
- [Lambda](/aws/lambda) — the functions these dashboards and alarms
  watch.
