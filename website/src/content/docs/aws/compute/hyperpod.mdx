...
prop. Reference the instance group through the cluster's
attributes — the workload is connected to the fleet through the
resource graph, and the node selector derives from it:
