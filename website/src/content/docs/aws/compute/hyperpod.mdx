...
Kubernetes workloads opt onto HyperPod nodes by spreading
`AWS.EKS.hyperpod(...)` into
...
| High-level DX   | —                                     | `Kubernetes.Job` / `Deployment` with `AWS.EKS.hyperpod(...)` |
...
[`Kubernetes.Deployment`](/providers/kubernetes/deployment) and
[`Kubernetes.Job`](/providers/kubernetes/job) opt onto them by
spreading `AWS.EKS.hyperpod(...)` into their props. The
instance-group keys carry through to the cluster's
...
const train = yield* Kubernetes.Job(
  "Train",
  {
    ...
    ...AWS.EKS.hyperpod({ instanceGroup: hyperpod.instanceGroups.workers }),
  },
  Effect.gen(function* () {
    return {
