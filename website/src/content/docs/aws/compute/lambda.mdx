...
There is deliberately no `waitUntil` on Lambda — no mechanism
delivers it reliably. A buffered invocation's response is not
released to the caller until the entire Invoke phase completes, so
deferring cleanup past the handler (we measured an extension-held
post-response window) still shows up as response latency, and
environments whose invoke phase is held open this way get recycled.
...
work entirely — silent data loss for anything that mattered. So the
contract is blocking and honest: request finalizers run before the
response, keep them cheap, and anything that must not be lost gets
written durably (a queue, a table) inside the handler itself.
