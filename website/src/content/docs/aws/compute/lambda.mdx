...
invocation. Each invocation then runs with a **fresh `Scope`**,
released after the response is returned.

Post-response cleanup works because the generated entry registers an
**internal extension** with the Extensions API. Lambda returns the
function response to the client as soon as the handler produces it,
but keeps the environment running until every registered extension
signals done — alchemy holds that signal until the invocation's
queued finalizers settle. The effect is workerd's `ctx.waitUntil`:
`Effect.addFinalizer` in a handler runs *after* the response,
without blocking it. The window extends the billed invocation and is
capped by the function timeout (queued work is raced against the
invocation deadline), and `Drizzle.postgres` uses it to close its
per-invocation pool off the response path.

The same extension registration buys the **Shutdown phase**: a
sandbox with no registered extensions is killed with no signal at
all; with one, Lambda sends `SIGTERM` and allows 500 ms before
`SIGKILL`, and the entry uses the window to close the instance
scope:
