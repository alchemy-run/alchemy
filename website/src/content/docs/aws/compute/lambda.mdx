...
is settled *inline* before the handler returns: Lambda freezes the
sandbox after the response, so unlike Workers there is no
`waitUntil` to detach cleanup to — `Effect.addFinalizer` in a
handler runs before the response leaves.
...
Treat it as best-effort: half a second, and not delivered on hard
failures (a timeout reset kills the runtime without the signal).
Flush caches and close connections there; anything that *must*
happen belongs in the handler, scoped to the invocation —
`Drizzle.postgres` opens its pool on the first query of an
invocation and closes it before the response returns. See
