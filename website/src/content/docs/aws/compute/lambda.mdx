...
That inline settle is a platform constraint, not a choice: a
buffered invocation's response is not released to the caller until
the entire Invoke phase completes, so any scheme that defers cleanup
past the handler (we measured an extension-held post-response
window) still shows up as response latency — and environments whose
invoke phase is held open this way get recycled. Response streaming
(`url: { invokeMode: "RESPONSE_STREAM" }`) is the genuine escape
hatch for latency-sensitive trailing work: bytes flush to the caller
as they're written.
