...
ordinary Kubernetes nodes. `Deployment` and `Job` opt onto those
nodes with the `hyperpod` prop, and passing a team's
`ComputeQuota` submits the workload through HyperPod task
governance:
...
const train = yield* AWS.EKS.Job("Train", {
  ...
  main: import.meta.url,
  hyperpod: {
    instanceGroup: workers, // the group object from the HyperPod cluster
...
See [HyperPod](/aws/compute/hyperpod) for provisioning the fleet
and the full governance surface.
