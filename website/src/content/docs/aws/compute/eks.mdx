...
For Secrets, prefer [`Kubernetes.Secret`](/providers/kubernetes/secret), which
keeps values `Redacted` in plans and state. The manifest is a literal object,
exactly as you would write it in YAML:
