...
SQS queue, and DynamoDB table publishes metrics into it automatically.

In Alchemy, a `Dashboard` is a structured widget document and an
`Alarm` is a threshold on a metric, both declared in the same stack as
the resources they watch. A changed threshold or a new graph is a diff
in the pull request.
...
`metrics`, and so on). Four widget types are supported: `metric` (graphs),
...
`CompositeAlarm` combines their states with a boolean rule. Alchemy
generates alarm names from the app, stage, and logical ID unless you
set `name`, so reference them through the `alarmName` output:

```typescript
import * as Output from "alchemy/Output";

// latency is a second AWS.CloudWatch.Alarm
const composite = yield* AWS.CloudWatch.CompositeAlarm("HighSeverity", {
  AlarmRule: Output.interpolate`ALARM("${errors.alarmName}") OR ALARM("${latency.alarmName}")`,
});
```

Each stage gets its own alarms, and renaming one replaces it.

## Where metrics come from

You rarely have to publish anything, because AWS services emit
metrics on their own. Every Lambda function reports `Invocations`,
`Errors`, `Duration`, and `Throttles` under the `AWS/Lambda` namespace,
keyed by the `FunctionName` dimension. That's why the snippets above
work against a stock [Lambda](/aws/compute/lambda) function with no
instrumentation.

Other services follow suit, such as `AWS/SQS` and `AWS/DynamoDB`.

For application-level signals, such as spans, structured logs, and
custom metrics from your Effect code, see
...
provisioned as resources in the same stack, like the alarms here.
