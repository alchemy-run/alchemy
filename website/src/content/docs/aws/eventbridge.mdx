...
`AWS.ECS.Task` (see [ECS](/aws/ecs) for standing those up). Under the
...
- [Lambda](/aws/lambda) — the runtime both the consumer and scheduled
  targets above deploy to.
- [SNS](/aws/sns) — pub/sub fan-out when you want subscriptions
  rather than pattern-matching rules.
- [ECS](/aws/ecs) — the cluster, task, and networking that
