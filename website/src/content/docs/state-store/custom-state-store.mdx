...
`Cloudflare.state()` on Cloudflare) cover most cases. If you
...
If you only need a Postgres store, use the built-in one from `alchemy/State/PostgresState`. It adds advisory locking. This guide builds a simplified version to teach the interface.
...
Alchemy accepts this layer, but it dies on every operation.
...
Initialization is deferred, so capture the scope at layer-build time
and provide it to the cached initializer. The connection opens lazily
on first use, and its release is still tied to the stack's lifetime:
...
inspection commands. They're plain `select distinct`
queries:
...
instance) and verify a resource round-trips through your store. The
test uses a double `yield*`. The first resolves the `State` service to
its `Effect<StateService>` initializer. The second runs it to get the
`StateService`.
