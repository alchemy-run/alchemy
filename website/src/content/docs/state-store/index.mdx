...
CI, set `CI=true`. Alchemy then resolves the credentials from the
...
Pass `workerName` to use a separate state store. For example, use
this when you want a dedicated store per Cloudflare account or per
team:
...
`postgresState()` persists state in a Postgres database you already run. It also locks state during a run. Each `(stack, stage)` holds a session advisory lock for the duration of a run, so two concurrent deploys of the same stage cannot interleave.
...
Import it from `alchemy/State/PostgresState`. It is not in the `State` barrel because it depends on the optional `pg` driver, which must stay out of bundled worker code. Install `pg` and `@effect/sql-pg` to use it.
...
A state store is an Effect `Layer` that provides the `State`
service. You can back it with Postgres, S3, Redis, DynamoDB, or any
other backend. See [Custom State Store](/state-store/custom-state-store)
