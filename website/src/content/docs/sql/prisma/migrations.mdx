...
`contract.d.ts`. Other environments consume the checked-in migration artifacts;
PSL-first applications also use the emitted contract in their generated client.
TypeScript-first applications can import the authored contract directly. The
deploy only plans a new package when the contract actually changed.
...
The resources shell the same `prisma` CLI you would run by hand,
against the same `prisma.config.ts`. Planning locally and letting the
deploy only apply is fully supported:

```sh
prisma contract emit
prisma migration plan --name add-posts
git add migrations src/prisma && git commit
```

The next deploy detects no drift (the committed package already covers
the contract) and `Prisma.Migrate` applies it. `prisma db migrate
