...
for dependencies and a complete deployment example.
:::
...
The [Postgres guide](/sql/prisma/postgres) shows Cloudflare Hyperdrive.
...
from tree-shaking. Follow [Configure validation for Workers](/sql/prisma/postgres#configure-validation-for-workers).
...
- [Postgres](/sql/prisma/postgres) — provision a database and query it from a Worker.
