...
`Cloudflare.MySQLBinding` switches the client to the text protocol
(Hyperdrive's MySQL proxy has no `COM_STMT_PREPARE`) and eval-free
row parsers (the isolate forbids runtime code generation) — see
