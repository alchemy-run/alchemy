...
The inner Effect is the instance init: it runs only in the deployed
object, when the instance activates and before any request reaches
its methods — the right place for runtime storage work like
migrations. Both the driver and the migrator are synchronous.
