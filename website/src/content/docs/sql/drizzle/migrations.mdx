...
Define the schema — tables plus `defineRelations` for typed
relational queries:
...
imports directly into your Durable Object. Commit it and hand it to
`Drizzle.DurableObject`, along with the `relations`:
...
`Drizzle.DurableObject` opens `drizzle-orm/durable-sqlite` over the
instance's own SQLite storage and applies the migrations before
returning the db. Both the driver and the migrator are synchronous,
so it lives in the object's inner Effect — the instance init, which
runs only in the deployed object, before any request reaches its
methods. The config passes the driver's options through — the
`relations` make `db.query.users.findMany({ with: { posts: true } })`
fully typed.
