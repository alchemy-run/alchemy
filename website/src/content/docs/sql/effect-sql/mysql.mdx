...
mysql2 has two habits that break on Cloudflare Workers, and the
Worker runtime provides a `MySQLDefaults` layer that turns both off:
...
Other runtimes — a Lambda or container connecting directly — provide
no defaults, so you keep prepared statements and the fast parsers. A
custom platform (or another text-protocol proxy) can provide its own:

```typescript
import { MySQLDefaults } from "alchemy/SQL/MySQLDefaults";
import * as Layer from "effect/Layer";

Layer.succeed(MySQLDefaults, { disablePreparedStatements: true });
```

And either flag can be forced per client:
