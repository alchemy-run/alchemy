...
These defaults avoid Hyperdrive's unsupported prepared-statement protocol and Workers' prohibition on eval-based row parsers. Outside workerd they stay off; [Cloudflare setup](/sql/providers/cloudflare) owns Worker deployment.
...
Choose a MySQL-compatible database in [SQL databases](/sql/databases); [AWS](/sql/providers/aws), [Cloudflare](/sql/providers/cloudflare), [Hetzner](/sql/providers/hetzner), [PlanetScale](/sql/providers/planetscale), and [Railway](/sql/providers/railway) cover provider setup. Those pages own provisioning and deployment; the client above only needs a connection URL.
