...
`Cloudflare.MySQLBinding` (provided in the Worker example above)
turns both off:
...
no binding, so you keep prepared statements and the fast parsers. A
custom platform (or another text-protocol proxy) provides its own
defaults with the same `MySQLDefaults` reference the Cloudflare
binding uses:
