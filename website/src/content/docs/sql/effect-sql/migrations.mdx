...
PlanetScale databases and branches. (`migrationsDir` and
`migrationsTable` still work as deprecated aliases.)
...
change, not a cosmetic one. `migrations: { dir, table }` points the
bookkeeping at a different table name when you need one.
