...
runtime binding. Client options pass through to `@effect/sql-pg`.
...
[`SQL.D1Layer`](/sql/effect-sql/d1#provide-as-a-service) reuses the
service interface, but SQL syntax and database-specific behavior must also be
compatible with the destination dialect.
