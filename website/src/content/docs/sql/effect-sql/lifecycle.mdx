...
For each constructed client, queries in the same scope reuse one underlying client; distinct client constructions do not share pools. The first query builds it and scope closure runs its finalizers, so merely constructing a client during plan or deploy does not connect.
...
Choose a database in [SQL databases](/sql/databases), then follow [AWS](/sql/providers/aws), [Cloudflare](/sql/providers/cloudflare), [Fly](/sql/providers/fly), [Hetzner](/sql/providers/hetzner), [Neon](/sql/providers/neon), [PlanetScale](/sql/providers/planetscale), [Prisma](/sql/providers/prisma), or [Railway](/sql/providers/railway) for connection and deployment setup. These choices do not change the client's scope-based lifecycle.
