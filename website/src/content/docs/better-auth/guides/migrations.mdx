...
```typescript
// auth-options.ts — alternative configuration for externally managed schema
...
This is a complete alternative options module, not an additional auth instance. To use it in the Worker above, import its `authOptions` instead of declaring that object in `worker.ts`.
