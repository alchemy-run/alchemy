...
Authentication identifies the caller; authorization still needs server-side ownership or permission checks. If cookie caching is enabled, sensitive checks should bypass it as described in [sessions](/better-auth/guides/sessions).
