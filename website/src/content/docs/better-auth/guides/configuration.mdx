---
title: Configuration
description: Configure Better Auth URLs, database layers, and deployment environments.
---
...
`satisfies BetterAuthProps` checks the options without discarding plugin inference. The wrapper owns `database` and `secondaryStorage`; supply those through layers rather than this object.
...
The client calls `/api/auth` on the page's origin; your frontend must be served from that origin or use an intentionally configured cross-origin arrangement. Never import `auth.ts` into a browser bundle.

The resolved auth origin is already trusted. Add exact additional origins to `trustedOrigins` only for intentional frontends; this allowlist does not configure CORS or cookie sharing, and disabling CSRF checks is not a URL fix.
