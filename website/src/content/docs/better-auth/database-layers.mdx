...
resolved only when a layer provides it. Better Auth 1.7 requires atomic
`getAndDelete` and `increment` alongside `get`/`set`/`delete`. There is
deliberately no Cloudflare KV implementation: Better Auth's guidance is a
Redis-style store with strongly consistent reads and precise sub-minute
TTLs, and KV's eventual consistency (plus its 60-second TTL floor) fights
session reads and rate-limit counters. A Durable Object-backed layer is
the right strongly-consistent Cloudflare option when one lands.
