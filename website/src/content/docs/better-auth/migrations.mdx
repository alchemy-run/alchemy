...
(`CREATE TABLE` / `ADD COLUMN` / `CREATE INDEX` on what's missing), so
re-running a greenfield or already-migrated database is always safe.

Populated databases missing `account.issuer` are upgraded in place from
provider IDs and stored Microsoft `id_token` claims. Deploy fails when
`(issuer, accountId)` collides, a Microsoft row has no `oid`, or leftover
`scimProvider` / `oauthApplication` tables still have rows — those last
two need the [upgrade guide](https://www.better-auth.com/docs/guides/1-7-upgrade-guide).
...
`npx auth@latest generate` and apply it through your own migration
