...
Better Auth 1.7 will **refuse** to add a required column with no default
to a table that already has rows (for example `account.issuer`). The
Action fails with Better Auth's message, which points at the
[1.7 upgrade guide](https://www.better-auth.com/docs/guides/1-7-upgrade-guide)
backfill. Complete that data step, then re-deploy.
