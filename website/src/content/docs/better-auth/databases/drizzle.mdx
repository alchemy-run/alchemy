---
title: Drizzle
description: Generate a Relations v2 auth schema and deploy a Better Auth Lambda with a request-scoped raw Drizzle database.
---
...
## Share the auth options

```typescript title="src/auth-options.ts"
...
Both the CLI and runtime import this complete options file so plugins, additional fields, and model names stay aligned. Keep Alchemy-only `id` and `migrate` options out of the upstream CLI configuration.
...
export default class AuthApi extends AWS.Lambda.Function<AuthApi>()(
  ...
  Effect.gen(function* () {
    const auth = yield* BetterAuth({ ...authOptions, migrate: false });
