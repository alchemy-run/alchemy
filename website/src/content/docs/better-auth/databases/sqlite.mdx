---
title: SQLite
description: Run Better Auth in a local Bun server with a persistent SQLite file.
---
...
```sh
bun add alchemy "effect@4.0.0-rc.115" "@effect/platform-bun@4.0.0-rc.115" @alchemy.run/better-auth better-auth@^1.7.5 kysely
```
...
The standalone Bun process has no Alchemy migration Action, so it applies the schema once at startup before listening. `SQLite` supplies migration support, and the scoped migration connection closes before requests begin.
