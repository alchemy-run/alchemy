...
Every App in the organization reaches the Service at `privateUrl`,
including other Services in their own Apps. Pass `users.privateUrl`
into a caller's `env` — see
[Call one Service from another](/fly/compute/services#call-one-service-from-another).
Prefer it over `{appName}.internal`: `.internal` resolves straight to
Machines and bypasses the proxy, so it ignores service checks,
...
stage. Services on it reach each other at `privateUrl`; Apps on any
other network, including the default one, get `bad address` for the
`.flycast` and `.internal` names. A public Service on the network
...
Other Apps in the organization call it at `http://{appName}.flycast`.
A Service in a shared App has no `privateUrl`; build the hostname
from the App's `appName`.
...
[Connect Services](/fly/compute/connecting-services) connects private
Services on a stack network.
