...
`http://{appName}.flycast`, which is also what the Service's `url`
reports once the Flycast address exists. Allocating the address in an
earlier deploy, or redeploying after adding it, updates `url`. Prefer
this over `{appName}.internal`:
