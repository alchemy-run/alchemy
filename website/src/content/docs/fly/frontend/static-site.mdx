...
[SvelteKit](/fly/frontend/sveltekit), [Vite](/fly/frontend/vite),
[Vocs](/fly/frontend/vocs), and [Waku](/fly/frontend/waku). Those
resources run the framework's own programmatic build and skip the
`command` / `outdir` contract. Reach for `StaticSite` when there is
no dedicated resource — Zola, Hugo, or any other generator.
