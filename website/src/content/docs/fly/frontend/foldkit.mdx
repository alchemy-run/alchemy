...
framework built on Effect. A client-only Foldkit app is a plain
`vite build`, so `Fly.Website.Foldkit` is
[`Fly.Website.Vite`](/fly/frontend/vite) with SPA fallback to
`index.html`. Deep links boot the app and the Foldkit router takes
over.
...
`ssr.build` is how Foldkit emits a server bundle and prerendered
pages from that same `vite build`. This resource does not run that
bundle: it is assets-only. Server-rendered and hybrid Foldkit
deployments are the
[Cloudflare Foldkit](/cloudflare/frontend/foldkit) page.
:::
...
`alchemy dev` runs Vite's own dev server (Foldkit's live reload
and devtools) instead of deploying; `site.url` is the local address and
