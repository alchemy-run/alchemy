...
token. Org API tokens are typed `Forbidden`. A full round-trip is
behind `FLY_TEST_KMS=1`.

## Where next

The [tutorial](/fly/tutorial/part-4) stores an App secret and reads
it from a Service. [Services](/fly/compute/services) covers
`Config.redacted`. [Postgres](/fly/data/postgres),
[Redis](/fly/data/redis), and [Tigris](/fly/data/tigris) Attach
bindings write Fly-owned secrets (`DATABASE_URL`, `REDIS_URL`,
`AWS_*`). See the
