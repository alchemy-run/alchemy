...
In [Part 1](/fly/tutorial/part-1) you deployed an App. Now you'll put
code on it: a `Fly.Service` — an Effect program that Alchemy
bundles, bakes into a Docker image, pushes to `registry.fly.io`, and
runs as a Machine. No Dockerfile to write, no `fly.toml`.
...
Fly Machines live in a region. The default is `iad` (Ashburn) —
set it explicitly so the Volume you add in Part 3 can match:
...
Changing `region` later **replaces** the Service (a new Machine in
the new region).
