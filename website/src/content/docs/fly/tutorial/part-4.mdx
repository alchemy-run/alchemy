...
`ReadSecret` is a **binding**: at deploy time it can mint an App
deploy token for the Machine, and at runtime it hands you a client
whose `get()` defaults to this Secret.

## Provide the binding layer

Bindings declare a capability; layers implement it. Provide
`ReadSecretHttp` next to `MountVolumeLive`:
...
- [Secrets](/fly/data/secrets) — `ReadSecret` / `WriteSecret`, plus
