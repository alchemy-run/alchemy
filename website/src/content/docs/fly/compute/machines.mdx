...
Use `containers` for a group that shares one Machine. Each named container has its own image and can set `cmd`, `entrypoint`, `exec`, `env`, startup `dependsOn`, and Pilot `healthChecks`. Check intervals and timeouts are numeric seconds. A dependency condition can be `started`, `healthy`, or `exited_successfully`.
