...
A raw image owns signal handling, admission control, and request/job drain;
these props do not inject an Effect bootstrap into it. Genuine unreachable-host
retirement can force-delete owned stateless predecessors after replacement
readiness, but cannot prove their work drained. See the
[deployment and recovery limits](/fly/compute/deployments).

Lifecycle mutations use native target Machine leases, not an App-wide lock.
Lease contention is a bounded failure, not permission to force-steal a lease.
Simultaneous first deployments and different snapshots are not globally
excluded; serialize CI for the same resource. LocalState is not a cross-process
lock, and App deletion is name-addressed without a Machine-lease precondition.

`autoDestroy: true` tears a Machine down when its process exits. Blue/green
rejects auto-destroy/one-shot configuration, explicit `skipLaunch`, attached
volumes (including bindings), and missing readiness checks. It does not
silently fall back to rolling. Stop/suspend autostop instead uses the documented
idle-capacity rules. Live acceptance remains distinct from a full flyctl-parity
claim.
