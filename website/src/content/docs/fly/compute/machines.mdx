...
Service builds the image for you.
...
defaults to shared-cpu 1× / 256 MB. `count` is how many Machines
to keep (default `1`). Fly's proxy load-balances published
`services` across them. Omit `services` (or pass `[]`) for a
worker that should not be reachable from the internet.
