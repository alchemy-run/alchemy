...
defaults to shared-cpu 1× / 256 MB. `services` is the Fly proxy:
published ports and handlers (`http`, `tls`, …). Omit `services`
(or pass `[]`) for a worker that should not be reachable from the
internet.
