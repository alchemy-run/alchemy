...
it or waiting for checks. By default, delete force-destroys and waits until
gone. Setting `shutdown` or enabling blue/green instead cordons, signals,
waits for a stopped Machine, and then deletes it. `Conflict` on create/update/delete is treated as a race and
