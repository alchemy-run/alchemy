...
Unchanged code is a no-op. Opt into `deploy: { strategy: "bluegreen" }` to
[prepare healthy replacements and drain the old generation](/fly/compute/deployments).
The same guide covers readiness, shutdown deadlines, and retrying interrupted deployments.
