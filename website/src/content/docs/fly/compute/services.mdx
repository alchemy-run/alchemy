...
Keep one Service declaration: Alchemy manages both generations. Health checks,
proxy traffic, and application shutdown are separate responsibilities.
The guide covers old-generation shutdown budgets, ordinary Effect finalizers,
native Machine lease limits, and recovery; it does not claim full live-tested
flyctl parity.
...
See [secret version limits](/fly/compute/deployments#treat-secret-versions-as-a-floor).
...
See the [worker resource-lifetime pattern](/fly/compute/deployments#own-the-job-drain-with-ordinary-scopes).
