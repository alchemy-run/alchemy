...
Fly's private network spans the organization, so any App in the org
reaches `Users` at `privateUrl`. Turning `public` on or off updates
the Service in place; its App and hostname stay the same.
