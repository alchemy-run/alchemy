...
App. `url` is `https://{appName}.fly.dev` whenever a port is
published, `privateUrl` is `undefined`, and `public` does not apply
(combining `app` and `public` fails with `Fly.InvalidServiceProps`).
