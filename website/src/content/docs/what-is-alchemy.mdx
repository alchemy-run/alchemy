...
This file is the whole application. It declares an R2 Bucket and a
Worker that serves files from it:
...
typed client the handler closes over. This page covers Infrastructure
as Code first, then what Infrastructure as Effects adds on top.
...
between them, and prints the URL. Every deploy targets a stage. A
stage is an isolated environment with its own copy of every resource,
so `dev` and `prod` never touch each other:
