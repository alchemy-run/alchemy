...
:::caution
**Updated** ([#888](https://github.com/alchemy-run/alchemy/pull/888)):
the example above no longer compiles. `proxyChain` requires a
channel-free `Effect<T>` — its handle is typed exactly `T`, so an
error or requirement channel on the cached effect would be silently
erased. Here that erasure was a real hazard: `PgClient.make`
requires `Scope | Reactivity`, `Reactivity` has no default and no
runtime bridge provides it, so the first query dies with a
missing-service defect the compiler could not warn about.

Use the packaged helper instead —
[`SQL.Postgres`](https://github.com/alchemy-run/alchemy/blob/main/packages/alchemy/src/SQL/Postgres.ts)
provides `Reactivity`, defers the connect, and memoizes the pool
per event:
...
To wrap a client yourself, discharge the channels first, then cast
to the explicit client interface. The cast is the deliberate
erasure point — everything it erases must be accounted for:

```typescript
const sql = proxyChain(
  (yield* makeExecutionMemo(
    // Reactivity: provide it — no bridge does
    PgClient.make({ url }).pipe(Effect.provide(Reactivity.layer)),
    // Scope: makeExecutionMemo defers it to the per-event scope
  )) as Effect.Effect<PgClient.PgClient>,
  // SqlError: erased deliberately — every statement already declares it
);
```
:::
