...
```typescript
// alchemy.run.ts
export const Events = Cloudflare.AnalyticsEngine.AnalyticsEngineDataset("Events", {
  dataset: "app-events",
});
...
```typescript
const Photos = Cloudflare.R2.R2Bucket("Photos");
...
```typescript
const Photos = Cloudflare.R2.R2Bucket("Photos", { emptyOnDestroy: false });
```
