...
The resources were implemented by fleets of agents working service by
service against [distilled](https://github.com/alchemy-run/distilled),
our generated Cloudflare SDK — now embedded in the workspace as a
submodule. Each agent implements a service's resources, then live-tests
them against the real API. The rule that makes this scale: **when a
test hits an API behavior the types don't capture, the fix goes into
the SDK, not a catch-block in the consumer.**

Concretely: a Turnstile lifecycle test reads back a widget it already
deleted and hits an unmatched error — Cloudflare code `10404`,
invisible to the type system. The fix is a four-line JSON patch in
distilled:
...
The service regenerates, and `WidgetNotFound` is now a tagged error in
`getWidget`'s inferred error union — so the provider handles it with
full inference, no casts, no status-code sniffing:
...
Run that loop across 288 live-test suites and you get an SDK whose
patch set grew from 369 to **1,087 operations spanning 94 of 114
Cloudflare services** within this one release cycle (the submodule
pointer changed ten times as the loop iterated). The test suite
didn't just verify the resources — it compiled Cloudflare's *actual*
API behavior, error codes and all, into the type system. Every future
consumer of the SDK inherits it.
...
- [Concepts › Providers](/concepts/provider)
- [Concepts › State Store](/concepts/state-store)
- [CLI guide](/guides/cli)
- [Deploy a Lambda Function](/tutorial/aws/lambda)
