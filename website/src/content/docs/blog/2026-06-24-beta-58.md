...
category: release
excerpt: v2.0.0-beta.58 reworks how Platform classes are wired — resolve `DurableObjectState` in the outer init Effect, `yield*` a Container to get a running instance and configure it with `Cloudflare.Containers.layerContainer`, pass props to `.make()` instead of the tag so one tag can have many implementations, and bind resources with least-privilege capabilities (`Cloudflare.R2.ReadWriteBucket`) backed by a native binding or a scoped HTTP token.
---
...
`yield*`-the-class + `Cloudflare.Containers.layerContainer`, standardizes
...
`Cloudflare.Containers.layerContainer(Class, options)` to configure how it
runs.
...
).pipe(Layer.provide(Cloudflare.Containers.layerContainer(Sandbox)));
```
...
them with `Cloudflare.Containers.layerContainer` from the hosting Durable
Object.
