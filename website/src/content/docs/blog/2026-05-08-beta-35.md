...
## `Cloudflare.Queue.messages(queue).subscribe(...)` — Effect-native queue consumer
  ...
  Effect.gen(function* () {
    ...
    Effect.provide(
      Layer.mergeAll(
        ...
        Cloudflare.Queue.QueueEventSourceLive, // required for `messages(...)` to dispatch
      ),
    ),
  ),
) {}
...
```typescript
const Assets = yield* Cloudflare.R2.R2Bucket("Assets", {
  customDomains: [
    "assets.example.com",
    "cdn.example.com",
  ],
