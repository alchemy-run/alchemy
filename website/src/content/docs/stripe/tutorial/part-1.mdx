---
title: "Part 1: Your First Product"
description: Install Alchemy, create a Stack with a Stripe Product, and deploy it.
sidebar:
  order: 1
---
...
Install Alchemy and Effect, create a Stack with a Stripe Product, and
deploy it. A Product is the catalog item Prices, invoices, and
Checkout attach to.

## Prerequisites

- [Bun](https://bun.sh) (recommended) or Node.js 22+
- A Stripe account and a secret key (`sk_test_…`) — see
  [Setup](/stripe/setup) if you haven't created those yet

:::note[Costs]
Stripe test-mode Products are free. `alchemy destroy` deletes them
when you're done.
:::
...
Stripe has no state backend of its own, so we'll keep state on disk
...
Let's declare a Stripe Product and observe the type error:
...
`Stripe.Providers` — the layer required by `Product`.

## Fix the Providers

Replace `Layer.empty` with `Stripe.providers()` to resolve the type
error:
...
talk to the Stripe API, and the type system ensures you never forget
to wire it up.

## Return Stack outputs

Stack outputs let you see important values after a deploy. Return an
object from the generator to expose the Product's id and name:
...
`product.id` is a Stripe id (`prod_…`). Nothing is for sale yet —
that's [Part 2](/stripe/tutorial/part-2).

## Deploy

Run `alchemy deploy` to create the Product:
...
The first time you deploy, Alchemy prompts for Stripe credentials —
paste the secret key you copied in [Setup](/stripe/setup). The key
...
[g]+[/g] [b]Pro[/b] [d](Stripe.Product)[/d]

Proceed?
◉ Yes ○ No
[g]✓[/g] [b]Pro[/b] [d](Stripe.Product)[/d] [g]created[/g]
{
[s2]productId: "prod_aaaaaaaaaaaaaaaa",
[s2]name: "Pro",
}`} />

Alchemy shows a plan, asks for confirmation, creates the Product, and
prints the stack outputs.

## Verify it worked

The Product is listed in the
[Stripe Dashboard](https://dashboard.stripe.com/test/products). Run
`alchemy deploy` again. Because nothing changed, the Product shows
as a no-op:
...
- An `alchemy.run.ts` with a Stack and a Stripe Product
- A live test-mode Product named `Pro`
- Stack outputs showing `productId` and `name`

In [Part 2](/stripe/tutorial/part-2), you'll put the catalog on a
Cloudflare Worker and add a Price.
