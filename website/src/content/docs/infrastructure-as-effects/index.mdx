...
infrastructure it runs on are one Effect program.
[What is Alchemy?](/what-is-alchemy) makes the case for that model;
this page is the map of the section, dissecting one running example —
the same Worker that opens that page:
...
One file, no `env` to reach through: `bucket` is a value the handler
closes over, next to the line that declares it. Everything in this
section is in that one Worker — here is the map.
