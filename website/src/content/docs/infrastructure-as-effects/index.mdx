...
applications, layered on top of its
[Infrastructure as Code](/infrastructure-as-code/stack) engine. The
code that runs inside a Worker or Lambda Function and the
infrastructure it runs on are one Effect program. A **Runtime**, such
as a Worker, is a [Resource](/infrastructure-as-code/resource) that
carries its own code, and the resources it uses are wired in as typed
values. This page is the map of the section, using one running
example — the same Worker that opens
...
bucket through an environment variable — two files held together by a
name, with nothing checking that they agree. Alchemy fully supports
that shape too ([What is Alchemy?](/what-is-alchemy#a-superset-of-infrastructure-as-code)
shows it side by side). The Effect form above is those two files as
one: `bucket` is a value the handler closes over, and the code that
uses the bucket sits next to the line that declares it.

Everything in this section is in that one Worker. Here is the map.

## Runtimes carry their code

`Cloudflare.Worker` is a **Runtime**: a Resource whose props are the
cloud configuration and whose Effect is the code it runs.
`main: import.meta.url` tells Alchemy to bundle this file's default
export as the Worker's script. The Effect always has the same shape —
bind what you need, then return what you expose — and a Worker, a
Lambda Function, a Durable Object, and a Workflow are all variations
on it. [Runtime](/infrastructure-as-effects/runtime) covers that
shape, the `fetch` and RPC interface it returns, and the three ways to
declare one.
...
work: a native binding on Cloudflare, an IAM statement scoped to the
one resource plus an environment variable on AWS. There is no
`env.Uploads` to reach for and no hand-written policy — the binding is
the SDK. [Bindings](/infrastructure-as-effects/binding) follows one
binding through the permissions, configuration, and client it
generates.
