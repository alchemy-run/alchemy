...
The same split works one level up, for your own services. Define the
contract as a `Context.Service`, a typed tag that names what the
service does:
...
Implement it with `Layer.effect`. The resources and bindings it needs
...
A second implementation can back the same contract with a different
resource, or a different cloud:

```typescript
export const JobServiceDynamo = Layer.effect(
  JobService,
  Effect.gen(function* () {
    const getItem = yield* AWS.DynamoDB.GetItem(AWS.DynamoDB.Table("Jobs", { partitionKey: "id" }));
    return { getJob: (id: string) => /* ... */ };
  }),
);
```

A Worker yields `JobService` and provides whichever Layer fits. The
namespace or table is created, bound, and hidden behind the service:
