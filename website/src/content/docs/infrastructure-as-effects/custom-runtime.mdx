...
On AWS the contract is env vars and IAM statements. A Cloudflare
...
| `MainShape`      | `Main<TaskServices>`                                    | What the constructor Effect may return, which is `{ fetch?: HttpEffect }` |
...
It is the shared implementation used by `AWS.ECS.Task` and
...
handler to `serve`. It then folds the context back onto the resource's
...
`hash` as the immutable artifact version. The Task tags its Docker
image `${repositoryUri}:${hash}`.
...
`Bundle.virtualEntryPlugin` wraps `main` in a bootstrap. The bootstrap
resolves the class, pulls `RuntimeContext.exports`, and runs the
collected program under your platform's Layers:
...
`action: "delete"`, so you can remove permissions as well as add them.
