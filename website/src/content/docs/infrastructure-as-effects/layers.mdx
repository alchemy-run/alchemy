...
your code — a different storage backend, a different cloud, or an
...
jobs now come from a DynamoDB table on AWS. The bindings follow the
Layer: the next deploy tears down the KV namespace, creates the table
`JobServiceDynamo` declares, and wires the Worker with the credentials
and permissions DynamoDB needs. `fetch` is byte-for-byte unchanged.
Tests do the same swap with a `JobServiceMemory` backed by a `Map`.
