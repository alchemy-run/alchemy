...
Consume the queue. Each batch arrives as `records`, a `Stream` of
...
Provide `AWS.Lambda.QueueEventSource` to implement it:
...
At deploy time this creates the queue consumer that dispatches batches
to this Worker. At runtime it registers the `queue` listener.
...
program, so every combinator composes. Map table changes to JSON and
run them straight into a queue:
...
write-side dual of an event source.
