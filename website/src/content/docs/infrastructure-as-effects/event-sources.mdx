...
Running code when a message lands on a queue takes more than a handler.
The Function needs permission to receive and delete messages, a mapping
that tells the queue to invoke it, and code that unpacks the raw event:
...
that does all three in one call. You name the queue and the handler,
and the messages arrive as an Effect `Stream`:
...
At deploy time, `consumeQueueMessages` attaches the receive, delete,
and get-attributes statement to the Function's role, scoped to the
queue's ARN. It creates the event source mapping that points the queue
at this Function, and removes it when the Function is destroyed. At
runtime it registers the listener that turns each invocation's batch
into `records`.
