...
pieces, which are permission to receive and delete messages, a mapping that tells
...
combinator composes. This example maps table changes to SQS messages
and runs them into a queue:
...
Each element is an SQS batch entry, so the message body goes in
`MessageBody`. A `consumeTableChanges` handler can't fail (its error
type is `never`), so `Effect.orDie` turns the sink's error into a
defect. Provide `AWS.Lambda.TableEventSource` and `AWS.SQS.QueueSinkHttp`
(which needs `AWS.SQS.SendMessageBatchHttp`), as shown on the
[Sinks](/infrastructure-as-effects/sinks) page.
...
Provide the matching Layer on the Function or Worker:

| Call | Layer to provide |
| ---- | ---------------- |
| `AWS.SQS.consumeQueueMessages` | `AWS.Lambda.QueueEventSource` |
| `AWS.Kinesis.consumeStreamRecords` | `AWS.Lambda.StreamEventSource` |
| `AWS.DynamoDB.consumeTableChanges` | `AWS.Lambda.TableEventSource` |
| `AWS.S3.consumeBucketEvents` | `AWS.Lambda.BucketEventSource` |
| `AWS.SNS.consumeTopicNotifications` | `AWS.Lambda.TopicEventSource` |
| `AWS.EventBridge.consumeBusEvents` | `AWS.Lambda.EventSource` |
| `Cloudflare.Queues.consumeQueueMessages` | `Cloudflare.Queues.EventSourceLive` |
| `Cloudflare.Workers.cron` | `Cloudflare.Workers.CronEventSourceLive` |
| `GitHub.consumeRepositoryEvents` | `Cloudflare.Workers.GitHubRepositoryEventSourceLive` |
...
Each source has its own guide. On AWS, see [SQS](/aws/messaging/sqs),
...
[EventBridge](/aws/messaging/eventbridge). On Cloudflare, see
[Queues](/cloudflare/messaging/queues), [Cron](/cloudflare/messaging/cron),
and [GitHub Events](/cloudflare/messaging/github-events).

## Where next

- [Sinks](/infrastructure-as-effects/sinks) are the write-side dual.
  They expose resources as Effect `Sink`s, with batching and IAM
  generated the same way.
- [Bindings](/infrastructure-as-effects/binding) covers the deploy-time
  mechanics every event source is built on.
- [Runtime](/infrastructure-as-effects/runtime) covers
