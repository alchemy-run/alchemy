...
A Function is usually defined twice. Once as a resource, with its
memory, runtime, and environment. Once as a handler file the resource
points at:

```typescript
// infrastructure
new Function("Api", { handler: "src/api.handler", memory: 512 });
...
The resource knows nothing about what the handler needs. The handler
knows nothing about what the resource was given. They meet at a string.
...
One deploy ships both. Let's look at the Effect.
