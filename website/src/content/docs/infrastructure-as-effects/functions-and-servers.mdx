...
Alchemy is a superset of that model, not a replacement. The same
Worker can point at a plain `async fetch` handler with the bucket on
its `env`:
...
artifact. [Effect handlers vs async handlers](#effect-handlers-vs-async-handlers)
compares them. The rest of this page follows the Effect.
