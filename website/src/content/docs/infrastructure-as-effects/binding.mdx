...
Worker, Lambda Function, or Container (see
[Functions & Servers](/infrastructure-as-effects/platform)).
One `bind()` call generates the IAM policies, the
...
bindings fit into a Function's runtime, see
[Functions & Servers › Bindings in action](/infrastructure-as-effects/platform#bindings-in-action).
...
Bindings work in both handler styles Workers and Lambdas support. Pick
...
guard](/infrastructure-as-effects/phases#the-__alchemy_runtime__-guard) for a deeper
look.
...
[Layers](/infrastructure-as-effects/layers) covers. When two Workers bind each
other, see [Circular Bindings](/infrastructure-as-effects/circular-bindings).

## Where next

- [Circular Bindings](/infrastructure-as-effects/circular-bindings) — two Workers that bind each other.
