...
One deploy ships both. `main: import.meta.url` says the code is this
file. Most Runtimes need no other props, because everything else they
depend on comes in through bindings.

In traditional IaC a Runtime is defined twice, once as a resource
with its configuration and once as a handler file the resource points
at, and the two meet at a string. Alchemy is a superset of that model,
not a replacement. The same Worker can point at a plain `async fetch`
handler with the bucket on its `env`:
...
// src/worker.ts
export default {
  async fetch(request: Request, env: { Uploads: R2Bucket }) {
    const obj = await env.Uploads.get("hello.txt");
    return new Response(obj?.body ?? "Not found", { status: obj ? 200 : 404 });
  },
};
```

Two files, held together by the name `Uploads`. The Effect form above
is those two files as one, and `bucket` is a value the handler closes
over instead of a key on `env`. Both styles deploy through the same
provider and produce the same artifact. [Effect handlers vs async handlers](#effect-handlers-vs-async-handlers)
