...
your own instead of a resource. Define the contract as a
`Context.Service`:
...
Implement it with a Layer that carries its own resources and bindings:
...
The Worker yields the service and provides the Layer. The namespace
comes with it, and swapping in a Layer backed by DynamoDB changes
nothing in the handler:
