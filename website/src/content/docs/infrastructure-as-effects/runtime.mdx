...
The Effect is the code. Here is the running example from the
[overview](/infrastructure-as-effects), which this page takes apart:
...
`Context.Service` for what the handler needs, implement it with a
Layer that owns its own resources and bindings, and provide that Layer
on the constructor. On the next deploy the Layer's resources are
created and bound to this Runtime, while the handler only ever sees
the service — swap the Layer for one backed by a different store, or a
different cloud, and the handler doesn't change. This is the
