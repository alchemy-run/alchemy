...
:::tip
Read this after [Functions & Servers](/infrastructure-as-effects/platform). The init/runtime
...
Function resources — Workers, Lambdas, Containers — express both in
a single program by **returning an Effect from inside an Effect**.
...
[Layers › Runtime as a colored function](/infrastructure-as-effects/layers#runtime-as-a-colored-function).
...
The `bind()` mechanics behind the guard are covered in
[Bindings](/infrastructure-as-effects/binding). Next: where all of this is
persisted — [State Store](/state-store).

## Where next

- [State Store](/state-store) — where deploy results persist between runs.
- [Bindings](/infrastructure-as-effects/binding) — what `bind(resource)` records at plantime.
- [Layers](/infrastructure-as-effects/layers) — runtime as a colored function.
