...
Asserts a narrower type for an `Output` — most commonly to drop
`undefined` from an attribute that is only ever absent under
configuration you didn't choose:

```typescript
// worker.url is Output<string | undefined> — undefined only when `url: false`
return { url: worker.url.as<string>() };
```

Stack outputs typed as `{ url: string }` need this: the attribute's
declared type keeps the `undefined` arm even though your configuration
guarantees a URL exists.

:::caution
`if (x === undefined)` does not narrow an `Output` — Outputs are lazy
references, not values, so control flow never sees what's inside. Use
`.as<T>()` to assert, or `Output.map` to transform the resolved value.
:::
...
| Assert a narrower type (e.g. drop `undefined`) | `output.as<string>()`                     |
