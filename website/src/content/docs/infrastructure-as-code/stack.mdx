...
Every deploy targets a **stage**. A stage is an isolated instance of
the stack, like `dev_sam`, `staging`, or `prod`. The stage defaults
to `dev_$USER`, so each developer gets their own environment. State
and physical names are namespaced by stage.
...
`yield* Backend` defaults to the current stage. The `sam` frontend
reads the `sam` backend, and `pr-42` reads `pr-42`. Use
`Backend.stage.<name>` to pin a specific stage.
