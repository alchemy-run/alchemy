...
:::note[Prerequisite]
This guide imports `StripeCredentials` from
[Custom Auth Provider](/environments/custom-auth-provider). Build that
first, or replace it with your own credentials service.
:::
...
:::note
Alchemy ships a full Stripe provider in `alchemy/Stripe`, and it
already uses the type `"Stripe.Product"`. This guide rebuilds a small
version to teach the pattern. In your own project, use a type string
no built-in uses, such as `"MyCo.Product"`.
:::
...
object with lifecycle handlers. `reconcile` and `delete` are
required. `list` is required by the typed
`StripeProduct.Provider.of(...)` constructor used below.
`Provider.succeed`/`Provider.effect` would default it to an empty
list if you passed a plain object (see
[Providers › list](/infrastructure-as-code/provider#list)). The
other hooks, such as `diff`, `read`, and `precreate`, are optional.
...
- **`Provider.effect`** is for services that need a dependency
  ...
  Don't use it to resolve credentials. Resolve those inside each
  handler (see [Credentials](#credentials)).
...
The body is idempotent by construction. If a state write fails, the
next deploy runs it again and it re-converges. Adoption needs no special case.
...
Like `reconcile`, `delete` must be idempotent. The next deploy runs
it again if persistence fails after the cloud call succeeds but before
...
Without a `catch` option, `Effect.tryPromise` wraps every rejection
in an `UnknownError` with a generic message. The `catch` option keeps
the SDK's error, so you can match its `code`. Stripe reports a
missing object with `resource_missing`.
...
- `undefined` / `void`: let the engine compare props and bindings.
  It plans an update if anything changed, otherwise a no-op. Return
  this for any change you don't need to special-case.
- `{ action: "update", stables?: [...] }`: apply in place
- `{ action: "replace", deleteFirst?: boolean }`: destroy and
  recreate
- `{ action: "noop" }`: ignore this change on purpose. This is rare.
...
To verify replacement semantics, change a prop that your `diff`
answers with `{ action: "replace" }` and assert that
`updated.productId !== created.productId`. `stables` does not trigger
replacement. It lists the attributes that stay the same across an
update. See
