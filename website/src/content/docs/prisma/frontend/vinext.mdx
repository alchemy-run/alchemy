...
`Prisma.Website.Vinext` runs `vinext build`, then hosts vinext's production
...
The same configuration selects the cache adapter for each deployment target.
Prisma uses the Node target. Leave Cloudflare plugins out of this configuration.
...
The `alchemy()` cache adapter uses Redis when `REDIS_URL` is set and memory
