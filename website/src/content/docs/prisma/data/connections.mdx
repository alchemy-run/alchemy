...
A `Prisma.Connection` is a named credential (an API key) for one
database. Alchemy appends the resource's instance identity to the
name so connections stay unambiguous, and every secret output is
`Redacted` so nothing prints in logs or plan output.
