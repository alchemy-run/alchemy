...
Generic Vite apps work with `build: "auto"`. For another static build command
or output directory, set `build.type` to `"static"`. Alchemy packages the files
with the HTTP entrypoint Prisma Compute requires; the application does not need
its own server file or runtime dependency:
