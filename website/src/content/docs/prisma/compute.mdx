---
title: Compute
description: Prisma Compute apps as Stack resources — build, upload, health-check, promote, and roll back deployments next to the database they use.
---

`Prisma.Compute` deploys an application onto Prisma's managed
runtime. One resource covers the whole path: build (optional),
upload, deploy, health-check, promote — and roll back when the new
deployment doesn't converge.

## Framework apps

Point `path` at the app and describe its build. The command runs
locally, the output directory is archived and uploaded, and the
entrypoint starts the server:

```typescript
import * as Prisma from "alchemy/Prisma";

const app = yield* Prisma.Compute("api", {
  project,
  path: "./app",
  build: {
    command: "bun run build",
    outdir: ".output",
    entrypoint: "server/index.mjs",
  },
  port: 3000,
  healthCheck: { path: "/api/health" },
  env: {
    DATABASE_URL: connection.databaseUrl,
  },
  destroyOldDeployment: true,
});
```

The server must listen on `PORT` (injected at runtime). With
`build: "auto"` — the default when `path` points at a recognizable
app — alchemy detects the framework and derives the build for you.

`app.url` is the deployed endpoint, and `destroyOldDeployment`
cleans up the previous generation once the new one is promoted.

## Effect-native apps

Pass `main` and an Effect body instead of a build, and the app is
bundled from your Stack file — same shape as an Effect-native
Cloudflare Worker or Lambda:

```typescript
export default Prisma.Compute(
  "api",
  { project, main: import.meta.filename },
  Effect.gen(function* () {
    const db = yield* Prisma.Connect(connection);
    const sql = yield* SQL.Postgres({ url: db.databaseUrl });

    return {
      fetch: Effect.gen(function* () {
        const users = yield* sql`SELECT * FROM users`;
        return yield* HttpServerResponse.json(users);
      }),
    };
  }).pipe(Effect.provide(Prisma.ConnectBinding)),
);
```

## Health checks and promotion

Every deploy creates a new deployment, probes `healthCheck.path`
until it answers, and only then promotes it to receive traffic. A
deployment that never becomes healthy is destroyed and the previous
one keeps serving — a failed deploy never takes the app down.

## Custom domains

```typescript
const domain = yield* Prisma.CustomDomain("api-domain", {
  app,
  hostname: "api.example.com",
});
```

## Local dev

Under `bun alchemy dev`, `dev.command` runs your app locally (e.g.
`vite dev`) with the same env the deployed app would see — including
connection bindings, which resolve to the local
[dev database](/prisma/data/postgres#local-dev):

```typescript
const app = yield* Prisma.Compute("web", {
  project,
  path: ".",
  dev: { command: "bun run dev" },
  env: { DATABASE_URL: connection.databaseUrl },
});
```

## Where next

- [Connections](/prisma/data/connections) — the env and binding
  wiring in detail.

Reference:

- [Compute API reference](/providers/prisma/compute)
- [CustomDomain API reference](/providers/prisma/customdomain)
