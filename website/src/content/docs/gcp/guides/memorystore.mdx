...
The binding grants `roles/redis.editor` on the host's runtime service
account and packs the connection URL into the revision as an Output —
it is resolved when the instance reconciles, not when you write the
binding, so the private IP is never something you paste in.
