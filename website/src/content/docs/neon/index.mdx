...
schema only, and expire on their own — see [Branching](/neon/branching).
...
See [Connections](/neon/connections) for the raw URIs, the parsed
...
1. [Hyperdrive](/cloudflare/hyperdrive) pools the branch's direct
   origin at the edge
2. [Drizzle](/cloudflare/guides/drizzle) gives the Worker a typed
   query layer over that connection
3. [Shared database](/cloudflare/guides/shared-database) keeps one
   long-lived project instead of provisioning a cluster per stage
4. [Branch from a shared database](/cloudflare/guides/branch-from-shared-database)
