---
title: Next.js
description: Build Next.js for a Neon Function Fetch entry and use next dev locally.
sidebar:
  order: 3
---
...
normalizes loopback IP hostnames to `localhost`. These checks do not prove live Neon
behavior: fresh deployment fails with `FunctionDeploymentFailed`. Live acceptance
remains incomplete, and the full 13-framework live matrix is not green.
:::
