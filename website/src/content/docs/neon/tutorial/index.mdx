...
The complete source is in [`examples/neon-tutorial`](https://github.com/alchemy-run/alchemy/tree/main/examples/neon-tutorial). Use that directory as the runnable project; the following pages explain its changes one concept at a time. Both the default Effect Function and the native Fetch alternative implement the same browser API.

## Prerequisites

Use Node.js 24, Bun, pnpm, and a Neon account with Object Storage and Functions available. Configure the Alchemy `testing` profile for Neon or supply `NEON_API_KEY` to the deployment process only. This tutorial selects `aws-us-east-2`, where the combined backend is available. Resources can incur normal Neon usage charges; no AI gateway, model call, credits, or plan purchase is required.
...
The example deliberately disables email verification and enables localhost. Turn verification on and localhost off before production. Signout revokes the managed session, but already issued JWTs can remain valid until their short expiry.
