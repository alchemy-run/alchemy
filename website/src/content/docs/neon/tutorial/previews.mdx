...
:::caution[Preview deployment verification]
The current provider rejected inherited Auth with `OwnedBySomeoneElse: Existing Auth requires explicit adoption` even with this resource-scoped policy. The fork was created, but the child API, trigger, and website were not deployed. Recovery reads now treat incomplete, never-created Auth and trusted-origin entries as absent without relaxing ownership checks on existing resources. Normal destroy of the retained preview and parent was verified after this fix. Preserve stack state for cleanup; do not use global adoption, remove state, or delete the parent around a failed dependency. The isolation test checks forked data separately from the new child trigger, so passing data checks alone do not prove a working preview application.
:::
...
Run this with the same stage and retained local state used for deployment. `forceDestroy: true` intentionally erases the tutorial bucket's files before removing its branch and project.
