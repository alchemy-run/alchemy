...
Every read filters by the verified JWT subject; the API does not accept an owner ID from the request body.
...
```diff lang="typescript"
+const [row] = yield* sql<UploadRow>`SELECT * FROM uploads WHERE id = ${id} AND owner_id = ${owner}`;
```
