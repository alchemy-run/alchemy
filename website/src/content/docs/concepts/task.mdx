...
Define a Task once with its type name and implementation, then call it
in a stack to register an instance:
...
```typescript
export class Sync extends Task<Sync, { table: string }, { rows: number }>()("Sync") {}
