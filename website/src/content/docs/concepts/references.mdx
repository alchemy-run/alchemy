...
Stacks and stages deploy independently; a **reference** lets one
deployment read another.
...
For the project-structuring patterns built on references —
long-lived staging stages feeding ephemeral `pr-*` previews,
infra/app stack splits, monorepo wiring — see
[Stack References](/guides/stack-references).
...
`yield* Backend` resolves at the current stage;
`Backend.stage.<name>` pins to a named one. Which to use when:
...
See [Monorepo](/guides/monorepo) for the full walkthrough.
...
- [Stack References](/guides/stack-references) — structuring
  projects with refs: shared staging databases, infra/app splits,
  monorepo wiring.
- [Inputs & Outputs](/concepts/outputs) — the broader Output
