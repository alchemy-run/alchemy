...
[`alchemy login`](/cli/login) and
[`alchemy profile show`](/cli/profile) in the CLI reference.
For environment isolation, see [Stages](/concepts/stages). To wire
profiles into GitHub Actions, see the [CI guide](/guides/ci).

## Where next

- [Secrets & Config](/concepts/secrets) — bind env vars and secrets onto your deploy targets.
- [CI](/guides/ci) — one profile per environment, minted and pushed to your repo as code.
