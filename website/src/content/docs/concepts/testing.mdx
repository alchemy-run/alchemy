...
For the full walkthrough, see [Testing a Stack](/guides/testing-a-stack).

For every option, hook, and variant, see the [Test harness](/concepts/test-harness) reference.
...
The per-call form is covered in [Test harness → stage](/concepts/test-harness#stage).

## Where next

- [Testing a Stack](/guides/testing-a-stack) — deploy a Stack and drive it over HTTP, end to end.
- [Testing Providers](/guides/testing-providers) — exercise a provider's create/update/replace/delete with `test.provider`.
- [Test harness](/concepts/test-harness) — every `Test.make` option, hook, and variant.
- [Observability](/concepts/observability) — wire exporters, monitors, and alarms into the same Stack.
