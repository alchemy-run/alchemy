...
See [Inputs & Outputs](/concepts/outputs) for the full set of
...
stages](/cloudflare/guides/shared-database) guide for the canonical
walkthrough.
...
Outputs draw the edges. [Bindings](/concepts/binding) attach
...
splitting each Function resource ([Functions & Servers](/concepts/platform)) into a
...
See [Circular Bindings](/guides/circular-bindings) for the full A↔B
build-up and how alchemy plans the two-phase create-then-wire deploy.

## Where next

- [Actions](/concepts/action) — deploy-time work without a lifecycle.
- [Inputs & Outputs](/concepts/outputs) — the lazy references that draw the graph.
- [Resource lifecycle](/concepts/resource-lifecycle) — what happens on deploy.
