...
## `ref` — reference a deployed resource {#ref}

Every resource constructor exposes a static `ref` that returns a
typed reference to a resource that has already been deployed
**elsewhere** — in another [stage](/concepts/stages) of this
stack, or in a different stack entirely.
...
The lookup is keyed by `{ stack, stage, id }`. Both `stack` and
`stage` default to the current stack/stage, so omitting both
points at the same resource the local stack would create — useful
for opting into "create OR adopt".

Three things to know:

- **Lazy.** Like every other Output expression, the ref doesn't
  call any cloud API at declaration time. Plan-time evaluation
  reads the persisted attributes from the state store.
- **Typed.** `Neon.Project.ref(...)` returns a `Neon.Project`. You
  can pass it to anything that accepts a `Neon.Project` — the
...
- **Strict.** If the target hasn't been deployed yet, evaluation
  fails with `InvalidReferenceError`. Deploy the upstream stage
  first, then the dependent one.
...
[Cross-stage references](/guides/cross-stage-references) (per-resource,
same stack, different stage) and
[Cross-stack references](/guides/cross-stack-references) (whole-stack
typed handles).
