...
is "declare a [Resource type](/concepts/resource), implement a
Layer" — and merge it alongside `Cloudflare.providers()` or
`AWS.providers()`. The [custom provider guide](/guides/custom-provider)
builds a Stripe `Product` provider end-to-end.

## Where next

- [Custom Provider](/guides/custom-provider) — step-by-step
  walkthrough of a Stripe `Product` provider.
- [Actions](/concepts/action) — deploy-time work that isn't a
  resource.
- [Resource lifecycle](/concepts/resource-lifecycle) — when each
  operation fires.
