...
backend. See [Custom State Store](/guides/custom-state-store) for
the full walkthrough.

## Where next

- [Custom State Store](/guides/custom-state-store) — implement the
  `StateService` interface end-to-end.
- [Testing Providers](/guides/testing-providers) — the in-memory store in practice.
- [References](/concepts/references) — reading another
  stack's outputs.
