...
`otelMetricsEndpoint` attributes are [Outputs](/concepts/outputs) —
...
[Secrets](/concepts/secrets). At runtime the exporter Layer from the
...
- [Axiom observability guide](/cloudflare/guides/axiom-observability) — end-to-end setup for a Cloudflare Worker
- [Secrets](/concepts/secrets) — how `Redacted` values like ingest tokens are bound
