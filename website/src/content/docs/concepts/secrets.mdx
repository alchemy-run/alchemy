...
to automatically bind env vars to your Worker or Lambda environment ([Functions & Servers](/concepts/platform)).
Bound values come from the env of whoever runs the deploy, so they naturally
vary per [stage](/concepts/stages) and [profile](/concepts/profiles).
...
see [Secrets & env on Cloudflare](/cloudflare/secrets-env) or
[on AWS](/aws/secrets-env).

## Where next

- [Local development](/concepts/local-development) — `alchemy dev`: local code, real cloud resources.
- [Secrets & env on Cloudflare](/cloudflare/secrets-env) — wire a real key from `.env` into a Worker.
- [Secrets & env on AWS](/aws/secrets-env) — the same walk for Lambda.
