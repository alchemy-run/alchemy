...
```typescript
yield* SQS.consumeQueue(InboundQueue, (records) =>
  records.pipe(
    Stream.map((r) => r.body),
...
| ----------------------------------- | --------------------- |
| `SQS.consumeQueue(queue, fn)`       | `SQSRecord`           |
| `Kinesis.consumeStream(stream, fn)` | `KinesisStreamRecord` |
| `DynamoDB.consumeStream(table, fn)` | `StreamRecord<T>`     |
...
yield* DynamoDB.consumeStream<Order>(OrdersTable, {
