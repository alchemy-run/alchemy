...
[Testing a Stack](/guides/testing-a-stack); for provider-lifecycle
testing, see [Testing Providers](/guides/testing-providers).
...
| `test.provider(name, fn)` | [Provider-lifecycle test](/guides/testing-providers) against a scratch in-memory stack. |
...
cold-start helpers, taught in [Testing a Stack](/guides/testing-a-stack).
...
built on it lives in [Testing a Stack](/guides/testing-a-stack).
...
[Testing Providers](/guides/testing-providers)):
...
see [Testing a Stack → Drive the live URL](/guides/testing-a-stack).
...
- [Testing a Stack](/guides/testing-a-stack) — deploy once, drive the live URL, tear down.
- [Testing Providers](/guides/testing-providers) — exercise create / update / replace / delete with `test.provider`.
- [Testing](/concepts/testing) — the testing overview.
- [State Store](/concepts/state-store) — choosing between `localState()`, `Cloudflare.state()`, and friends.
- [Profiles](/concepts/profiles) — how `ALCHEMY_PROFILE` and the `profile` factory option resolve credentials.
