...
operators, [Stack References](/guides/stack-references) for the
structuring patterns, and [Monorepo](/guides/monorepo) for the
end-to-end walkthrough (package layout, schema sharing, deploy
ordering).

A stack is just the container — the things it deploys are
[Resources](/concepts/resource).

## Where next

- [Resources](/concepts/resource) — the named entities a stack deploys.
- [Stages](/concepts/stages) — naming, isolation, per-stage configuration.
- [Monorepo](/guides/monorepo) — one stack or many across packages.
