...
A stack can read the deployed outputs of another stack. Declare a
typed handle once, deploy with it on one side, reference its stage
on the other:

```typescript
// backend/src/Stack.ts — shared between both stacks
...
```typescript
// backend/alchemy.run.ts — uses .make to deploy
...
```typescript
// frontend/alchemy.run.ts — reads a deployed stage
...
`Backend.stage.<name>` resolves the named stage's outputs from the
state store at plan time. Each stack keeps its own state file,
deploys, and destroys independently.

Yielding the bare handle defaults to the **current stage** of the
referencing stack — `dev_sam` of the frontend reads `dev_sam` of
the backend, `pr-42` reads `pr-42`, and so on:

```typescript
const backend = yield* Backend; // current stage
const backend = yield* Backend.stage.prod; // pinned stage
```

For the full monorepo walkthrough — package layout, sharing the
HTTP API schema, deploy order — see
